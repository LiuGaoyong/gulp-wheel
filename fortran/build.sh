...
pixi update && pixi upgrade && pixi list
...
#########################################################################
# The following code was coped from Src/mkgulp. Then Modify for pixi.   #
#########################################################################
#
...
rm -f makefile #  Clean up by remove temporary makefile
if [ "$task" = "gulp" ] ; then
...
cd "${THISDIR}/Src"
if [[ -f gulp ]] ; then
    TDIR=${THISDIR}/../gulp/_gulpexec
    for so in libgfortran.so.5 libgcc_s.so.1 libquadmath.so.0; do
      cp ${THISDIR}/../.pixi/envs/default/lib/$so $TDIR/$so
    done; mv gulp $TDIR/_gulp
else
    echo "Compile error !!!"
    exit 0
fi
