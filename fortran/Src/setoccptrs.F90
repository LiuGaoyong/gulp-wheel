  subroutine setoccptrs(ncfoc,nsfoc,nbfoc,iocptr,ibocptr,lregion1only,lanypartial)
!
...
!   7/20 Argument added to flag the presence of a partially occupied site
...
!  Passed variables
!
  integer(i4),                    intent(out)  :: ibocptr(*)
  integer(i4),                    intent(out)  :: iocptr(*)
  integer(i4),                    intent(out)  :: nbfoc
  integer(i4),                    intent(out)  :: ncfoc
  integer(i4),                    intent(out)  :: nsfoc
  logical,                        intent(in)   :: lregion1only
  logical,                        intent(out)  :: lanypartial
!
!  Local variables
!
...
!************************************
  ...
  ii = 0
  do i = 1,numat
    if (.not.lregion1only.or.nregionno(nrelf2a(i)).eq.1) then
      ii = ii + 1
      iiptr(i) = ii
      ibocptr(ii) = 0
      if (occuf(i).eq.1.0_dp.and..not.lallowgt1) then
!
!  Fully occupied site
!
...
!
        ...
        do while (ni.lt.nspcellat(ixyz).and..not.lfound)
          ...
          if (j.lt.i) then
            if (.not.lregion1only.or.nregionno(nrelf2a(j)).eq.1) then
              jj = iiptr(j)
