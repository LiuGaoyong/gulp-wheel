...
!
!  Elemental data (including EEM and EHT)
!
  module element
    use datatypes
    ...
    real(dp),         save :: rqeq                                 ! QEq radius for integrals
    real(dp),         save :: rqeqtaper                            ! QEq radius taper range
...
!
  module m_aaclp
    use datatypes
    real(dp),                         save :: aaclp_dscale = 650.0_dp
    real(dp),                         save :: aaclp_pscale = 235.0_dp
    real(dp),                         save :: aaclp_qscale = 0.41_dp
    real(dp),                         save :: aaclp_rscale = 77000.0_dp
    real(dp),                         save :: aaclp_dscale_ev
    real(dp),                         save :: aaclp_pscale_ev
    real(dp),                         save :: aaclp_rscale_ev
  end module m_aaclp
!
!  Bond charges
!
...
!
  module bondorderdata
    ...
    logical,     dimension(:),   pointer, save :: lBOgikA  => null()
    logical,     dimension(:),   pointer, save :: lBOgikR  => null()
    logical,     dimension(:),   pointer, save :: lBOsymA  => null()
    logical,     dimension(:),   pointer, save :: lBOsymR  => null()
...
!
!  Bond valence potentials
!
  module bondvalence
    use datatypes
    integer(i4),                          save :: maxvalbond = 1            ! Maximum number of valence bond potentials
    integer(i4),                          save :: maxvalener = 1            ! Maximum number of valence energy terms
    integer(i4),                          save :: maxVBparB = 6             ! Maximum number of valence bond potential parameters
    integer(i4),                          save :: maxVBparE = 6             ! Maximum number of valence energy parameters
    integer(i4),                          save :: nvalbond = 0              ! Number of valence bond potentials
    integer(i4),                          save :: nvalener = 0              ! Number of valence energy terms
    integer(i4),                          save :: nlibvalbond = 0           ! Number of valence bond terms in libraries
    integer(i4),                          save :: nlibvalener = 0           ! Number of valence energy terms in libraries
    integer(i4), dimension(:),   pointer, save :: nVBspecE => null()
    integer(i4), dimension(:),   pointer, save :: nVBspecB1 => null()
    integer(i4), dimension(:),   pointer, save :: nVBspecB2 => null()
    integer(i4), dimension(:),   pointer, save :: nVBtypeE => null()
    integer(i4), dimension(:),   pointer, save :: nVBtypeB1 => null()
    integer(i4), dimension(:),   pointer, save :: nVBtypeB2 => null()
    integer(i4), dimension(:),   pointer, save :: nvalbondtype => null()    ! Indicator of type of valence bond
    integer(i4), dimension(:),   pointer, save :: nvalenertype => null()    ! Indicator of type of valence energy
    logical,                              save :: lPrintVB = .false.        ! Flag as to whether to print the valence bond energies
    real(dp),    dimension(:,:), pointer, save :: VBparB => null()          ! Parameters for valence bond potentials
    real(dp),    dimension(:,:), pointer, save :: VBparE => null()          ! Parameters for valence energy terms
    real(dp),    dimension(:),   pointer, save :: VBwgtB => null()          ! Weights for valence bond terms
...
!
  module configurations
    ...
    integer(i4), dimension(:),                  pointer, save :: nregionnocfg => null()
    ...
    logical,     dimension(:),                  pointer, save :: lbsmatcfg => null()
    logical,     dimension(:),                  pointer, save :: leinsteinat => null()
    logical,     dimension(:),                  pointer, save :: ltibeinsteinat => null()        ! If true then apply TI backward
    logical,     dimension(:),                  pointer, save :: ltifeinsteinat => null()        ! If true then apply TI forward
    ...
    real(dp),    dimension(:),                  pointer, save :: xceinsteinat => null()            ! Einstein spring position in Cartesian x
    real(dp),    dimension(:),                  pointer, save :: yceinsteinat => null()            ! Einstein spring position in Cartesian y
    real(dp),    dimension(:),                  pointer, save :: zceinsteinat => null()            ! Einstein spring position in Cartesian z
    real(dp),    dimension(:),                  pointer, save :: xfeinsteinat => null()            ! Einstein spring position in fractional x
    real(dp),    dimension(:),                  pointer, save :: yfeinsteinat => null()            ! Einstein spring position in fractional y
    real(dp),    dimension(:),                  pointer, save :: zfeinsteinat => null()            ! Einstein spring position in fractional z
!
!  Reference geometries for configurations for GFNFF
!
    logical,     dimension(:),                  pointer, save :: lgfnff_ref_rv  => null() ! Reference initial cell input?
    logical,     dimension(:),                  pointer, save :: lgfnff_ref_crd => null() ! Reference initial coords input?
    real(dp),    dimension(:,:,:),              pointer, save :: gfnff_ref_rv   => null() ! Reference initial cell
    real(dp),    dimension(:,:,:),              pointer, save :: gfnff_ref_crd  => null() ! Reference initial coordinates
    real(dp),    dimension(:,:),                pointer, save :: gfnff_ref_q    => null() ! Reference initial charges
!
...
!
  module control
    ...
    logical,                          save :: laaclp               ! If true, then include contribution from AA-CLP potentials
    ...
    logical,                          save :: leht                 ! If true, determine charges by Extended Huckel Theory
    logical,                          save :: leigen               ! If true, print out the phonon eigenvectors
    logical,                          save :: lequipartition       ! If true, then use classical free energy rather than quantised
    ...
    logical,                          save :: lga
    logical,                          save :: lgfnff = .false.     ! This is the flag the controls whether GFNFF is used
    ...
    logical,                          save :: lusefcalg            ! If true, use new FC algorithm
    ...
    logical,                          save :: lsync                ! If true, perform synchronous transit calculation
...
!
  module current
    ...
    logical,     dimension(:),     pointer, save :: lbsmat => null()            ! If true then this atom has a breathing shell
    real(dp),    dimension(:),     pointer, save :: aaclp_d => null()           ! AA-CLP one-centre contribution to D coefficient
    real(dp),    dimension(:),     pointer, save :: aaclp_p => null()           ! AA-CLP one-centre contribution to P coefficient
    real(dp),    dimension(:),     pointer, save :: aaclp_t => null()           ! AA-CLP one-centre contribution to T coefficient
    ...
    real(dp),    dimension(:),     pointer, save :: extpot => null()
    ...
    real(dp),                               save :: xvoffset
    real(dp),                               save :: yvoffset
    real(dp),                               save :: zvoffset
...
!
  module derivatives
    ...
    real(dp),                                   save :: d1tol = 1.0d-16              ! Tolerance on first derivatives being non-zero
    real(dp),                                   save :: d2tol = 1.0d-16              ! Tolerance on second derivatives being non-zero
    ...
    real(dp),    dimension(:,:),       pointer, save :: dqdVxyz=> null()             ! First derivatives of charge w.r.t. electric field
    real(dp),    dimension(:,:),       pointer, save :: d2qdxyz2 => null()
    real(dp),    dimension(:,:),       pointer, save :: dedqc => null()              ! Sum of first partial derivatives of energy with respect to a charge and per coordinate
    real(dp),    dimension(:,:),       pointer, save :: d2edqc => null()             ! First partial derivatives of energy with respect to a charge and per coordinate
    real(dp),    dimension(:,:),       pointer, save :: d2edqdq => null()            ! Second derivative of energy with respect to two charges
    real(dp),    dimension(:),         pointer, save :: d2edq2 => null()             ! Second derivative of energy with respect to a single charge
    real(dp),    dimension(:,:),       pointer, save :: ds2g => null()               ! Sum of charge derivatives for strain for atom
    real(dp),    dimension(:,:),       pointer, save :: ds2gs => null()              ! Sum of charge derivatives for strain for atom
    real(dp),    dimension(:,:),       pointer, save :: d2s2gs => null()             ! Sum of charge derivatives for strain for atom
...
! Bulk
    ...
    real(dp),                                      save :: eeht                    ! Energy from Extended Huckel Theory
    ...
    real(dp),                                      save :: egfnff                  ! Energy from GFNFF force field
    ...
    real(dp),                                      save :: eval                    ! Energy from valence bond potentials
...
! Defect
    ...
    real(dp),                                      save :: fhenergy_eq            ! Phonon contribution to free energy (equipartition)
...
!
  module gulp_files
    ...
    character(len=maxwordlength), save :: cfgfile
    ...
    character(len=maxwordlength), save :: jdftxfile
    ...
    logical,                      save :: lcfg                                   ! Output CFG file for Ovito/AtomEye
    logical,                      save :: lcfg_e                                 ! Flags for CFG output: - energy
    logical,                      save :: lcfg_fx                                !                       - Forces in X
    logical,                      save :: lcfg_fy                                !                       - Forces in Y
    logical,                      save :: lcfg_fz                                !                       - Forces in Z
    logical,                      save :: lcfg_f                                 !                       - RMS force
    logical,                      save :: lcfg_q                                 !                       - Charges
    logical,                      save :: lcfg_dq                                !                       - Change in charges
    logical,                      save :: ldcd
    logical,                      save :: ldip
    logical,                      save :: ldlv
    logical,                      save :: leig
...
!             
  module fc_data
    ...
    real(dp),                     save :: rfcinteraction_in = 0.0_dp             ! Force constant interaction range from input
  end module fc_data
!*******************************
!  Variables to pass to GFNFF  *
!*******************************
  module gulp_gfnff
    use datatypes
!
...
!
!  Current configuration whose parameters are being stored
!
    integer(i4),                              save :: ngfnff_current_cfg = 0
!
    integer(i4),                              save :: max_gfnff_ele = 86              ! Maximum element number for GFNFF
!
    integer(i4),                              save :: maxat_gfnff = 1                 ! Maximum number of atoms for GFNFF
!
    real(dp),                                 save :: gfnff_autoangs = 0.52917726_dp  ! Unit conversion from au to Angs for pGFNFF
    real(dp),                                 save :: gfnff_autoev = 27.21138505_dp   ! Unit conversion from au to eV for PGFNFF
!
!  Radius arrays
!
    real(dp),    dimension(:),       pointer, save :: gfnff_rcov => null()            ! GFNFF covalent radii
    real(dp),    dimension(:),       pointer, save :: gfnff_rad => null()             ! GFNFF general radii
    real(dp),    dimension(:,:),     pointer, save :: gfnff_rad_cn => null()          ! GFNFF radius related CN factors
!
!  EEQ arrays
!
    real(dp),                                 save :: gfnff_eeq_rad                   ! Radius for EEQ difference of erf(gam.r) from 1
    real(dp),    dimension(:),       pointer, save :: gfnff_eeq_alp => null()         ! Alpha
    real(dp),    dimension(:),       pointer, save :: gfnff_eeq_chi => null()         ! Chi
    real(dp),    dimension(:),       pointer, save :: gfnff_eeq_cnf => null()         ! Coordination number factor
    real(dp),    dimension(:),       pointer, save :: gfnff_eeq_gam => null()         ! Gamma
!
!  Fragments
!
    integer(i4),                              save :: nfrag                           ! Number of fragments
    integer(i4), dimension(:),       pointer, save :: nfraglist => null()             ! Fragment list
    real(dp),    dimension(:),       pointer, save :: qfrag     => null()             ! Fragment charges
!
!  Pi system flag
!
...
!
    ...
    real(dp),                                 save :: gfnff_rtopo_scale = 1.175_dp    ! Scale factor for topological distances
!
!  Neighbour list cutoff
!
    real(dp),                                 save :: gfnff_cut2_nbr
!
!  Variables for damping of ATM dispersion
!
    real(dp),                                 save :: atm_alpha1 = 1.0_dp
!
...
!
    real(dp),                                 save :: gfnff_cnmax                     ! Maximum coordination number parameter
    ...
    real(dp),                                 save :: gfnff_cnthr                     ! Coordination number threshold
    real(dp),                                 save :: gfnff_cnhbthr                   ! Coordination number (HB) threshold
    real(dp),                                 save :: gfnff_cnerfcut_hb               ! Coordination number threshold scale factor for r0 (HB)
    real(dp),                                 save :: gfnff_cnerfcut_cn               ! Coordination number threshold scale factor for r0 (CN)
!
!  Dispersion
!
    real(dp),                                 save :: gfnff_dispthr                   ! Dispersion threshold
    real(dp),                                 save :: t_dispthr                       ! Tapered dispersion threshold
!
!  Bond parameters
!
    real(dp),                                 save :: gfnff_bondscale                 ! Bond scale factor
!
!  Repulsion parameters
!
    real(dp),                                 save :: gfnff_repscale_b                ! Repulsion scale factor
    real(dp),                                 save :: gfnff_repscale_n                ! Repulsion scale factor
    real(dp),                                 save :: gfnff_repscale_13               ! Repulsion scale factor for 1-3 interactions
    real(dp),                                 save :: gfnff_repscale_14               ! Repulsion scale factor for 1-4 interactions
    real(dp),                                 save :: gfnff_repthr                    ! Repulsion threshold
    real(dp),                                 save :: t_repthr                        ! Tapered repulsion threshold
    real(dp),    dimension(:),       pointer, save :: gfnff_repulsion_a => null()     ! GFNFF repulsion parameter A
    real(dp),    dimension(:),       pointer, save :: gfnff_repulsion_z => null()     ! GFNFF repulsion parameter Z
    real(dp),    dimension(:),       pointer, save :: gfnff_repulsion_p => null()     ! GFNFF pair-specific repulsion parameter
!
!  Bend parameters
!
    integer(i4),                              save :: max_gfnff_angles = 0            ! Maximum number of angles
    integer(i4),                              save :: n_gfnff_angles = 0              ! Number of angles for bends
    integer(i4), dimension(:,:),     pointer, save :: n_gfnff_angleptr => null()      ! Pointer to atoms in angle
    real(dp),                                 save :: gfnff_angle_damp                ! Angle damping parameter
    real(dp),    dimension(:,:),     pointer, save :: par_gfnff_angle => null()       ! Bending parameters
!
!  Torsion parameters
!
    integer(i4),                              save :: max_gfnff_torsions = 0          ! Maximum number of torsion angles
    integer(i4),                              save :: n_gfnff_torsions = 0            ! Number of torsion angles
    integer(i4), dimension(:,:),     pointer, save :: n_gfnff_torsionptr => null()    ! Pointer to atoms in torsion
    real(dp),                                 save :: gfnff_torsion_damp              ! Torsion damping parameter
    real(dp),    dimension(:,:),     pointer, save :: par_gfnff_torsion => null()     ! Torsion parameters
!
!  Hydrogen bonding lists
!
    integer(i4),                              save :: maxbond_hb_Bn = 1               ! Maximum for value of bond_hb_Bn
    integer(i4),                              save :: maxbond_hb_nr = 0               ! Maximum for value of bond_hb_nr
    integer(i4),                              save :: nbond_hb_nr                     ! bond_hb_nr
    integer(i4),                              save :: nhb1                            ! nhb1
    integer(i4),                              save :: nhb2                            ! nhb2
    integer(i4),                              save :: nhb3                            ! nhb3
    integer(i4), dimension(:,:),     pointer, save :: nbond_hb_AH => null()           ! bond_hb_AH
    integer(i4), dimension(:),       pointer, save :: nbond_hb_Bn => null()           ! bond_hb_Bn
    integer(i4), dimension(:,:),     pointer, save :: nbond_hb_B => null()            ! bond_hb_B
!
!  Hydrogen bond - pre final search data
!
    integer(i4),                              save :: maxathbH = 1
    integer(i4),                              save :: maxatxbAB = 1
    integer(i4),                              save :: n_gfnff_hb_H = 0
    integer(i4),                              save :: n_gfnff_hb_AB = 0               ! Number of atoms that could be A or B for hydrogen bond
    integer(i4),                              save :: n_gfnff_xb_AB = 0               ! Number of atoms that could be A or B for halogen bond
    integer(i4), dimension(:),       pointer, save :: n_gfnff_hb_ABptr                ! Pointer to atoms that could be A or B for hydrogen bond
    integer(i4), dimension(:,:),     pointer, save :: n_gfnff_xb_ABptr                ! Pointer to atoms that could be A or B for halogen bond
    integer(i4), dimension(:),       pointer, save :: n_gfnff_hb_Hptr                 ! Pointer to atoms that could be H for hydrogen bond
    integer(i4), dimension(:),       pointer, save :: n_gfnff_hb_Hrptr                ! Reverse pointer to atoms that could be H for hydrogen bond
!
!  Hydrogen/halogen bonding parameters
!
    real(dp),                                 save :: gfnff_bend_hb                   ! Hydrogen bonding parameter
    real(dp),                                 save :: gfnff_tors_hb                   ! Hydrogen bonding parameter
    real(dp),                                 save :: gfnff_hbabmix                   ! Hydrogen bonding parameter
    real(dp),                                 save :: gfnff_hb_alp                    ! Hydrogen bonding cutoff scaling
    real(dp),                                 save :: gfnff_hb_a_cut                  ! Hydrogen bonding cutoff
    real(dp),                                 save :: gfnff_hb_long_cut               ! Hydrogen bonding cutoff
    ...
    real(dp),    dimension(:),       pointer, save :: gfnff_hb_acid => null()         ! Hydrogen bonding parameter
    real(dp),    dimension(:),       pointer, save :: gfnff_hb_base => null()         ! Hydrogen bonding parameter
    real(dp),                                 save :: gfnff_xb_a_cut                  ! Halogen bonding cutoff
    real(dp),                                 save :: gfnff_xb_short_cut              ! Halogen bonding cutoff
    real(dp),                                 save :: gfnff_xb_long_cut               ! Halogen bonding cutoff
    real(dp),    dimension(:,:),     pointer, save :: gfnff_xb_ABq => null()          ! Halogen bonding charge-scaling parameter
    real(dp),    dimension(:),       pointer, save :: gfnff_xb_scale => null()        ! Halogen bonding element-specific scaling parameter
  end module gulp_gfnff
!
!  Fitting data
!
  module fitting
    ...
    character(len=15),                    save :: fitlabel(30,65,10)
    character(len=10),                    save :: fitlabelunits(30,65,10)
    integer(i4),                          save :: nfitlabel(65,10)
...
!
  module four
    ...
    logical,          dimension(:),     pointer, save :: ltib4body => null()         ! If true then lambda is applied in backward direction for TI
    logical,          dimension(:),     pointer, save :: ltif4body => null()         ! If true then lambda is applied in forward direction for TI
...
!
  module frequencies
    ...
    complex(dpc), dimension(:),         pointer, save :: aphase => null()    ! Array to store atomic phases for each k point
    complex(dpc), dimension(:),         pointer, save :: fphase => null()    ! Array to store mode phases for each k point
...
!
  module ksample
    ...
    logical,     dimension(:),   pointer, save :: lkptgammaonlycfg => null()    ! If true this configuration only has the gamma point
...
!
  module library
    ...
    character(len=16),            dimension(:), pointer, save :: lib2ndspec => null()
    logical,                      dimension(:), pointer, save :: llib2ndspec => null()
...
!
  module maths
    use datatypes
    integer(i4),                              save :: elpatype = 1                    ! Selects the ELPA algorithm if appropriate
    integer(i4),                              save :: qsolver = 1                     ! Selects the iterative charge solver
    logical,                                  save :: leispack_eigensolve = .false.
    logical,                                  save :: lelpa = .false.                 ! Selects the ELPA maths library if available
    logical,                                  save :: ldivide_and_conquer = .false.
    logical,                                  save :: leig_2stage = .false.
    logical,                                  save :: leig_mrrr = .false.
    ...
    logical,                                  save :: lblock2D = .false.              ! Selects whether transform to 2D block cyclic in parallel for maths
    real(dp),                                 save :: eigtol                          ! Tolerance on eigenvalues if needed
  end module maths
!
!  Monte Carlo data
!
  module montecarlo
    ...
    integer(i4), dimension(:,:),   pointer, save :: nptrtranable => null()
    ...
    integer(i4), dimension(:,:),   pointer, save :: nmctrannat => null()
    integer(i4), dimension(:,:),   pointer, save :: nmctranspec => null()
    integer(i4), dimension(:,:),   pointer, save :: nmctrantype => null()
    integer(i4), dimension(:),     pointer, save :: nswapable => null()
    integer(i4), dimension(:),     pointer, save :: ntranable => null()
    ...
    real(dp),    dimension(:),     pointer, save :: ptran => null()
...
!
  module mdlogic
    ...
    logical,                                save :: llist6
    logical,                                save :: lmd
    logical,                                save :: lmd_inc_com
    logical,                                save :: lmd_inc_rot
  end module mdlogic
!
...
!
  module observables
    ...
    integer(i4), dimension(:),     pointer, save :: nobstyle => null()     ! Style of observable: 0 = standard, 1 = + only, -1 = - only
    ...
    character(len=13),                      save :: nameobs(35)
    ...
    data nameobs/'Energy       ','Derivative   ','Elastic Const', &
                 ...
                 'Stress       ','Free energy  ','Free ener eqp', &
                 'Reaction FE  ','Reactn FE Eq '/
...
!
  module parallel
    ...
    logical,                              save :: lscaleMPI                   ! If true, then algorithms are chosen that suit more cores
...
!
  module phonout
    ...
    logical,                              save :: leigenvectors_original      ! If true then keep eigenvectors mass-weighted
    logical,                              save :: leigenvectors_phase         ! If true then phase eigenvectors
...
!
  module potentialnames
    use datatypes
    integer(i4),                          save :: max2pottype = 65
    integer(i4),                          save :: max3pottype = 26
    integer(i4),                          save :: max4pottype = 17
    character(len=13),                    save :: name2pot(65)             ! Names of twobody potentials
...
!
  module reaxFFdata
    ...
    logical,                                       save :: lreaxff_ffield = .false.
    character(len=maxwordlength),                  save :: reaxff_ffield
    ...
    logical,                                       save :: lreaxFF_ebond = .true.         ! Flag to control whether bond energy is computed
    logical,                                       save :: lreaxFF_ebpen = .true.         ! Flag to control whether bpen energy is computed
    logical,                                       save :: lreaxFF_elone = .true.         ! Flag to control whether lone pair energy is computed
    logical,                                       save :: lreaxFF_eover = .true.         ! Flag to control whether over energy is computed
    logical,                                       save :: lreaxFF_eunder = .true.        ! Flag to control whether under energy is computed
    logical,                                       save :: lreaxFF_eval = .true.          ! Flag to control whether valence energy is computed
    logical,                                       save :: lreaxFF_epen = .true.          ! Flag to control whether penalty energy is computed
    logical,                                       save :: lreaxFF_ecoa = .true.          ! Flag to control whether coa energy is computed
    logical,                                       save :: lreaxFF_etors = .true.         ! Flag to control whether torsion energy is computed
    logical,                                       save :: lreaxFF_econj = .true.         ! Flag to control whether conjugation energy is computed
    logical,                                       save :: lreaxFF_ehb = .true.           ! Flag to control whether hydrogen bond energy is computed
    logical,                                       save :: lreaxFF_evdw = .true.          ! Flag to control whether VDW energy is computed
    logical,                                       save :: lreaxFF_ecoul = .true.         ! Flag to control whether Coulomb energy is computed
...
!
  module six
    ...
    logical,          dimension(:),   pointer, save :: ltib6body => null()         ! If true then lambda is applied in backward direction for TI
    logical,          dimension(:),   pointer, save :: ltif6body => null()         ! If true then lambda is applied in forward direction for TI
...
!
  module spatial
    ...
    logical,                              save :: lupdateinbox = .true.      ! If true then update coordinates in the box in energy
...
!
  module species
    ...
    logical,                                    save :: ltianyqspec = .false.      ! If true there is a species whose charge will change during TI
    ...
    logical,           dimension(:),   pointer, save :: ltibqspec => null()        ! If true then lambda is applied in backward direction for TI to species charge
    logical,           dimension(:),   pointer, save :: ltifqspec => null()        ! If true then lambda is applied in forward direction for TI to species charge
...
!
    real(dp),          dimension(:),   pointer, save :: aaclp_b_spec => null()     ! AA-CLP B coefficient for species
    real(dp),          dimension(:),   pointer, save :: aaclp_h_spec => null()     ! AA-CLP H coefficient for species
    real(dp),          dimension(:),   pointer, save :: aaclp_ip_spec => null()    ! AA-CLP ionisation potential for species
    real(dp),          dimension(:),   pointer, save :: aaclp_pol_spec => null()   ! AA-CLP polarisability for species
...
!
  module m_three
    ...
    logical,          dimension(:),     pointer, save :: ltib3body => null()         ! If true then lambda is applied in backward direction for TI
    logical,          dimension(:),     pointer, save :: ltif3body => null()         ! If true then lambda is applied in forward direction for TI
...
!
!  Thermodynamic integration data
!
  module m_ti
    use datatypes
    integer(i4),                                 save :: maxlambda = 1            ! Maximum number of lambda steps + 1
    integer(i4),                                 save :: nlambda = -1             ! Number of lambda steps
    integer(i4),                                 save :: nlambdanow = 1           ! Current lambda step
    integer(i4),                                 save :: ntistep = 1              ! Number of steps for each lambda change in TI
    integer(i4),                                 save :: ntinow = 0               ! Number of steps since last lambda change in TI
    logical,                                     save :: ltirun = .false.         ! If true then this is a TI run
    logical,                                     save :: lti = .false.            ! If true then TI to be performed now
    real(dp),         dimension(:),     pointer, save :: dUdlambda => null()      ! Derivative of energy w.r.t. lambda for each step
    real(dp),                                    save :: lambda_initial = 0.0_dp  ! Initial value of lambda
    real(dp),                                    save :: lambda_final = 1.0_dp    ! Final value of lambda
    real(dp),                                    save :: lambda_step = 0.05_dp    ! Step size between lambda values
    real(dp),                                    save :: lambda = 1.0_dp          ! The current value of lambda
    real(dp),                                    save :: rtistep                  ! The inverse of ntistep for scaling lambda additions
...
!
  module times
    ...
    real(dp), save :: tgfnff = 0.0_dp
    real(dp), save :: tgfnffsetup = 0.0_dp
...
!
  module two
    ...
    integer(i4),      dimension(:),   pointer, save :: nspecptr1 => null()         ! Species number of species for atom 1
    integer(i4),      dimension(:),   pointer, save :: nspecptr2 => null()         ! Species number of species for atom 2
    ...
    logical,          dimension(:),   pointer, save :: ltib2body => null()         ! If true then lambda is applied in backward direction for TI
    logical,          dimension(:),   pointer, save :: ltif2body => null()         ! If true then lambda is applied in forward direction for TI
