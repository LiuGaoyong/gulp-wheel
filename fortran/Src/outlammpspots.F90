...
!
!   3/11 Created
!  10/12 deriv correct for the fact that GULP uses 1/r dE/dr
...
!   4/23 lc6 turned off during output of potentials since this needs to 
!        be handled separately
!   6/23 Actual species numbers passed to twobody
!
!  Conditions of use:
!
...
!  Local variables
!
  character(len=maxwordlength) :: unique_label
...
!
!  Save value of lc6 and then turn off here
!
  lsavec6 = lc6
  lc6 = .false.
!
...
!
    unique_label = ' '
    ind = index(name2pot(nptype(n)),' ')
    nlen = ind - 1
    unique_label(1:nlen) = name2pot(nptype(n))(1:nlen)
    nlen = nlen + 1
...
!
    write(iout,*)
    write(iout,'(a)') trim(unique_label)
...
!
      call twobody1(eatom,ereal,ec6,.true.,.false.,.false.,1_i4,1_i4,r,x1,y1,z1,d0i,d0j,deriv, &
                    ...
                    0.0_dp,.false.,0_i4,angstoev,1.0_dp,1.0_dp,0.0_dp,rtrm,rtrm2,rtrm3,rtrm32,sctrm1, &
                    sctrm2,0.0_dp,0.0_dp,ns1,ns2,.false.,.false.,.false.,.false.,.false.,.false.,0_i4, &
                    0_i4,.false.,.true.,d1i,d1j,d2i2,d2ij,d2j2)
!
!  Write out result - note that table requires force, not derivative, and so sign of deriv is changed
!
      write(iout,'(i10,2x,3(g20.10,2x))') np,r,eatom,-deriv*r
!
