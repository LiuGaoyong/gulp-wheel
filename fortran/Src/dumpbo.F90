...
!   8/12 Check on nfpot during setting of flags for reaxff2 potentials removed
!   8/12 Dumping of reaxff_gamma corrected since elements were coming out wrong
!   9/12 Dumping of reaxff2_bo corrected for non-over & non-bo13 case
!  12/12 reaxff_q0 option added 
!  11/13 Error in referencing of atom numbers and types for nCCspec loop corrected
!   1/14 lBOzrl arrays added
!   1/14 EDIP2p, lEDIP3orig and lEDIPpi added
!   7/14 lEDIP3mod added
!   9/15 BOccoeffA/R changed to 2-D arrays
!   9/15 Fitting parameter numbers changed
!  12/15 nBOtapertype added
!  12/15 Format statements corrected for edip_twobody & threebody
!   3/16 BO coordination potential added 
!   3/16 BO coordination switch function parameters added 
!   3/16 BO coordination tolerance added
...
!   8/20 Correction to handling of BO h fitting flag
!   9/20 Correction to output of reaxff2_morse when some parameters are less than zero
!  11/20 Tersoff reorganised
!   6/21 lBOgik arrays added
!   6/21 Atom2 for borepulsive and boattractive now defaults to wildcard
!   7/21 Bond valence added
!   9/21 Symmetric flags added for bond order potentials
!  10/22 boonebody option was missing and is now added
!   4/23 reaxff options not output if a reaxff format library was used
!  11/23 Correction to borep and boatt options for core/shell type
!
!  Conditions of use:
!
...
!
  ...
  integer(i4)                                    :: ntype3
  integer(i4)                                    :: ntype4
  integer(i4)                                    :: nvalbondloc
  integer(i4)                                    :: nvalenerloc
  integer(i4)                                    :: status
  logical                                        :: first
  logical                                        :: l2atoms
  real(dp)                                       :: sum
#ifdef TRACE
  call trace_in('dumpbo')
#endif
!
  if (nlib.gt.0) then
    if (.not.llibdump) then
      ...
      nvalbondloc = nlibvalbond
      nvalenerloc = nlibvalener
    else
      nbopotloc = nbopot 
      nboAloc = nboA
      nboOloc = nboO
      ...
      nvalbondloc = nvalbond
      nvalenerloc = nvalener
    endif
  else
    nbopotloc = nbopot
    nboAloc = nboA
...
!**********************************
  if (nboOloc.gt.0) then
    if (lfit) then
      allocate(itmp(4*nboO),stat=status)
      ...
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).le.8.and.nfvar(i).gt.4) then
            itmp(4*(nfpot(i)-1)+nfvar(i)-4) = 1
          endif
        endif
      enddo
    endif
    write(iout,'(''boonebody'')')
    do i = 1,nboOloc
!
...
!
      if (lfit) then
        write(iout,'(a5,1x,a4,1x,4(g12.6,1x),4i2)') lab1,ctmp(1),BOecoeffR(i),BOecoeffA(i),BOncoeffR(i),BOncoeffA(i), &
          (itmp(4*(i-1)+j),j=1,4)
      else
        write(iout,'(a5,1x,a4,1x,4(g12.6,1x))') lab1,ctmp(1),BOecoeffR(i),BOecoeffA(i),BOncoeffR(i),BOncoeffA(i)
      endif
    enddo
    if (lfit) then
    ...
    if (lfit) then
      ...
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          ...
          elseif (nfvar(i).eq.25.or.nfvar(i).eq.26) then
            itmp(4*(nfpot(i)-1)+nfvar(i)-24) = 1
          endif
        endif
      enddo
    endif
...
!
      if (BOcombi(i)) then
        if (nBOtapertype(i).eq.2) then
          write(iout,'(''botwobody combine cosine '')')
        elseif (nBOtapertype(i).eq.3) then
          write(iout,'(''botwobody combine murty '')')
        else
        ...
        if (nBOtapertype(i).eq.2) then
          write(iout,'(''botwobody cosine '')')
        elseif (nBOtapertype(i).eq.3) then
          write(iout,'(''botwobody murty '')')
        else
...
!***************************
  if (nboRloc.gt.0) then
    if (lfit) then
      allocate(itmp(8*nboR),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,8*nboR
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).ge.9.and.nfvar(i).le.16) then
            if (nfvar(i).eq.16) then
              itmp(8*(nfpot(i)-1)+5) = 1
            else
              itmp(8*(nfpot(i)-1)+nfvar(i)-8) = 1
            endif
          endif
        endif
      enddo
...
!
      if (nBOtypeR(i).eq.1) then
        if (lBOsymR(i)) then
          if (lBOzrlR(i)) then
            write(iout,'(''borepulsive sym zrl '')')
          else
            write(iout,'(''borepulsive sym '')')
          endif
        else
          if (lBOzrlR(i)) then
            write(iout,'(''borepulsive sym zrl '')')
        ...
        if (lfit) then
          if (l2atoms) then
            write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),2i2)')  &
              lab1,ctmp(1),lab2,ctmp(2),nint(BOmcoeffR(i)),BOocoeffR(i),BOlcoeffR(i),(itmp(9*(i-1)+j),j=1,2)
          else
            write(iout,'(a5,1x,a4,1x,i3,1x,2(g12.6,1x),2i2)')  &
              lab1,ctmp(1),nint(BOmcoeffR(i)),BOocoeffR(i),BOlcoeffR(i),(itmp(9*(i-1)+j),j=1,2)
          endif
        else
          if (l2atoms) then
            write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x))')  &
              lab1,ctmp(1),lab2,ctmp(2),nint(BOmcoeffR(i)),BOncoeffR(i),BOlcoeffR(i)
          else
            write(iout,'(a5,1x,a4,1x,i3,1x,2(g12.6,1x))')  &
              lab1,ctmp(1),nint(BOmcoeffR(i)),BOncoeffR(i),BOlcoeffR(i)
          endif
        endif
      elseif (nBOtypeR(i).eq.2) then
        if (lBOsymR(i)) then
          if (lBOzrlR(i)) then
            if (lBOgikR(i)) then
              write(iout,'(''borepulsive theta sym gik zrl'')')
            else
              write(iout,'(''borepulsive theta sym zrl'')')
            endif
          else
            if (lBOgikR(i)) then
              write(iout,'(''borepulsive theta sym gik'')')
            else
              write(iout,'(''borepulsive theta sym '')')
            endif
          endif
        else
          if (lBOzrlR(i)) then
            if (lBOgikR(i)) then
              write(iout,'(''borepulsive theta gik zrl'')')
            else
              write(iout,'(''borepulsive theta zrl'')')
            endif
          else
            if (lBOgikR(i)) then
              write(iout,'(''borepulsive theta gik'')')
            else
        ...
        if (l2atoms) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),''&'')')  &
            lab1,ctmp(1),lab2,ctmp(2),nint(BOmcoeffR(i)),BOocoeffR(i),BOlcoeffR(i)
        else
          write(iout,'(a5,1x,a4,1x,i3,1x,2(g12.6,1x),''&'')')  &
            lab1,ctmp(1),nint(BOmcoeffR(i)),BOocoeffR(i),BOlcoeffR(i)
        endif
        if (lfit) then
          write(iout,'(2x,3(g12.6,1x),5i2)') BOccoeffR(1,i),BOccoeffR(2,i),BOhcoeffR(i),(itmp(9*(i-1)+j),j=1,5)
        else
          write(iout,'(2x,3(g12.6,1x))') BOccoeffR(1,i),BOccoeffR(2,i),BOhcoeffR(i)
        endif
      elseif (nBOtypeR(i).eq.3) then
        if (lBOsymR(i)) then
          if (lBOzrlR(i)) then
            write(iout,'(''borepulsive kumagai sym zrl'')')
          else
            write(iout,'(''borepulsive kumagai sym '')')
          endif
        else
          if (lBOzrlR(i)) then
            write(iout,'(''borepulsive kumagai zrl'')')
          else
            write(iout,'(''borepulsive kumagai'')')
          endif
        endif
        if (l2atoms) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),''&'')')  &
        ...
        if (lfit) then
          write(iout,'(2x,6(g12.6,1x),''&'')') (BOccoeffR(j,i),j=1,5),BOhcoeffR(i)
          write(iout,'(2x,8i2)') (itmp(9*(i-1)+j),j=1,8)
        else
          write(iout,'(2x,6(g12.6,1x))') (BOccoeffR(j,i),j=1,5),BOhcoeffR(i)
        endif
      elseif (nBOtypeR(i).eq.4) then
        if (lBOsymR(i)) then
          if (lBOzrlR(i)) then
            write(iout,'(''borepulsive mmp sym zrl'')')
          else
            write(iout,'(''borepulsive mmp sym '')')
          endif
        else
          if (lBOzrlR(i)) then
            write(iout,'(''borepulsive mmp zrl'')')
          else
            write(iout,'(''borepulsive mmp '')')
          endif
        endif
        if (l2atoms) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),''&'')')  &
        ...
        if (lfit) then
          write(iout,'(2x,5(g12.6,1x),''&'')') (BOccoeffR(j,i),j=1,5)
          write(iout,'(2x,7i2)') (itmp(8*(i-1)+j),j=1,7)
        else
          write(iout,'(2x,5(g12.6,1x))') (BOccoeffR(j,i),j=1,5)
        endif
      endif
...
!****************************
  if (nboAloc.gt.0) then
    if (lfit) then
      allocate(itmp(8*nboA),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,8*nboA
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).ge.17.and.nfvar(i).le.24) then
            if (nfvar(i).eq.24) then
              itmp(8*(nfpot(i)-1)+5) = 1
            else
              itmp(8*(nfpot(i)-1)+nfvar(i)-16) = 1
            endif
          endif
        endif
      enddo
...
!
      if (nBOtypeA(i).eq.1) then
        if (lBOsymA(i)) then
          if (lBOzrlA(i)) then
            write(iout,'(''boattractive sym zrl'')')
          else
            write(iout,'(''boattractive sym '')')
          endif
        else
          if (lBOzrlA(i)) then
            write(iout,'(''boattractive zrl'')')
          else
        ...
        if (lfit) then
          if (l2atoms) then
            write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),2i2)')  &
              lab1,ctmp(1),lab2,ctmp(2),nint(BOmcoeffA(i)),BOocoeffA(i),BOlcoeffA(i),(itmp(8*(i-1)+j),j=1,2)
          else
            write(iout,'(a5,1x,a4,1x,i3,1x,2(g12.6,1x),2i2)')  &
              lab1,ctmp(1),nint(BOmcoeffA(i)),BOocoeffA(i),BOlcoeffA(i),(itmp(8*(i-1)+j),j=1,2)
          endif
        else
          if (l2atoms) then
            write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x))')  &
              lab1,ctmp(1),lab2,ctmp(2),nint(BOmcoeffA(i)),BOocoeffA(i),BOlcoeffA(i)
          else
            write(iout,'(a5,1x,a4,1x,i3,1x,2(g12.6,1x))')  &
              lab1,ctmp(1),nint(BOmcoeffA(i)),BOocoeffA(i),BOlcoeffA(i)
          endif
        endif
      elseif (nBOtypeA(i).eq.2) then
        if (lBOsymA(i)) then
          if (lBOzrlA(i)) then
            if (lBOgikA(i)) then
              write(iout,'(''boattractive theta sym gik zrl'')')
            else
              write(iout,'(''boattractive theta sym zrl'')')
            endif
          else
            if (lBOgikA(i)) then
              write(iout,'(''boattractive theta sym gik'')')
            else
              write(iout,'(''boattractive theta sym '')')
            endif
          endif
        else
          if (lBOzrlA(i)) then
            if (lBOgikA(i)) then
              write(iout,'(''boattractive theta gik zrl'')')
            else
              write(iout,'(''boattractive theta zrl'')')
            endif
          else
            if (lBOgikA(i)) then
              write(iout,'(''boattractive theta gik'')')
            else
        ...
        if (lfit) then
          write(iout,'(2x,3(g12.6,1x),5i2)') BOccoeffA(1,i),BOccoeffA(2,i),BOhcoeffA(i),(itmp(8*(i-1)+j),j=1,5)
        else
          write(iout,'(2x,3(g12.6,1x))') BOccoeffA(1,i),BOccoeffA(2,i),BOhcoeffA(i)
        endif
      elseif (nBOtypeA(i).eq.3) then
        if (lBOsymA(i)) then
          if (lBOzrlA(i)) then
            write(iout,'(''boattractive kumagai sym zrl'')')
          else
            write(iout,'(''boattractive kumagai sym '')')
          endif
        else
          if (lBOzrlA(i)) then
            write(iout,'(''boattractive kumagai zrl'')')
          else
            write(iout,'(''boattractive kumagai'')')
          endif
        endif
        if (l2atoms) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),''&'')')  &
        ...
        if (lfit) then
          write(iout,'(2x,6(g12.6,1x),''&'')') (BOccoeffA(j,i),j=1,5),BOhcoeffA(i)
          write(iout,'(2x,8i2)') (itmp(8*(i-1)+j),j=1,8)
        else
          write(iout,'(2x,6(g12.6,1x))') (BOccoeffA(j,i),j=1,5),BOhcoeffA(i)
        endif
      elseif (nBOtypeA(i).eq.4) then
        if (lBOsymA(i)) then
          if (lBOzrlA(i)) then
            write(iout,'(''boattractive mmp sym zrl'')')
          else
            write(iout,'(''boattractive mmp sym '')')
          endif
        else
          if (lBOzrlA(i)) then
            write(iout,'(''boattractive mmp zrl'')')
          else
            write(iout,'(''boattractive mmp'')')
          endif
        endif
        if (l2atoms) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,i3,1x,2(g12.6,1x),''&'')')  &
        ...
        if (lfit) then
          write(iout,'(2x,5(g12.6,1x),''&'')') (BOccoeffA(j,i),j=1,5)
          write(iout,'(2x,7i2)') (itmp(8*(i-1)+j),j=1,7)
        else
          write(iout,'(2x,5(g12.6,1x))') (BOccoeffA(j,i),j=1,5)
        endif
...
!******************************
!  Bond order - coordination  *
!******************************
  if (nboZloc.gt.0) then
    if (lfit) then
      allocate(itmp(4*nboZ),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,4*nboA
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).ge.27.and.nfvar(i).le.30) then
            itmp(4*(nfpot(i)-1)+nfvar(i)-26) = 1
          endif
        endif
      enddo
    endif
...
!
      write(iout,'(''bocoordination '')')
      if (lfit) then
        write(iout,'(a5,1x,a4,1x,4(g12.6,1x),4i2)')  &
          lab1,ctmp(1),BOccoeffZ(1,i),BOccoeffZ(2,i),BOzcoeffZ(i),BOecoeffZ(i),(itmp(4*(i-1)+j),j=1,4)
      else
        write(iout,'(a5,1x,a4,1x,4(g12.6,1x))')  &
          lab1,ctmp(1),BOccoeffZ(1,i),BOccoeffZ(2,i),BOzcoeffZ(i),BOecoeffZ(i)
      endif
    enddo
    if (lfit) then
      deallocate(itmp,stat=status)
...
!*************************************************
!  Bond order - coordination switching function  *
!*************************************************
  if (lBOcnzin) then
    write(iout,'(''bocnswitch '',f12.8,1x,f12.8)') BOcnzb,BOcnzt
  endif
!****************************************
!  Bond order - coordination tolerance  *
!****************************************
  if (BOcntol.ne.0.000001_dp) then
    write(iout,'(''bocntolerance '',f16.14)') BOcntol
  endif
!*************************************
...
!
  ...
  if (.not.lreaxff_ffield) then
!###################################################################
!  Stop output of ReaxFF options if a ReaxFF format file was used  #
!###################################################################
...
!******************************
    ...
    if (lreaxFFlpsmooth.and.lreaxFFovsmooth) then
      if (reaxFFlpsmooth.ne.80.0_dp) then
        write(iout,'(''reaxFFsmooth both '',f12.6)') reaxFFlpsmooth
      else
        write(iout,'(''reaxFFsmooth both '')')
      endif
    elseif (lreaxFFlpsmooth) then
      if (reaxFFlpsmooth.ne.80.0_dp) then
        write(iout,'(''reaxFFsmooth lone '',f12.6)') reaxFFlpsmooth
      else
        write(iout,'(''reaxFFsmooth lone '')')
      endif
    elseif (lreaxFFovsmooth) then
      if (reaxFFlpsmooth.ne.80.0_dp) then
        write(iout,'(''reaxFFsmooth over '',f12.6)') reaxFFlpsmooth
      else
        write(iout,'(''reaxFFsmooth over '')')
      endif
    endif
...
!
      if (lfit) then
        allocate(itmp2(10,nreaxFFspecloc),stat=status)
        if (status/=0) call outofmemory('dumpbo','itmp2')
        itmp2(1:8,1:nreaxFFspecloc) = 0
        do i = 1,nfit
          if (nftyp(i).eq.8) then
...
!           
      ...
      do i = 1,nreaxFFspecloc
        ...
        if (lfit) then
          write(iout,'(a5,1x,a4,1x,f12.6,1x,i1)') lab1,ctmp(1),reaxFFchi(i),itmp2(1,i)
        else
          write(iout,'(a5,1x,a4,1x,f12.6)') lab1,ctmp(1),reaxFFchi(i)
        endif
      enddo
!
...
!
      ...
      do i = 1,nreaxFFspecloc
        ...
        if (lfit) then
          write(iout,'(a5,1x,a4,1x,f12.6,1x,i1)') lab1,ctmp(1),reaxFFmu(i),itmp2(2,i)
        else
          write(iout,'(a5,1x,a4,1x,f12.6)') lab1,ctmp(1),reaxFFmu(i)
        endif
      enddo
!
!  ReaxFF EEM q0 parameters for species present
!
      first = .true.
      do i = 1,nreaxFFspecloc
        nat1 = natreaxFFspec(i)
        ntype1 = ntypreaxFFspec(i)
        if (abs(reaxFFq0(i)).gt.1.0d-12) then
          if (first) then
            write(iout,'(''reaxff_q0'')')
            first = .false.
          endif
          call label(nat1,ntype1,lab1)
          if (nat1.gt.maxele) then
          ...
          write(iout,'(a5,1x,a4,1x,f12.6)') lab1,ctmp(1),reaxFFq0(i)
        endif
      enddo
!   
...
!           
      ...
      do i = 1,nreaxFFspecloc
        nat1 = natreaxFFspec(i)
        ntype1 = ntypreaxFFspec(i)
        if (abs(reaxFFshell(1,i)).gt.1.0d-12) then
          call label(nat1,ntype1,lab1)
          if (nat1.gt.maxele) then
            ctmp(1) = 'shel'
          else 
          ...
          if (lfit) then
            write(iout,'(a5,1x,a4,3(1x,f12.6),1x,3i2)') lab1,ctmp(1),(reaxFFshell(j,i),j=1,3),(itmp2(k,i),k=4,6)
          else
            write(iout,'(a5,1x,a4,3(1x,f12.6))') lab1,ctmp(1),(reaxFFshell(j,i),j=1,3)
          endif
        endif
      enddo
...
!           
        ...
        do i = 1,nreaxFFspecloc
          ...
          if (lfit) then
            write(iout,'(a5,1x,a4,1x,f12.6,1x,i1)') lab1,ctmp(1),reaxFFgamma(i),itmp2(2,i)
          else
...
!
      ...
      do i = 1,nreaxFFspecloc
        ...
        if (nat1.le.10.and..not.lreaxFFunder(i)) then
          if (first) then
            first = .false.
            write(iout,'(''reaxff1_include_under'')')
          endif
          write(iout,'(a5,1x,a4,1x,''0'')') lab1,ctmp(1)
        elseif (nat1.gt.10.and.lreaxFFunder(i)) then
          if (first) then
            first = .false.
            write(iout,'(''reaxff1_include_under'')')
          endif
          write(iout,'(a5,1x,a4,1x,''1'')') lab1,ctmp(1)
        endif
      enddo
      if (lfit) then
        deallocate(itmp2,stat=status)
...
!##############################################################################
!  End of stopping output of ReaxFF options if a ReaxFF format file was used  #
!##############################################################################
  endif
!********************
!  EDIP potentials  *
...
!       
    ...
    if (lfit) then
      allocate(itmp3(7,2,nEDIPspec2),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp3')
      itmp3(1:7,1:2,1:nEDIPspec2) = 0
      do i = 1,nfit
...
!       
          ...
          if (lEDIPpi(ind)) then
            ...
            if (lfit) then
              write(iout,'(2(a5,1x,a4,1x),3(f12.6,1x),i2)') &
                lab1,ctmp(1),lab2,ctmp(2),EDIPalpha(ind),EDIPflow(ind),EDIPfhigh(ind),itmp3(1,1,ind)
            else
              write(iout,'(2(a5,1x,a4,1x),3(f12.6,1x))') &
                lab1,ctmp(1),lab2,ctmp(2),EDIPalpha(ind),EDIPflow(ind),EDIPfhigh(ind)
            endif
...
!
          ...
          write(iout,'(2(a5,1x,a4,1x),4(f12.6,1x),''&'')') &
            lab1,ctmp(1),lab2,ctmp(2),EDIP2epsilon(ind),EDIP2B(ind),EDIP2p(ind),EDIP2beta(ind)
          if (lfit) then
            write(iout,'(12x,3(f12.6,1x),7i2)') EDIP2sigma(ind),EDIP2a(ind),EDIP2aprime(ind),(itmp3(l,2,ind),l=1,7)
          else
...
!
            ...
            if (lEDIP3mod(ind,i)) then
              write(iout,'(''edip_threebody modified '')')
              write(iout,'(3(a5,1x,a4,1x),3(f10.5,1x),''&'')') &
              ...
              if (lEDIP3orig(ind,i)) then
                write(iout,'(''edip_threebody original '')')
              else
              ...
              if (lfit) then
                write(iout,'(3(a5,1x,a4,1x),3(f10.5,1x),''&'')') &
                  lab1,ctmp(1),lab2,ctmp(2),lab3,ctmp(3),EDIP3lambda0(ind,i),EDIP3lambdap(ind,i),EDIP3Z0(ind,i)
                  write(iout,'(22x,3(f10.5,1x),''&'')') EDIP3gamma0(ind,i),EDIP3gammap(ind,i),EDIP3q(ind,i)
                  write(iout,'(22x,6i2)') (itmp3(m,ind,i),m=1,6)
              else
                write(iout,'(3(a5,1x,a4,1x),3(f10.5,1x),''&'')') &
                  lab1,ctmp(1),lab2,ctmp(2),lab3,ctmp(3),EDIP3lambda0(ind,i),EDIP3lambdap(ind,i),EDIP3Z0(ind,i)
                  write(iout,'(22x,3(f10.5,1x))') EDIP3gamma0(ind,i),EDIP3gammap(ind,i),EDIP3q(ind,i)
...
!******************************
!  Bond valence energy terms  *
!******************************
  if (nvalenerloc.gt.0) then
    if (lfit) then
      allocate(itmp(4*nvalener),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,4*nvalener
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.10) then
          if (nfvar(i).le.7.and.nfvar(i).gt.4) then
            itmp(4*(nfpot(i)-1)+nfvar(i)-3) = 1
          endif
        endif
      enddo
    endif
...
!
      if (lfit) then
        write(iout,'(a5,1x,a4,1x,4(g12.6,1x),4i2)') lab1,ctmp(1),VBparE(1,i),VBparE(2,i),VBparE(3,i),VBparE(4,i), &
          (itmp(4*(i-1)+j),j=1,4)
      else
        write(iout,'(a5,1x,a4,1x,4(g12.6,1x))') lab1,ctmp(1),VBparE(1,i),VBparE(2,i),VBparE(3,i),VBparE(4,i)
      endif
    enddo
    if (lfit) then
      deallocate(itmp,stat=status)
...
!***************************
  if (nvalbondloc.gt.0) then
    if (lfit) then
      allocate(itmp(2*nvalbond),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,2*nvalbond
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.10) then
          if (nfvar(i).eq.1) then
            itmp(2*(nfpot(i)-1)+nfvar(i)) = 1
          elseif (nfvar(i).eq.2) then
            itmp(2*(nfpot(i)-1)+nfvar(i)+1) = 1
          elseif (nfvar(i).eq.3) then
            itmp(2*(nfpot(i)-1)+nfvar(i)+1) = 1
          endif
        endif
      enddo
...
!
      if (nvalbondtype(i).eq.2) then
        write(iout,'(''valence_bond power'')')
        write(iout,'(2(a5,1x,a4,1x),2(g12.6,1x),''&'')') lab1,ctmp(1),lab2,ctmp(2),VBparB(1,i),VBparB(2,i)
      elseif (nvalbondtype(i).eq.1) then
        write(iout,'(''valence_bond exp'')')
        write(iout,'(2(a5,1x,a4,1x),2(g12.6,1x),''&'')') lab1,ctmp(1),lab2,ctmp(2),VBparB(1,i),VBparB(3,i)
      endif
      if (lfit) then
        write(iout,'(2x,f8.6,1x,2(f8.4,1x),2i2)') VBwgtB(i),rVBmin(i),rVBmax(i),(itmp(2*(i-1)+j),j=1,2)
      else
        write(iout,'(2x,f8.6,1x,2(f8.4,1x))') VBwgtB(i),rBOmin(i),rBOmax(i)
      endif
    enddo
    if (lfit) then
      deallocate(itmp,stat=status)
