...
!  Set COM coordinates
!
          if (lrigid.and.nmj.gt.0.and.lmolokij) then
...
!
!  Check that one of the atoms is equal to matom
!
                        if (i.ne.matom.and.j.ne.matom.and.k.ne.matom.and. &
                            m.ne.matom.and.l.ne.matom.and.n.ne.matom) cycle nloop
!
                        nn = nat(n)
                        ntypn = nftype(n)
...
!
                          if (lsg1) then
                            call realstrterms(ndim,15_i4,15_i4,xv6,yv6,zv6,xcom,ycom,zcom,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                          endif
!***********************
!  Strain derivatives  *
!***********************
...
!
                            do ivec = 1,15
                              do kl = 1,nstrains
                                ks = nstrptr(kl)
                                rstrd(kl) = rstrd(kl) + e1d(ivec)*dr2ds(ks,ivec)
                              enddo
                            enddo
                          endif
!*************************
