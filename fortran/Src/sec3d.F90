...
!  Radial
!
        if (lbsmat(nrelf2a(node2atom(i)))) then
          do j = 1,numat
            dx = derv2(noff+j,indi+1)
            dy = derv2(noff+j,indi+2)
            dz = derv2(noff+j,indi+3)
...
!
        if (nbsm.gt.0.and.lbsmat(i)) then
          ibs = nofa + i
          do j = 1,nasym
            jj = nrela2f(j)
            neqj = neqv(j)
...
!
    ...
    do i = 1,nrecv
      ...
      call MPI_IRecv(datarecv(1,i),ninternalatm,MPI_double_precision,nrecvnode(i), &
                     nrecvptr(2,i),MPI_Comm_GULP,Request(nlocalcomm),MPIerror)
    enddo
    do i = 1,nsend
      nlocalcomm = nlocalcomm + 1
      call MPI_ISend(datasend(1,i),ninternalatm,MPI_double_precision,nsendnode(i), &
                     nsendptr(2,i),MPI_Comm_GULP,Request(nlocalcomm),MPIerror)
    enddo
!
