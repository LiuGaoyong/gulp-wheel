  subroutine bondorder12d(ebondorder,imode,lgrad1,lgrad2)
!
...
!
!   6/17 Created from bondorder12.f
!   2/18 Trace added
!   7/20 Separate routine for sumall with 1 argument added
!  11/20 Tersoff reorganised
!   6/21 Correction to call to GthetaBO as ij indices replace ik 
!   6/21 Wildcard introduced for second atom in borepulsive and boattractive
!   9/21 Symmetric flags added for bond order potentials
!  10/22 mneigh added to d2add and variants arguments
!  12/22 Call to d1add/d2add changed to include lextended argument
!
!  Conditions of use:
!
...
!
  ...
  if (status/=0) call outofmemory('bondorder12d','nboatomRptr')
!
...
!
  ...
  if (status/=0) call outofmemory('bondorder12d','lopanyneigh')
  allocate(nauatom(ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','nauatom')
  allocate(nfreeatom(ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','nfreeatom')
  allocate(nfreeatomau(ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','nfreeatomau')
  allocate(nneigh(ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','nneigh')
  allocate(rBOcutmax(ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','rBOcutmax')
  allocate(Zcn(ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','Zcn')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('bondorder12d','d2BtotiR')
    deallocate(d2BtotiA,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','d2BtotiA')
    deallocate(d2i,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','d2i')
    deallocate(d1BtotiR,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','d1BtotiR')
    deallocate(d1BtotiA,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','d1BtotiA')
    deallocate(d1i,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','d1i')
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','rneigh')
    deallocate(nbopotptr,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','nbopotptr')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('bondorder12d','neighno')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('bondorder12d','neighno')
  allocate(nbopotptr(maxneigh,ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','nbopotptr')
  allocate(rneigh(maxneigh,ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','rneigh')
  allocate(xneigh(maxneigh,ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','xneigh')
  allocate(yneigh(maxneigh,ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','yneigh')
  allocate(zneigh(maxneigh,ntot),stat=status)
  if (status/=0) call outofmemory('bondorder12d','zneigh')
  if (lgrad1) then
    allocate(d1i(maxneigh2),stat=status)
    if (status/=0) call outofmemory('bondorder12d','d1i')
    allocate(d1BtotiA(maxneigh2),stat=status)
    if (status/=0) call outofmemory('bondorder12d','d1BtotiA')
    allocate(d1BtotiR(maxneigh2),stat=status)
    if (status/=0) call outofmemory('bondorder12d','d1BtotiR')
  else
  ...
  if (lgrad2) then
    ...
    if (status/=0) call outofmemory('bondorder12d','d2i')
    allocate(d2BtotiA(maxneigh22),stat=status)
    if (status/=0) call outofmemory('bondorder12d','d2BtotiA')
    allocate(d2BtotiR(maxneigh22),stat=status)
    if (status/=0) call outofmemory('bondorder12d','d2BtotiR')
  else
...
!*****************************
  iloop: do i = 1,ntot
    if (i.le.nr1) then
!
!  Region 1 - use parallel decomposition
!
      lilocal = (reg12local(i).gt.0)
    else
!
!  Region 2 - divide work over nodes
!
      nn = mod((i - nr1),nprocs)
      lilocal = (nn.eq.procid)
    endif
!
!  If not second derivative call then we can parallelise using lilocal
!
    if (.not.lgrad2.and..not.lilocal) cycle iloop
!
...
!
!  Check for self energy terms for atom i if in region 1
!
    if (lregion1i) then
      if (nboZ.gt.0) then
        lfound = .false.
...
!
      ...
      do while (.not.lfound.and.nboRij.lt.nboR) 
        nboRij = nboRij + 1
        if (nBOspecR1(nboRij).eq.nati.and.(nBOspecR2(nboRij).eq.natj.or.nBOspecR2(nboRij).eq.maxele)) then
        ...
        if (lBOsymR(nboRij).and..not.lfound) then
          if (nBOspecR2(nboRij).eq.nati.and.(nBOspecR1(nboRij).eq.natj.or.nBOspecR1(nboRij).eq.maxele)) then
            if ((nBOtypR2(nboRij).eq.ntypi.or.nBOtypR2(nboRij).eq.0).and. &
                (nBOtypR1(nboRij).eq.ntypj.or.nBOtypR1(nboRij).eq.0)) then
              lfound = .true.
            elseif (nati.eq.natj) then
...
!
      ...
      do while (.not.lfound.and.nboAij.lt.nboA)
        nboAij = nboAij + 1
        if (nBOspecA1(nboAij).eq.nati.and.(nBOspecA2(nboAij).eq.natj.or.nBOspecA2(nboAij).eq.maxele)) then
        ...
        if (lBOsymA(nboAij).and..not.lfound) then
          if (nBOspecA2(nboAij).eq.nati.and.(nBOspecA1(nboAij).eq.natj.or.nBOspecA1(nboAij).eq.maxele)) then
            if ((nBOtypA2(nboAij).eq.ntypi.or.nBOtypA2(nboAij).eq.0).and. &
                (nBOtypA1(nboAij).eq.ntypj.or.nBOtypA1(nboAij).eq.0)) then
              lfound = .true.
            elseif (nati.eq.natj) then
...
!
              ...
              kk = neighno(k,i)
!
!  Set variables for k according to region
!
              if (kk.le.nr1) then
!
!  Region 1 ion
...
!
              ...
              do while (.not.lfound.and.nboRik.lt.nboR)
                nboRik = nboRik + 1
                if (nBOspecR1(nboRik).eq.nati.and.(nBOspecR2(nboRik).eq.natk.or.nBOspecR2(nboRik).eq.maxele)) then
                  if ((nBOtypR1(nboRik).eq.ntypi.or.nBOtypR1(nboRik).eq.0).and. &
                      (nBOtypR2(nboRik).eq.ntypk.or.nBOtypR2(nboRik).eq.0)) then
                    lfound = .true.
                  endif
                endif
                if (lBOsymR(nboRik).and..not.lfound) then
                  if (nBOspecR2(nboRik).eq.nati.and.(nBOspecR1(nboRik).eq.natk.or.nBOspecR1(nboRik).eq.maxele)) then
                    if ((nBOtypR2(nboRik).eq.ntypi.or.nBOtypR2(nboRik).eq.0).and. &
                        (nBOtypR1(nboRik).eq.ntypk.or.nBOtypR1(nboRik).eq.0)) then
                      lfound = .true.
                    endif
                  endif
                endif
...
!
                  ...
                  d2BtotiR(nn) = d2BtotiR(nn) - 2.0_dp*dGijkdr(3)*fik*dexpijkdr*rrik*drlambdadrik
                endif
              endif
            endif
!
...
!
              ...
              do while (.not.lfound.and.nboAik.lt.nboA)
                nboAik = nboAik + 1
                if (nBOspecA1(nboAik).eq.nati.and.(nBOspecA2(nboAik).eq.natk.or.nBOspecA2(nboAik).eq.maxele)) then
                  if ((nBOtypA1(nboAik).eq.ntypi.or.nBOtypA1(nboAik).eq.0).and. &
                      (nBOtypA2(nboAik).eq.ntypk.or.nBOtypA2(nboAik).eq.0)) then
                    lfound = .true.
                  endif
                endif
                if (lBOsymA(nboAik).and..not.lfound) then
                  if (nBOspecA2(nboAik).eq.nati.and.(nBOspecA1(nboAik).eq.natk.or.nBOspecA1(nboAik).eq.maxele)) then
                    if ((nBOtypA2(nboAik).eq.ntypi.or.nBOtypA2(nboAik).eq.0).and. &
                        (nBOtypA1(nboAik).eq.ntypk.or.nBOtypA1(nboAik).eq.0)) then
                      lfound = .true.
                    endif
                  endif
                endif
...
!
                  ...
                  d2BtotiA(nn) = d2BtotiA(nn) - 2.0_dp*dGijkdr(3)*fik*dexpijkdr*rrik*drlambdadrik
                endif
              endif
            endif
          endif
...
!
            if (bijsumA.gt.0.0_dp) then
              ...
              rtmp = 0.25_dp*zAi3*bijA*rbijsumA1*(zAi3*(1.0_dp + 0.5_dp/BOncoAi)*(bijsumAn1**2.0_dp)* &
                BOncoAi*rbijsumA1 - ((BOncoAi-1)*bijsumA**(BOncoAi-2)))
              nn = 0 
              do nn1 = 1,nneighi2
                do nn2 = 1,nn1
                  nn = nn + 1
            ...
            if (bijsumR.gt.0.0_dp) then
              ...
              rtmp = 0.25_dp*zRi3*bijR*rbijsumR1*(zRi3*(1.0_dp + 0.5_dp/BOncoRi)*(bijsumRn1**2.0_dp)* &
                BOncoRi*rbijsumR1 - ((BOncoRi-1)*bijsumR**(BOncoRi-2)))
              nn = 0 
              do nn1 = 1,nneighi2
                do nn2 = 1,nn1
                  nn = nn + 1
...
!
    if (lgrad1) then
      if (ldsl) then
        call d1adds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,nauatom,ndeqv,d1i,.false.,.false.)
      else
        if (lilocal) then
          call d1add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,d1i,.false.,.false.,.false.)
        endif
      endif
      if (lgrad2) then
        if (ldsl) then
          call d2addds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nfreeatomau,nboatomRptr,nauatom,ndeqv, &
            nr1,d1i,d2i,.false.)
        else     
          call d2addddm(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,nr1,d1i,d2i,.false.)
        endif
      endif
    endif
  enddo iloop
...
!
    if (lgrad2) then
      ...
      if (status/=0) call outofmemory('bondorder12d','d1ZiCN')
    endif
!**************************************
...
!  If not second derivative call then we can parallelise using lilocal
!
      if (.not.lgrad2.and..not.lilocal) cycle iloop2
!
!  Set variables relating to i
!
      if (i.le.nr1) then
...
!  Attractive component
!
                  if (nboAij.gt.0.and.rik.lt.rBOmax(npki)) then
!
!  Find nboAik
!
                    lfound = .false.
...
!
          if (ldsl) then
            call d1adds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,nauatom,ndeqv,d1i, &
                        .false.,.false.)
          else
            if (lilocal) then
              call d1add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,d1i,.false.,.false.,.false.)
          ...
          if (lgrad2) then
            if (ldsl) then
              call d2addds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nfreeatomau,nboatomRptr, &
                nauatom,ndeqv,nr1,d1i,d2i,.false.)
            else     
              call d2addddm(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,nr1,d1i,d2i, &
                .false.)
            endif
          endif
!
...
!
    if (lgrad2) then
      ...
      if (status/=0) call deallocate_error('bondorder12d','d1ZiCN')
    endif
  endif
!
!  Globalise energy if needed
!
  if (.not.lgrad2) then
    call sumone(ebondorder,esum,"bondorder12d","ebondorder")
    ebondorder = esum
  endif
!
!  Free local memory
!
  ...
  if (status/=0) call deallocate_error('bondorder12d','Zcn')
  deallocate(rBOcutmax,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','rBOcutmax')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','nneigh')
  deallocate(nfreeatomau,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','nfreeatomau')
  deallocate(nfreeatom,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','nfreeatom')
  deallocate(nauatom,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','nauatom')
  deallocate(lopanyneigh,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','lopanyneigh')
  deallocate(nboatomRptr,stat=status)
  if (status/=0) call deallocate_error('bondorder12d','nboatomRptr')
!
  t2 = g_cpu_time()
  tbondorder = tbondorder + t2 - t1
#ifdef TRACE
  call trace_out('bondorder12d')
#endif
!
  return
  end
