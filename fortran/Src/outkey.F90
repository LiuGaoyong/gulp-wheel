...
!***************************
  ...
  if (leht) then
    write(ioout,'(''*  eht          - use Extended Huckel Theory to compute charges                *'')')
  endif
  if (leembond) then
...
!
!  Trap case where EEM and EHT are both specified
!
  if (leem.and.leht) then
    call outerror('EEM and EHT cannot be used to compute charges in the same run',0_i4)
    call stopnow('outkey')
  endif
!
  return
