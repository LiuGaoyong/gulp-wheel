...
!  11/13 Check for points on SAS modified so that all atoms that
!        interact with the SAS lead to exclusion of a point rather
!        than just atoms that can have points on the SAS of their
!        own.
!   3/14 Call to switch renamed to sasswitch for benefit of ChemShell
!   4/17 Call added to set parallel distribution
!   2/18 Trace added
...
!
  ...
  use shells,           only : ncore, ncoptr, ncsptr
...
!
  buildloop: do nc = 1,nsasparticlespart
    ...
    if (nregionno(nrelf2a(i)).gt.1) cycle buildloop
    ds = sqrt(4.0_dp/nspa)
        ...
        do while (lInSAS(j).and.ii.lt.iimax)
          ...
          if (k.ne.i.or.ii.ne.iimid) then
            ...
            elseif (r .lt. 0.0_dp) then
              lpartial = .true.
              call sasswitch(abs(r),switchfn,.false.,dwdr,.false.,d2wdr2)
...
!
!  Set up parallel distribution of SAS points
!
  call setptsnoded2
!
