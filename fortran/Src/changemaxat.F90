...
!   1/22 dedqc added for variable charge second derivatives
!   6/23 AA-CLP arrays added
!   7/23 Check on size of node2var removed
!   7/23 lbsmat added
!   7/23 nregionno added
!   7/23 extpot added
!   9/23 dqdVxyz added
!
!  Conditions of use:
...
!
  ...
  use configurations, only : maxcfg, gfnff_ref_crd, gfnff_ref_q
  use control,        only : lcosmo, lscatter, lDoQDeriv2
  use cosmic
  use current
  use defects,        only : maxr1at
  use derivatives,    only : atomicstress, sumatomicstress, d2edqdq, d2edq2, dedqc, maxd2qu, ds2g, ds2gs, d2s2gs
  use derivatives,    only : dqdVxyz
...
#endif
  use gulp_gfnff,     only : gfnff_eeq_alp, gfnff_eeq_chi, gfnff_eeq_cnf, gfnff_eeq_gam, nfraglist, qfrag
  ...
  use parallel,       only : node2var, nvar2node, node2atomv, nvar2local, nprocs
  use partial
  use phononatoms
...
!
  ...
  call realloc(aaclp_d,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','aaclp_d')
  call realloc(aaclp_p,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','aaclp_p')
  call realloc(aaclp_t,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','aaclp_t')
  ...
  call realloc(extpot,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','extpot')
...
!
!  For these next few arrays the size can be determined by maxat or maxr1at
!
  nsize = max(4_i4*maxat+6_i4,4_i4*maxr1at)
  call realloc(node2var,nsize,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','node2var')
  call realloc(nvar2local,nsize,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','nvar2local')
  call realloc(nvar2node,nsize,ierror)
...
!
  ...
  call realloc(lbsmat,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','lbsmat')
  ...
  call realloc(nregionno,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','nregionno')
...
!
  ...
  if (ierror.gt.0) call outofmemory('changemaxat','gfnff_ref_crd')
  call realloc(gfnff_ref_q,maxat,maxcfg,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','gfnff_ref_q')
  call realloc(nfraglist,maxat,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','nfraglist')
  call realloc(qfrag,maxat,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','qfrag')
  call realloc(gfnff_eeq_alp,maxat,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','gfnff_eeq_alp')
  call realloc(gfnff_eeq_chi,maxat,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','gfnff_eeq_chi')
  call realloc(gfnff_eeq_cnf,maxat,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','gfnff_eeq_cnf')
  call realloc(gfnff_eeq_gam,maxat,ierror)
  if (ierror.gt.0) call outofmemory('changemaxat','gfnff_eeq_gam')
#ifdef OLDCS
...
#endif
!
!  Variable charge arrays
!
  call realloc(dqdVxyz,3_i4,maxat,ierror)
  if (ierror.ne.0) call outofmemory('changemaxat','dqdVxyz')
  if (lDoQDeriv2) then
    call realloc(d2edq2,maxat,ierror)
    if (ierror.gt.0) call outofmemory('changemaxat','d2edq2')
    call realloc(d2edqdq,maxat,maxd2qu,ierror)
    if (ierror.gt.0) call outofmemory('changemaxat','d2edqdq')
!
    if (nprocs.gt.0) then
!
!  Arrays needed only for parallel runs
!
      call realloc(dedqc,3_i4,maxat,ierror)
      if (ierror.gt.0) call outofmemory('changemaxat','dedqc')
      call realloc(ds2g,6_i4,maxat,ierror)
      if (ierror.gt.0) call outofmemory('changemaxat','ds2g')
      call realloc(ds2gs,6_i4,maxat,ierror)
      if (ierror.gt.0) call outofmemory('changemaxat','ds2gs')
      call realloc(d2s2gs,6_i4,maxat,ierror)
      if (ierror.gt.0) call outofmemory('changemaxat','d2s2gs')
    endif
  endif
!
