...
!  10/12 OpenKIM cutoff now added in here
!   8/18 Modified for version 2 of OpenKIM
!  11/20 Modifications for GFNFF added
!   3/21 GFNFF cutoff set here to avoid issues
!  10/21 lgfnff moved to control
!  10/21 radeeq now obtained from call to subroutine
!
!  On exit :
!
...
!
  ...
  use control,       only : lbrenner, lwolf, lgfnff
  use current,       only : lewald, numat
...
!
  ...
  real(dp)                  :: cmax2
!
!  Set GFNFF cutoff if needed
!
  if (lgfnff) then
    call gfnff_set_radeeq(numat,gfnff_eeq_rad)
  endif
!
...
!
  ...
  if (lgfnff) cutoffmax = max(cutoffmax,gfnff_eeq_rad)
!
...
!
  ...
  enddo
!
!  Compare with KIM model cutoffs
!
  if (lkim_model) then
    cutoffmax = max(cutoffmax,kim_cutoff)
  endif
!
  return
  end
