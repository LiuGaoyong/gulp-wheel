...
!   7/13 When calling epot the shell flag is no longer hard coded to
!        true, but is set to the logical nshell > 0
!  10/17 Modified so that absolute coordinates are not overwritten for MD
!   1/18 Trace added
!   8/19 Correction to allocation of v array for reaxffpot case where symmetry is not used
!   9/19 nrel2 / nrelat changed to nrela2f / nrelf2a
!   2/20 Call to sitepot removed so that it can be handled independently
!   2/20 Timing output corrected to better reflect activity
...
!
    ...
    call epot(.true.,nasym,v,xalat,yalat,zalat,.true.,vx,vy,vz,lgrad2,v2xyz,nshell.gt.0)
  endif
  time2 = g_cpu_time()
  tspot = tspot + time2 - time1
!
...
!
  if (ioproc) then
    time = g_cpu_time() - time0
    write(ioout,'(''  Total time to end of atomic site potentials = '',f12.4,'' s'',/)') time
  endif
#ifdef TRACE
  call trace_out('latpot')
#endif
!
  return
  end
