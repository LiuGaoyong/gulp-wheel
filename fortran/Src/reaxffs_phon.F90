  subroutine reaxFFs_phon(xkv,ykv,zkv)
!
!  Calculates the dynamical matrix contribution for the ReaxFF force field.
!  Uses sparsity to speed up calculation.
!
!   8/23 Created from reaxffs.F90 after sparsity changes
!   8/23 Calling of d2addspp corrected
...
!
  ...
  use iochannels
  use m_qderiv,       only : d2chargep, d2chargeselfp
...
!
  if (nprocs.ne.1) then
    call outerror('reaxFFs_phon called in parallel!',0_i4)
    call stopnow('reaxFFs_phon')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('reaxFFs_phon','nboatomptr')
  allocate(nboatomRptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nboatomRptr')
  allocate(nbodummyptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nbodummyptr')
!
!  Allocate memory that does not depend on maxneigh
!
  allocate(nbos(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nbos')
  allocate(nbosptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nbosptr')
  allocate(latomdone(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','latomdone')
  allocate(nneigh(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nneigh')
  allocate(sumsij(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','sumsij')
  allocate(sum(max(3*numat,14_i4)),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','sum')
  allocate(sum0(max(3*numat,14_i4)),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','sum0')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('reaxFFs_phon','d2sboddeltaidbo')
    deallocate(d2sboproddbo2,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2sboproddbo2')
    deallocate(dsboproddbo,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','dsboproddbo')
    deallocate(nonzeroptr_j2k2,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzeroptr_j2k2')
    deallocate(nonzeroptr_jk,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzeroptr_jk')
    deallocate(nonzeroptr_ij2,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzeroptr_ij2')
    deallocate(nonzeroptr_ik,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzeroptr_ik')
    deallocate(nonzeroptr_ij,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzeroptr_ij')
    deallocate(nonzeroptr,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzeroptr')
    deallocate(nonzerorptr,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','nonzerorptr')
    deallocate(d1BOjk2_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk2_pipi')
    deallocate(d1BOjk2_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk2_pi')
    deallocate(d1BOjk2_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk2_s')
    deallocate(d1BOjk2,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk2')
    deallocate(d1BOjk_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk_pipi')
    deallocate(d1BOjk_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk_pi')
    deallocate(d1BOjk_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk_s')
    deallocate(d1BOjk,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOjk')
    deallocate(d2BOp_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOp_pipi')
    deallocate(d2BOp_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOp_pi')
    deallocate(d2BOp,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOp')
    deallocate(d2BOjk_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOjk_pipi')
    deallocate(d2BOjk_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOjk_pi')
    deallocate(d2BOjk_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOjk_s')
    deallocate(d2BOjk,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOjk')
    deallocate(d2BOi_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOi_pipi')
    deallocate(d2BOi_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOi_pi')
    deallocate(d2BOi_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOi_s')
    deallocate(d2BOi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2BOi')
    deallocate(d2mix,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2mix')
    deallocate(d1BOik_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOik_pipi')
    deallocate(d1BOik_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOik_pi')
    deallocate(d1BOik_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOik_s')
    deallocate(d1BOik,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOik')
    deallocate(d1BOij2_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOij2_pipi')
    deallocate(d1BOij2_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOij2_pi')
    deallocate(d1BOij2_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOij2_s')
    deallocate(d1BOij2,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOij2')
    deallocate(d1BOi_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOi_pipi')
    deallocate(d1BOi_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOi_pi')
    deallocate(d1BOi_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOi_s')
    deallocate(d1BOi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOi')
    deallocate(devaldsbo_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','devaldsbo_neigh')
    deallocate(devalddeltaj_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','devalddeltaj_neigh')
    deallocate(d2etorsdthetaddeltai,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2etorsdthetaddeltai')
    deallocate(d2etorsdBOdBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2etorsdBOdBOpi_neigh')
    deallocate(d2etorsdBOpi2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2etorsdBOpi2_neigh')
    deallocate(detorsdBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','detorsdBOpi_neigh')
    deallocate(d2evaldthetaddeltai_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evaldthetaddeltai_neigh')
    deallocate(d2evaldthetadBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evaldthetadBOpi_neigh')
    deallocate(d2evalddeltajdBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evalddeltajdBO_neigh')
    deallocate(d2etorsddeltajdBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2etorsddeltajdBOpi_neigh')
    deallocate(d2etorsddeltaidBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2etorsddeltaidBOpi_neigh')
    deallocate(d2etorsddeltadBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2etorsddeltadBOpi_neigh')
    deallocate(d2evaldsbo2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evaldsbo2_neigh')
    deallocate(d2evaldsbo_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evaldsbo_neigh')
    deallocate(d2evalddeltajk_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evalddeltajk_neigh')
    deallocate(d2evalddeltaiddeltaj_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evalddeltaiddeltaj_neigh')
    deallocate(d2evalddeltaidBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evalddeltaidBO_neigh')
    deallocate(d2evaldBO2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2evaldBO2_neigh')
    deallocate(devaldBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','devaldBO_neigh')
    deallocate(d2ebpenddeltaidBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2ebpendddeltaiBO_neigh')
    deallocate(d2ebpendBO2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d2ebpendBO2_neigh')
    deallocate(debpendBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','debpendBO_neigh')
    deallocate(d1BOp_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOp_pipi')
    deallocate(d1BOp_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOp_pi')
    deallocate(d1BOp,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','d1BOp')
    deallocate(deltalplp,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','deltalplp')
    deallocate(deltalp,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','deltalp')
    deallocate(deltap,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','deltap')
    deallocate(delta,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','delta')
    deallocate(BOp_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','BOp_pipi')
    deallocate(BOp_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','BOp_pi')
    deallocate(BOp,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','BOp')
    deallocate(BO_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','BO_pipi')
    deallocate(BO_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','BO_pi')
    deallocate(BO,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','BO')
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','rneigh')
    deallocate(ineigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','ineigh')
    deallocate(neighnoRptr,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','neighnoRptr')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('reaxFFs_phon','neighno')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('reaxFFs_phon','neighno')
  allocate(neighnoRptr(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','neighnoRptr')
  allocate(ineigh(3_i4,maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','ineigh')
  allocate(rneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','rneigh')
  allocate(xneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','xneigh')
  allocate(yneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','yneigh')
  allocate(zneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','zneigh')
  allocate(BO(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','BO')
  allocate(BO_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','BO_pi')
  allocate(BO_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','BO_pipi')
  allocate(BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','BOp')
  allocate(BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','BOp_pi')
  allocate(BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','BOp_pipi')
  allocate(delta(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','delta')
  allocate(deltap(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','deltap')
  allocate(deltalp(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','deltalp')
  allocate(deltalplp(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','deltalplp')
!
  allocate(nonzerorptr(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzerorptr')
  allocate(nonzeroptr(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzeroptr')
  allocate(nonzeroptr_ij(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzeroptr_ij')
  allocate(nonzeroptr_ij2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzeroptr_ij2')
  allocate(nonzeroptr_ik(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzeroptr_ik')
  allocate(nonzeroptr_jk(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzeroptr_jk')
  allocate(nonzeroptr_j2k2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','nonzeroptr_j2k2')
  allocate(d1BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOp')
  allocate(d1BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOp_pi')
  allocate(d1BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOp_pipi')
  allocate(debpendBO_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','debpendBO_neigh')
  allocate(devaldBO_neigh(maxneigh1a),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','devaldBO_neigh')
  allocate(detorsdBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','detorsdBOpi_neigh')
  allocate(devalddeltaj_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','devalddeltaj_neigh')
  allocate(devaldsbo_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','devaldsbo_neigh')
  allocate(d1BOi(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOi')
  allocate(d1BOi_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOi_s')
  allocate(d1BOi_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOi_pi')
  allocate(d1BOi_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOi_pipi')
  allocate(d1BOij2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOij2')
  allocate(d1BOij2_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOij2_s')
  allocate(d1BOij2_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOij2_pi')
  allocate(d1BOij2_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOij2_pipi')
  allocate(d1BOik(maxneigh2t),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOik')
  allocate(d1BOik_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOik_s')
  allocate(d1BOik_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOik_pi')
  allocate(d1BOik_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOik_pipi')
  allocate(d1BOjk(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk')
  allocate(d1BOjk_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk_s')
  allocate(d1BOjk_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk_pi')
  allocate(d1BOjk_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk_pipi')
  allocate(d1BOjk2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk2')
  allocate(d1BOjk2_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk2_s')
  allocate(d1BOjk2_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk2_pi')
  allocate(d1BOjk2_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d1BOjk2_pipi')
  allocate(dsboproddbo(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','dsboproddbo')
  allocate(d2mix(maxneigh2t),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2max')
  allocate(d2BOi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOi')
  allocate(d2BOi_s(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOi_s')
  allocate(d2BOi_pi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOi_pi')
  allocate(d2BOi_pipi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOi_pipi')
  allocate(d2BOjk(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOjk')
  allocate(d2BOjk_s(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOjk_s')
  allocate(d2BOjk_pi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOjk_pi')
  allocate(d2BOjk_pipi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOjk_pipi')
  allocate(d2BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOp')
  allocate(d2BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOp_pi')
  allocate(d2BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2BOp_pipi')
  allocate(d2ebpenddeltaidBO_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2ebpenddeltaidBO_neigh')
  allocate(d2ebpendBO2_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2ebpendBO2_neigh')
  allocate(d2etorsdBOpi2_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2etorsdBOpi2_neigh')
  allocate(d2etorsdBOdBOpi_neigh(maxneigh1a,maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2etorsdBOdBOpi_neigh')
  allocate(d2etorsdthetaddeltai(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2etorsdthetaddeltai')
  allocate(d2evaldBO2_neigh(maxneigh2a),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evaldBO2_neigh')
  allocate(d2evalddeltaidBO_neigh(maxneigh1a),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evalddeltaidBO_neigh')
  allocate(d2evalddeltaiddeltaj_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evalddeltaiddeltaj_neigh')
  allocate(d2evalddeltajk_neigh(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evalddeltajk_neigh')
  allocate(d2evaldsbo_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evaldsbo_neigh')
  allocate(d2evaldsbo2_neigh(maxneigh1),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evaldsbo2_neigh')
  allocate(d2etorsddeltadBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2etorsddeltadBOpi_neigh')
  allocate(d2etorsddeltaidBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2etorsddeltaidBOpi_neigh')
  allocate(d2etorsddeltajdBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2etorsddeltajdBOpi_neigh')
  allocate(d2evalddeltajdBO_neigh(maxneigh,maxneigh1a),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evalddeltajdBO_neigh')
  allocate(d2evaldthetadBOpi_neigh(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evaldthetadBOpi_neigh')
  allocate(d2evaldthetaddeltai_neigh(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2evaldthetaddeltai_neigh')
  allocate(d2sboproddbo2(maxneigh1),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2sboproddbo2')
  allocate(d2sboddeltaidbo(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFs_phon','d2sboddeltaidbo')
!
!  Initial allocation of sparse arrays
!
  call realloc(d1is,maxsparse,status)
  if (status.ne.0) call outofmemory('reaxFFs_phon','d1is')
  maxsparse2 = maxsparse*(maxsparse+1)/2
  call realloc(d2is,maxsparse2,status)
  if (status.ne.0) call outofmemory('reaxFFs_phon','d2is')
  call realloc(d2evaldthetadBO_neigh,maxsparse,maxneigh1a,status)
  if (status.ne.0) call outofmemory('reaxFFs_phon','d2evaldthetadBO_neigh')
  call realloc(d2etorsdthetaddeltaj_neigh,maxsparse,maxneigh,status)
  if (status.ne.0) call outofmemory('reaxFFs_phon','d2etorsdthetaddeltaj_neigh')
  call realloc(d2etorsdthetadBOpi_neigh,maxsparse,maxneigh,status)
  if (status.ne.0) call outofmemory('reaxFFs_phon','d2etorsdthetadBOpi_neigh')
!*************************************
...
!
          call d2addspp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nboatomRptr,d1is,d2is, &
...
!
      call d2addspp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nboatomRptr, &
                    d1is,d2is,xkv,ykv,zkv,nonzero,nonzeroptr,.true.,lextended)
!
...
!
                          ...
                          call add_drv2_1p(ix,iy,iz,jx,jy,jz,xji,yji,zji,dehbdrij,d2ehbdrij2,d2r2dx2(1,1))
!
!  i-j / i-k
!
                          ...
                          call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,kx,ky,kz,xji,yji,zji,xki,yki,zki,d2ehbdrijdrik)
!
!  i-j / j-k
!
                          ...
                          call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,kx,ky,kz,xji,yji,zji,xkj,ykj,zkj,d2ehbdrijdrjk)
!
!  i-k / i-k
!
                          ...
                          call add_drv2_1p(ix,iy,iz,kx,ky,kz,xki,yki,zki,dehbdrik,d2ehbdrik2,d2r2dx2(1,2))
!
!  i-k / j-k
!
                          ...
                          call add_drv2_2p(ix,iy,iz,kx,ky,kz,jx,jy,jz,kx,ky,kz,xki,yki,zki,xkj,ykj,zkj,d2ehbdrikdrjk)
!
!  j-k / j-k
!
                          ...
                          call add_drv2_1p(jx,jy,jz,kx,ky,kz,xkj,ykj,zkj,dehbdrjk,d2ehbdrjk2,d2r2dx2(1,3))
!
!  Derivatives of hydrogen bond energy with respect to bond orders -> add to eval terms
!
...
!
                          ...
                          enddo
                          call d2addmixspp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nbodummyptr,d2mix, &
                                           nonzero_ij,nonzeroptr_ij,ix,iy,iz,jx,jy,jz,xji,yji,zji,.true.)
!
!  i-k
!
                          ...
                          enddo
                          call d2addmixspp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nbodummyptr,d2mix, &
                                           nonzero_ij,nonzeroptr_ij,ix,iy,iz,kx,ky,kz,xki,yki,zki,.true.)
!
!  j-k
!
                          ...
                          enddo
                          call d2addmixspp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nbodummyptr,d2mix, &
                                           nonzero_ij,nonzeroptr_ij,jx,jy,jz,kx,ky,kz,xkj,ykj,zkj,.true.)
!
...
!
                              call d2chargep(i,j,1_i4,ix,iy,iz,jx,jy,jz,xkv,ykv,zkv,d0i,d0j,d1ix,d1iy,d1iz, &
                                             d1jx,d1jy,d1jz,d2i2,d2ij,d2j2,d2self,derive0selfi, &
                                             derive0selfj,.true.,.false.,.false.,nbosptr)
                            else
                              decouldrij = 0.0_dp
                              d2ecouldrij2 = 0.0_dp
                            endif
...
!
                            ...
                            call add_drv2_1p(ix,iy,iz,jx,jy,jz,xji,yji,zji,d1trm,d2trm,d2r2dx2(1,1))
                          endif
                        endif
                      endif
!
...
!  Contribution from charge derivatives  *
!*****************************************
              call d2chargeselfp(i,j,ix,iy,iz,jx,jy,jz,nbosptr)
!
!  End of loop over j
!
            enddo
...
!*****************************************
            if (lreaxFF_ecoul.and.lreaxFFQ) then
              ...
              call d2chargep(i,j,nor,ix,iy,iz,jx,jy,jz,xkv,ykv,zkv,d0i,d0j,d1ix,d1iy,d1iz, &
                             d1jx,d1jy,d1jz,d2i2,d2ij,d2j2,d2self,derive0selfi, &
                             derive0selfj,.true.,.true.,.false.,nbosptr)
            endif
!
!  End of loop over j
!
...
!
  ...
  if (status/=0) call deallocate_error('reaxFFs_phon','d2ebpendBO2_neigh')
  deallocate(d2ebpenddeltaidBO_neigh,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','d2ebpenddeltaidBO_neigh')
  deallocate(debpendBO_neigh,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','debpendBO_neigh')
  deallocate(d1BOp_pipi,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','d1BOp_pipi')
  ...
  if (status/=0) call deallocate_error('reaxFFs_phon','sum0')
  deallocate(sum,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','sum')
  deallocate(sumsij,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','sumsij')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','nneigh')
  deallocate(latomdone,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','latomdone')
  deallocate(nbosptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','nbosptr')
  deallocate(nbos,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','nbos')
  deallocate(nbodummyptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','nbodummyptr')
  deallocate(nboatomRptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','nboatomRptr')
  deallocate(nboatomptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFs_phon','nboatomptr')
!     
