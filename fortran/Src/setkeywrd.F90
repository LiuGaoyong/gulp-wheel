  subroutine setkeyword(llibraryonly)
!
...
!   7/21 If charge equilibration is being used then set cuts to small value
!   9/21 lPrintVB option added
!  11/21 Modifications for TI added
!  11/21 lmd_inc_com added
!  12/21 lnomottlittleton traps now added
!  12/21 pGFNFF modifications added
!   2/22 lsync added
!   2/22 lequipartition added
!   5/23 eht added
!  12/23 Modified for library calls
!
!  Conditions of use:
!
...
!  Passed arguments
!
  logical,     intent(in)  :: llibraryonly
!****************************
...
!
  if (llibraryonly) then
    ...
    if (.not.lzdipole) lzdipole = (index(keyword,' zdip').ne.0.or.index(keyword,'zdip').eq.1)
    ...
    if (.not.leht) leht = (index(keyword,' eht').ne.0.or.index(keyword,'eht').eq.1)
    if (.not.leigen) leigen = (index(keyword,' eige').ne.0.or.index(keyword,'eige').eq.1)
    if (.not.lequipartition) lequipartition = (index(keyword,' equi').ne.0.or.index(keyword,'equi').eq.1)
    ...
    if (.not.lgfnff) lgfnff = (index(keyword,' gfn').ne.0.or.index(keyword,'gfn').eq.1)
    if (.not.lgfnff_topowolf) lgfnff_topowolf = (index(keyword,' gwol').ne.0.or.index(keyword,'gwol').eq.1)
    ...
    if (.not.loldd2q) loldd2q = (index(keyword,' oldd').ne.0.or.index(keyword,'oldd').eq.1)
    ...
    if (.not.lPrintVB) lPrintVB = (index(keyword,' prt_vb').ne.0.or.index(keyword,'prt_vb').eq.1)
    ...
    if (.not.lusefcalg) lusefcalg = (index(keyword,' newfc').ne.0.or.index(keyword,'newfc').eq.1)
...
!   
!  Set keywords that will be required for a library call
!   
    lnoexclude = .true.
    lconp = .true. 
  else
    ...
    if (.not.lmd_inc_com) lmd_inc_com = (index(keyword,' inc_c').ne.0.or.index(keyword,'inc_c').eq.1)
    if (.not.lmd_inc_rot) lmd_inc_rot = (index(keyword,' inc_r').ne.0.or.index(keyword,'inc_r').eq.1)
    ...
    if (.not.lstressout) lstressout = (index(keyword,' stre').ne.0.or.index(keyword,'stre').eq.1)
    if (.not.lsymoff) lsymoff = (index(keyword,' symoff').ne.0.or.index(keyword,'symoff').eq.1)
    if (.not.lsync) lsync = (index(keyword,' sync').ne.0.or.index(keyword,'sync').eq.1)
    if (.not.lthermal) lthermal = (index(keyword,' ther').ne.0.or.index(keyword,'ther').eq.1)
    if (.not.ltirun) ltirun = (index(keyword,' ti').ne.0.or.index(keyword,'ti').eq.1)
    if (.not.ltors) ltors = (index(keyword,' tors').ne.0.or.index(keyword,'tors').eq.1)
    ...
    if (.not.lusefcalg) lusefcalg = (index(keyword,' newfc').ne.0.or.index(keyword,'newfc').eq.1)
    if (.not.luseprecon) luseprecon = (index(keyword,' prec').ne.0.or.index(keyword,'prec').eq.1)
    if (.not.lxray) lxray = (index(keyword,' xray').ne.0.or.index(keyword,'xray').eq.1)
    if (.not.lzsisa) lzsisa = (index(keyword,' zsis').ne.0.or.index(keyword,'zsis').eq.1)
  endif
!
...
!
  if (llibraryonly) then
    ...
    if (lnoflags) then
      ...
      if (lopt.or.lgrad.or.lharmrelax.or.lfit.or.lrfo.or.lmc.or.lmd.or.lneb.or.lsync) then
...
!
!  If variable charge then there are no third derivatives but we do need charge second derivatives
!
...
!
!  For charge equilibration models set the core-shell cutoff to a small value as shells can't be present
!
  if (leem) cuts = 0.01_dp
!
...
!
!  If bond order charge potentials or ReaxFF or GFNFF are present turn off symmetry for derivatives
!
  if (nboQ.gt.0.or.lreaxFF.or.lEDIP.or.lgfnff) then
    lsymdok = .false.
    lnomottlittleton = .true.
  endif
!
...
!
!  GFNFF currently needs lmodco to be false
!
  if (lgfnff) lmodco = .false.
#ifdef TRACE
