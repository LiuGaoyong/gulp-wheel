...
!
  ...
  real(dp)                                         :: zkj
#ifdef TRACE
  call trace_in('brennerfc')
#endif
!
  t1 = g_cpu_time()
!
  allocate(nREBOatomptr(numat),stat=status)
  if (status/=0) call outofmemory('brennerfc','nREBOatomptr')
  allocate(nREBOatomRptr(numat),stat=status)
  if (status/=0) call outofmemory('brennerfc','nREBOatomRptr')
!
...
!
  ...
  if (status/=0) call outofmemory('brennerfc','latomdone')
  allocate(nneigh(nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','nneigh')
  allocate(nsneigh(nREBOspecies,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','nsneigh')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('brennerfc','d2Ndr2')
    deallocate(d2Nconjdj2,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d2Nconjdj2')
    deallocate(d2Nconjdi2,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d2Nconjdi2')
    deallocate(d2Btotj,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d2Btotj')
    deallocate(d2Btoti,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d2Btoti')
    deallocate(d2j,stat=status)   
    if (status/=0) call deallocate_error('brennerfc','d2j')
    deallocate(d2i,stat=status)   
    if (status/=0) call deallocate_error('brennerfc','d2i')
!
    deallocate(dNdr,stat=status)
    if (status/=0) call deallocate_error('brennerfc','dNdr')
    deallocate(dNconjdj,stat=status)
    if (status/=0) call deallocate_error('brennerfc','dNconjdj')
    deallocate(dNconjdi,stat=status)
    if (status/=0) call deallocate_error('brennerfc','dNconjdi')
    deallocate(d1Btotj,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d1Btotj')
    deallocate(d1Btoti,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d1Btoti')
    deallocate(d1j,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d1j')
    deallocate(d1i,stat=status)
    if (status/=0) call deallocate_error('brennerfc','d1i')
!
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('brennerfc','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('brennerfc','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('brennerfc','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('brennerfc','rneigh')
    deallocate(ineigh,stat=status)
    if (status/=0) call deallocate_error('brennerfc','ineigh')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('brennerfc','neighno')
    deallocate(nREBObond,stat=status)
    if (status/=0) call deallocate_error('brennerfc','nREBObond')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('brennerfc','nREBObond')
  allocate(neighno(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','neighno')
  allocate(ineigh(3_i4,maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','ineigh')
  allocate(rneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','rneigh')
  allocate(xneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','xneigh')
  allocate(yneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','yneigh')
  allocate(zneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','zneigh')
!
!  First derivative arrays
!
  allocate(d1i(maxneigh2),stat=status)
  if (status/=0) call outofmemory('brennerfc','d1i')
  allocate(d1j(maxneigh2),stat=status)
  if (status/=0) call outofmemory('brennerfc','d1j')
  allocate(d1Btoti(maxneigh2),stat=status)
  if (status/=0) call outofmemory('brennerfc','d1Btoti')
  allocate(d1Btotj(maxneigh2),stat=status)
  if (status/=0) call outofmemory('brennerfc','d1Btotj')
  allocate(dNconjdi(maxneigh),stat=status)
  if (status/=0) call outofmemory('brennerfc','dNconjdi')
  allocate(dNconjdj(maxneigh),stat=status)
  if (status/=0) call outofmemory('brennerfc','dNconjdj')
  allocate(dNdr(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','dNdr')
! 
!  Second derivative arrays
!
  allocate(d2i(maxneigh22),stat=status)   
  if (status/=0) call outofmemory('brennerfc','d2i')
  allocate(d2j(maxneigh22),stat=status)   
  if (status/=0) call outofmemory('brennerfc','d2j')
  allocate(d2Btoti(maxneigh22),stat=status)
  if (status/=0) call outofmemory('brennerfc','d2Btoti')
  allocate(d2Btotj(maxneigh22),stat=status)
  if (status/=0) call outofmemory('brennerfc','d2Btotj')
  allocate(d2Nconjdi2(maxneigh*(maxneigh+1)/2),stat=status)
  if (status/=0) call outofmemory('brennerfc','d2Nconjdi2')
  allocate(d2Nconjdj2(maxneigh*(maxneigh+1)/2),stat=status)
  if (status/=0) call outofmemory('brennerfc','d2Nconjdj2')
  allocate(d2Ndr2(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennerfc','d2Ndr2')
!****************************************
...
!
        call d2addfc(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,ineigh,nREBOatomRptr,d1j,d2j,.true.)
!
...
!
    call d2addfc(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,ineigh,nREBOatomRptr,d1i,d2i,.true.)
  enddo
!
!  Free local memory
!
  ...
  if (status/=0) call deallocate_error('brennerfc','nsneigh')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('brennerfc','nneigh')
  deallocate(latomdone,stat=status)
  if (status/=0) call deallocate_error('brennerfc','latomdone')
  deallocate(nREBOatomRptr,stat=status)
  if (status/=0) call deallocate_error('brennerfc','nREBOatomRptr')
  deallocate(nREBOatomptr,stat=status)
  if (status/=0) call deallocate_error('brennerfc','nREBOatomptr')
!
  t2 = g_cpu_time()
  tbrenner = tbrenner + t2 - t1
#ifdef TRACE
  call trace_out('brennerfc')
#endif
!
  return
  end
