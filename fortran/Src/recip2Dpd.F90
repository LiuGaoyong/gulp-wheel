  subroutine recip2Dpd(xkv,ykv)
!
!  Calculate phased second derivatives in reciprocal space for 2-D phonon calculations.
...
!   6/20 Correction to the complex part of one group velocity term
!   1/22 Call to d2chargep2D changed to d2chargep2Dd
!  12/22 m_qderiv added
!
!  Conditions of use:
!
...
!
  ...
  use ksample
  use kspace
  use m_qderiv,       only : d2chargep2Dd
  use molecule
  use parallel
  use shells
  use symmetry
...
!
  ...
  if (status/=0) call outofmemory('recip2Dpd','kmod0')
  allocate(ktrm0(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','ktrm0')
  allocate(ktrm3(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','ktrm3')
  allocate(ktrm3z(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','ktrm3z')
  allocate(ktrm4(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','ktrm4')
  allocate(ktrm4z(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','ktrm4z')
  allocate(tmp1(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','tmp1')
  allocate(tmp2(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','tmp2')
  allocate(tmp3(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip2Dpd','tmp3')
  if (lgroupvelocity) then
    allocate(ktrmdk(2_i4,maxkvec),stat=status)
    if (status/=0) call outofmemory('recip2Dpd','ktrmdk')
  endif
!
...
!
!  Check magnitude of charge
!
    if (abs(qli).lt.thresh_q) cycle iloop
!
    xci = xclat(i)
    yci = yclat(i)
    zci = zclat(i)
...
!
      do iv = 1,nkvec
        ...
        if (kvec.le.Gmax) then
          ...
          yrkk = yrk(iv)
          if (lphasecom) then
            arg = xrkk*xd - xkv*(xd-xcom) + yrkk*yd - ykv*(yd-ycom)
          else
...
!
          ...
          d24q = d24q + yrkk*sinq*ztrm1
          d25q = d25q + xrkk*sinq*ztrm1
          d26q = d26q + cosq*tmp3(iv)*kexperfc
          d21i = d21i - sinq*tmp1(iv)*kexperfc
          d22i = d22i - sinq*tmp2(iv)*kexperfc
          d23i = d23i + sinq*trmzz
          d24i = d24i + yrkk*cosq*ztrm1
          d25i = d25i + xrkk*cosq*ztrm1
          d26i = d26i - sinq*tmp3(iv)*kexperfc
!
          if (lgroupvelocity) then
!
!  Group velocities
!
            dexp1dk = zd*dexp1
            dexp2dk = - zd*dexp2
            dexp3dk = - 2.0_dp*darg1*rhseta*dexp3
            dexp4dk = - 2.0_dp*darg2*rhseta*dexp4
            derfc1dk = - dexp3*rpieta
            derfc2dk = - dexp4*rpieta
            kexperfcdk = (dexp1*derfc1dk + dexp2*derfc2dk + dexp1dk*derfc1 + dexp2dk*derfc2)
            trmzzdk = (2.0_dp*kvec*kexperfc + kvec*kvec*kexperfcdk - 2.0_dp*tweatpi*( &
                      (dexp1*dexp3 + dexp2*dexp4) + &
                      kvec*(dexp1dk*dexp3 + dexp1*dexp3dk + dexp2dk*dexp4 + dexp2*dexp4dk) &
                      - seta*(rhseta*dexp1*dexp3 + darg1*(dexp1dk*dexp3 + dexp1*dexp3dk) + &
                              rhseta*dexp2*dexp4 + darg2*(dexp2dk*dexp4 + dexp2*dexp4dk))))/kvec
!
            kexperfcdk = kexperfcdk/kvec
            ztrm1dk = ((dexp1*derfc1-dexp2*derfc2) + kvec*(dexp1dk*derfc1-dexp2dk*derfc2) + &
                       kvec*(dexp1*derfc1dk-dexp2*derfc2dk) - &
                       tweatpi*(dexp1dk*dexp3 + dexp1*dexp3dk - dexp2dk*dexp4 - dexp2*dexp4dk))/kvec
!
            cdk(1:2,1) = cdk(1:2,1) + dcmplx(cosa*ktrmdk(1:2,iv)*tmp1(iv)*kexperfc,-sina*ktrmdk(1:2,iv)*tmp1(iv)*kexperfc)
            cdk(1,1)   = cdk(1,1)   + dcmplx(cosq*tmp1(iv)*kexperfcdk*xrkk,-sinq*tmp1(iv)*kexperfcdk*xrkk)
            cdk(2,1)   = cdk(2,1)   + dcmplx(cosq*tmp1(iv)*kexperfcdk*yrkk,-sinq*tmp1(iv)*kexperfcdk*yrkk)
            cdk(1,1)   = cdk(1,1)   + dcmplx(2.0_dp*cosq*xrkk*kexperfc,-2.0_dp*sinq*xrkk*kexperfc)
!
            cdk(1:2,2) = cdk(1:2,2) + dcmplx(cosa*ktrmdk(1:2,iv)*tmp2(iv)*kexperfc,-sina*ktrmdk(1:2,iv)*tmp2(iv)*kexperfc)
            cdk(1,2)   = cdk(1,2)   + dcmplx(cosq*tmp2(iv)*kexperfcdk*xrkk,-sinq*tmp2(iv)*kexperfcdk*xrkk)
            cdk(2,2)   = cdk(2,2)   + dcmplx(cosq*tmp2(iv)*kexperfcdk*yrkk,-sinq*tmp2(iv)*kexperfcdk*yrkk)
            cdk(2,2)   = cdk(2,2)   + dcmplx(2.0_dp*cosq*yrkk*kexperfc,-2.0_dp*sinq*yrkk*kexperfc)
!
            cdk(1:2,3) = cdk(1:2,3) + dcmplx(-cosa*ktrmdk(1:2,iv)*trmzz,sina*ktrmdk(1:2,iv)*trmzz)
            cdk(1,3)   = cdk(1,3)   + dcmplx(-cosq*trmzzdk*xrkk,sinq*trmzzdk*xrkk)
            cdk(2,3)   = cdk(2,3)   + dcmplx(-cosq*trmzzdk*yrkk,sinq*trmzzdk*yrkk)
!
            cdk(1:2,4) = cdk(1:2,4) + dcmplx(sina*ktrmdk(1:2,iv)*yrkk*ztrm1,cosa*ktrmdk(1:2,iv)*yrkk*ztrm1)
            cdk(1,4)   = cdk(1,4)   + dcmplx(sinq*yrkk*ztrm1dk*xrkk,cosq*yrkk*ztrm1dk*xrkk)
            cdk(2,4)   = cdk(2,4)   + dcmplx(sinq*yrkk*ztrm1dk*yrkk,cosq*yrkk*ztrm1dk*yrkk)
            cdk(2,4)   = cdk(2,4)   + dcmplx(sinq*ztrm1,cosq*ztrm1)
!
            cdk(1:2,5) = cdk(1:2,5) + dcmplx(sina*ktrmdk(1:2,iv)*xrkk*ztrm1,cosa*ktrmdk(1:2,iv)*xrkk*ztrm1)
            cdk(1,5)   = cdk(1,5)   + dcmplx(sinq*xrkk*ztrm1dk*xrkk,cosq*xrkk*ztrm1dk*xrkk)
            cdk(2,5)   = cdk(2,5)   + dcmplx(sinq*xrkk*ztrm1dk*yrkk,cosq*xrkk*ztrm1dk*yrkk)
            cdk(1,5)   = cdk(1,5)   + dcmplx(sinq*ztrm1,cosq*ztrm1)
!
            cdk(1:2,6) = cdk(1:2,6) + dcmplx(cosa*ktrmdk(1:2,iv)*tmp3(iv)*kexperfc,-sina*ktrmdk(1:2,iv)*tmp3(iv)*kexperfc)
            cdk(1,6)   = cdk(1,6)   + dcmplx(cosq*tmp3(iv)*kexperfcdk*xrkk,-sinq*tmp3(iv)*kexperfcdk*xrkk)
            cdk(2,6)   = cdk(2,6)   + dcmplx(cosq*tmp3(iv)*kexperfcdk*yrkk,-sinq*tmp3(iv)*kexperfcdk*yrkk)
            cdk(1,6)   = cdk(1,6)   + dcmplx(cosq*yrkk*kexperfc,-sinq*yrkk*kexperfc)
            cdk(2,6)   = cdk(2,6)   + dcmplx(cosq*xrkk*kexperfc,-sinq*xrkk*kexperfc)
!
!  Terms for rigid molecules
!
            if (lrigid) then
              cdk(1,1) = cdk(1,1) + dcmplx(-sinq*tmp1(iv)*kexperfc*xcom,-cosq*tmp1(iv)*kexperfc*xcom)
              cdk(2,1) = cdk(2,1) + dcmplx(-sinq*tmp1(iv)*kexperfc*ycom,-cosq*tmp1(iv)*kexperfc*ycom)
!
              cdk(1,2) = cdk(1,2) + dcmplx(-sinq*tmp2(iv)*kexperfc*xcom,-cosq*tmp2(iv)*kexperfc*xcom)
              cdk(2,2) = cdk(2,2) + dcmplx(-sinq*tmp2(iv)*kexperfc*ycom,-cosq*tmp2(iv)*kexperfc*ycom)
!
              cdk(1,3) = cdk(1,3) + dcmplx(sinq*trmzz*xcom,cosq*trmzz*xcom)
              cdk(2,3) = cdk(2,3) + dcmplx(sinq*trmzz*ycom,cosq*trmzz*ycom)
!
              cdk(1,4) = cdk(1,4) + dcmplx(yrkk*cosq*ztrm1*xcom,-yrkk*sinq*ztrm1*xcom)
              cdk(2,4) = cdk(2,4) + dcmplx(yrkk*cosq*ztrm1*ycom,-yrkk*sinq*ztrm1*ycom)
!
              cdk(1,5) = cdk(1,5) + dcmplx(xrkk*cosq*ztrm1*xcom,-xrkk*sinq*ztrm1*xcom)
              cdk(2,5) = cdk(2,5) + dcmplx(xrkk*cosq*ztrm1*ycom,-xrkk*sinq*ztrm1*ycom)
!
              cdk(1,6) = cdk(1,6) + dcmplx(-sinq*tmp3(iv)*kexperfc*xcom,-cosq*tmp3(iv)*kexperfc*xcom)
              cdk(2,6) = cdk(2,6) + dcmplx(-sinq*tmp3(iv)*kexperfc*ycom,-cosq*tmp3(iv)*kexperfc*ycom)
            endif
          endif
        endif
      enddo
...
!
        d2self = 0.0_dp
        call d2chargep2Dd(iloc,i,j,nkvec,xrk,yrk,ix,iy,iz,jx,jy,jz,tmp1,ktrm0,ktrm3,ktrm3z, &
...
!
  if (lgroupvelocity) then
    ...
    if (status/=0) call deallocate_error('recip2Dpd','ktrmdk')
  endif
  deallocate(tmp3,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','tmp3')
  deallocate(tmp2,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','tmp2')
  deallocate(tmp1,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','tmp1')
  deallocate(ktrm4z,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','ktrm4z')
  deallocate(ktrm4,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','ktrm4')
  deallocate(ktrm3z,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','ktrm3z')
  deallocate(ktrm3,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','ktrm3')
  deallocate(ktrm0,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','ktrm0')
  deallocate(kmod0,stat=status)
  if (status/=0) call deallocate_error('recip2Dpd','kmod0')
!
!  Timing
!
  time1 = g_cpu_time()
