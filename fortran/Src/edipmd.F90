...
!        torsional term.
!   1/14 Use of pi coordination terms made optional
!   4/16 ldoregions flag added to dutils calls
!  11/17 ineigh added to pass to getEDIPneighbour
!   1/18 Modified to allow for second derivative arguments to 
!        called subroutines
!   2/18 Trace added
!   9/18 Strain module added
!   1/19 Finite strain modifications added to cross terms
!   9/19 nrel2 / nrelat changed to nrela2f / nrelf2a
!  11/19 Call to cartstrterm modified to allow for rigid molecules
...
!
  use datatypes
  use configurations, only : nregions, lsliceatom, nregiontype, QMMMmode
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('edipmd','ineigh')
...
!
  ...
  if (status/=0) call outofmemory('edipmd','ineigh')
...
!
      do ni = 1,nneigh(i)
        ...
        if (lstr) then
          call real1strterm(ndim,xd,yd,zd,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
          do is = 1,nstrains
            ns = nstrptr(is)
            d1sZi(is,ni) = d1Z(ni,i)*dr2ds(ns)
          enddo
        endif
      enddo
    endif
...
!
                  ...
                  if (lstr) then
                    call real1strterm(ndim,xij,yij,zij,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                    do is = 1,nstrains
                      ns = nstrptr(is)
                      d1sZi(is,ni) = d1sZi(is,ni) + ztrm2*dr2ds(ns)
                    enddo
                  endif
!
                  ...
                  if (lstr) then
                    ...
                    do is = 1,nstrains
                      ns = nstrptr(is)
                      d1sZi(is,nk) = d1sZi(is,nk) + ztrm2*dr2ds(ns)
                    enddo
                  endif
!
...
!  Derivatives w.r.t. i-j
                  ...
                  if (lstr) then
                    ...
                    do is = 1,nstrains
                      ...
                      d1sZi(is,ni) = d1sZi(is,ni) - ztrm2*dr2ds(ns)
                    enddo
                    call cartstrterm(ndim,xij,yij,zij,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                    do is = 1,nstrains
                      d1sZi(is,ni) = d1sZi(is,ni) + ztrm*rrij*rrik*xik*dxyzds(is,1)
                      d1sZi(is,ni) = d1sZi(is,ni) + ztrm*rrij*rrik*yik*dxyzds(is,2)
                      d1sZi(is,ni) = d1sZi(is,ni) + ztrm*rrij*rrik*zik*dxyzds(is,3)
                    enddo
                  endif
!  Derivatives w.r.t. i-k
                  ...
                  if (lstr) then
                    ...
                    do is = 1,nstrains
                      ...
                      d1sZi(is,nk) = d1sZi(is,nk) - ztrm2*dr2ds(ns)
                    enddo
                    call cartstrterm(ndim,xik,yik,zik,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                    do is = 1,nstrains
                      d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrij*rrik*xij*dxyzds(is,1)
                      d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrij*rrik*yij*dxyzds(is,2)
                      d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrij*rrik*zij*dxyzds(is,3)
                    enddo
                  endif
!
...
!
                  ...
                  do nn = 1,nneigh(i)
                    ...
                    if (lstr) then
                      ...
                      do is = 1,nstrains
                        ns = nstrptr(is)
                        d1sZi(is,nn) = d1sZi(is,nn) + ztrm*d1Z(nn,i)*dr2ds(ns)
                      enddo
                    endif
                  enddo
!
...
!
                  ...
                  do nn = 1,nneigh(j)
                    ...
                    if (lstr) then
                      ...
                      do is = 1,nstrains
                        ns = nstrptr(is)
                        d1sZi(is,ijbase+nn) = d1sZi(is,ijbase+nn) + ztrm*d1Z(nn,j)*dr2ds(ns)
                      enddo
                    endif
                  enddo
                endif
...
!
                      ...
                      if (lstr) then
                        ...
                        do is = 1,nstrains
                          ns = nstrptr(is)
                          d1sZi(is,nl) = d1sZi(is,nl) + ztrm2*dr2ds(ns)
                        enddo
                      endif
!
...
!  Derivatives w.r.t. i-j
                      ...
                      if (lstr) then
                        ...
                        call cartstrterm(ndim,xij,yij,zij,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                        do is = 1,nstrains
                          d1sZi(is,ni) = d1sZi(is,ni) + ztrm*rrij*xcross*dxyzds(is,1)
                          d1sZi(is,ni) = d1sZi(is,ni) + ztrm*rrij*ycross*dxyzds(is,2)
                          d1sZi(is,ni) = d1sZi(is,ni) + ztrm*rrij*zcross*dxyzds(is,3)
                        enddo
                      endif
!  Derivatives w.r.t. i-k
                      ...
                      if (lstr) then
                        ...
                        call cartstrterm(ndim,xik,yik,zik,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                        do is = 1,nstrains
                          d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrij*rrik*rril*(yil*zij - zil*yij)*dxyzds(is,1)
                          d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrij*rrik*rril*(zil*xij - xil*zij)*dxyzds(is,2)
                          d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrij*rrik*rril*(xil*yij - yil*xij)*dxyzds(is,3)
                        enddo
                      endif
!  Derivatives w.r.t. i-l
                      ...
                      if (lstr) then
                        ...
                        do is = 1,nstrains
                          ...
                          d1sZi(is,nl) = d1sZi(is,nl) - ztrm2*dr2ds(ns)
                        enddo
                        call cartstrterm(ndim,xil,yil,zil,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                        do is = 1,nstrains
                          d1sZi(is,nl) = d1sZi(is,nl) + ztrm*rrij*rrik*rril*(zik*yij - yik*zij)*dxyzds(is,1)
                          d1sZi(is,nl) = d1sZi(is,nl) + ztrm*rrij*rrik*rril*(xik*zij - zik*xij)*dxyzds(is,2)
                          d1sZi(is,nl) = d1sZi(is,nl) + ztrm*rrij*rrik*rril*(yik*xij - xik*yij)*dxyzds(is,3)
                        enddo
                      endif
!
...
!
                          ...
                          if (lstr) then
                            call real1strterm(ndim,xjm,yjm,zjm,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                            do is = 1,nstrains
                              ns = nstrptr(is)
                              d1sZi(is,nn) = d1sZi(is,nn) + ztrm2*dr2ds(ns)
                            enddo
                          endif
!
...
!  Derivatives w.r.t. j-m
                          ...
                          if (lstr) then
                            ...
                            do is = 1,nstrains
                              ...
                              d1sZi(is,nn) = d1sZi(is,nn) - ztrm2*dr2ds(ns)
                            enddo
                            call cartstrterm(ndim,xjm,yjm,zjm,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                            do is = 1,nstrains
                              d1sZi(is,nn) = d1sZi(is,nn) + ztrm*rrjm*xcross*dxyzds(is,1)
                              d1sZi(is,nn) = d1sZi(is,nn) + ztrm*rrjm*ycross*dxyzds(is,2)
                              d1sZi(is,nn) = d1sZi(is,nn) + ztrm*rrjm*zcross*dxyzds(is,3)
                            enddo
                          endif
!  Derivatives w.r.t. i-k
                          ...
                          if (lstr) then
                            ...
                            call cartstrterm(ndim,xik,yik,zik,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                            do is = 1,nstrains
                              d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrik*rril*rrjm*(yil*zjm - zil*yjm)*dxyzds(is,1)
                              d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrik*rril*rrjm*(zil*xjm - xil*zjm)*dxyzds(is,2)
                              d1sZi(is,nk) = d1sZi(is,nk) + ztrm*rrik*rril*rrjm*(xil*yjm - yil*xjm)*dxyzds(is,3)
                            enddo
                          endif
!  Derivatives w.r.t. i-l
                          ...
                          if (lstr) then
                            ...
                            call cartstrterm(ndim,xil,yil,zil,0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
                            do is = 1,nstrains
                              d1sZi(is,nl) = d1sZi(is,nl) + ztrm*rrik*rril*rrjm*(zik*yjm - yik*zjm)*dxyzds(is,1)
                              d1sZi(is,nl) = d1sZi(is,nl) + ztrm*rrik*rril*rrjm*(xik*zjm - zik*xjm)*dxyzds(is,2)
                              d1sZi(is,nl) = d1sZi(is,nl) + ztrm*rrik*rril*rrjm*(yik*xjm - xik*yjm)*dxyzds(is,3)
                            enddo
                          endif
!
...
!*************************************
        call EDIP_twobody(nzij,rij,Zitot,e2i,de2idrij,de2idZi, &
                          d2e2idrij2,d2e2idrijdZi,d2e2idZi2,lgrad1,.false.)
!
...
!
        if (lgrad1) then
          ...
          if (lstr) then
            ...
            do is = 1,nstrains
              ns = nstrptr(is)
              d1si(is,ni) = d1si(is,ni) + deijdrij*dr2ds(ns)
            enddo
          endif
!
...
!
            ...
            if (lstr) then
              ...
              do is = 1,nstrains
                ns = nstrptr(is)
                d1si(is,ni) = d1si(is,ni) + de3idr(1)*dr2ds(ns)
              enddo
            endif
!
            ...
            if (lstr) then
              ...
              do is = 1,nstrains
                ns = nstrptr(is)
                d1si(is,nk) = d1si(is,nk) + de3idr(2)*dr2ds(ns)
              enddo
            endif
!
            xjk = xik - xij
            ...
            if (lstr) then
              call real1strterm(ndim,xjk,yjk,zjk,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
              do is = 1,nstrains
                ns = nstrptr(is)
                d1si(is,njk) = d1si(is,njk) + de3idr(3)*dr2ds(ns)
              enddo
            endif
!
            do nn = 1,nneighi3
