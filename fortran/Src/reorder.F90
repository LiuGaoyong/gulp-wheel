...
!
  itmp(1:nasym) = nregionnocfg(nsft+1:nsft+nasym)
  call icollect(nasym,itmp,itmp2,iptr)
  nregionnocfg(nsft+1:nsft+nasym) = itmp(1:nasym)
!
...
!
  ltmp(1:nasym) = lbsmatcfg(nsft+1:nsft+nasym)
  call lcollect(nasym,ltmp,ltmp2,iptr)
  lbsmatcfg(nsft+1:nsft+nasym) = ltmp(1:nasym)
!
