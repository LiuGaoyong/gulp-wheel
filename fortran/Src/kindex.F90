...
!   3/13 Error messages changed to handle higher dimensions
!  10/13 Maximum kindex turned into a variable
!   2/18 Trace added
!   7/23 Increment for maxkvec changed from 100 to 1000
!  
!  Conditions of use:
!
...
!
!  Calculate and store k-vector indices as (e+maxindk)*maxindk3+(f+maxindk)*maxindk2+g+maxindk
!
!  maxindk2 = 2*maxindk
!  maxindk3 = (2*maxindk)**2
!
  if (lra) then
    max1l = 0
  ...
  if (max1l.gt.maxindk.or.max1u.gt.maxindk) then
    if (ioproc) then
      write(ioout,'(/)')
      ...
      elseif (ndim.eq.2) then
        ...
        write(ioout,'(/,''  Current cell parameters : '',/)')
        write(ioout,'(''    a = '',f8.4,''    alpha = '',f8.4)') a,alpha
        write(ioout,'(''    b = '',f8.4)') b
      elseif (ndim.eq.1) then
        write(ioout,'(/,''  Current cell parameter : '',/)')
        write(ioout,'(''    a = '',f8.4)') rv(1,1)
      endif
    endif
    call stopnow('kindex')
  endif
...
!
      ...
      if (perpk.ge.0.0_dp) then
        ...
        max2l1 = max2l + 1
!
        if (max2l.gt.maxindk.or.max2u.gt.maxindk) then
          if (ioproc) then
            write(ioout,'(/)')
            write(ioout,'(''  **** Too many reciprocal lattice vectors needed ****'')')
            write(ioout,'(''  **** Probably due to unphysical cell dimensions ****'')')
...
!
          ...
          if (perpk.ge.0.0_dp) then
            ...
            max3l1 = max3l + 1
!
            if (max3l.gt.maxindk.or.max3u.gt.maxindk) then
              if (ioproc) then
                write(ioout,'(/)')
                write(ioout,'(''  **** Too many reciprocal lattice vectors needed ****'')')
                write(ioout,'(''  **** Probably due to unphysical cell dimensions ****'')')
...
!
              if (rk2.gt.zero.and.rk2.le.rrmx2) then
                ...
                indk(nkvec) = maxindk3*(e + maxindk) + maxindk2*(f + maxindk) + g + maxindk
              endif
!
!  End of e,f,g loops
!
