...
!   6/21 lBOgik flags added for bond order potentials
!   7/21 Bond valence added
!  12/21 pGFNFF modifications added
!   8/22 OpenMM taper added
!   6/23 Output of AA-CLP parameters added
!   8/23 Wildcard excluded from AA-CLP species output
!
!  Conditions of use:
!
...
!
  if (none.gt.0.or.npote.gt.0.or.lbrenner.or.lreaxFF.or.lEDIP.or.lkim_model.or.lgfnff.or.laaclp) then
!**********************************
...
!**********************************
    ...
    if ((tapermax - tapermin).gt.0.01_dp) then
      ...
      elseif (tapertype.eq.7) then
        write(ioout,'(/,''  Taper potentials to zero over '',f8.4,'' Angstroms using OpenMM form'')') (tapermax - tapermin)
      else
    ...
    if (laaclp) then
      write(ioout,'(/,''  AA-CLP charge scale factor = '',f12.4)') aaclp_qscale
      write(ioout,'(''  AA-CLP D      scale factor = '',f12.4)') aaclp_dscale
      write(ioout,'(''  AA-CLP P      scale factor = '',f12.4)') aaclp_pscale
      write(ioout,'(''  AA-CLP R      scale factor = '',f12.4,/)') aaclp_rscale
!       
!  Species terms
!       
      write(ioout,'(/,''  AA-CLP species parameters:'',/)')
      write(ioout,'(''--------------------------------------------------------------------------------'')')
      write(ioout,'(''  Atom Type    Polarisability (Ang^3)  IP (eV)        B          H  '')')
      write(ioout,'(''--------------------------------------------------------------------------------'')')
      do i = 1,nspec
        nat1 = natspec(i)
        if (nat1.ne.maxele) then
          ntype1 = ntypspec(i)
          call label(nat1,ntype1,lab1)
          cstyp1 = 'c'
          if (nat1.gt.maxele) cstyp1 = 's'
          write(ioout,'(2x,a5,1x,a1,7x,f10.5,8x,f12.5,2x,f9.4,2x,f9.4)') lab1,cstyp1,aaclp_pol_spec(i), &
            aaclp_ip_spec(i),aaclp_b_spec(i),aaclp_h_spec(i)
        endif
      enddo
      write(ioout,'(''--------------------------------------------------------------------------------'')')
    endif
...
!
    ...
    if (lgfnff) then
      call gfnff_outpot
      if (nnobo.gt.0) then
        write(ioout,'(/,''  GFNFF topology will exclude bonds between the following atom types : '',/)')
        do i = 1,nnobo
          nvar1 = nobond(i)/1000
      ...
      if (nBOtypeR(i).eq.1) then
        ...
        elseif (nBOtypeR(i).eq.4) then
          ...
          write(ioout,'(a5,a1,2x,''Theta'',1x,2(f16.8,1x),4x,i3)') lab1,cstyp1,BOocoeffR(i), &
            BOlcoeffR(i),nint(BOmcoeffR(i))
          if (lBOgikR(i)) then
            write(ioout,'(a5,a1,2x,''Gik'',3x,3(f16.8,1x))') lab2,cstyp2,BOccoeffR(1,i),BOccoeffR(2,i),BOhcoeffR(i)
          else
      ...
      if (nBOtypeA(i).eq.1) then
        ...
        elseif (nBOtypeA(i).eq.4) then
          ...
          write(ioout,'(a5,a1,2x,''Theta'',1x,2(f16.8,1x),4x,i3)') lab1,cstyp1,BOocoeffA(i), &
            BOlcoeffA(i),nint(BOmcoeffA(i))
          if (lBOgikA(i)) then
            write(ioout,'(a5,a1,2x,''Gik'',3x,3(f16.8,1x))') lab2,cstyp2,BOccoeffA(1,i),BOccoeffA(2,i),BOhcoeffA(i)
          else
...
!
      if (lout) then
        lout = .false.
        write(ioout,'(/,''  Bond valence energy:'',/)')
        write(ioout,'(''--------------------------------------------------------------------------------'')')
        write(ioout,'(''Atom  Type            K_v             v0              K_w             w0        '')')
        write(ioout,'(''                      (eV)                         (eV/Ang**4)       (Ang)      '')')
        write(ioout,'(''--------------------------------------------------------------------------------'')')
      endif
      write(ioout,'(a5,a1,3x,4x,4(f15.6,1x))') lab1,cstyp1,(VBparE(j,i),j=1,4)
      if (.not.lout) then
        write(ioout,'(''--------------------------------------------------------------------------------'')')
      endif
    enddo
...
!****************************
  if (nvalbond.gt.0) then
    lout = .true.
    do i = 1,nvalbond
      if (nvalbondtype(i).eq.1) then
        nat1 = nVBspecB1(i)
        nat2 = nVBspecB2(i)
        ntype1 = nVBtypeB1(i)
        ntype2 = nVBtypeB2(i)
...
!
        if (lout) then
          lout = .false.
          write(ioout,'(/,''  Bond valence potentials : Exponential :'',/)')
          write(ioout,'(''--------------------------------------------------------------------------------'')')
          write(ioout,'(''Atom  Types            r0            rho         Weight          Cutoffs(Ang)   '')')
          write(ioout,'(''  1     2             (Ang)         (Ang)                        Min      Max   '')')
          write(ioout,'(''--------------------------------------------------------------------------------'')')
        endif
        write(ioout,'(2(a5,a1,1x),5x,f10.6,4x,f10.6,2x,f6.4,2x,2(1x,f8.4))') lab1,cstyp1,lab2,cstyp2, &
          VBparB(1,i),VBparB(3,i),VBwgtB(i),rVBmin(i),rVBmax(i)
        if (.not.lout) then
          write(ioout,'(''--------------------------------------------------------------------------------'')')
        endif
      endif
...
!
        if (lout) then
          lout = .false.
          write(ioout,'(/,''  Bond valence potentials : Power law :'',/)')
          write(ioout,'(''--------------------------------------------------------------------------------'')')
          write(ioout,'(''Atom  Types            r0             m          Weight          Cutoffs(Ang)   '')')
          write(ioout,'(''  1     2             (Ang)                                      Min      Max   '')')
          write(ioout,'(''--------------------------------------------------------------------------------'')')
        endif
        write(ioout,'(2(a5,a1,1x),5x,f10.6,4x,f10.6,6x,f6.4,6x,2(1x,f8.4))') lab1,cstyp1,lab2,cstyp2, &
          VBparB(1,i),VBparB(2,i),VBwgtB(i),rVBmin(i),rVBmax(i)
        if (.not.lout) then
          write(ioout,'(''--------------------------------------------------------------------------------'')')
        endif
      endif
