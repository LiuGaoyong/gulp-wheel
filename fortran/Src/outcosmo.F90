...
!   4/18 Truncation of file names removed
...
!  Local variables
!
  character(len=maxwordlength)  :: cosmofilel
  character(len=5)              :: lab
  integer(i4)                   :: i
  integer(i4)                   :: ii
  integer(i4)                   :: inat
...
!
  ...
  if (ind.eq.0) then
    call endstring(cosmofilel,len(cosmofilel),ind)
    cosmofilel(ind:ind+5) = '.cosmo'
  endif
  open(iout,file = cosmofilel,status='unknown')
  write(ioout,'(''  COSMO file written as '',a)') trim(cosmofilel)
!*************************
