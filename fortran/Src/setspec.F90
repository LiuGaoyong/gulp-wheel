...
!   1/16 Handling of vdwspec added
!   4/16 Error in handling of vdwspec corrected so that default values are used
...
!
  ...
  logical                                     :: lfound
!
!  For each species in the configuration list check whether it is present in the species list
!
  nspecin = nspec
  do i = 1,nspecin
    lmask(i) = .true.
...
!
      if (j.le.nspecin.and.lqinspec(j)) then
        ...
        if (.not.lbsmatcfg(i)) lbsmatcfg(i) = lbrspec(j)
      endif
!
!  Set VDW radius to default if not input
!
      if (.not.lvdwinspec(j)) then
        if (natspec(j).le.maxele) then
          vdwspec(j) = rvdw(natspec(j))
        else 
          vdwspec(j) = rvdw(natspec(j)-maxele)
        endif
      endif
!
