...
!   1/09 Modified to allow for different levels of integer precision between GULP and MPI
...
!   7/21 Variable charge derivative arrays added
!   1/22 dedqc added for variable charge second derivatives
!   2/22 Correction to use of nprocs for serial case
!   5/23 Initialise of charge second derivatives now excluded if numerical 
!        second derivatives are being used
!
!  Conditions of use:
!
...
!
  ...
  use control,        only : latomicstress, lDoQDeriv2, lnumerical
  use current
  use derivatives
  use iochannels
  use optimisation
...
#endif
#ifdef MPI
  use parallel,       only : MPI_comm_GULP, nprocs, ioproc
#endif
  use parallel,       only : natomsonnode
  use symmetry
#ifdef TRACE
...
!
  if (lDoQDeriv2.and.lgrad2.and..not.lnumerical) then
    d2edq2(1:numat) = 0.0_dp
    d2edqdq(1:numat,1:natomsonnode) = 0.0_dp
#ifdef MPI
    if (nprocs.gt.0) then
      dedqc(1:3,1:numat) = 0.0_dp
      d2edqc(1:3*numat,1:numat) = 0.0_dp
      ds2g(1:nstrains,1:numat) = 0.0_dp
      ds2gs(1:nstrains,1:numat) = 0.0_dp
      d2s2gs(1:nstrains,1:numat) = 0.0_dp
    endif
#endif
  endif
#ifdef TRACE
