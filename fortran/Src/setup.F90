...
!   8/21 Valence bond added
!  12/21 pGFNFF modifications added
!  10/22 Call to setoptptr added
!   2/23 lstr now set to true if lphon and 3D, as lprop will get set later based on this
!   6/23 Setup added for AA-CLP force field parameters
!   6/23 lvariablecharge now set to be true for EHT case
!   7/23 lbsmatcfg added
!   7/23 nregionnocfg added
!   7/23 extpot added
!   8/23 Set up offsets for field calculation
!
!  Conditions of use:
!
...
!
  ...
  use gulp_gfnff,    only : ngfnff_current_cfg
...
!
  ...
  do i = 1,nasym
    ...
    nregionno(i) = nregionnocfg(nsft+i)
    ...
    lbsmat(i) = lbsmatcfg(nsft+i)
    if (lbsmat(i)) nbsmat = nbsmat + 1
    oxa(i) = oxcfg(nsft+i)
    cna(i) = cncfg(nsft+i)
    extpot(i) = extpotcfg(nsft+i)
  enddo
!
...
!
!  Set up shifts based on initial coordinates
!
  xvoffset = 0.0_dp
  yvoffset = 0.0_dp
  zvoffset = 0.0_dp
  do i = 1,nasym
    xvoffset = xvoffset + xinitial(i)
    yvoffset = yvoffset + yinitial(i)
    zvoffset = zvoffset + zinitial(i)
  enddo
  xvoffset = xvoffset/dble(nasym)
  yvoffset = yvoffset/dble(nasym)
  zvoffset = zvoffset/dble(nasym)
!
!  COSMO parameters
!
...
!******************************************************************
  ...
  do i = 1,numat
    ...
    if (nregionno(nrelf2a(i)).eq.1) then
...
!
!  GFNFF currently don't work with partial occupancy and so check for this
!
  if (lpocc.and.lgfnff) then
    call outerror('partial occupancies not current supported with GFNFF',0_i4)
    call stopnow('setup')
  endif
!
  if (lall) then
...
!
  ...
  if (lrigid) then
    do i = 1,numatnomol
      ii = numatnomolptr(i)
      if (nregionno(nrelf2a(ii)).eq.1) then
...
!
  lstr = (ncell.gt.0.or.lprop.or.(lphon.and.ndim.gt.0).or.(lmd.and.(lpre.or.ndim.eq.3)).or.lstressout)
!
...
!
    if (lseok.and.nregions(ncf).gt.1) then
      call outerror('GFNFF cannot currently be used for surfaces with two or more regions',0_i4)
      call stopnow('setup')
    endif
    if (ncf.ne.ngfnff_current_cfg) then
      call setup_gfnff
!
!  Reset cutoff since this changes if GFNFF is being used
!
      call setcutoffmax
    endif
  endif
