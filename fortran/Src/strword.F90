...
!  12/12 Units scaling removed from time constants in td_external_force
!   5/13 Custom bond type added
!   7/13 Error message shortened to keep with usual length
!   7/13 Symmetry number input added
!   9/13 nsuper changed to be a 2-D array
!   3/14 Fractional bond orders added
!   4/15 Ghost supercell array added
!   7/15 Option exte now changed to external_f in check to distinguish
!        from external_p
!   7/15 Option added to read external potential for sites
!   8/15 translate noise option added
!   9/15 Bug fixed in setting of lopfreg
!   9/15 Added number of cells for lmodco increased from 10 to 1000
!   9/16 hmssgio added
!  11/16 ldumpconnectivity set to true if connect option is present
!   3/17 fix_atom option added
!  10/17 Initial coordinates added to restart info
!   1/18 Trap to avoid out of bounds reference to icos arrays in connect option
!   2/18 Translate thermal option added
!   3/18 Sign option added to translate
!   4/18 Scan_cell option added
!   4/18 Twist option added
!   5/18 Strain sub-option added for scan_cell
!   6/18 Strain cell added
!   7/18 Atom number checking for connectivity corrected
...
!   2/19 Check for blank lines after initial coordinates added
!   7/19 Automatic check to exclude indentity from symmetry operators added
!   8/19 molatom option added
!   9/19 Primitive cell option added
...
!   3/21 Flags set to indicate GFNFF reference configurations have been read
!   3/21 Correction to reading of gfnff_vectors
!   3/21 Reference charges added
!   4/21 gfnff_shrink added
!   5/21 Correction to reading of gfnff coordinates and vectors
...
!
  ...
  use configurations
  use current
  use dump,             only : ldumpconnectivity
...
!
  ...
  if (index(word,'external_f').eq.1) goto 160
  if (index(word,'td_e').eq.1) goto 170
  if (index(word,'prim').eq.1) goto 180
  if (index(word,'external_p').eq.1) goto 190
  if (index(word,'init').eq.1) goto 200
  if (index(word,'spac').eq.1) goto 270
  if (index(word,'symmetry_o').eq.1) goto 280
  if (index(word,'symmetry_c').eq.1) goto 290
  if (index(word,'symmetry_n').eq.1) goto 300
  ...
  if (index(word,'ghos').eq.1) goto 660
  if (index(word,'fix_a').eq.1) goto 670
  if (index(word,'scan').eq.1) goto 680
  if (index(word,'twis').eq.1) goto 690
  if (index(word,'cstr').eq.1) goto 700
  if (index(word,'shear_f').eq.1) goto 710
  if (index(word,'mola').eq.1) goto 720
  if (index(word,'molr').eq.1) goto 730
  if (index(word,'unfr').eq.1) goto 750
  if (index(word,'gfnff_co').eq.1) goto 760
  if (index(word,'gfnff_vec').eq.1) goto 770
  return
!************************************************************
...
!
        ...
        if (lregionrigid(nregioncurr,ncurr)) then
          do j = 1,3
            lopfreg(3*(nregioncurr-1)+j,ncurr) = .false.
          enddo
          if (nword.gt.2) then       
            do nwi = 3,nword    
              if (index(words(nwi),'x').ne.0) lopfreg(3*(nregioncurr-1)+1,ncurr) = .true.
              if (index(words(nwi),'y').ne.0) lopfreg(3*(nregioncurr-1)+2,ncurr) = .true.
              if (index(words(nwi),'z').ne.0) lopfreg(3*(nregioncurr-1)+3,ncurr) = .true.
            enddo
          endif
        endif
      endif
...
!
      do i = 2,nword
        ...
        if (index(words(i),'t-').eq.1.or.index(words(i),'T-').eq.1) ltranatminus(nasum+nasym) = .true.
...
!
  ...
  elseif (nfloat.eq.3) then
    ...
    if (ioproc) then
      write(ioout,'(''!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'')')
      write(ioout,'(''!! This is probably caused by missing flags if the keywords are missing:'')')
...
!
  do i = 1,nasym
    ...
    nregionnocfg(nasum) = nregioncurr
    xcfg(nasum) = xfrac(i)
    ycfg(nasum) = yfrac(i)
    zcfg(nasum) = zfrac(i)
    qlcfg(nasum) = qa(i)
...
!
  if (lmodco) then
    do i = 1,nasym
      ...
      yold = yfrac(i)
      zold = zfrac(i)
      xfrac(i) = mod(xfrac(i)+1000.0_dp,1.0_dp)
      yfrac(i) = mod(yfrac(i)+1000.0_dp,1.0_dp)
      zfrac(i) = mod(zfrac(i)+1000.0_dp,1.0_dp)
!
...
!
  if (nfloat.ge.4) then
    ...
    tdforcecfg(2,j,nlsft+nn) = floats(3)
    tdforcecfg(3,j,nlsft+nn) = floats(4)
  else
    call outerror('Missing data in external force input',iline)
    call stopnow('strword')
  endif
...
180 continue
  ...
  if (nfloat.gt.0) then
    ncorepcfg(ncurr) = abs(nint(floats(1)))
  else
    call outerror('Number of cores missing for primitive cell input',iline)
    call stopnow('strword')
  endif
  lvecpin(ncurr) = .true.
  do i = 1,3
    read(nru,*,err=99)(rvpcfg(j,i,ncurr),j=1,3)
    iline = iline + 1
  enddo
!
!  Scale cell if necessary
!
  if (scalefactor*units.ne.1.0_dp) then
    do i = 1,3
      rvpcfg(1,i,ncurr) = scalefactor*units*rvpcfg(1,i,ncurr)
      rvpcfg(2,i,ncurr) = scalefactor*units*rvpcfg(2,i,ncurr)
      rvpcfg(3,i,ncurr) = scalefactor*units*rvpcfg(3,i,ncurr)
    enddo
  endif
  lwordok = .true.
  return
...
!
  if (nfloat.ge.2) then
    nn = abs(nint(floats(1)))
    if (nn.gt.nascfg(ncurr)) then
      call outerror('Atom number for external potential is too large',iline)
      call stopnow('strword')
    endif
    extpotcfg(nlsft+nn) = floats(2)*units
  else
    call outerror('Missing data in external potential input',iline)
    call stopnow('strword')
  endif
  goto 195
!
...
200 continue
  ii = nasum - nasym
  if (ii.lt.0) then
    call outerror('atom pointer is incorrect in initial coords',iline)
    call stopnow('strword')
  endif
  linitcfg(ncurr) = .true.
202 line = '  '
...
!
  ...
  if (nfloat.lt.3) then
    call outerror('initial coordinates are missing from input',iline)
    call stopnow('strword')
  endif
  ii = ii + 1
  if (ii.gt.nasum) then
    call outerror('atom pointer is too large in initial coords',iline)
    call stopnow('strword')
  endif
  xinitcfg(ii) = floats(1)
  yinitcfg(ii) = floats(2)
  zinitcfg(ii) = floats(3)
  goto 202
...
!
    ...
    if (nfloat.eq.1) then
      ...
      do i = 1,16
        hmssgio(i,ncurr) = gronam(nspcg(ncurr))(i:i)
      enddo
    else
      ...
      if (iflags(ncurr).eq.0) then
        ...
        do i = 1,16
          ...
          hmssgio(i,ncurr) = string(i:i)
          call stouc(hmssg(i,ncurr))
          call stouc(hmssgio(i,ncurr))
        enddo
      endif
    endif
  else
...
280 lreverse = .false.
  ...
  enddo
!
!  Check whether symmetry operator is the identity and if so exclude
!
  diff = abs(ropin(1,1) - 1.0_dp) + abs(ropin(2,2) - 1.0_dp) + abs(ropin(3,3) - 1.0_dp) + &
         abs(ropin(2,1)) + abs(ropin(3,1)) + abs(ropin(3,2)) + &
         abs(ropin(1,2)) + abs(ropin(1,3)) + abs(ropin(2,3))
  if (diff.gt.1.0d-6) then
!
!  Symmetry operator accepted and so copy to configuration array
!
    ngocfg(ncurr) = ngocfg(ncurr) + 1
    if (ngocfg(ncurr).gt.maxsymop) then
      maxsymop = ngocfg(ncurr)
      call changemaxsymop
    endif
    ropcfg(1:3,1:3,ngocfg(ncurr),ncurr) = ropin(1:3,1:3)
    vitcfg(1:3,ngocfg(ncurr),ncurr) = vitin(1:3)
    lsymset(ncurr) = .true.
  endif
!
  lwordok = .true.
...
290 if (index(words(2),'tri').eq.1) then
  ...
  return
!******************************************
!  Input of symmetry number for rotation  *
!******************************************
300 continue
  if (nfloat.gt.0) then
    symnocfg(ncurr) = nint(floats(1))
    if (symnocfg(ncurr).lt.1) then
      call outerror('Symmetry number cannot be less than 1',iline)
      call stopnow('strword')
    endif
  else
    call outerror('Missing value in symmetry_number input',iline)
    call stopnow('strword')
  endif
  lwordok = .true.
  return
...
630 continue
  if (nword.gt.1) then
    if (index(words(2),'zyx').eq.1) then
      ordersuper(ncurr) = 2
    elseif (index(words(2),'xyz').eq.1) then
      ordersuper(ncurr) = 1
    else
      call outerror('Unrecognised supercell ordering specified',iline)
      call stopnow('strword')
    endif
  endif
...
!
!  First read the main translation input
!
  if (nfloat.ge.4) then
...
!
!  Now look for optional extras
!
  if (nword.ge.2) then
    nfread = 4
    do i = 2,nword
      if (index(words(i),'noi').eq.1) then
        ltrannoise(ncurr) = .true.
        if (nfloat.ge.nfread+1) then
          nfread = nfread + 1
          trannoise(ncurr) = abs(floats(nfread))
        else
          call outerror('Insufficient data for translate option',iline)
          call stopnow('strword')
        endif
      elseif (index(words(i),'the').eq.1) then
        ltrantherm(ncurr) = .true.
        if (nfloat.ge.nfread+1) then
          nfread = nfread + 1
          trantherm(ncurr) = abs(floats(nfread))
        else
          call outerror('Insufficient data for translate option',iline)
          call stopnow('strword')
        endif
...
!
!  Check values against maxat
!
    if (n1connect(nconnect).gt.maxat) then
      maxat = n1connect(nconnect)
      call changemaxat
    endif
    if (n2connect(nconnect).gt.maxat) then
      maxat = n2connect(nconnect)
      call changemaxat
    endif
!
!  Set flag to dump connectivity to true
!
...
!
  ...
  if (nword.gt.1) then
    do i = 2,nword
      ...
      elseif (index(words(i),'cu').eq.1) then
        nconnecttype(1,nconnect) = 7
      elseif (index(words(i),'ha').eq.1) then
        nconnecttype(1,nconnect) = 8
      elseif (index(words(i),'quar').eq.1) then
        nconnecttype(1,nconnect) = 9
      elseif (index(words(i),'th').eq.1) then
        nconnecttype(1,nconnect) = 10
...
660 continue
  if (nword.gt.1) then
    ...
    elseif (index(words(2),'xyz').eq.1) then
      ...
      call outerror('Unrecognised ghost_supercell ordering specified',iline)
      call stopnow('strword')
    endif
  endif
  if (ndimen(ncurr).eq.2) then
    if (nfloat.ge.2) then
      ...
      if (nfloat.ge.2) then
        ...
        call outerror('Insufficient data for ghost_supercell input',iline)
        call stopnow('strword')
      endif
    endif
    if (ii.eq.0.or.jj.eq.0) then
      call outerror('Zero supplied for ghost_supercell input',iline)
      call stopnow('strword')
    endif
  elseif (ndimen(ncurr).eq.1) then
    if (nfloat.ge.1) then
  ...
  nsuperghost(1,ncurr) = ii
  nsuperghost(2,ncurr) = jj
  nsuperghost(3,ncurr) = kk
  lwordok = .true.
  return
!********************
!  Fix atom option  *
...
!  5 => none
!
    if (index(words(2),'fir').eq.1) then
      nfixatomtypecfg(ncurr) = 1_i4
    elseif (index(words(2),'las').eq.1) then
      nfixatomtypecfg(ncurr) = 2_i4
    elseif (index(words(2),'cen').eq.1) then
      nfixatomtypecfg(ncurr) = 3_i4
    elseif (index(words(2),'non').eq.1) then
      nfixatomtypecfg(ncurr) = 5_i4
    else
      call outerror('Unrecognised specification type for fix_atom option',iline)
      call stopnow('strword')
    endif
  elseif (nfloat.ge.1) then
!
!  Atom specified by number
!
    nfixatomtypecfg(ncurr) = 4_i4
    nfixatomcfg(ncurr) = nint(abs(floats(1)))
  else
    call outerror('Missing input for fix_atom option',iline)
    call stopnow('strword')
  endif
  lwordok = .true.
  return
...
680 continue
  if (nword.gt.1) then
    lcscanstrain(ncurr) = (index(words(2),'str').eq.1)
  endif
  if (nfloat.eq.0) then
    line = '  '
    read(nru,'(a)') line
...
!
!  Check that strain has been specified as a keyword
!
  if (lcscanstrain(ncurr).and..not.lstraincell) then
    call outerror('scan_cell strain can only be used when the keyword strain is given',iline)
    call stopnow('strword')
  endif
!
!  Read the main cell scan input
!
  if (ndimen(ncurr).eq.3) then
    if (nfloat.ge.7) then
      cscan(1,ncurr) = floats(1)
      cscan(2,ncurr) = floats(2)
      cscan(3,ncurr) = floats(3)
      cscan(4,ncurr) = floats(4)
      cscan(5,ncurr) = floats(5)
      cscan(6,ncurr) = floats(6)
      ncscan(ncurr) = nint(abs(floats(7)))
    else
      call outerror('Insufficient data for scan_cell option',iline)
      call stopnow('strword')
    endif
  elseif (ndimen(ncurr).eq.2) then
    if (nfloat.ge.4) then
      ...
      cscan(2,ncurr) = floats(2)
      cscan(3,ncurr) = floats(3)
      ncscan(ncurr) = nint(abs(floats(4)))
    else
      call outerror('Insufficient data for scan_cell option',iline)
      call stopnow('strword')
    endif
  elseif (ndimen(ncurr).eq.1) then
    if (nfloat.ge.2) then
      cscan(1,ncurr) = floats(1)
      ncscan(ncurr) = nint(abs(floats(2)))
    else
      call outerror('Insufficient data for scan_cell option',iline)
      call stopnow('strword')
    endif
  else
    call outerror('scan_cell option cannot be used for clusters',iline)
    call stopnow('strword')
  endif
  lwordok = .true.
  return
...
!*****************
690 continue
  if (ndimen(ncurr).ne.1) then
    call outerror('twist option can only be used for polymers',iline)
    call stopnow('strword')
  endif
  if (nfloat.ge.1) then
    ntwistcfg(ncurr) = nint(floats(1))
  else
    ntwistcfg(ncurr) = 1_i4
  endif
  lwordok = .true.
  return
!***********************
...
700 continue
  if (ndimen(ncurr).eq.3) then
    if (nfloat.ge.6) then
      straincfg(1,ncurr) = floats(1)
      straincfg(2,ncurr) = floats(2)
      straincfg(3,ncurr) = floats(3)
      straincfg(4,ncurr) = floats(4)
      straincfg(5,ncurr) = floats(5)
      straincfg(6,ncurr) = floats(6)
    else
      call outerror('insufficient strains specified for cstrain',iline)
      call stopnow('strword')
    endif
  elseif (ndimen(ncurr).eq.2) then
    if (nfloat.ge.3) then
...
710 continue
  if (nfloat.ge.7) then
    lshearforcecfg(ncurr) = .true.
    shearforcecfg(ncurr) = floats(1)
    shearforcedircfg(1,ncurr) = floats(2)
    shearforcedircfg(2,ncurr) = floats(3)
    shearforcedircfg(3,ncurr) = floats(4)
    shearforcenormcfg(1,ncurr) = floats(5)
    shearforcenormcfg(2,ncurr) = floats(6)
    shearforcenormcfg(3,ncurr) = floats(7)
!
!  Check that force direction and normal aren't the same
!
    dot1 = shearforcedircfg(1,ncurr)*shearforcenormcfg(1,ncurr) + &
           shearforcedircfg(2,ncurr)*shearforcenormcfg(2,ncurr) + &
           shearforcedircfg(3,ncurr)*shearforcenormcfg(3,ncurr)
    dot2 = shearforcedircfg(1,ncurr)*shearforcedircfg(1,ncurr) + &
           shearforcedircfg(2,ncurr)*shearforcedircfg(2,ncurr) + &
           shearforcedircfg(3,ncurr)*shearforcedircfg(3,ncurr)
    dot3 = shearforcenormcfg(1,ncurr)*shearforcenormcfg(1,ncurr) + &
           shearforcenormcfg(2,ncurr)*shearforcenormcfg(2,ncurr) + &
           shearforcenormcfg(3,ncurr)*shearforcenormcfg(3,ncurr)
    dot2 = sqrt(dot2)
    dot3 = sqrt(dot3)
    if (dot2.lt.1.0d-12) then
      call outerror('Direction for shear_force is ill-defined',iline)
      call stopnow('strword')
    endif
    if (dot3.lt.1.0d-12) then
      call outerror('Normal for shear_force is ill-defined',iline)
      call stopnow('strword')
    endif
    dotp = abs(dot1)/(dot2*dot3)
    if (abs(dotp-1.0_dp).lt.1.0d-12) then
      call outerror('Direction and normal for shear_force cannot be the same',iline)
      call stopnow('strword')
    endif
!
!  Normalise shear direction and normal
!
    shearforcedircfg(1,ncurr) = shearforcedircfg(1,ncurr)/dot2
    shearforcedircfg(2,ncurr) = shearforcedircfg(2,ncurr)/dot2
    shearforcedircfg(3,ncurr) = shearforcedircfg(3,ncurr)/dot2
    shearforcenormcfg(1,ncurr) = shearforcenormcfg(1,ncurr)/dot3
    shearforcenormcfg(2,ncurr) = shearforcenormcfg(2,ncurr)/dot3
    shearforcenormcfg(3,ncurr) = shearforcenormcfg(3,ncurr)/dot3
  else
    call outerror('Insufficient input for shear_force',iline)
    call stopnow('strword')
  endif
!
!  Force symmetry to be turned off since this will break the symmetry
!
  lsymoff = .true.
!
!  Force wrapping of coordinates to be turned off since this is incompatible with this option
!
  lmodco = .false.
!
!  Force output in terms of cell vectors to preserve absolute orientation
!
  lusevectors  = .true.
!
  lwordok = .true.
...
!
!  Check that molrigid hasn't been used - can't mix
!
  if (lmolrigid) then
    call outerror('Cannot mix molatom and molrigid in a run',iline)
    call stopnow('strword')
  endif
!
!  Exclude molecules with one atom or less
!
  if (nfloat.gt.1) then
    lmolatom = .true.
!
!  Check there is enough space to hold the atom lists
!
    if (nfloat+nmolatomtotcfg(ncurr).gt.maxat) then
      maxat = nfloat + nmolatomtotcfg(ncurr)
      call changemaxat
    endif
!
!  Add atoms to the list
!
    nmolcfg(ncurr) = nmolcfg(ncurr) + 1
    if (nmolcfg(ncurr).gt.maxmol) then
      maxmol = nmolcfg(ncurr) + 20
      call changemaxmol
    endif
    nmolatomcfg(nmolcfg(ncurr),ncurr) = nfloat
    do i = 1,nfloat
      nmollistcfg(nmolatomtotcfg(ncurr)+i,ncurr) = nint(abs(floats(i)))
    enddo
    nmolatomtotcfg(ncurr) = nmolatomtotcfg(ncurr) + nfloat
  endif
  lwordok = .true.
  return
!******************************************
!  Molecule - restart for rigid molecule  *
!******************************************
730 if (nfloat.eq.0) then
    line = '  '
    read(nru,'(a)') line
    iline = iline + 1
    call linepro(nru,line,iline)
...
!
!  Check that molatom hasn't been used - can't mix
!
  if (lmolatom) then
    call outerror('Cannot mix molatom and molrigid in a run',iline)
    call stopnow('strword')
...
!
!  Set atom and core counts based on the input
!
  if (nfloat.ge.2) then
    natomin = nint(abs(floats(1)))
    ncorein = nint(abs(floats(2)))
  elseif (nfloat.eq.1) then
    natomin = nint(abs(floats(1)))
    ncorein = natomin
  else
    call outerror('Missing number of atoms and cores for rigid molecule',iline)
    call stopnow('strword')
  endif
!
!  Read next line for atom numbers
!
  read(nru,'(a)',end=99,err=99) line
  iline = iline + 1
  call linepro(nru,line,iline)
  if (nfloat.lt.natomin) then
    call outerror('Insufficient input for molrigid - atom numbers',iline)
    call stopnow('strword')
  endif
!
!  Exclude molecules with one atom or less
...
!
    ...
    nmolatomcfg(nmolcfg(ncurr),ncurr) = nfloat
    nmolcorecfg(nmolcfg(ncurr),ncurr) = ncorein
    do i = 1,nfloat
      nmollistcfg(nmolatomtotcfg(ncurr)+i,ncurr) = nint(abs(floats(i)))
    enddo
!
!  Read next line for quaternions
!
    read(nru,'(a)',end=99,err=99) line
    iline = iline + 1
    call linepro(nru,line,iline)
    if (nfloat.lt.3) then
      call outerror('Insufficient input for molrigid - quaternions',iline)
      call stopnow('strword')
    endif
    molQcfg(1,nmolcfg(ncurr),ncurr) = floats(1)
    molQcfg(2,nmolcfg(ncurr),ncurr) = floats(2)
    molQcfg(3,nmolcfg(ncurr),ncurr) = floats(3)
!
!  Read next line for centre of mass
!
    read(nru,'(a)',end=99,err=99) line
    iline = iline + 1
    call linepro(nru,line,iline)
    if (nfloat.lt.3) then
      call outerror('Insufficient input for molrigid - centre of mass',iline)
      call stopnow('strword')
    endif
    molcomcfg(1,nmolcfg(ncurr),ncurr) = floats(1)
    molcomcfg(2,nmolcfg(ncurr),ncurr) = floats(2)
    molcomcfg(3,nmolcfg(ncurr),ncurr) = floats(3)
!
!  Read lines for atom reference coordinates
!
    do i = 1,nmolcorecfg(nmolcfg(ncurr),ncurr)
      read(nru,'(a)',end=99,err=99) line
      iline = iline + 1
      call linepro(nru,line,iline)
      if (nfloat.lt.3) then
        call outerror('Insufficient input for molrigid - atom coordinates',iline)
        call stopnow('strword')
      endif
      molQxyzcfg(1,nmolatomtotcfg(ncurr)+i,ncurr) = floats(1)
      molQxyzcfg(2,nmolatomtotcfg(ncurr)+i,ncurr) = floats(2)
      molQxyzcfg(3,nmolatomtotcfg(ncurr)+i,ncurr) = floats(3)
    enddo
!
!  Increment the number of atoms in molecules to allow for this molecule
!
    nmolatomtotcfg(ncurr) = nmolatomtotcfg(ncurr) + nmolatomcfg(nmolcfg(ncurr),ncurr)
!
!  Set flag that we are using rigid molecule restarting
!
    lmolrestart = .true.
  endif
  lwordok = .true.
...
!
  ...
  return
!********************************
!  GFNFF reference coordinates  *
!********************************
760 continue
  ii = 0
765 line = '  '
  read(nru,'(a)',end=768) line
  iline = iline + 1
  call linepro(nru,line,iline)
!
  if ((nword+nfloat).eq.0) goto 765
!
  if (nword.gt.0) then
    l55 = .true.
    goto 768
  ...
  if (nfloat.ge.4) then
    ii = ii + 1
!
!  Check there is enough space to hold this atom
!
    if (ii.gt.maxat) then
      maxat = ii
      call changemaxat
    endif
    gfnff_ref_crd(1,ii,ncurr) = floats(1)
    gfnff_ref_crd(2,ii,ncurr) = floats(2)
    gfnff_ref_crd(3,ii,ncurr) = floats(3)
    gfnff_ref_q(ii,ncurr) = floats(4)
  else
    call outerror('Insufficient coordinates in gfnff_coords',iline)
    call stopnow('strword')
  endif
  lgfnff_ref_crd(ncurr) = .true.
  goto 765
768 if (.not.l55) l1000 = .true.
  lwordok = .true.
  return
!****************************
!  GFNFF reference vectors  *
!****************************
770 continue
  ii = 0
775 line = '  '
  read(nru,'(a)',end=778) line
  iline = iline + 1
  call linepro(nru,line,iline)
!
  if ((nword+nfloat).eq.0) goto 775
!
  if (nword.gt.0) then
    l55 = .true.
    goto 778
  ...
  if (ii.eq.1) then
    if (nfloat.eq.1) then
      gfnff_ref_rv(1,ii,ncurr) = floats(1)
      gfnff_ref_rv(2,ii,ncurr) = 0.0_dp
      gfnff_ref_rv(3,ii,ncurr) = 0.0_dp
    elseif (nfloat.eq.2) then
      gfnff_ref_rv(1,ii,ncurr) = floats(1)
      gfnff_ref_rv(2,ii,ncurr) = floats(2)
      gfnff_ref_rv(3,ii,ncurr) = 0.0_dp
    elseif (nfloat.ge.3) then
      gfnff_ref_rv(1,ii,ncurr) = floats(1)
      gfnff_ref_rv(2,ii,ncurr) = floats(2)
      gfnff_ref_rv(3,ii,ncurr) = floats(3)
    else
      call outerror('Insufficient coordinates in gfnff_vectors',iline)
      call stopnow('strword')
    endif
  elseif (ii.eq.2) then
    if (nfloat.eq.2) then
  ...
  elseif (ii.eq.3) then
    ...
    call outerror('Too many vectors in gfnff_vectors',iline)
    call stopnow('strword')
  endif
  lgfnff_ref_rv(ncurr) = .true.
  if (ii.lt.3) goto 775
778 continue
  lwordok = .true.
  return
