...
!   1/16 Call to potwordk corrected
!   1/18 Trace added
!   1/19 maxwordlength changes added
!   8/19 Call to potwordm corrected
!  11/23 Handling of the case where a word is present in a library file
!        that has no relevance added
!  12/23 llibrarycall flag added to setkeyword call
!
!  Conditions of use:
!
...
!
  ...
  character(len=maxlinelength+4) :: lib
...
!
    do j = maxlinelength+1,maxlinelength+4
      lib(j:j) = ' '
    enddo
    lib = libname(i)
    ind = index(lib,'.lib')
    if (ind.eq.0) then
      call endstring(lib,len(lib),ilen)
      lib(ilen:ilen+3) = '.lib'
    endif
    iline = 0
    lint = .true.
...
!
30  call endstring(gulpdir,len(gulpdir),ilend)
    ilend = ilend - 1
...
!
    ...
    if (l50) goto 50
    if (lwordok) goto 40
    call potwordm(7_i4,word,lwordok,iline,line,l45,l50,.true.,lfflags)
    if (l45) goto 45
    if (l50) goto 50
    if (lwordok) goto 40
    call potwordk(7_i4,word,lwordok,iline,line,l45,l50)
    if (l45) goto 45
    if (l50) goto 50
    if (lwordok) goto 40
    ...
    if (lwordok) goto 40
!
!  In case a word has been found that isn't relevant to a library, return to input file to find next word
!
    goto 40
50  continue
  enddo
