...
!
  ...
  real(dp), dimension(:),   allocatable       :: csinkdlambda
  ...
  real(dp), dimension(:),   allocatable       :: sinekdlambda
...
!
  if (lti) then
    allocate(csinkdlambda(nlocalkvec),stat=status)
    if (status/=0) call outofmemory('recip3Dmd','csinkdlambda')
    allocate(sinekdlambda(nlocalkvec),stat=status)
    if (status/=0) call outofmemory('recip3Dmd','sinekdlambda')
  endif
!
!  Setup
!
...
!
      if (lti) then
        c6tot = 0.0_dp
        csinkdlambda(1:nlocalkvec) = 0.0_dp
        sinekdlambda(1:nlocalkvec) = 0.0_dp
...
!
          ...
          do iv = 1,nlocalkvec
            ...
            csinkdlambda(iv)  = csinkdlambda(iv) + qi0*cosi
            sinekdlambda(iv)  = sinekdlambda(iv) + qi0*sini
          enddo
        enddo
      else
        c6tot = 0.0_dp
...
!
      if (lti) then
        call sumall(csinkdlambda,sum,nlocalkvec,"recip3Dmd","csinkdlambda")
        csinkdlambda(1:nlocalkvec) = sum(1:nlocalkvec)
        call sumall(sinekdlambda,sum,nlocalkvec,"recip3Dmd","sinekdlambda")
        sinekdlambda(1:nlocalkvec) = sum(1:nlocalkvec)
      endif
      c6tot = esum6
!
      esum  = 0.0_dp
...
!
      if (lti) then
        dUdl = 0.0_dp
        do iv = procid+1,nlocalkvec,nprocs
          phsqk = (csink(iv)*csinkdlambda(iv) + sinek(iv)*sinekdlambda(iv))
          dUdl = dUdl + 2.0_dp*ktrm(iv)*phsqk
        enddo
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + 0.5_dp*angstoev*dUdl*rtistep
      endif
!*******************
!  Lattice energy  *
!*******************
...
!  C6 case  |
!------------
      csink(1:nlocalkvec) = 0.0_dp
      sinek(1:nlocalkvec) = 0.0_dp
      csink6(1:nlocalkvec) = 0.0_dp
      sinek6(1:nlocalkvec) = 0.0_dp
      if (lti) then
        csinkdlambda(1:nlocalkvec) = 0.0_dp
        sinekdlambda(1:nlocalkvec) = 0.0_dp
      endif
...
!
            ...
            csinkdlambda(iv)  = csinkdlambda(iv) + qi0*csin(i)
            sinekdlambda(iv)  = sinekdlambda(iv) + qi0*sine(i)
          enddo
        else
          do ii = 1,nchargec6
            i = nchargec6ptr(ii)
...
!
        if (lti) then
!
!  Factor of 2 cancels with 0.5 in erecip
!
          dUdl = (csink(iv)*csinkdlambda(iv) + sinek(iv)*sinekdlambda(iv))
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + trmk*dUdl*rtistep
        endif
!**********************
!  Lattice energy     *
!**********************
...
!
  if (lti) then
    deallocate(sinekdlambda,stat=status)
    if (status/=0) call deallocate_error('recip3Dmd','sinekdlambda')
    deallocate(csinkdlambda,stat=status)
    if (status/=0) call deallocate_error('recip3Dmd','csinkdlambda')
  endif
!
