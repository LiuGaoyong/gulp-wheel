...
!   3/21 Initialisation of d2self added when skipping pair
!  11/21 Species added to twobody calls
!  12/21 pGFNFF modifications added
!   3/22 derive0self now set during conditional goto
...
!*************************************************
      ...
      call twobody(eatoml,ereall,ec6l,lgrad1p,lgrad2,.false.,nor,1_i4,npots,npotl,cut2r, &
                   ...
                   nspeci,nspecj,lcspair,lewaldtype,.false.,.false.,lQMMMelectro)
      ...
      if (leem.or.lgfnff) then
        if (lqeq.or.lSandM.or.lgfnff) then
          ...
          elseif (lgfnff) then
            ...
            call gfnffbody(eqeql,zetaij,lgrad1,lgrad2,nor,1_i4,fct,qli,qlj)
          endif
          esum = eqeql
          if (lreg2one) then
            esregion12 = esregion12 + esum
...
!
    lreg2pair = .false.
    if (lseok.and.nregions(ncf).ge.2) then
      lreg2pair = (nregionno(nrelf2a(i)).gt.1)
    endif
!
...
!***********************
    if (ndim.eq.3) then
      call rsearch3D(0.0_dp,0.0_dp,0.0_dp,lmolok,lcspair,i,i,nmi,ixj,iyj,izj,nor,nmolonly,lself,cut2,cut2q)
    elseif (ndim.eq.2) then
      call rsearch2D(0.0_dp,0.0_dp,0.0_dp,lmolok,lcspair,i,i,nmi,ixj,iyj,izj,nor,nmolonly,lself,cut2,cut2q)
    elseif (ndim.eq.1) then
      call rsearch1D(0.0_dp,0.0_dp,0.0_dp,lmolok,lcspair,i,i,nmi,ixj,iyj,izj,nor,nmolonly,lself,cut2,cut2q)
    endif
!
    ereall = 0.0_dp
    erecipl = 0.0_dp
...
!
    ldog1 = (lmdl.or.lsg1.or.lbsmat(nrelf2a(i)).or.(leem.or.lgfnff).or.lDoQDeriv1)
    ldog2 = (lsg2.or.lbsmat(nrelf2a(i)).or.(leem.or.lgfnff).or.lDoQDeriv2)
...
!*************************************************
    ...
    call twobody(eatoml,ereall,ec6l,ldog1,ldog2,.false.,nor,1_i4,npots,npotl,cut2r,cut2q,cut2s,nmolonly, &
                 factor,ofct,ospfct,radsum,sctrm1,sctrm2,qli,qli,nspeci,nspeci,.false.,lewaldtype,.false., &
                 .false.,lQMMMelectro)
    esum = eatoml + ereall + ec6l
    if (lreg2pair) then
      esregion2 = esregion2 + esum
...
!
    ...
    if (leem.or.lgfnff) then
      if (lqeq.or.lSandM.or.lgfnff) then
        ...
        elseif (lgfnff) then
          zetaij = 1.0_dp/sqrt(gfnff_eeq_alp(i)+gfnff_eeq_alp(i))
          call gfnffbody(eqeql,zetaij,ldog1,ldog2,nor,1_i4,fct,qli,qli)
        endif
        if (lreg2pair) then
...
1210 continue
    if (lbsmat(nrelf2a(i)).and.lopi) then
!***********************
!  Radial derivatives  *
!***********************
!
