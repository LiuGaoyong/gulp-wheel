...
!  12/12 reaxff_q0 input added
!   1/14 lBOzrl arrays added
!   1/14 EDIP2p, lEDIP3orig and lEDIPpi added
!   3/14 Unused arguments linr, lint removed
!   7/14 lEDIP3mod added
!   1/15 lint and linr added back to argument list
!   9/15 Kumagai variant of Tersoff potential added
!   9/15 Fitting flag numbers changed for bond order terms
!  12/15 Taper selection option added to botwobody
!   3/16 BO coordination potential added
!   3/16 BOcnswitch option added
!   3/16 BOcntolerance option added
!   5/16 Murty taper option added for Kumagai form
!   1/18 MMP potential added
!   1/19 Incorrect reference of nreaxFFspec corrected to nEDIPspec
!   1/19 Setting of fitting flags corrected for some EDIP inputs
!   1/19 Worsy changed to use word length parameter
!   3/19 Lone pair smoothing added to ReaxFF
!   9/19 ReaxFF charge parameters now depend on species rather than element
!   9/19 Order of species changed when passed to lmatch
!   8/20 Correction BO fitting flags for h
!  11/20 Tersoff reorganised
!   6/21 gik sub-option added for Tersoff
!   6/21 Atom2 for borepulsive and boattractive now defaults to wildcard
!   7/21 Correction to propagation of lBOzrl and lBOgik
!   7/21 Bond valence added
!   9/21 Symmetric flags added for bond order potentials
!  12/21 lnomottlittleton set to be true for EDIP
...
!
  ...
  integer(i4)                                    :: nrs
  integer(i4)                                    :: nval3
  integer(i4)                                    :: nvalbond1
  integer(i4)                                    :: nvalener1
  integer(i4)                                    :: nvar1
  integer(i4)                                    :: nvar2
  integer(i4)                                    :: nvar3
  integer(i4)                                    :: nvar4
...
!
  ...
  if (index(word,'reaxff_q0').eq.1) goto 570
  if (index(word,'boco').eq.1) goto 580
  if (index(word,'bocns').eq.1) goto 590
  if (index(word,'bocnt').eq.1) goto 600
  if (index(word,'reaxffs').eq.1) goto 610
  if (index(word,'boon').eq.1) goto 620
  if (index(word,'valence_b').eq.1) goto 630
  if (index(word,'valence_e').eq.1) goto 640
  if (index(word,'reaxff_e').eq.1) goto 650
  if (index(word,'reaxff_l').eq.1) goto 660
  return
!**********************
!  Brenner potential  *
!**********************
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'cos').eq.1) then
        nBOtapertypeloc = 2
      elseif (index(words(i),'mur').eq.1) then
        nBOtapertypeloc = 3
      endif
      i = i + 1
    enddo
...
!
  ...
  nBOtapertype(nbopot) = nBOtapertypeloc
!
!  Fitting flags
!
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      if (index(words(i),'the').eq.1) then
        nBOtypeR(nboR1) = 2
      elseif (index(words(i),'kum').eq.1) then
        nBOtypeR(nboR1) = 3
      elseif (index(words(i),'mmp').eq.1) then
        nBOtypeR(nboR1) = 4
      elseif (index(words(i),'zrl').eq.1) then
        lBOzrlR(nboR1) = .true.
      elseif (index(words(i),'gik').eq.1) then
        lBOgikR(nboR1) = .true.
      elseif (index(words(i),'sym').eq.1) then
        lBOsymR(nboR1) = .true.
      endif
      i = i + 1
    enddo
...
!
  ...
  lBOgikR(nboR) = lBOgikR(nboR1)
  lBOsymR(nboR) = lBOsymR(nboR1)
  lBOzrlR(nboR) = lBOzrlR(nboR1)
!******************
!  Fitting flags  *
!******************
...
!
  ...
  elseif (nBOtypeR(nboR).eq.3) then
!
!  Kumagai theta term form
!
    if (lfit.and.lfflags) then
      if (nfloat.lt.8) then
        call outerror('Incorrect input during fitting flags',iline)
  ...
  elseif (nBOtypeR(nboR).eq.4) then
!
!  MMP theta term form
!
    if (lfit.and.lfflags) then
...
!
!  If only a single atom input then set atom 2 to be a wildcard
!
  if (l2atoms) then
...
!
    if (nfloat.ge.3) then
      BOmcoeffR(nboR) = floats(1+nbeg)
      BOocoeffR(nboR) = floats(2+nbeg)
      BOlcoeffR(nboR) = floats(3+nbeg)
    elseif (nfloat.eq.2) then
      BOmcoeffR(nboR) = floats(1+nbeg)
      BOocoeffR(nboR) = 1.0_dp
      BOlcoeffR(nboR) = floats(2+nbeg)
    elseif (nfloat.eq.1) then
      BOmcoeffR(nboR) = 3.0_dp
      BOocoeffR(nboR) = 1.0_dp
      BOlcoeffR(nboR) = floats(1+nbeg)
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('boword')
    endif
...
!
    if (nfloat.ge.6) then
      ...
      BOocoeffR(nboR) = floats(2+nbeg)
      BOlcoeffR(nboR) = floats(3+nbeg)
      BOccoeffR(1,nboR) = floats(4+nbeg)
      BOccoeffR(2,nboR) = floats(5+nbeg)
      BOhcoeffR(nboR) = floats(6+nbeg)
    elseif (nfloat.eq.5) then
      BOmcoeffR(nboR) = floats(1+nbeg)
      BOocoeffR(nboR) = 1.0_dp
      BOlcoeffR(nboR) = floats(2+nbeg)
      BOccoeffR(1,nboR) = floats(3+nbeg)
      BOccoeffR(2,nboR) = floats(4+nbeg)
      BOhcoeffR(nboR) = floats(5+nbeg)
    elseif (nfloat.eq.4) then
      BOmcoeffR(nboR) = 3.0_dp
      BOocoeffR(nboR) = 1.0_dp
      BOlcoeffR(nboR) = floats(1+nbeg)
      BOccoeffR(1,nboR) = floats(2+nbeg)
      BOccoeffR(2,nboR) = floats(3+nbeg)
      BOhcoeffR(nboR) = floats(4+nbeg)
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('boword')
    endif
...
!
    if (nfloat.ge.9) then
      ...
      BOccoeffR(1,nboR) = floats(4+nbeg)
      BOccoeffR(2,nboR) = floats(5+nbeg)
      BOccoeffR(3,nboR) = floats(6+nbeg)
      BOccoeffR(4,nboR) = floats(7+nbeg)
      BOccoeffR(5,nboR) = floats(8+nbeg)
      BOhcoeffR(nboR) = floats(9+nbeg)
    elseif (nfloat.eq.8) then
      ...
      BOccoeffR(1,nboR) = floats(3+nbeg)
      BOccoeffR(2,nboR) = floats(4+nbeg)
      BOccoeffR(3,nboR) = floats(5+nbeg)
      BOccoeffR(4,nboR) = floats(6+nbeg)
      BOccoeffR(5,nboR) = floats(7+nbeg)
      BOhcoeffR(nboR) = floats(8+nbeg)
    elseif (nfloat.eq.7) then
      ...
      BOccoeffR(1,nboR) = floats(2+nbeg)
      BOccoeffR(2,nboR) = floats(3+nbeg)
      BOccoeffR(3,nboR) = floats(4+nbeg)
      BOccoeffR(4,nboR) = floats(5+nbeg)
      BOccoeffR(5,nboR) = floats(6+nbeg)
...
!
  ...
  lBOgikA(nboA) = lBOgikA(nboA1)
  lBOsymA(nboA) = lBOsymA(nboA1)
  lBOzrlA(nboA) = lBOzrlA(nboA1)
!******************
!  Fitting flags  *
!******************
...
!
  ...
  elseif (nBOtypeA(nboA).eq.3) then
!
!  Kumagai theta term form
!
    if (lfit.and.lfflags) then
  ...
  elseif (nBOtypeA(nboA).eq.4) then
!
!  MMP theta term form
!
    if (lfit.and.lfflags) then
...
!
    if (nfloat.ge.3) then
      BOmcoeffA(nboA) = floats(1+nbeg)
      BOocoeffA(nboA) = floats(2+nbeg)
      BOlcoeffA(nboA) = floats(3+nbeg)
    elseif (nfloat.eq.2) then
      BOmcoeffA(nboA) = 3.0_dp
      BOocoeffA(nboA) = floats(1+nbeg)
      BOlcoeffA(nboA) = floats(2+nbeg)
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('boword')
    endif
...
!
    if (nfloat.ge.6) then
      ...
      BOocoeffA(nboA) = floats(2+nbeg)
      BOlcoeffA(nboA) = floats(3+nbeg)
      BOccoeffA(1,nboA) = floats(4+nbeg)
      BOccoeffA(2,nboA) = floats(5+nbeg)
      BOhcoeffA(nboA) = floats(6+nbeg)
    elseif (nfloat.eq.5) then
      BOmcoeffA(nboA) = floats(1+nbeg)
      BOocoeffA(nboA) = 1.0_dp
      BOlcoeffA(nboA) = floats(2+nbeg)
      BOccoeffA(1,nboA) = floats(3+nbeg)
      BOccoeffA(2,nboA) = floats(4+nbeg)
      BOhcoeffA(nboA) = floats(5+nbeg)
    elseif (nfloat.eq.4) then
      BOmcoeffA(nboA) = 3.0_dp
      BOocoeffA(nboA) = 1.0_dp
      BOlcoeffA(nboA) = floats(1+nbeg)
      BOccoeffA(1,nboA) = floats(2+nbeg)
      BOccoeffA(2,nboA) = floats(3+nbeg)
      BOhcoeffA(nboA) = floats(4+nbeg)
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('boword')
    endif
...
!
    if (nfloat.ge.9) then
      ...
      BOccoeffA(1,nboA) = floats(4+nbeg)
      BOccoeffA(2,nboA) = floats(5+nbeg)
      BOccoeffA(3,nboA) = floats(6+nbeg)
      BOccoeffA(4,nboA) = floats(7+nbeg)
      BOccoeffA(5,nboA) = floats(8+nbeg)
      BOhcoeffA(nboA) = floats(9+nbeg)
    elseif (nfloat.eq.8) then
      BOmcoeffA(nboA) = floats(1+nbeg)
      BOocoeffA(nboA) = 1.0_dp
      BOlcoeffA(nboA) = floats(2+nbeg)
      BOccoeffA(1,nboA) = floats(3+nbeg)
      BOccoeffA(2,nboA) = floats(4+nbeg)
      BOccoeffA(3,nboA) = floats(5+nbeg)
      BOccoeffA(4,nboA) = floats(6+nbeg)
      BOccoeffA(5,nboA) = floats(7+nbeg)
      BOhcoeffA(nboA) = floats(8+nbeg)
    elseif (nfloat.eq.7) then
      BOmcoeffA(nboA) = 3.0_dp
      BOocoeffA(nboA) = 1.0_dp
      BOlcoeffA(nboA) = floats(1+nbeg)
      BOccoeffA(1,nboA) = floats(2+nbeg)
      BOccoeffA(2,nboA) = floats(3+nbeg)
      BOccoeffA(3,nboA) = floats(4+nbeg)
      BOccoeffA(4,nboA) = floats(5+nbeg)
      BOccoeffA(5,nboA) = floats(6+nbeg)
...
!
  nmatch3 = 0
  do nrs = 1,nreaxFFspec
    if (lmatch(natreaxFFspec(nrs),ntypreaxFFspec(nrs),nvar3,itype3,.false.)) then
      nmatch3 = nmatch3 + 1
  ...
  if (nmatch3.eq.0) then
    ...
    if (nvar3.le.10) lreaxFFunder(nreaxFFspec) = .true.
...
!
  nmatch4 = 0
  do nrs = 1,nreaxFFspec
    if (lmatch(natreaxFFspec(nrs),ntypreaxFFspec(nrs),nvar4,itype4,.false.)) then
      nmatch4 = nmatch4 + 1
  ...
  if (nmatch4.eq.0) then
    if (nvar4.eq.maxele) then
      ...
      if (nvar4.le.10) lreaxFFunder(nreaxFFspec) = .true.
...
!
  do nm1 = 1,nmatch1
    ns1 = nsmatch1(nm1)
    if (nfloat.ge.ind+1) then
      reaxFFmu(ns1) = floats(ind+1)*units
    else
...
!
  do nm1 = 1,nmatch1
    ns1 = nsmatch1(nm1)
    if (nfloat.ge.ind+1) then
      reaxFFchi(ns1) = floats(ind+1)*units
    else
...
!
  do nm1 = 1,nmatch1
    ns1 = nsmatch1(nm1)
    if (nfloat.ge.ind+3) then
      reaxFFshell(1,ns1) = floats(ind+1)*units
      reaxFFshell(2,ns1) = floats(ind+2)
      reaxFFshell(3,ns1) = floats(ind+3)
    else
...
!
  nmatch1 = 0
  do nrs = 1,nEDIPspec
    if (lmatch(natEDIPspec(nrs),ntypEDIPspec(nrs),nvar1,itype1,.false.)) then
      nmatch1 = nmatch1 + 1
      if (nmatch1.gt.maxmatch) then
        call outerror('maxmatch parameter exceeded - increase and recompile',iline)
        call stopnow('boword')
...
!
  nmatch2 = 0
  do nrs = 1,nEDIPspec
    if (lmatch(natEDIPspec(nrs),ntypEDIPspec(nrs),nvar2,itype2,.false.)) then
      nmatch2 = nmatch2 + 1
      if (nmatch2.gt.maxmatch) then
        call outerror('maxmatch parameter exceeded - increase and recompile',iline)
        call stopnow('boword')
...
!
    maxdone = nEDIPspec*(nEDIPspec+1)/2
    allocate(ldone(maxdone),stat=status)
    if (status/=0) call outofmemory('boword','ldone')
!
    if (lpi) then
    ...
    do ifl = 1,iflmax
      n1 = nint(floats(nfloat-iflmax+ifl))
      if (n1.eq.1) then
!
!  Loop over pairs of matches
!
        ldone(1:maxdone) = .false.
        nfit0 = nfit + 1
        do nm1 = 1,nmatch1
          ns1 = nsmatch1(nm1)
          do nm2 = 1,nmatch2
...
!
      if (nfloat.ge.7) then
        ...
        EDIP2p(ind)       = floats(3+nbeg)
        EDIP2beta(ind)    = floats(4+nbeg)
        EDIP2sigma(ind)   = floats(5+nbeg)
        EDIP2a(ind)       = floats(6+nbeg)
        EDIP2aprime(ind)  = floats(7+nbeg)
      else
...
!
  ...
  if (nword.gt.1) then
    do i = 2,nword
      if (index(words(i),'ori').eq.1) then
        loriginal = .true.
      elseif (index(words(i),'mod').eq.1) then
        lmodified = .true.
      endif
    enddo
  endif
!
!  Check that modified and original haven't both been specified
!
  if (lmodified.and.loriginal) then
    call outerror('both modified and original cannot be specified for edip3body',iline)
    call stopnow('boword')
  endif
525 line = '  '
...
!
  nmatch3 = 0
  do nrs = 1,nEDIPspec
    if (lmatch(natEDIPspec(nrs),ntypEDIPspec(nrs),nvar3,itype3,.false.)) then
      nmatch3 = nmatch3 + 1
      if (nmatch3.gt.maxmatch) then
        call outerror('maxmatch parameter exceeded - increase and recompile',iline)
        call stopnow('boword')
...
!
!  Set flag as to whether this is original or new form
!
        lEDIP3mod(ind,ns1)  = lmodified
        lEDIP3orig(ind,ns1) = loriginal
!
...
!
    if (nfloat.gt.0) then
      ...
      if (ii.eq.1) then
        lreaxFFunder(ns1) = .true.
      else
        lreaxFFunder(ns1) = .false.
      endif
    endif
  enddo
  goto 565
...
570 continue
575 line = '  '
  read(iin,'(a)',end=578) line
  iline = iline + 1
  call linepro(iin,line,iline)
!
!  Check whether input is symbol or option
!
  if ((nword+nfloat).eq.0) goto 575
  if (nword.gt.0) then
    word = words(1)
    call worsy(word,lsymbol,.true.)
    if (.not.lsymbol) then
...
!
  do nm1 = 1,nmatch1
    ns1 = nsmatch1(nm1)
    if (nfloat.ge.ind+1) then
      reaxFFq0(ns1) = floats(ind+1)
    else
      call outerror('Incorrect input for reaxFF_q0',iline)
      call stopnow('boword')
    endif
  enddo
  goto 575
578 if (.not.l55) l1000 = .true.
  lwordok = .true.
  return
!*******************************************
!  Parameters for bond-order coordination  *
!*******************************************
580 if (nboZ.eq.maxnboZ) then
    maxnboZ = nboZ + 10
    call changemaxnboz
  endif
  lnoanald3 = .true.
!
  nboZ1 = nboZ + 1
585 line = '  '
  read(iin,'(a)',end=588) line
  iline = iline + 1
  call linepro(iin,line,iline)
!
!  Check whether input is symbol or option
!
  if ((nword+nfloat).eq.0) goto 585
  if (nword.gt.0) then
    word = words(1)
    call worsy(word,lsymbol,.true.)
    if (.not.lsymbol) then
...
!
  if (nfloat.ge.4) then
    BOccoeffZ(1,nboZ) = floats(1+nbeg)
    BOccoeffZ(2,nboZ) = floats(2+nbeg)
    BOzcoeffZ(nboZ) = floats(3+nbeg)
    BOecoeffZ(nboZ) = floats(4+nbeg)
  elseif (nfloat.eq.3) then
    BOccoeffZ(1,nboZ) = floats(1+nbeg)
    BOccoeffZ(2,nboZ) = floats(2+nbeg)
    BOzcoeffZ(nboZ) = floats(3+nbeg)
    BOecoeffZ(nboZ) = 0.0_dp
  else
    call outerror('Incorrect potential coefficient input',iline)
    call stopnow('boword')
  endif
!
  goto 585
588 if (.not.l55) l1000 = .true.
  lwordok = .true.
  return
!**************************************************************
!  Parameters for bond-order coordination switching function  *
!**************************************************************
590 continue
  lBOcnzin = .true.
  if (nfloat.ge.2) then
...
!  Check that Zt > Zb
!
  if (BOcnzb.gt.BOcnzt) then
    call outerror('Zt must be greater than Zb',iline)
    call stopnow('boword')
  endif
!
!  Check that Zt-Zb is not less than 0
!
  if (BOcnzt-BOcnzb.lt.0.0_dp) then
    call outerror('Zt-Zb must be greater than or equal to 0',iline)
    call stopnow('boword')
  endif
!
!  Check that Zt+Zb is greater than 1
!
  if (BOcnzt+BOcnzb.gt.1.0_dp) then
    call outerror('Zt+Zb must be less than or equal to 1',iline)
    call stopnow('boword')
  endif
  lwordok = .true.
  return
!***************************************************************
!  Parameter for bond-order coordination derivative tolerance  *
!***************************************************************
600 continue
  if (nfloat.ge.1) then
    BOcntol = abs(floats(1))
  else
    call outerror('Incorrect BOcntolerance input',iline)
    call stopnow('boword')
  endif
!
!  Check that tolerance is sensible
!
  if (BOcntol.gt.1.0_dp) then
    call outerror('BOcntol cannot be greater than 1',iline)
    call stopnow('boword')
  endif
  lwordok = .true.
  return
!*******************************
!  ReaxFF lone pair smoothing  *
!*******************************
610 continue
  if (nword.gt.1) then
    if (index(words(2),'lone').eq.1) then
      lreaxFFlpsmooth = .true.
    elseif (index(words(2),'over').eq.1) then
      lreaxFFovsmooth = .true.
    else
      lreaxFFlpsmooth = .true.
      lreaxFFovsmooth = .true.
    endif
  ...
  if (nfloat.gt.0) then
    reaxFFlpsmooth = abs(floats(1))
    if (reaxFFlpsmooth.lt.20.0_dp) then
      call outwarning('Lone pair smoothing factor for reaxFF is too low',iline)
    endif
  endif
  lwordok = .true.
  return
!******************************************************************
!  Parameters for one-body component of the bond-order potential  *
!******************************************************************
620 continue
  lnoanald3 = .true.
!
...
!
  if (nfloat.ge.4) then
    BOecoeffR(nboO) = floats(1+nbeg)
    BOecoeffA(nboO) = floats(2+nbeg)
    BOncoeffR(nboO) = floats(3+nbeg)
    BOncoeffA(nboO) = floats(4+nbeg)
  else
    call outerror('Incorrect potential coefficient input',iline)
    call stopnow('boword')
  endif
...
628 if (.not.l55) l1000 = .true.
  ...
  return
!******************************************
!  Parameters for bond valence potential  *
!******************************************
630 if (nvalbond.eq.maxvalbond) then
    maxvalbond = nvalbond + 10
    call changemaxvalbond
  endif
!
  nvalbond1 = nvalbond + 1
  nvalbondtype(nvalbond1) = 1
  if (nword.gt.1) then
    i = 2
    do while (i.le.nword)
...
!
  ...
  nvalbond = nvalbond + 1
  if (nvalbond.gt.maxvalbond) then
    maxvalbond = nvalbond + 10
    call changemaxvalbond
  endif
  nvalbondtype(nvalbond) = nvalbondtype(nvalbond1)
!******************
!  Fitting flags  *
!******************
  if (nvalbondtype(nvalbond).eq.1) then
!
!  Exponential form
!
    if (lfit.and.lfflags) then
  ...
  elseif (nvalbondtype(nvalbond).eq.2) then
!
!  Power law form
!
    if (lfit.and.lfflags) then
...
!
  if (l2atoms) then
    nVBspecB1(nvalbond) = nvar1
    nVBtypeB1(nvalbond) = itype1
    nVBspecB2(nvalbond) = nvar2
    nVBtypeB2(nvalbond) = itype2
  else
    nVBspecB1(nvalbond) = nvar1
    nVBtypeB1(nvalbond) = itype1
    nVBspecB2(nvalbond) = maxele
    nVBtypeB2(nvalbond) = 0_i4
  endif
!
  if (nvalbondtype(nvalbond).eq.1) then
!*********************
...
!
    if (nfloat.ge.5) then
      VBparB(1,nvalbond) = floats(1+nbeg)
      VBparB(3,nvalbond) = floats(2+nbeg)
      VBwgtB(nvalbond) = floats(3+nbeg)
      rVBmin(nvalbond) = floats(4+nbeg)
      rVBmax(nvalbond) = floats(5+nbeg)
    elseif (nfloat.eq.4) then
      VBparB(1,nvalbond) = floats(1+nbeg)
      VBparB(3,nvalbond) = floats(2+nbeg)
      VBwgtB(nvalbond) = floats(3+nbeg)
      rVBmin(nvalbond) = 0.0_dp
      rVBmax(nvalbond) = floats(4+nbeg)
    elseif (nfloat.eq.3) then
      VBparB(1,nvalbond) = floats(1+nbeg)
      VBparB(3,nvalbond) = floats(2+nbeg)
      VBwgtB(nvalbond) = 1.0_dp
      rVBmin(nvalbond) = 0.0_dp
      rVBmax(nvalbond) = floats(3+nbeg)
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('boword')
    endif
...
!
    if (nfloat.ge.5) then
      VBparB(1,nvalbond) = floats(1+nbeg)
      VBparB(2,nvalbond) = floats(2+nbeg)
      VBwgtB(nvalbond) = floats(3+nbeg)
      rVBmin(nvalbond) = floats(4+nbeg)
      rVBmax(nvalbond) = floats(5+nbeg)
    elseif (nfloat.eq.4) then
...
638 if (.not.l55) l1000 = .true.
  ...
  return
!**************************************
!  Parameters for bond valence energy *
!**************************************
640 if (nvalener.eq.maxvalener) then
    maxvalener = nvalener + 10
    call changemaxvalener
  endif
!
  nvalener1 = nvalener + 1
  nvalenertype(nvalener1) = 1
645 line = '  '
...
!
  ...
  nvalener = nvalener + 1
  lnomottlittleton = .true.
  if (nvalener.gt.maxvalener) then
    maxvalener = nvalener + 10
    call changemaxvalener
  endif
  nvalenertype(nvalener) = nvalenertype(nvalener1)
!******************
!  Fitting flags  *
!******************
  if (nvalenertype(nvalener).eq.1) then
    if (lfit.and.lfflags) then
      if (nfloat.lt.4) then
        call outerror('Incorrect input during fitting flags',iline)
        call stopnow('boword')
...
!
  ...
  nVBspecE(nvalener) = nvar1
  nVBtypeE(nvalener) = itype1
!
  if (nvalenertype(nvalener).eq.1) then
!******************
!  Standard form  *
...
!
    if (nfloat.ge.4) then
      VBparE(1,nvalener) = floats(1+nbeg)
      VBparE(2,nvalener) = floats(2+nbeg)
      VBparE(3,nvalener) = floats(3+nbeg)
      VBparE(4,nvalener) = floats(4+nbeg)
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('boword')
    endif
...
648 if (.not.l55) l1000 = .true.
  ...
  return
!**************************************
!  Turn off ReaxFF energy components  *
!**************************************
650 continue
  if (nword.gt.1) then
    do i = 2,nword
      if (index(words(i),'bon').eq.1) then
        lreaxFF_ebond = .false.
      elseif (index(words(i),'bpe').eq.1) then
        lreaxFF_ebpen = .false.
      elseif (index(words(i),'lon').eq.1) then
        lreaxFF_elone = .false.
      elseif (index(words(i),'ove').eq.1) then
        lreaxFF_eover = .false.
      elseif (index(words(i),'und').eq.1) then
        lreaxFF_eunder = .false.
      elseif (index(words(i),'val').eq.1) then
        lreaxFF_eval = .false.
      elseif (index(words(i),'coa').eq.1) then
        lreaxFF_ecoa = .false.
      elseif (index(words(i),'pen').eq.1) then
        lreaxFF_epen = .false.
      elseif (index(words(i),'tor').eq.1) then
        lreaxFF_etors = .false.
      elseif (index(words(i),'con').eq.1) then
        lreaxFF_econj = .false.
      elseif (index(words(i),'hb').eq.1) then
        lreaxFF_ehb = .false.
      elseif (index(words(i),'vdw').eq.1) then
        lreaxFF_evdw = .false.
      elseif (index(words(i),'cou').eq.1) then
        lreaxFF_ecoul = .false.
      endif
    enddo 
  endif
...
660 continue
  ...
  lreaxFF = .true.
  if (nword.gt.1) then
    reaxff_ffield = words(2)
  else
    call outerror('Name of reaxff library file is missing',iline)
    call stopnow('boword')
  endif
  lwordok = .true.
  return
