  subroutine putpar(xc,ncfold,xf,lput)
!
...
!          14 = (SM) zeta
!          15 = (SM) Znuc
!          16 = Used to be electronegativity related, but not used after changes
!          17 = Used to be electronegativity related, but not used after changes
!          18 = Used to be electronegativity related, but not used after changes
!          19 = Used to be electronegativity related, but not used after changes
!          20 = UFF radius
!          21 = UFF theta
!          22 = UFF distance
!          23 = UFF energy
...
!          39 = Gasteiger species A
!          40 = Gasteiger species B
!          41 = Gasteiger species C
!          42 = Gasteiger species Q0
!
...
!  11/12 Unused argument n removed
!   1/14 EDIP2p added
!   8/14 Modified to allow for twopot parameters 8 & 9
!   2/15 setlenn renamed to setcombine
!   3/15 MM3stretch added
!   3/15 Gasteiger species parameters added
!   3/15 Call to gasteiger calculation added if parameters change
!   9/15 Added number of cells for lmodco increased from 10 to 1000
!   9/15 Bond order terms changed to allow for more fitting parameters
!  11/17 Modified for buffered LJ potential
!  12/17 Call to applyconcfg added for simultaneous fitting to ensure
!        structure in restart file has correct symmetry
!   2/18 Trace added
!   5/18 Electronegativity parameter numbers changed
!   3/19 Referencing of fit variables changed for shell coordinates
!   9/19 ReaxFF charge arrays moved to reaxFFdata
!   3/20 Handling of screened Coulomb potential added for first potential term
!  11/20 Tersoff reorganised
!   1/21 BO lambda and omega fitting variables swapped
!   7/21 Bond valence added
!  11/22 labs now applies to epsilon and sigma of LJ potential
!
!  Conditions of use:
!
...
!
        ...
        dum = xc(i)*scale(i)
        if (nv2.eq.iopt_xf) then
          if (ndimen(ncf).ge.1.and.lmodco) dum = mod(dum+1000.0_dp,1.0_dp)
          xcfg(nsft+nv) = dum
        elseif (nv2.eq.iopt_yf) then
          if (ndimen(ncf).ge.2.and.lmodco) dum = mod(dum+1000.0_dp,1.0_dp)
          ycfg(nsft+nv) = dum
        elseif (nv2.eq.iopt_zf) then
          if (ndimen(ncf).eq.3.and.lmodco) dum = mod(dum+1000.0_dp,1.0_dp)
          zcfg(nsft+nv) = dum
        endif
        if (lput) xf(i) = dum
!
!  Apply constraints to ensure that symmetry is maintained in the restart file
!
        call applyconcfg
      elseif (nf.eq.2) then
!
...
!
        if (nv.ge.1.and.nv.le.maxele) then
          chirange(nv2,nv,nv3) = xc(i)*scale(i)
          if (lput) xf(i) = chirange(nv2,nv,nv3)
        endif
...
!
        if (nv.ge.1.and.nv.le.maxele) then
          murange(nv2,nv,nv3) = xc(i)*scale(i)
          if (lput) xf(i) = murange(nv2,nv,nv3)
        endif
...
!
        if (nv.ge.1.and.nv.le.maxele) then
          radrange(nv2,nv,nv3) = xc(i)*scale(i)
          if (lput) xf(i) = radrange(nv2,nv,nv3)
        endif
...
!
        if (nv.ge.1.and.nv.le.maxele) then
          zetarange(nv2,nv,nv3) = xc(i)*scale(i)
          if (lput) xf(i) = zetarange(nv2,nv,nv3)
        endif
...
!
        if (nv.ge.1.and.nv.le.maxele) then
          znucrange(nv2,nv,nv3) = xc(i)*scale(i)
          if (lput) xf(i) = znucrange(nv2,nv,nv3)
        endif
...
!-----------------------
      ...
      if (nv.eq.1) then
        ...
        if (np.eq.1.or.np.eq.2.or.(np.ge.4.and.np.le.7).or.np.eq.8.or.np.eq.12.or.np.eq.13.or. &
            np.eq.14.or.np.eq.17.or.np.eq.31.or.np.eq.58.or.np.eq.60) then
          if (.not.lnoabs) twopot(1,nf) = abs(twopot(1,nf))
        elseif (np.eq.64) then
!
!  For screened Coulomb potential the saturation dielectric constant must be >= 1
!
          if (.not.lnoabs) twopot(1,nf) = abs(twopot(1,nf))
          twopot(1,nf) = max(1.0_dp,twopot(1,nf))
        endif
        if (lput) xf(i) = twopot(1,nf)
      elseif (nv.eq.2) then
        twopot(2,nf) = xc(i)*scale(i)
        if (np.le.2.or.np.eq.5.or.np.eq.6.or.np.eq.12.or.np.eq.13.or.np.eq.14.or.np.eq.16.or. &
            np.eq.28.or.np.eq.29.or.np.eq.58.or.np.eq.60) then
          twopot(2,nf) = abs(twopot(2,nf))
        endif
        if (lput) xf(i) = twopot(2,nf)
      elseif (nv.eq.3) then
        twopot(3,nf) = xc(i)*scale(i)
        if (np.eq.1.or.np.eq.7.or.np.eq.10.or.np.eq.16.or.np.eq.17.or.np.eq.31.or.np.eq.46.or.np.eq.60) then
...
! For VBO_twobody potential the cutoff is also related to the delta parameter
      ...
      elseif (nv.eq.8) then
        twopot(8,nf) = xc(i)*scale(i)
        if (lput) xf(i) = twopot(8,nf)
      elseif (nv.eq.9) then
        twopot(9,nf) = xc(i)*scale(i)
        if (lput) xf(i) = twopot(9,nf)
...
!
        ...
        elseif (nv2.eq.3) then
          EDIP2p(nf) = xc(i)*scale(i)
          if (lput) xf(i) = EDIP2p(nf)
        elseif (nv2.eq.4) then
...
!---------------------------
!  Bond valence variables  -
!---------------------------
      if (nv.eq.1) then
!
!  VB bond potential parameter 1
!
        VBparB(1,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparB(1,nf)
      elseif (nv.eq.2) then
!
!  VB bond potential parameter 2
!
        VBparB(2,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparB(2,nf)
      elseif (nv.eq.3) then
!
!  VB bond potential parameter 3
!
        VBparB(3,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparB(3,nf)
      elseif (nv.eq.4) then
!
!  VB energy parameter 1
!
        VBparE(1,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparE(1,nf)
      elseif (nv.eq.5) then
!
!  VB energy parameter 2
!
        VBparE(2,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparE(2,nf)
      elseif (nv.eq.6) then
!
!  VB energy parameter 3
!
        VBparE(3,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparE(3,nf)
      elseif (nv.eq.7) then
!
!  VB energy parameter 4
!
        VBparE(4,nf) = xc(i)*scale(i)
        if (lput) xf(i) = VBparE(4,nf)
      endif
    endif
  enddo
!*******************
...
!*******************
  ...
  if (lgastchange) call gasteiger(.false.)
  if (lqchange) call qupdate
  if (ncf.gt.0) call setup(.true.)
!**********************
!  Update potentials  *
!**********************
  if (leschange) call setcombine
