...
!   8/21 manysd2 replaces manysd for all symmetry cases
!   8/21 Call to getinbox added
!  12/21 pGFNFF modifications added
!   2/22 Memory check added for charge derivatives
!   5/22 Option added to call gfnff_energy for parallel second derivatives
!        with improved scaling in dispersion
!   6/22 Arguments for one call of valencebondd corrected
!   7/22 Call to reaxff added
!  12/22 Charge derivative arrays now resized for reaxFF as well
!  12/22 ReaxFF now called before d2charge_finish
!  12/22 m_qderiv added
!   7/23 lupdateinbox added
!   8/23 lgrad2 passed to electricfield
!
!  Conditions of use:
!
...
!
  ...
  use bondvalence,   only : nvalener
  use cellmultipole
  use configurations
  use g_constants
  use control
  use current
  use derivatives,   only : maxd2qu, maxd2q
  ...
  use kspace
  use m_qderiv,      only : d2charge_finish, d2charged_finish
  use m_three
  use molecule
  use one,           only : none
  use optimisation
  ...
  use shifts
  use six
  use spatial,       only : lspatialok, lupdateinbox
...
!*************************************************
!  Setup images of atom within the central cell  *
!*************************************************
  if (lupdateinbox) call getinbox
!*******************************************
...
!
!  Check the memory for the charge derivatives if necessary
!
...
!*******************************************************************************
!  GFNFF forcefield - must be called before electrostatics to set the charges  *
!*******************************************************************************
  if (lgfnff) then
    if (lgrad2) then
      if (nprocs.gt.1) then
        if (lscaleMPI) then
          call gfnff_energyd_s(egfnff,lgrad1,lgrad2)
        else
          call gfnff_energyd(egfnff,lgrad1,lgrad2)
        endif
      else
        call gfnff_energy(egfnff,lgrad1,lgrad2)
      endif
    else
      call gfnffmd(egfnff,lgrad1)
    endif
  endif
!*******************************
...
!**********************
  if (lreaxFF) then
    if (lgrad2) then
      if (nprocs.gt.1) then
        call reaxFFds(ereaxFF,lgrad1,lgrad2)
      else
        call reaxFFs(ereaxFF,lgrad1,lgrad2)
      endif
    else
      call reaxFFmds(ereaxFF,lgrad1)
    endif
  endif
!**********************************************************
!  Variable charge second derivatives                     *
!  NB: Must be called after all variable charge methods!  *
!**********************************************************
  if (lgrad2.and.lDoQDeriv2.and..not.loldd2q) then
    if (nprocs.gt.1) then
      call d2charged_finish
    else
      call d2charge_finish
    endif
  endif
!*****************************
...
!****************************
  if (nvalener.gt.0) then
    if (lsymopt.and.lsymderv) then
      if ((lgrad2.and..not.lsymderv2).or.lsymoffloc) then
        if (nprocs.gt.1) then
          call valencebondd(eval,lgrad1,lgrad2)
        else
          call valencebond(eval,lgrad1,lgrad2)
        endif
      else
        call valencebondsd2(eval,lgrad1,lgrad2)
      endif
    else
      if (lgrad2) then
        if (nprocs.gt.1) then
        ...
        call valencebondmd(eval,lgrad1)
      endif
    endif
  endif
!*******************
...
!*******************
  if (lfieldcfg(ncf)) then
    call electricfield(efield,lgrad1,lgrad2)
  endif
!*******************
!  Einstein model  *
!*******************
...
!*****************************
  if ((ldipole.or.lddipole.or.lzdipole).and.(lewald.or.lwolf).and.ndim.gt.0) then
    if (lsymopt.and.lsymderv) then
      if ((lgrad2.and..not.lsymderv2).or.lsymoffloc) then
        if (nprocs.gt.1) then
...
!********************************
  if (nprocs.gt.1) then
    call psumall(eatom,ereal,erecip,ec6,eqeq,eattach,esregion12,esregion2,ethb,efor, &
      ...
      eval,egfnff,lgrad1,lgrad2,(lsymderv.and..not.lgrad2))
  endif
!********************************
