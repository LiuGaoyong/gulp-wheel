  subroutine fourimpd(eimp,esregion12,esregion2,eattach,lgrad1,lgrad2,xderv,yderv,zderv)
!
!  Subroutine for four-body energy from improper torsions
!  Distributed memory parallel version
!
!   1/17 Created from fourimp
...
!
  ...
  use mdlogic
  use molecule
  use parallel,       only : ioproc, natomsonnode, atom2local, node2atom
  use optimisation
  use symmetry
#ifdef TRACE
...
!
  ...
  if (status/=0) call outofmemory('fourimpd','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourimpd','ntypmiddle')
  allocate(nptrnforimp(nfor),stat=status)
  if (status/=0) call outofmemory('fourimpd','nptrnforimp')
  allocate(nuniqueiptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimpd','nuniqueiptr')
  allocate(nuniquekptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimpd','nuniquekptr')
  allocate(nuniquelptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimpd','nuniquelptr')
!
...
!**************************************************
!  Loop over middle site 2 / j - middle atom case *
!**************************************************
  jxl = - 2
  jyl = - 1
  jzl =   0
...
!
          ...
          if (lgrad2) then
            ...
            n2z = jzl
!
!  lopj will be true for the following
!
            n2x1 = jxl
            n2x3 = jxl
            n2x4 = jxl
...
!
                derv3(n2x,kk) = derv3(n2x,kk) - e1d(4)*d2r2dsdx(ks,1,4) + &
                  e1d(1)*d2r2dsdx(ks,1,1) - e1d(5)*d2r2dsdx(ks,1,5)
                derv3(n2y,kk) = derv3(n2y,kk) - e1d(4)*d2r2dsdx(ks,2,4) + &
                  e1d(1)*d2r2dsdx(ks,2,1) - e1d(5)*d2r2dsdx(ks,2,5)
                derv3(n2z,kk) = derv3(n2z,kk) - e1d(4)*d2r2dsdx(ks,3,4) + &
                  e1d(1)*d2r2dsdx(ks,3,1) - e1d(5)*d2r2dsdx(ks,3,5)
!
...
!
  ...
  enddo ljloop
!***********************************************
!  Loop over middle site 2 / j - end atom case *
!***********************************************
  jx = - 2
  jy = - 1
  jz =   0
...
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"fourimpd","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!
                derv3(n1x,kk) = derv3(n1x,kk) - e1d(1)*d2r2dsdx(ks,1,1) - &
                  e1d(2)*d2r2dsdx(ks,1,2) - e1d(3)*d2r2dsdx(ks,1,3)
                derv3(n1y,kk) = derv3(n1y,kk) - e1d(1)*d2r2dsdx(ks,2,1) - &
                  e1d(2)*d2r2dsdx(ks,2,2) - e1d(3)*d2r2dsdx(ks,2,3)
                derv3(n1z,kk) = derv3(n1z,kk) - e1d(1)*d2r2dsdx(ks,3,1) - &
                  e1d(2)*d2r2dsdx(ks,3,2) - e1d(3)*d2r2dsdx(ks,3,3)
!
!  End of loop over strain
!
              enddo
...
!
          if (QMMMmode(ncf).gt.0) then
            if (nregiontypj.eq.1.and.nregiontypk.eq.1.and.lbtyp) cycle pots3
          endif
!************************************
!  Validate potential for j-k pair  *
!************************************
...
!  then skip this four body term
!
                if (.not.lopi.and..not.lopj.and..not.lopk.and..not.lopl) cycle l4loop3
!
                if (lmolloc.and.lanyneedmol) then
!
!  Molecule handling
...
!
                if (QMMMmode(ncf).gt.0) then
                  if (nregiontypi.eq.1.and.nregiontypj.eq.1.and.nregiontypk.eq.1.and.nregiontypl.eq.1) cycle l4loop3
                endif
!
                xc4t = xclat(l)
                yc4t = yclat(l)
...
!
          ...
          if (lgrad2) then
            ...
            n4z = lzl
!
!  lopl must be true for the following
!
            n4x1 = lxl
            n4x2 = lxl
            n4x3 = lxl
...
!
                derv3(n4x,kk) = derv3(n4x,kk) + e1d(6)*d2r2dsdx(ks,1,6) + &
                  e1d(5)*d2r2dsdx(ks,1,5) + e1d(3)*d2r2dsdx(ks,1,3)
                derv3(n4y,kk) = derv3(n4y,kk) + e1d(6)*d2r2dsdx(ks,2,6) + &
                  e1d(5)*d2r2dsdx(ks,2,5) + e1d(3)*d2r2dsdx(ks,2,3)
                derv3(n4z,kk) = derv3(n4z,kk) + e1d(6)*d2r2dsdx(ks,3,6) + &
                  e1d(5)*d2r2dsdx(ks,3,5) + e1d(3)*d2r2dsdx(ks,3,3)
!
!  End of loop over strain
!
              enddo
...
!
  ...
  enddo ljloop2
!**************************************************
!  Loop over middle site 2 / j - second site case *
!**************************************************
  jx = - 2
  jy = - 1
  jz =   0
...
!
          if (QMMMmode(ncf).gt.0) then
            if (nregiontypj.eq.1.and.nregiontypk.eq.1.and.lbtyp) cycle pots4
          endif
!************************************
!  Validate potential for j-k pair  *
!************************************
...
!  then skip this four body term
!
                if (.not.lopi.and..not.lopj.and..not.lopk.and..not.lopl) cycle l4loop4
!
                if (lmolloc.and.lanyneedmol) then
!
!  Molecule handling
...
!
                if (QMMMmode(ncf).gt.0) then
                  if (nregiontypi.eq.1.and.nregiontypj.eq.1.and.nregiontypk.eq.1.and.nregiontypl.eq.1) cycle l4loop4
                endif
!
                xc4t = xclat(l)
                yc4t = yclat(l)
...
!
          ...
          if (lgrad2) then
            ...
            n3z = kzl
!
!  lopk must be true for what follows
!
            n3x1 = kxl
            n3x2 = kxl
            n3x4 = kxl
            if (lopi.and.lopj.and.lopl) then
              n1x2 = ix
              n1x3 = ix
              n1x4 = ix
              n2x1 = jx
...
!
                derv3(n3x,kk) = derv3(n3x,kk) + e1d(4)*d2r2dsdx(ks,1,4) - &
                  e1d(6)*d2r2dsdx(ks,1,6) + e1d(2)*d2r2dsdx(ks,1,2)
                derv3(n3y,kk) = derv3(n3y,kk) + e1d(4)*d2r2dsdx(ks,2,4) - &
                  e1d(6)*d2r2dsdx(ks,2,6) + e1d(2)*d2r2dsdx(ks,2,2)
                derv3(n3z,kk) = derv3(n3z,kk) + e1d(4)*d2r2dsdx(ks,3,4) - &
                  e1d(6)*d2r2dsdx(ks,3,6) + e1d(2)*d2r2dsdx(ks,3,2)
!
!  End of loop over strain
!
              enddo
...
!
  ...
  if (status/=0) call deallocate_error('fourimpd','nuniquelptr')
  deallocate(nuniquekptr,stat=status)
  if (status/=0) call deallocate_error('fourimpd','nuniquekptr')
  deallocate(nuniqueiptr,stat=status)
  if (status/=0) call deallocate_error('fourimpd','nuniqueiptr')
  deallocate(nptrnforimp,stat=status)
  if (status/=0) call deallocate_error('fourimpd','nptrnforimp')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimpd','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimpd','natmiddle')
#ifdef TRACE
  call trace_out('fourimpd')
#endif
!
  return
  end
