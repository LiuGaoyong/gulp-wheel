  subroutine phonond_newfc(lprint,fc,nobsmodeptr0,nobsmode)
!
!  Calculates the phonons at a given set of k points.
!  Uses a new force constant algorithm in which a supercell is built to allow generation of phased values.
...
!
!   8/23 Created from phonon_newfc and phonond
!   8/23 Cutoff setting corrected
!
!  Conditions of use:
!
...
!
  ...
  use phononatoms
#ifdef MPI
  use phonout,         only : leigenvectors_phase, leigenvectors_original
#endif
  use projectdos
  use properties
#ifdef MPI
...
!
  ...
  integer(i4)                                       :: maxd2fc
  integer(i4)                                       :: maxd2fcu
  integer(i4)                                       :: maxd2save
  integer(i4)                                       :: maxd2usave
  integer(i4)                                       :: maxd2qsave
  integer(i4)                                       :: maxd2qusave
  integer(i4)                                       :: maxeigc
  integer(i4)                                       :: maxeigr
  integer(i4)                                       :: maxlim
  integer(i4)                                       :: mcnmv
  ...
  integer(i4)                                       :: natomsonnodefc
  integer(i4),  dimension(:),     allocatable       :: node2atomfc    
  integer(i4),  dimension(:),     allocatable       :: atom2localfc
  integer(i4),  dimension(:),     allocatable       :: atom2nodefc
  integer(i4)                                       :: nd
  integer(i4)                                       :: nf
  integer(i4)                                       :: nfccells
  integer(i4)                                       :: nfcsuper(3)
  integer(i4)                                       :: nfitmode
  integer(i4)                                       :: nk
  integer(i4)                                       :: nldpt
  integer(i4)                                       :: nlkpt
...
!
  ...
  real(dp)                                          :: factor
  real(dp),     dimension(:,:),   allocatable       :: fccell
  real(dp),     dimension(:,:),   allocatable       :: fcderv2
  real(dp)                                          :: frq
  real(dp)                                          :: fscale
  real(dp)                                          :: mI(3)
...
!  wavenumbers for the frequencies. The factor of 10^23 comes from Ang^2 -> m^2 and g -> kg.
!
...
!
!  Requires k points to be sorted such that all points relating a given structure are consecutive.
!  
  nlkpt = 0
  do i = 1,nkpt
    nk = nkptcfg(i)
...
!*************************************************
!  Set range of interaction for force constants  *
!*************************************************
  if (rfcinteraction_in.ne.0.0_dp) then
!
!  If a value has been input then use this
!
    rfcinteraction = rfcinteraction_in
  else
!   
!  Otherwise use the maximum interaction range or 18 Angstroms, which ever is smaller
!  
    rfcinteraction = max(cutoffmax,cutoffmaxbo,18.0_dp)
    if (lreaxFF) rfcinteraction = max(rfcinteraction,reaxFFcutoffQ)
  endif
!*************************************************************
!  Findout size of supercell required for interaction range  *
!*************************************************************
  if (ndim.eq.3) then
    nfcsuper(1) = int(rfcinteraction/a) + 1
    nfcsuper(2) = int(rfcinteraction/b) + 1
    nfcsuper(3) = int(rfcinteraction/c) + 1
    nfccells = nfcsuper(1)*nfcsuper(2)*nfcsuper(3)
  elseif (ndim.eq.2) then
    nfcsuper(1) = int(rfcinteraction/a) + 1
    nfcsuper(2) = int(rfcinteraction/b) + 1
    nfcsuper(3) = 1
    nfccells = nfcsuper(1)*nfcsuper(2)
  else
    nfcsuper(1) = int(rfcinteraction/a) + 1
    nfcsuper(2) = 1
    nfcsuper(3) = 1
...
!
  allocate(fccell(3_i4,nfccells),stat=status)
  if (status/=0) call outofmemory('phonon_newfc','fccell')
!
!  Build cell shifts
!
  nfccells = 0
  do ii = 0,nfcsuper(1)-1
    do jj = 0,nfcsuper(2)-1
      do kk = 0,nfcsuper(3)-1
        nfccells = nfccells + 1
        fccell(1,nfccells) = dble(ii)*rv(1,1) + dble(jj)*rv(1,2) + dble(kk)*rv(1,3)
        fccell(2,nfccells) = dble(ii)*rv(2,1) + dble(jj)*rv(2,2) + dble(kk)*rv(2,3)
        fccell(3,nfccells) = dble(ii)*rv(3,1) + dble(jj)*rv(3,2) + dble(kk)*rv(3,3)
      enddo
    enddo
  enddo
!************************************
...
!
  ...
  if (status/=0) call outofmemory('phonond_newfc','meanKEperatom')
!
...
!
  ...
  if (status/=0) call outofmemory('phonond_newfc','mtvptr')
  allocate(mtvnptr(mtsv),stat=status)
  if (status/=0) call outofmemory('phonond_newfc','mtvnptr')
  allocate(mtvrptr(mtsv),stat=status)
  if (status/=0) call outofmemory('phonond_newfc','mtvrptr')
!
  if (lrigid) then
!
...
!
  if (leigloc) then
    ...
    if (status/=0) call outofmemory('phonond_newfc','savefreq')
    allocate(oscstrength(3,3,mtv),stat=status)
    if (status/=0) call outofmemory('phonond_newfc','oscstrength')
    allocate(ramstrength(3,3,mtv),stat=status)
    if (status/=0) call outofmemory('phonond_newfc','ramstrength')
  endif
!
...
!
  if (lthermal) then
    ...
    if (status/=0) call outofmemory('phonond_newfc','Sij')
  endif
!
...
!
  if (lprinloc) then
    if (lcluster) then
      ...
      if (leigen) then
        if (leigenvectors_original) then
          write(ioout,'(''  Eigenvectors will be output with mass-weighting '',/)')
        else
          write(ioout,'(''  Eigenvectors will be output without mass-weighting '',/)')
        endif
      endif
    else
      write(ioout,'(/,''  Phonon Calculation : '',/)')
      write(ioout,'(''  Using force constant supercell algorithm with: '',/)')
      write(ioout,'(''    Maximum force field cutoff = '',f8.4,'' Ang'')') cutoffmax
      write(ioout,'(''    Maximum interaction range  = '',f8.4,'' Ang'')') rfcinteraction
      write(ioout,'(''    Supercell dimensions       = '',3i4,/)') nfcsuper
      ...
      if (leigen) then
        if (leigenvectors_original.and.leigenvectors_phase) then
          write(ioout,'(''  Eigenvectors will be output with mass-weighting and phased '',/)')
        elseif (leigenvectors_original.and..not.leigenvectors_phase) then
          write(ioout,'(''  Eigenvectors will be output with mass-weighting and unphased '',/)')
        elseif (.not.leigenvectors_original.and.leigenvectors_phase) then
          write(ioout,'(''  Eigenvectors will be output without mass-weighting and phased '',/)')
        elseif (.not.leigenvectors_original.and..not.leigenvectors_phase) then
          write(ioout,'(''  Eigenvectors will be output without mass-weighting and unphased '',/)')
        endif
      endif
    endif
...
!*******************************************************
  if (leig.and.ioproc) then
    if (eigfile.ne.' ') then
      ...
      open(53,status='unknown',form='formatted')
    endif
...
!
    if (casfile.ne.' ') then
      ...
      open(54,status='unknown',form='formatted')
    endif
!
!  Open file header
!
...
!************************************************
  if (leigloc) then
    ...
    if (lgammaonly) then
      ...
      if (status/=0) call outofmemory('phonond_newfc','eigr')
      if (nbornstep(ncf).gt.0.or..not.lnonanal) then
        allocate(savederv2(maxeigr,mtvloc),stat=status)
        if (status/=0) call outofmemory('phonond_newfc','savederv2')
      endif
    else
      if (lgamma) then
        allocate(eigr(maxeigr,mtsvloc),stat=status)
      ...
      if (status/=0) call outofmemory('phonond_newfc','eigc')
      if (nbornstep(ncf).gt.0.or.lnonanal) then
        allocate(savederv2(maxeigr,mtvloc),stat=status)
        if (status/=0) call outofmemory('phonond_newfc','savederv2')
        allocate(savedervi(maxeigr,mtvloc),stat=status)
        if (status/=0) call outofmemory('phonond_newfc','savedervi')
      endif
    endif
  else
    if (msv.gt.0) then
...
!****************************************
!  Build supercell for force constants  *
!****************************************
!
!  Save maxd2 before building the supercell
!
  maxd2save = maxd2
  maxd2usave = maxd2u
  maxd2qsave = maxd2q
  maxd2qusave = maxd2qu
!
  call build_fc_supercell(nfcsuper)
!
!  Save number of atom on node for supercell and associated pointers
!
  natomsonnodefc = natomsonnode
!
  allocate(node2atomfc(natomsonnodefc),stat=status)
  if (status/=0) call outofmemory('phonond_newfc','node2atomfc')
  allocate(atom2localfc(nfccells*numat),stat=status)
  if (status/=0) call outofmemory('phonond_newfc','atom2localfc')
  allocate(atom2nodefc(nfccells*numat),stat=status)
  if (status/=0) call outofmemory('phonond_newfc','atom2nodefc')
!
  node2atomfc(1:natomsonnodefc) = atom2local(1:natomsonnodefc)
  atom2localfc(1:numat) = atom2local(1:numat)
  atom2nodefc(1:numat) = atom2node(1:numat)
!******************************************** 
!  Generate dynamical matrix for supercell  *
!********************************************
  if (lcluster) then
! 
...
!******************************************
!  Unbuild supercell for force constants  *
!******************************************
  call unbuild_fc_supercell(nfcsuper)
!********************************************
!  Save dynamical matrix for the supercell  *
!********************************************
  if (.not.lcluster) then
!   
!  Allocate array to store force constants from supercell
!     
    maxd2fcu = 3*natomsonnode
    maxd2fc = 3*numat*nfccells
    allocate(fcderv2(maxd2fc,maxd2fcu),stat=status)
    if (status/=0) call outofmemory('phonon_newfc','fcderv2')
!
    allocate(dtmp(maxd2fc,3_i4),stat=status)
    if (status/=0) call outofmemory('phonond_newfc','dtmp')
    allocate(StatMPI(MPI_Status_Size),stat=status)
    if (status/=0) call outofmemory('phonond_newfc','StatMPI')
!
!  Copy derv2 to fcderv2 while changing distribution to match that of the initial cell
!  NB: For phase_newfcd only the first image of each atom is required for the right-hand index
!
    do i = 1,numat
      is = nfccells*(i-1) + 1       ! Number of atom i in fc supercell
      nodewithfc = atom2nodefc(is)  ! Node that currently has this column
      nodewantfc = atom2node(i)     ! Node that wants this column
!
      if (nodewithfc.eq.nodewantfc) then
        if (nodewithfc.eq.procid) then
          iwith = atom2localfc(is)
          iwant = atom2local(i)
!
!  This column is local so just copy across
!
          indwant = 3*(iwant-1)
          indwith = 3*(iwith-1)
          do ii = 1,3
            do j = 1,maxd2fc
              fcderv2(j,indwant+ii) = derv2(j,indwith+ii)
            enddo
          enddo
        endif
      else
!
!  Column needs to be redistributed
!
        ntmp = 3*maxd2fc
        if (procid.eq.nodewantfc) then
!
!  Post receive
!
          ntag = i
          call MPI_IRecv(dtmp,ntmp,MPI_double_precision,nodewithfc, &
                         ntag,MPI_Comm_GULP,Request,MPIerror)
        elseif (procid.eq.nodewithfc) then
!
!  Post send
!
          ix = 3*(atom2localfc(is)-1) + 1
          iy = ix + 1
          iz = ix + 2
          dtmp(1:maxd2fc,1) = derv2(1:maxd2fc,ix)
          dtmp(1:maxd2fc,2) = derv2(1:maxd2fc,iy)
          dtmp(1:maxd2fc,3) = derv2(1:maxd2fc,iz)
          ntag = i
          call MPI_ISend(dtmp,ntmp,MPI_double_precision,nodewantfc, &
                         ntag,MPI_Comm_GULP,Request,MPIerror)
        endif
!
        if (procid.eq.nodewantfc.or.procid.eq.nodewithfc) then
...
!
          ix = 3*(atom2local(i)-1) + 1
          iy = ix + 1
          iz = ix + 2
          fcderv2(1:maxd2fc,ix) = dtmp(1:maxd2fc,1)
          fcderv2(1:maxd2fc,iy) = dtmp(1:maxd2fc,2)
          fcderv2(1:maxd2fc,iz) = dtmp(1:maxd2fc,3)
        endif
!
        call mpbarrier
      endif
...
!
    ...
    if (status/=0) call deallocate_error('phonond_newfc','StatMPI')
    deallocate(dtmp,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','dtmp')
  endif
!***************************************************************
!  Restore maxd2 after building the supercell and reset sizes  *
!***************************************************************
  maxd2 = maxd2save
  maxd2u = maxd2usave
  call changemaxd2
!
  if (leem.or.lgfnff.or.lreaxFF) then
    maxd2q = maxd2qsave
    maxd2qu = maxd2qusave
    call changemaxd2q
  endif
...
!  Set atomic phases
!
    call setatomicphase(xkt,ykt,zkt)
    if (.not.leigenvectors_phase) then
      do i = 1,mtv
        fphase(i) = 0.0_dpc
      enddo
      do i = 1,nphonatc
        ii = 3*(iocptr(i) - 1)
        fphase(ii+1) = fphase(ii+1) + aphase(nphonatcptr(i))*dcmplx(occuf(nphonatcptr(i)),0.0_dp)
        fphase(ii+2) = fphase(ii+2) + aphase(nphonatcptr(i))*dcmplx(occuf(nphonatcptr(i)),0.0_dp)
        fphase(ii+3) = fphase(ii+3) + aphase(nphonatcptr(i))*dcmplx(occuf(nphonatcptr(i)),0.0_dp)
      enddo
      if (lrigid) then
        do i = 1,nmol
          ii = mcnmv + 6*(i-1)
!
!  Sum atomic phases for rigid molecule translations
!
          do j = 1,nmolcore(i)
            l = nmollist(nmolptr(i)+j)
            fphase(ii+1) = fphase(ii+1) + aphase(l)*dcmplx(occuf(l),0.0_dp)
            fphase(ii+2) = fphase(ii+2) + aphase(l)*dcmplx(occuf(l),0.0_dp)
            fphase(ii+3) = fphase(ii+3) + aphase(l)*dcmplx(occuf(l),0.0_dp)
          enddo
!
!  Phasing of rotations of rigid molecules is not supported at present
!
          fphase(ii+4) = dcmplx(1.0_dp,0.0_dp)
          fphase(ii+5) = dcmplx(1.0_dp,0.0_dp)
          fphase(ii+6) = dcmplx(1.0_dp,0.0_dp)
        enddo
      endif
    endif
!
    if (.not.lcluster) then
!
!  Generate phased second derivatives for original cell from supercell
!
      call phase_newfcd(xkt,ykt,zkt,nfccells,nfcsuper,fccell,fcderv2,maxd2fc,maxd2fcu)
    endif
!
    if (.not.lnoanald2loc.and..not.lfinitediff2.and..not.lcluster) then
!
...
!
        if (.not.leigcOK) then
          ...
          if (status/=0) call deallocate_error('phonond_newfc','eigc')
        endif
#else
!
...
!***************************************************
      if (.not.lgamma) then
        ...
        if (lgroupvelocity) then
          ...
          if (status/=0) call outofmemory('phonond_newfc','ctmp')
        endif
      else
...
!
          ...
          ntag = i
          call MPI_IRecv(dtmp,nsize,MPI_double_precision,snode,ntag,MPI_Comm_GULP,Request,MPIerror)
!
          if (.not.lgamma) then
            if (lgroupvelocity) then
              ntag = ncorenomol + i
              nsizec = 27*ncorenomol
              call MPI_IRecv(ctmp,nsizec,MPI_double_complex,snode,ntag,MPI_Comm_GULP,Requestc,MPIerror)
            endif
          endif
        elseif (ld2loc) then
!
...
!
          ...
          ntag = i
          call MPI_ISend(dtmp,nsize,MPI_double_precision,snode,ntag,MPI_Comm_GULP,Request,MPIerror)
!
          if (.not.lgamma) then
            if (lgroupvelocity) then
              do j = 1,ncorenomol
              ...
              ntag = ncorenomol + i
              nsizec = 27*ncorenomol
              call MPI_ISend(ctmp,nsizec,MPI_double_complex,snode,ntag,MPI_Comm_GULP,Requestc,MPIerror)
            endif
          endif
        endif
!
...
!
      ...
      if (.not.lgamma.and.lgroupvelocity) then
        ...
        if (status/=0) call deallocate_error('phonond_newfc','ctmp')
      endif
      deallocate(dtmp,stat=status)
      if (status/=0) call deallocate_error('phonond_newfc','dtmp')
!****************************************
...
!
          ...
          if (status/=0) call outofmemory('phonond_newfc','averfreq')
!
!  Store derv2
!
...
!
          ...
          if (status/=0) call deallocate_error('phonond_newfc','averfreq')
        endif
!
...
!
        ...
        if (status/=0) call outofmemory('phonond_newfc','sum')
!
        call sumall(meanKEperatom,sum,mtv,"phonond_newfc","meanKEperatom")
        do i = 1,mtv
          meanKEperatom(i) = sum(i)
        enddo
!
        deallocate(sum,stat=status)
        if (status/=0) call deallocate_error('phonond_newfc','sum')
      endif
    endif
!
...
!**************************************************
  if (allocated(fcderv2)) then
    deallocate(fcderv2,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','fcderv2')
  endif
!
  deallocate(atom2nodefc,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','atom2nodefc')
  deallocate(atom2localfc,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','atom2localfc')
  deallocate(node2atomfc,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','node2atomfc')
!
  if (lgammaonly) then
    if (allocated(savederv2)) then
      deallocate(savederv2,stat=status)
      if (status/=0) call deallocate_error('phonond_newfc','savederv2')
    endif
    deallocate(eigr,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','eigr')
  else
    if (allocated(savederv2)) then
      deallocate(savedervi,stat=status)
      if (status/=0) call deallocate_error('phonond_newfc','savedervi')
      deallocate(savederv2,stat=status)
      if (status/=0) call deallocate_error('phonond_newfc','savederv2')
    endif
    deallocate(eigc,stat=status)
...
!
    ...
    if (status/=0) call outofmemory('phonond_newfc','sum2')
!
    sum(1:ncfoc) = msdx(1:ncfoc)
    sum(ncfoc+1:2*ncfoc) = msdy(1:ncfoc)
    sum(2*ncfoc+1:3*ncfoc) = msdz(1:ncfoc)
!
    call sumall(sum,sum2,3_i4*ncfoc,"phonond_newfc","msd")
!
...
!
    ...
    if (status/=0) call deallocate_error('phonond_newfc','sum2')
    deallocate(sum,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','sum')
!
    if (lprinloc) then
...
!
  if (lthermal) then
    ...
    if (status/=0) call deallocate_error('phonond_newfc','Sij')
  endif
  if (leigloc) then
    deallocate(ramstrength,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','ramstrength')
    deallocate(oscstrength,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','oscstrength')
    deallocate(savefreq,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','savefreq')
  endif
  deallocate(mtvrptr,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','mtvrptr')
  deallocate(mtvnptr,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','mtvnptr')
  deallocate(mtvptr,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','mtvptr')
  if (lrigid.and.leckart) then
    deallocate(mcvptr,stat=status)
    if (status/=0) call deallocate_error('phonond_newfc','mcvptr')
  endif
  deallocate(meanKEperatom,stat=status)
  if (status/=0) call deallocate_error('phonond_newfc','meanKEperatom')
!
...
#endif
#else
  call outerror('phonond_newfc called when not compiled with MPI',0_i4)
  call stopnow('phonond_newfc')
#endif
!
  return
