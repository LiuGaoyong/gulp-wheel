...
!   3/22 lhessreset now given default value for non-Hessian methods
!   4/22 Call to resetx0rv wrapped so that it is only called for a bulk calculation
!   5/23 Reduction of stepmax added if line minimisation fails
!
!  Conditions of use:
!
...
!
  ...
  frepf = 1.0d15
!
!  Save original stepmax
!
  stepmax_original = stepmax
!
...
!
!  Set lhessreset by default so that value is set even for non-Hessian-based methods
  lhessreset = .false.
!
  if (llbfgsloc) then
!************************
...
!
!  Scale back stepmax as line minimisation has failed
!
        stepmax = 0.25_dp*stepmax
!
        alp = 1.0_dp
        call olinmin(xc,alp,pvect,nvar,fc,okf,gg,imode)
      else
