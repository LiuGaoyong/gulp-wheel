...
!  53 = gcoulomb
!  54 = Becke_Johnson_C6
!  55 = Baskes potential with a4 term
!  56 = ZBL potential
!  57 = MM3 Buckingham
!  58 = MM3 Stretch
...
!   8/14 Baskes potential modified to allow for a4 sub-option, Z2S & s as input parameters
!        and n as a power of the 2NN correction
!   8/14 tpot terms from 5-12 removed from Baskes potential since they are now obtained
!        from the meam_density directly
!   8/14 rho0 replaced with wF in input to Baskes potential
!   8/14 MEAM lattice type indicator added to Baskes potential
!   8/14 lorder12 added
!   8/14 s coefficients removed from Baskes potential
!   3/15 Tsuneyuki fitting flag to variable reference corrected
!   3/15 x14 added as a sub-option
!  11/17 Buffered_LJ potential added
!   3/18 Error in fitting flag handling for Baskes potential fixed
!   6/18 Coulomb subtract parameter moved to twobody 1
!   1/19 Worsy changed to use word length parameter
!   6/19 j2 potential added
!  11/19 Trap for blank lines after the spline data added
!   3/20 SW2gen potential added to include p, q and C
!   3/20 Screened Coulomb potential added
!   6/21 Baskes setting of self term pointer now moved to setpote
!  10/21 Order of defaults for j2 potential changed
!  11/21 Modifications for TI added
!  11/21 Correction to referencing npote+1 -> npote for spline
!  11/21 Energy shift removed from qerfc
!  11/21 Einstein positions changed
!  12/21 lnomottlittleton set to true for einstein model and planepots
!   2/23 Molecule warning extended
!
!  Conditions of use:
!
...
!
!  Julian Gale, CIC, Curtin University, February 2023
!
  use configurations
  use g_constants
  use control
...
!
  ...
  if (index(word,'buff').eq.1) goto 760
  ...
  if (index(word,'j2').eq.1) goto 840
  if (index(word,'sw2g').eq.1) goto 850
  if (index(word,'scre').eq.1) goto 860
  return
!**********************
!  General potential  *
!**********************
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'x13').eq.1) then
        mmexc(npote+1) = 3
      elseif (index(words(i),'x14').eq.1) then
        mmexc(npote+1) = 6
        scale14(npote+1) = 0.0_dp
      ...
      elseif (index(words(i),'cus').eq.1) then
        n2botype(1,npote+1) = 7
      elseif (index(words(i),'hal').eq.1) then
        n2botype(1,npote+1) = 8
      elseif (index(words(i),'quar').eq.1) then
        n2botype(1,npote+1) = 9
      elseif (index(words(i),'thi').eq.1) then
        n2botype(1,npote+1) = 10
      elseif (index(words(i),'cyc').eq.1) then
        n2botype(2,npote+1) = 2
      elseif (index(words(i),'exo').eq.1) then
        n2botype(2,npote+1) = 3
      elseif (index(words(i),'flam').eq.1) then
        ltif = .true.
      ...
      if (lfound.and.lwarnp) then
        nwarn = nwarn + 1
        if (ioproc) then
          write(ioout,'(/,''**** Warning - inter/intra/both/bond/molmec on the potential line ****'')')
...
!
  if (ltib.and.ltif) then
    call outerror('Lambda cannot be forward and backward for the same potential',iline)
...
!
  ...
  ltib2body(npote) = ltib
  ltif2body(npote) = ltif
  ...
  if (nword.gt.1) then
    do nw = 2,nword
      ...
      if (index(words(nw),'cart').eq.1) then
        lcartin = .true.
      elseif (index(words(nw),'blam').eq.1) then
        ltib = .true.
      elseif (index(words(nw),'flam').eq.1) then
        ltif = .true.
      endif
    enddo
  endif
...
!
  ...
  leinsteinat(n) = .true.
  ltibeinsteinat(n) = ltib
  ltifeinsteinat(n) = ltif
  if (lcartin) then
    xceinsteinat(n) = floats(2)
    yceinsteinat(n) = floats(3)
    zceinsteinat(n) = floats(4)
  else
    xfeinsteinat(n) = floats(2)
    yfeinsteinat(n) = floats(3)
    zfeinsteinat(n) = floats(4)
  endif
...
!
!  Convert coordinates from Cartesian to fractional if necessary or vice versa
!
  n = nascfg(ncurr)
  nlsft = nasum - nascfg(ncurr)
  if (lcartin) then
    do i = 1,n
      xloc = xceinsteinat(nlsft+i)
      yloc = yceinsteinat(nlsft+i)
      zloc = zceinsteinat(nlsft+i)
      call cart2frac(ndimen(ncfg),xloc,yloc,zloc,rvcfg(1,1,ncurr),xfeinsteinat(nlsft+i),yfeinsteinat(nlsft+i), &
                     zfeinsteinat(nlsft+i),icx,icy,icz)
    enddo
  else
    if (ndimen(ncurr).eq.3) then
      do i = 1,n
        xceinsteinat(nlsft+i) = rvcfg(1,1,ncurr)*xfeinsteinat(nlsft+i) + &
                                rvcfg(1,2,ncurr)*yfeinsteinat(nlsft+i) + &
                                rvcfg(1,3,ncurr)*zfeinsteinat(nlsft+i)
        yceinsteinat(nlsft+i) = rvcfg(2,1,ncurr)*xfeinsteinat(nlsft+i) + &
                                rvcfg(2,2,ncurr)*yfeinsteinat(nlsft+i) + &
                                rvcfg(2,3,ncurr)*zfeinsteinat(nlsft+i)
        zceinsteinat(nlsft+i) = rvcfg(3,1,ncurr)*xfeinsteinat(nlsft+i) + &
                                rvcfg(3,2,ncurr)*yfeinsteinat(nlsft+i) + &
                                rvcfg(3,3,ncurr)*zfeinsteinat(nlsft+i)
      enddo
    elseif (ndimen(ncurr).eq.2) then
      do i = 1,n
        xceinsteinat(nlsft+i) = rvcfg(1,1,ncurr)*xfeinsteinat(nlsft+i) + rvcfg(1,2,ncurr)*yfeinsteinat(nlsft+i)
        yceinsteinat(nlsft+i) = rvcfg(2,1,ncurr)*xfeinsteinat(nlsft+i) + rvcfg(2,2,ncurr)*yfeinsteinat(nlsft+i)
        zceinsteinat(nlsft+i) = zfeinsteinat(nlsft+i)
      enddo
    elseif (ndimen(ncurr).eq.1) then
      do i = 1,n
        xceinsteinat(nlsft+i) = rvcfg(1,1,ncurr)*xfeinsteinat(nlsft+i)
        yceinsteinat(nlsft+i) = yfeinsteinat(nlsft+i)
        zceinsteinat(nlsft+i) = zfeinsteinat(nlsft+i)
      enddo
    else
      do i = 1,n
        xceinsteinat(nlsft+i) = xfeinsteinat(nlsft+i)
        yceinsteinat(nlsft+i) = yfeinsteinat(nlsft+i)
        zceinsteinat(nlsft+i) = zfeinsteinat(nlsft+i)
      enddo
    endif
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'bon').eq.1) then
        ...
        linter(npote+1) = .false.
        lfound = .true.
      elseif (index(words(i),'a4').eq.1) then
        la4 = .true.
      elseif (index(words(i),'x12').eq.1) then
        mmexc(npote+1) = 2
      elseif (index(words(i),'x13').eq.1) then
      ...
      elseif (index(words(i),'exo').eq.1) then
        n2botype(2,npote+1) = 3
      elseif (index(words(i),'fcc').eq.1) then
        ilattice = 1
      elseif (index(words(i),'bcc').eq.1) then
        ilattice = 2
      elseif (index(words(i),'nacl').eq.1) then
        ilattice = 3
      elseif (index(words(i),'hcp').eq.1) then
        ilattice = 4
      elseif (index(words(i),'dim').eq.1) then
        ilattice = 5
      elseif (index(words(i),'l12ab3').eq.1) then
        ilattice = 6
      elseif (index(words(i),'l12a3b').eq.1) then
        ilattice = 7
      elseif (index(words(i),'zns').eq.1) then
        ilattice = 8
      elseif (index(words(i),'dia').eq.1) then
        ilattice = 9
      elseif (index(words(i),'flam').eq.1) then
        ltif = .true.
      elseif (index(words(i),'blam').eq.1) then
...
!  Copy first line info from first potential
!
!  For Baskes potential the array ipot contains several pieces of info:
!
!  ipot(1,*) = number of iterations for 2NN approximation
!  ipot(2,*) = lattice type pointer for this pair potential
!  ipot(3,*) = pointer to self-pair potential for atom 1 (NB: Now set in setpote)
!  ipot(4,*) = pointer to self-pair potential for atom 2 (NB: Now set in setpote)
!
  ipot(1,npote) = nbaskes
  ipot(2,npote) = ilattice
  mmexc(npote) = mmexc(npot1)
  n2botype(1:2,npote) = n2botype(1:2,npot1)
  lintra(npote) = lintra(npot1)
  linter(npote) = linter(npot1)
...
!
  ...
  if (lfit.and.lfflags) then
    if (.not.la4) then
      ...
      n1 = int(floats(nfloat-6))
      n2 = int(floats(nfloat-5))
      n3 = int(floats(nfloat-4))
      n4 = int(floats(nfloat-3))
      n5 = int(floats(nfloat-2))
      n6 = int(floats(nfloat-1))
      n7 = int(floats(nfloat))
      nfloat = nfloat - 7
      if (n1.eq.1) then
        nfit = nfit + 1
        if (nfit.ge.maxfit) then
...
!
!  If number of floats is greater than 9/11, assume that fitting flags have been left on line
!
  if (la4) then
    if (nbaskes.gt.0) then
      if (nfloat.gt.11) nfloat = nfloat - 7
!
!  Assign coefficients and cutoffs
!
      if (nfloat.ge.11) then
        ...
        twopot(8,npote) = floats(8+nbeg)
        twopot(9,npote) = floats(9+nbeg)
        rpot2(npote)    = floats(10+nbeg)
        rpot(npote)     = floats(11+nbeg)
      ...
      elseif (nfloat.eq.9) then
        ...
        twopot(8,npote) = floats(7+nbeg)
        twopot(9,npote) = floats(8+nbeg)
...
!
!  Check wF is between zero and one
!
  if (twopot(6,npote).lt.0.0_dp.or.twopot(6,npote).gt.1.0_dp) then
    call outerror('Baskes potential has an wF outside range of 0 to 1',iline)
    call stopnow('potword22')
  endif
  if (nbaskes.gt.0) then
!
!  Read Z2S and s factors
!
!  Line 1
!
    line = '  '
    ...
    if (nfloat.ge.4) then
      ...
      call outerror('incorrect Z2S and scale input for baskes potential',iline)
      call stopnow('potword22')
    endif
!
!  Check Z2S is positive
!
    if (tpot(5,npote).lt.0.0_dp) then
      call outerror('Baskes potential has negative Z2S',iline)
      call stopnow('potword22')
    endif
    if (tpot(7,npote).lt.0.0_dp) then
      call outerror('Baskes potential has negative Z2S',iline)
      call stopnow('potword22')
    endif
!
!  Check s is greater than 1
!
    if (tpot(6,npote).lt.1.0_dp) then
      call outerror('Baskes potential has s less than 1',iline)
      call stopnow('potword22')
    endif
    if (tpot(8,npote).lt.1.0_dp) then
      call outerror('Baskes potential has s less than 1',iline)
      call stopnow('potword22')
    endif
  endif
!
  ...
  elseif (nvar1.lt.nvar2) then
    ...
    symbol2(2,npote) = sym21
!
!  Correct wF to reflect the change of order
!
    twopot(6,npote) = 1.0_dp - twopot(6,npote)
  endif
!
!  If order has changed then switch around tpot values
!
  if (.not.lorder12(npote)) then
    rtmp = tpot(5,npote)
    tpot(5,npote) = tpot(7,npote)
    tpot(7,npote) = rtmp
    rtmp = tpot(6,npote)
    tpot(6,npote) = tpot(8,npote)
    tpot(8,npote) = rtmp
  endif
!
  if (la4) then
...
!
  if (twopot(2,npote).lt.1.0d-8) then
    call outerror('Buffered_LJ potential has an r0 of zero',iline)
    call stopnow('potword22')
  endif
  if (nvar1.eq.nvar2) then
    nspec1(npote) = nvar1
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'exo').eq.1) then
        n2botype(2,npote) = 3
      elseif (index(words(i),'flam').eq.1) then
        ltif2body(npote) = .true.
      elseif (index(words(i),'blam').eq.1) then
        ltib2body(npote) = .true.
      endif
      i = i + 1
      if (lfound.and.lwarnp) then
        nwarn = nwarn + 1
...
!
  ...
  do while (.not.leof)
    ...
    iline = iline + 1
    call linepro(iin,line,iline)
    if (nword.eq.0.and.nfloat.eq.0) cycle
...
838 if (.not.l55) l1000 = .true.
  ...
  return
!********************************
!  Constant 2body J2 potential  *
!********************************
840 if (npote.eq.maxpot) then
    maxpot = npote + 10
    call changemaxpot
  endif
  call init2bodydefaults(npote+1_i4)
...
!
  ...
  npot1 = npote + 1
845 line  =  '  '
  read(iin,'(a)',end = 848) line
  iline = iline + 1
  call linepro(iin,line,iline)
!
!  Check whether input is symbol or option
!
  if ((nword+nfloat).eq.0) goto 845
  if (nword.gt.0) then
    word = words(1)
    call worsy(word,lsymbol,.true.)
    if (.not.lsymbol) then
...
!
  ...
  nptype(npote) = 62
  goto 845
848 if (.not.l55) l1000 = .true.
  lwordok = .true.
  return
!*****************************************************
!  Stillinger-Weber twobody potential - general form *
!*****************************************************
850 if (npote.eq.maxpot) then
    maxpot = npote + 10
    call changemaxpot
  endif
  call init2bodydefaults(npote+1_i4)
...
!
  ...
  npot1 = npote + 1
855 line = '  '
  read(iin,'(a)',end=858) line
  iline = iline + 1
  call linepro(iin,line,iline)
!
!  Check whether input is symbol or option
!
  if ((nword+nfloat).eq.0) goto 855
  if (nword.gt.0) then
    word = words(1)
    call worsy(word,lsymbol,.true.)
    if (.not.lsymbol) then
...
!
  if (nfloat.eq.7) then
    twopot(1,npote) = floats(1+nbeg)*units
    twopot(2,npote) = abs(floats(2+nbeg))
    twopot(3,npote) = floats(3+nbeg)
    twopot(4,npote) = abs(floats(4+nbeg))
    twopot(5,npote) = abs(floats(5+nbeg))
    rpot2(npote)    = floats(6+nbeg)
    rpot(npote)     = floats(7+nbeg)
  elseif (nfloat.eq.4) then
    twopot(1,npote) = floats(1+nbeg)*units
...
!
  if (twopot(2,npote).lt.1.0d-8) then
    call outerror('SW2gen potential has a sigma of zero',iline)
    call stopnow('potword22')
  endif
  if (nvar1.eq.nvar2) then
    nspec1(npote) = nvar1
  ...
  nptype(npote) = 63
  goto 855
858 if (.not.l55) l1000 = .true.
  lwordok=.true.
  return
!*******************************
!  Screened Coulomb potential  *
!*******************************
860 if (npote.eq.maxpot) then
    maxpot = npote + 10
    call changemaxpot
  endif
  call init2bodydefaults(npote+1_i4)
...
!
  ...
  npot1 = npote + 1
865 line = '  '
  read(iin,'(a)',end=868) line
  iline = iline + 1
  call linepro(iin,line,iline)
!
!  Check whether input is symbol or option
!
  if ((nword+nfloat).eq.0) goto 865
  if (nword.gt.0) then
    word = words(1)
    call worsy(word,lsymbol,.true.)
    if (.not.lsymbol) then
...
!
  if (nfloat.eq.5) then
    twopot(1,npote) = abs(floats(1+nbeg))
    twopot(2,npote) = abs(floats(2+nbeg))
    twopot(3,npote) = abs(floats(3+nbeg))
    rpot2(npote)    = floats(4+nbeg)
    rpot(npote)     = floats(5+nbeg)
...
!
!  Check saturation values is greater than or equal to 1
!
  if (twopot(1,npote).lt.1.0_dp) then
    call outerror('Screened Coulomb potential has saturation dielectric less than 1',iline)
    call stopnow('potword22')
  endif
  if (nvar1.eq.nvar2) then
    nspec1(npote) = nvar1
