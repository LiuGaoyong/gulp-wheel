...
!   7/20 MPI communicator changed to MPI_Comm_GULP
!   9/20 lpolarisation added
!   9/20 tmp2 no longer allocated if linternal is false
!   7/21 Call to matrix_inversion_library changed
!   8/23 Calculation of dielectric and piezo properties disabled for variable charges
!   9/23 High frequency dielectric constants added for variable charges
!   9/23 Piezoelectric constants added for variable charges
!   9/23 sumall no longer applied to piezoelectric constants as global sum of tmp2 is now
!        stored in tmp2 rather than tmp2s at this point
!  10/23 Static dielectric constants added for variable charge models
!
!  Conditions of use:
!
...
!
    ...
    elseif (ldlch.and.(leem.or.lreaxFF.or.lgfnff)) then
!****************************************************
!  Variable charge model high frequency properties  *
!****************************************************
      diconh(1:3,1:3) = 0.0_dp
!
      do i = 1,numat
        diconh(1,1) = diconh(1,1) + dqdVxyz(1,i)*(xclat(i) - xclat(numat))
        diconh(2,1) = diconh(2,1) + 0.5_dp*(dqdVxyz(2,i)*(xclat(i) - xclat(numat)) + dqdVxyz(1,i)*(yclat(i) - yclat(numat)))
        diconh(3,1) = diconh(3,1) + 0.5_dp*(dqdVxyz(3,i)*(xclat(i) - xclat(numat)) + dqdVxyz(1,i)*(zclat(i) - zclat(numat)))
        diconh(2,2) = diconh(2,2) + dqdVxyz(2,i)*(yclat(i) - yclat(numat))
        diconh(3,2) = diconh(3,2) + 0.5_dp*(dqdVxyz(3,i)*(yclat(i) - yclat(numat)) + dqdVxyz(2,i)*(zclat(i) - zclat(numat)))
        diconh(3,3) = diconh(3,3) + dqdVxyz(3,i)*(zclat(i) - zclat(numat))
      enddo
!
      diconh(1,2) = diconh(2,1)
      diconh(1,3) = diconh(3,1)
      diconh(2,3) = diconh(3,2)
!
      diconh = diconh*dlcfactor
      do i = 1,3
        diconh(i,i) = 1.0_dp + diconh(i,i)
      enddo
!         
!  Calculate refractive indices
!       
...
!
      ifail = 1
      call matrix_inversion_library(ninv,1_i4,maxd2,3_i4*nblocksize,dervi,0_i4,.true.,ifail)
!
      if (ifail.gt.0) then
        nwarn = nwarn + 1
        call outwarning('Properties cannot be calculated - matrix is singular',0_i4)
...
!
  if ((leem.or.lreaxff.or.lgfnff).and.ldlcs) then
    allocate(tmp3(ninv,3),stat=status)
    if (status/=0) call outofmemory('property3','tmp3')
  endif
!*********************
!  Elastic constants *
!*********************
...
!
!  Copy tmp2s to tmp2 for later use
!
          tmp2(1:mint,1:6) = tmp2s(1:mint,1:6)
#endif
!
...
!****************************
  if (lpiezo) then
    if (leem.or.lreaxFF.or.lgfnff) then
!
      ...
      do i = 1,6
        do j = 1,3
          ...
          do k = 1,nqfo
            indk = 3*(k-1) + j
!
!  For variable charges need to correct the displacements relative to the fixed atom
!
            call cartstrterm(ndim,xclat(k)-xclat(numat),yclat(k)-yclat(numat),zclat(k)-zclat(numat), &
                             0.0_dp,0.0_dp,0.0_dp,dxyzds,d2xyzdsdx,d2xyzds2,.false.)
            sum = sum + (tmp2(indk,i)-dxyzds(i,j))*qfo(k)
          enddo
          piezo(i,j) = - piefactor*sum
        enddo
      enddo
!
!  Contribution due to variable charges
!
      do k = 1,nqfo
!
!  Compute charge derivative with respect to strain, corrected for internal relaxation
!
        dqds_corrected(1:6) = dqds(1:6,k)
        do j = 1,nqfo
          indj = 3*(j-1)
          do i = 1,6
            dqds_corrected(i) = dqds_corrected(i) - dqdxyz(indj+1,k)*tmp2(indj+1,i) &
                                                  - dqdxyz(indj+2,k)*tmp2(indj+2,i) &
                                                  - dqdxyz(indj+3,k)*tmp2(indj+3,i)
          enddo
        enddo
!
!  Apply correction to piezoelectric constants with polarisation relative to the fixed atom
!
        do i = 1,6
          piezo(i,1) = piezo(i,1) + piefactor*dqds_corrected(i)*(xclat(k)-xclat(numat))
          piezo(i,2) = piezo(i,2) + piefactor*dqds_corrected(i)*(yclat(k)-yclat(numat))
          piezo(i,3) = piezo(i,3) + piefactor*dqds_corrected(i)*(zclat(k)-zclat(numat))
        enddo
      enddo
    elseif (linternal) then
!
!  Piezoelectric stress constants
!
      do i = 1,6
      ...
      if (nprocs.gt.1) then
        if (lrigid) then
          do i = 1,3
            ...
            enddo
          enddo
        elseif (leem.or.lreaxFF.or.lgfnff) then
!
! Compute displacements due to electric field and store in tmp3
!
          tmp3(1:3*nqfo,1:3) = 0.0_dp
          do i = 1,3
            do k = 1,natomsonnodem1
              ki = node2atom(k)
              do j = 1,3
                indk = 3*(k-1) + j
                do indl = 1,3*nqfo
                  tmp3(indl,i) = tmp3(indl,i) + dervi(indl,indk)*bornq(j,i,ki)
                enddo
              enddo
            enddo
          enddo
!
          dicons(1:3,1:3) = 0.0_dp
!
          do i = 1,3
            sums(1:3) = 0.0_dp
            do k = 1,nqfo
              indk = 3*(k-1) 
!     
!  Find corrected charge derivative with respect to the electric field
!     
              dqdVxyzcorr = dqdVxyz(i,k)/dble(nprocs)
              do l = 1,nqfo
                indl = 3*(l-1)
                dqdVxyzcorr = dqdVxyzcorr + dqdxyz(indl+1,k)*tmp3(indl+1,i) &
                                          + dqdxyz(indl+2,k)*tmp3(indl+2,i) &
                                          + dqdxyz(indl+3,k)*tmp3(indl+3,i)
              enddo
!
              sums(1) = sums(1) + qfo(k)*tmp3(indk+1,i) + dqdVxyzcorr*(xclat(k)-xclat(numat))
              sums(2) = sums(2) + qfo(k)*tmp3(indk+2,i) + dqdVxyzcorr*(yclat(k)-yclat(numat))
              sums(3) = sums(3) + qfo(k)*tmp3(indk+3,i) + dqdVxyzcorr*(zclat(k)-zclat(numat))
            enddo
!
            dicons(1,i) = dicons(1,i) + dlcfactor*sums(1)
            dicons(2,i) = dicons(2,i) + dlcfactor*sums(2)
            dicons(3,i) = dicons(3,i) + dlcfactor*sums(3)
          enddo
        else
          do i = 1,3
            do j = 1,3
          ...
          do i = 1,3
            do k = 1,nqfo
              do j = 1,3
                indk = 3*(k-1) + j
                do indl = 1,3*nqfo
                  tmp3(indl,i) = tmp3(indl,i) + dervi(indl,indk)*bornq(j,i,k)
                enddo
              enddo
            enddo
          enddo
...
!**********************
  if (lprint.and.ioproc) then
    ...
    if (ldlch.and.(nshell.gt.0.or.(leem.or.lreaxFF.or.lgfnff))) then
...
!
  ...
  if (allocated(tmp3)) then
    ...
    if (status/=0) call deallocate_error('property3','tmp3')
  endif
!
