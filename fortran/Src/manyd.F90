  subroutine manyd(emany,esregion12,esregion2,eattach,lgrad1,lgrad2)
!
...
!
  use configurations, only : nregions, nregiontype, QMMMmode
  use control
  use current
  use derivatives
  use eam
...
!     
    ...
    if (neamspeci.gt.0.and.rhoi.gt.1.0d-12.and.lQMMMok) then
      ...
      if (lreg2ok.and.nregionno(nrelf2a(i)).gt.1) then
...
!
  ...
  if (status/=0) call outofmemory('manyd','npotl')
  if (lgrad2) then
    allocate(npotikptr(npote),stat=status)
    if (status/=0) call outofmemory('manyd','npotikptr')
    allocate(npotjkptr(npote),stat=status)
    if (status/=0) call outofmemory('manyd','npotjkptr')
  endif
!
!  Set up cutoffs
!
...
!
                        ...
                        if (lstr) then
                          ...
                          enddo
                          if (latomicstress.and.i.ge.j) then
                            if (.not.lfreeze) then
                              do kl = 1,nstrains
...
!
                  ...
                  if (lsg1) then
                    drhoijs(1:nstrains,1:maxmeamcomponent) = Sij*drhoijs(1:nstrains,1:maxmeamcomponent)
                    drhojis(1:nstrains,1:maxmeamcomponent) = Sij*drhojis(1:nstrains,1:maxmeamcomponent)
                  endif
                  if (lgrad2) then
                    ...
                    if (lsg2) then
                      drhoij2s(1:nstrains2,1:maxmeamcomponent) = Sij*drhoij2s(1:nstrains2,1:maxmeamcomponent)
                      drhoji2s(1:nstrains2,1:maxmeamcomponent) = Sij*drhoji2s(1:nstrains2,1:maxmeamcomponent)
                      drhoij2m(1:nstrains,1:3,1:maxmeamcomponent) = Sij*drhoij2m(1:nstrains,1:3,1:maxmeamcomponent)
                      drhoji2m(1:nstrains,1:3,1:maxmeamcomponent) = Sij*drhoji2m(1:nstrains,1:3,1:maxmeamcomponent)
                    endif
                  endif
!
...
!**********************
              if (lanyvalidik) then
                ...
                if (lsg2) then
                  drhototikssum(1:6) = drhototikssum(1:6) + drhototiks(1:6)
                  drhototijk2ssum(1:6,1:6) = drhototijk2ssum(1:6,1:6) + drhototijk2s(1:6,1:6)
                  drhototijk2msum(1:6,1:3) = drhototijk2msum(1:6,1:3) + drhototijk2m(1:6,1:3)
                endif
              endif
              if (lanyvalidjk) then
                ...
                if (lsg2) then
                  drhototjkssum(1:6) = drhototjkssum(1:6) + drhototjks(1:6)
                  drhototjik2ssum(1:6,1:6) = drhototjik2ssum(1:6,1:6) + drhototjik2s(1:6,1:6)
                  drhototjik2msum(1:6,1:3) = drhototjik2msum(1:6,1:3) + drhototjik2m(1:6,1:3)
                endif
              endif
!
!  Only need to do internal derivatives if i not equals j
...
!**************************************
  ...
  enddo iloop
!  
!  Double counting correction - note that there is an extra factor of a half in this version
!  
  if (.not.lfreeze) then
    if (lsg1) then
      do i = 1,nstrains
        rstrd(i) = rstrd(i) + 0.25_dp*rstrl(i)
      enddo
    endif
    if (lsg2) then
      do i = 1,nstrains
        do j = 1,i
          sderv2(i,j) = sderv2(i,j) + 0.25_dp*elcom(i,j)
        enddo
      enddo
    endif
  else
...
!
  if (lgrad2) then
    ...
    if (status/=0) call deallocate_error('manyd','npotjkptr')
    deallocate(npotikptr,stat=status)
    if (status/=0) call deallocate_error('manyd','npotikptr')
  endif
  deallocate(npotl,stat=status)
  if (status/=0) call deallocate_error('manyd','npotl')
!
!  Exit point
!
1000 continue
