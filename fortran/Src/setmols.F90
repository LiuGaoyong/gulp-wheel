...
!  10/19 Test on nprocs corrected 
...
!   9/23 Symmetry setup added for rigid molecules
!   9/23 Increasing of maxmol improved to avoid slow down for large number of molecules
!
!  Conditions of use:
!
...
!
      ...
      nbat(nbond) = i
!
!  Check if any of the bonded atoms are in molecule already and find lowest molecule number
!
      nml = 0
      lfirstlow = .true.
      do k = 1,nbond
...
!
!  Estimate how many more molecules are likely
!
          maxmolest = nmol*numat/i
          if (maxmolest.gt.100*maxmol) then
            maxmol = 100*maxmol
          else
            maxmol = maxmolest + 10
          endif
...
!
    do ico = 1,nconnect
      if (nconnectcfg(ico).eq.ncf) then
        ...
        nmolconnectptr(nmolconnect(nm),nm) = ico
      endif
    enddo
  endif
!**************************************
...
!
  ...
  enddo
!
!  Set up number of atoms not in a rigid molecule
!
  if (lrigid) then
!   
...
!
    ...
    do i = 1,numat
      if (natmol(i).eq.0.or.nat(i).gt.maxele) then
        ...
        if (lbsmat(i)) nbsmatnomol = nbsmatnomol + 1
      else
        numatnomolrptr(i) = 0
      endif
    enddo
...
!
!  Dummy symmetry setup as symmetry is not used with spatial
!
    allocate(lmoldone(nmol),stat=status)
    if (status/=0) call outofmemory('setmols','lmoldone')
    allocate(lmolunique(nmol),stat=status)
    if (status/=0) call outofmemory('setmols','lmolunique')
!
...
!
    ...
    if (status/=0) call deallocate_error('setmols','lmolunique')
    deallocate(lmoldone,stat=status)
    if (status/=0) call deallocate_error('setmols','lmoldone')
!
