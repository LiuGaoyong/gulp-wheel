  subroutine fournosd(efor,eoop,eimp,esregion12,esregion2,eattach,lgrad1,lgrad2)
!
!  Subroutine for four-body energy.
!  Distributed memory parallel version.
!
!   1/17 Created from fournos
...
!
!  Julian Gale, Curtin University, July 2023
!
  use configurations, only : lsliceatom, nregions, nregiontype, QMMMmode
  use g_constants
  use control,        only : lmarvreg2, lseok, latomicstress, lnotorXduplicates, lrigid
  use current
  use derivatives
  ...
  use mdlogic
  use molecule
  use parallel,       only : ioproc, natomsonnode, node2atom, atom2local
  use optimisation
  use symmetry
  use times
#ifdef TRACE
...
!
  ...
  if (status/=0) call outofmemory('fournosd','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fournosd','ntypmiddle')
  allocate(nptrnfornonoop(nfor),stat=status)
  if (status/=0) call outofmemory('fournosd','nptrnfornonoop')
  allocate(nuniqueiptr(numat),stat=status)
  if (status/=0) call outofmemory('fournosd','nuniqueiptr')
  allocate(nuniquekptr(numat),stat=status)
  if (status/=0) call outofmemory('fournosd','nuniquekptr')
  allocate(nuniquelptr(numat),stat=status)
  if (status/=0) call outofmemory('fournosd','nuniquelptr')
  allocate(xderv(numat),stat=status)
  if (status/=0) call outofmemory('fournosd','xderv')
  allocate(yderv(numat),stat=status)
  if (status/=0) call outofmemory('fournosd','yderv')
  allocate(zderv(numat),stat=status)
  if (status/=0) call outofmemory('fournosd','zderv')
  if (ndim.gt.0) then
    allocate(xvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fournosd','xvec')
    allocate(yvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fournosd','yvec')
    allocate(zvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fournosd','zvec')
  else
...
!
      ...
      if (status/=0) call deallocate_error('fournosd','zvec')
      deallocate(yvec,stat=status)
      if (status/=0) call deallocate_error('fournosd','yvec')
      deallocate(xvec,stat=status)
      if (status/=0) call deallocate_error('fournosd','xvec')
...
!********************************************************
!  Loop over middle site 2 / j - parallel over j sites  *
!********************************************************
  jxl = - 2
  jyl = - 1
  jzl =   0
...
!
!  For energy and gradients we need to ensure that j/k are not duplicated
!
          if (k.ge.j) then
            if (lreg2qtet) then
...
!
          ...
          if (lgrad2) then
            ...
            n2z = jzl
!
!  For the following then lopj must be true
!
            n2x1 = jxl
            n2x3 = jxl
            n2x4 = jxl
            if (lopi.and.lopk.and.lopl) then
              n1x2 = ix
              n3x2 = kx
              n4x2 = lx
...
!
!  End of outer loop over strains
!
              enddo
            endif
          endif
...
!
  ...
  enddo ljloop
!************************************************************************
!  Loop over middle site 2 / j  - loop for end atom second derivatives  *
!************************************************************************
  jx = - 2
  jy = - 1
  jz =   0
...
!  Loop over cell vectors
!
      jjloop2: do jj = 1,nvector
        r322 = (xvec(jj)+x32t)**2 + (yvec(jj)+y32t)**2 + (zvec(jj)+z32t)**2
        if (r322.lt.1d-12) cycle jjloop2
        if (k.eq.j.and.jj.eq.nmid) cycle jjloop2
!
!  Molecule checking
!
        lbondedjk = .false.
...
!
          if (QMMMmode(ncf).gt.0) then
            if (nregiontypj.eq.1.and.nregiontypk.eq.1.and.lbtyp) cycle pots2
          endif
!************************************
!  Validate potential for j-k pair  *
!************************************
...
!  then skip this four body term
!
                if (.not.lopi.and..not.lopj.and..not.lopk.and..not.lopl) cycle l4loop2
!
                if (lmolloc.and.lanyneedmol) then
!
!  Molecule handling
...
!
                if (QMMMmode(ncf).gt.0) then
                  if (nregiontypi.eq.1.and.nregiontypj.eq.1.and.nregiontypk.eq.1.and.nregiontypl.eq.1) cycle l4loop2
                endif
!
                xc4t = xclat(l)
                yc4t = yclat(l)
...
!  Loop over cell vectors
!
                llloop2: do ll = 1,nvector
                  r432 = (xvec(ll)+x43t)**2 + (yvec(ll)+y43t)**2 + (zvec(ll)+z43t)**2
                  if (r432.lt.1d-12) cycle llloop2
!
!  Molecule checking
!
                  lbondedkl = .false.
...
!
                  ...
                  z41 = z43 + z32 + z21
                  r412 = x41*x41 + y41*y41 + z41*z41
                  if (r412.gt.tr4.and.tr4.gt.0.0_dp.and..not.lbtyp) cycle llloop2
...
!
                  ...
                  z42 = z32 + z43
                  r422 = x42*x42 + y42*y42 + z42*z42
                  if (r422.lt.1.0d-12) cycle llloop2
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"fournosd","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!
          ...
          if (lgrad2) then
            ...
            n1z = izl
!
!  lopi must be true for the following
!
            n1x2 = ixl
            n1x3 = ixl
            n1x4 = ixl
            if (lopj.and.lopk.and.lopl) then
              n2x1 = jx
              n2x3 = jx
              n2x4 = jx
              n3x1 = kx
...
!****************************
  if (noofp.gt.0) call fouroopd(eoop,esregion12loop,esregion2,eattach,lgrad1,lgrad2,xderv,yderv,zderv)
!************************
!  Improper potentials  *
!************************
  if (nimproper.gt.0) call fourimpd(eimp,esregion12limp,esregion2,eattach,lgrad1,lgrad2,xderv,yderv,zderv)
!   
...
!   
  if (lmarvreg2) then
    ...
    eimp = eimp + esregion12limp
  else
    esregion12 = esregion12 + esregion12l + esregion12loop + esregion12limp
  endif
!
...
!
  ...
  if (status/=0) call deallocate_error('fournosd','zderv')
  deallocate(yderv,stat=status)
  if (status/=0) call deallocate_error('fournosd','yderv')
  deallocate(xderv,stat=status)
  if (status/=0) call deallocate_error('fournosd','xderv')
  deallocate(nuniquelptr,stat=status)
  if (status/=0) call deallocate_error('fournosd','nuniquelptr')
  deallocate(nuniquekptr,stat=status)
  if (status/=0) call deallocate_error('fournosd','nuniquekptr')
  deallocate(nuniqueiptr,stat=status)
  if (status/=0) call deallocate_error('fournosd','nuniqueiptr')
  deallocate(nptrnfornonoop,stat=status)
  if (status/=0) call deallocate_error('fournosd','nptrnfornonoop')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fournosd','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fournosd','natmiddle')
!
!  Timing
!
  time2 = g_cpu_time()
