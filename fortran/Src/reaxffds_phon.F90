  subroutine reaxFFds_phon(xkv,ykv,zkv)
!
!  Calculates the dynamical matrix contribution for the ReaxFF force field.
...
!
!   8/23 Created from reaxFFds.F90 and old version of reaxffds_phon.F90
...
!
  use datatypes
  use control,        only : keyword, lreaxFFQ
  use g_constants,    only : evtokcal
  use current
  use derivatives,    only : derv2
  use iochannels
  use m_qderiv,       only : d2chargepd, d2chargeselfpd
...
!
  ...
  if (status/=0) call outofmemory('reaxFFds_phon','nboatomptr')
  allocate(nboatomRptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nboatomRptr')
  allocate(nbodummyptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nbodummyptr')
!
!  Allocate memory that does not depend on maxneigh
!
  allocate(numattodoptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','numattodoptr')
  allocate(numattodonptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','numattodonptr')
  allocate(numattodoRptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','numattodoRptr')
  allocate(nbos(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nbos')
  allocate(nbosptr(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nbosptr')
  allocate(latomdone(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','latomdone')
  allocate(nneigh(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nneigh')
  allocate(sumsij(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','sumsij')
  allocate(sum(max(3*numat,14_i4)),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','sum')
  allocate(sum0(max(3*numat,14_i4)),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','sum0')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('reaxFFds_phon','d2sboddeltaidbo')
    deallocate(d2sboproddbo2,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2sboproddbo2')
    deallocate(dsboproddbo,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','dsboproddbo')
    deallocate(nonzeroptr_j2k2,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzeroptr_j2k2')
    deallocate(nonzeroptr_jk,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzeroptr_jk')
    deallocate(nonzeroptr_ij2,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzeroptr_ij2')
    deallocate(nonzeroptr_ik,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzeroptr_ik')
    deallocate(nonzeroptr_ij,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzeroptr_ij')
    deallocate(nonzeroptr,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzeroptr')
    deallocate(nonzerorptr,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','nonzerorptr')
    deallocate(d1BOjk2_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk2_pipi')
    deallocate(d1BOjk2_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk2_pi')
    deallocate(d1BOjk2_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk2_s')
    deallocate(d1BOjk2,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk2')
    deallocate(d1BOjk_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk_pipi')
    deallocate(d1BOjk_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk_pi')
    deallocate(d1BOjk_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk_s')
    deallocate(d1BOjk,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOjk')
    deallocate(d2BOp_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOp_pipi')
    deallocate(d2BOp_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOp_pi')
    deallocate(d2BOp,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOp')
    deallocate(d2BOjk_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOjk_pipi')
    deallocate(d2BOjk_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOjk_pi')
    deallocate(d2BOjk_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOjk_s')
    deallocate(d2BOjk,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOjk')
    deallocate(d2BOi_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOi_pipi')
    deallocate(d2BOi_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOi_pi')
    deallocate(d2BOi_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOi_s')
    deallocate(d2BOi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2BOi')
    deallocate(d2mix,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2mix')
    deallocate(d1BOik_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOik_pipi')
    deallocate(d1BOik_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOik_pi')
    deallocate(d1BOik_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOik_s')
    deallocate(d1BOik,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOik')
    deallocate(d1BOij2_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOij2_pipi')
    deallocate(d1BOij2_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOij2_pi')
    deallocate(d1BOij2_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOij2_s')
    deallocate(d1BOij2,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOij2')
    deallocate(d1BOi_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOi_pipi')
    deallocate(d1BOi_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOi_pi')
    deallocate(d1BOi_s,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOi_s')
    deallocate(d1BOi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOi')
    deallocate(devaldsbo_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','devaldsbo_neigh')
    deallocate(devalddeltaj_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','devalddeltaj_neigh')
    deallocate(d2etorsdthetaddeltai,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2etorsdthetaddeltai')
    deallocate(d2etorsdBOdBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2etorsdBOdBOpi_neigh')
    deallocate(d2etorsdBOpi2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2etorsdBOpi2_neigh')
    deallocate(detorsdBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','detorsdBOpi_neigh')
    deallocate(d2evaldthetaddeltai_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evaldthetaddeltai_neigh')
    deallocate(d2evaldthetadBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evaldthetadBOpi_neigh')
    deallocate(d2evalddeltajdBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evalddeltajdBO_neigh')
    deallocate(d2etorsddeltajdBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2etorsddeltajdBOpi_neigh')
    deallocate(d2etorsddeltaidBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2etorsddeltaidBOpi_neigh')
    deallocate(d2etorsddeltadBOpi_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2etorsddeltadBOpi_neigh')
    deallocate(d2evaldsbo2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evaldsbo2_neigh')
    deallocate(d2evaldsbo_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evaldsbo_neigh')
    deallocate(d2evalddeltajk_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evalddeltajk_neigh')
    deallocate(d2evalddeltaiddeltaj_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evalddeltaiddeltaj_neigh')
    deallocate(d2evalddeltaidBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evalddeltaidBO_neigh')
    deallocate(d2evaldBO2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2evaldBO2_neigh')
    deallocate(devaldBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','devaldBO_neigh')
    deallocate(d2ebpenddeltaidBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2ebpendddeltaiBO_neigh')
    deallocate(d2ebpendBO2_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d2ebpendBO2_neigh')
    deallocate(debpendBO_neigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','debpendBO_neigh')
    deallocate(d1BOp_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOp_pipi')
    deallocate(d1BOp_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOp_pi')
    deallocate(d1BOp,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','d1BOp')
    deallocate(deltalplp,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','deltalplp')
    deallocate(deltalp,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','deltalp')
    deallocate(deltap,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','deltap')
    deallocate(delta,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','delta')
    deallocate(BOp_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','BOp_pipi')
    deallocate(BOp_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','BOp_pi')
    deallocate(BOp,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','BOp')
    deallocate(BO_pipi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','BO_pipi')
    deallocate(BO_pi,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','BO_pi')
    deallocate(BO,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','BO')
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','rneigh')
    deallocate(ineigh,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','ineigh')
    deallocate(neighnoRptr,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','neighnoRptr')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('reaxFFds_phon','neighno')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('reaxFFds_phon','neighno')
  allocate(neighnoRptr(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','neighnoRptr')
  allocate(ineigh(3_i4,maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','ineigh')
  allocate(rneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','rneigh')
  allocate(xneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','xneigh')
  allocate(yneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','yneigh')
  allocate(zneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','zneigh')
  allocate(BO(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','BO')
  allocate(BO_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','BO_pi')
  allocate(BO_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','BO_pipi')
  allocate(BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','BOp')
  allocate(BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','BOp_pi')
  allocate(BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','BOp_pipi')
  allocate(delta(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','delta')
  allocate(deltap(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','deltap')
  allocate(deltalp(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','deltalp')
  allocate(deltalplp(numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','deltalplp')
!
  allocate(nonzerorptr(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzerorptr')
  allocate(nonzeroptr(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzeroptr')
  allocate(nonzeroptr_ij(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzeroptr_ij')
  allocate(nonzeroptr_ij2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzeroptr_ij2')
  allocate(nonzeroptr_ik(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzeroptr_ik')
  allocate(nonzeroptr_jk(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzeroptr_jk')
  allocate(nonzeroptr_j2k2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','nonzeroptr_j2k2')
  allocate(d1BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOp')
  allocate(d1BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOp_pi')
  allocate(d1BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOp_pipi')
  allocate(debpendBO_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','debpendBO_neigh')
  allocate(devaldBO_neigh(maxneigh1a),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','devaldBO_neigh')
  allocate(detorsdBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','detorsdBOpi_neigh')
  allocate(devalddeltaj_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','devalddeltaj_neigh')
  allocate(devaldsbo_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','devaldsbo_neigh')
  allocate(d1BOi(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOi')
  allocate(d1BOi_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOi_s')
  allocate(d1BOi_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOi_pi')
  allocate(d1BOi_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOi_pipi')
  allocate(d1BOij2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOij2')
  allocate(d1BOij2_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOij2_s')
  allocate(d1BOij2_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOij2_pi')
  allocate(d1BOij2_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOij2_pipi')
  allocate(d1BOik(maxneigh2t),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOik')
  allocate(d1BOik_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOik_s')
  allocate(d1BOik_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOik_pi')
  allocate(d1BOik_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOik_pipi')
  allocate(d1BOjk(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk')
  allocate(d1BOjk_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk_s')
  allocate(d1BOjk_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk_pi')
  allocate(d1BOjk_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk_pipi')
  allocate(d1BOjk2(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk2')
  allocate(d1BOjk2_s(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk2_s')
  allocate(d1BOjk2_pi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk2_pi')
  allocate(d1BOjk2_pipi(maxnonzero),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d1BOjk2_pipi')
  allocate(dsboproddbo(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','dsboproddbo')
  allocate(d2mix(maxneigh2t),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2max')
  allocate(d2BOi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOi')
  allocate(d2BOi_s(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOi_s')
  allocate(d2BOi_pi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOi_pi')
  allocate(d2BOi_pipi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOi_pipi')
  allocate(d2BOjk(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOjk')
  allocate(d2BOjk_s(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOjk_s')
  allocate(d2BOjk_pi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOjk_pi')
  allocate(d2BOjk_pipi(maxnonzero2),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOjk_pipi')
  allocate(d2BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOp')
  allocate(d2BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOp_pi')
  allocate(d2BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2BOp_pipi')
  allocate(d2ebpenddeltaidBO_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2ebpenddeltaidBO_neigh')
  allocate(d2ebpendBO2_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2ebpendBO2_neigh')
  allocate(d2etorsdBOpi2_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2etorsdBOpi2_neigh')
  allocate(d2etorsdBOdBOpi_neigh(maxneigh1a,maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2etorsdBOdBOpi_neigh')
  allocate(d2etorsdthetaddeltai(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2etorsdthetaddeltai')
  allocate(d2evaldBO2_neigh(maxneigh2a),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evaldBO2_neigh')
  allocate(d2evalddeltaidBO_neigh(maxneigh1a),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evalddeltaidBO_neigh')
  allocate(d2evalddeltaiddeltaj_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evalddeltaiddeltaj_neigh')
  allocate(d2evalddeltajk_neigh(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evalddeltajk_neigh')
  allocate(d2evaldsbo_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evaldsbo_neigh')
  allocate(d2evaldsbo2_neigh(maxneigh1),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evaldsbo2_neigh')
  allocate(d2etorsddeltadBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2etorsddeltadBOpi_neigh')
  allocate(d2etorsddeltaidBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2etorsddeltaidBOpi_neigh')
  allocate(d2etorsddeltajdBOpi_neigh(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2etorsddeltajdBOpi_neigh')
  allocate(d2evalddeltajdBO_neigh(maxneigh,maxneigh1a),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evalddeltajdBO_neigh')
  allocate(d2evaldthetadBOpi_neigh(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evaldthetadBOpi_neigh')
  allocate(d2evaldthetaddeltai_neigh(maxneigh2e),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2evaldthetaddeltai_neigh')
  allocate(d2sboproddbo2(maxneigh1),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2sboproddbo2')
  allocate(d2sboddeltaidbo(maxneigh),stat=status)
  if (status/=0) call outofmemory('reaxFFds_phon','d2sboddeltaidbo')
!
!  Initial allocation of sparse arrays
!
  call realloc(d1is,maxsparse,status)
  if (status.ne.0) call outofmemory('reaxFFds_phon','d1is')
  maxsparse2 = maxsparse*(maxsparse+1)/2
  call realloc(d2is,maxsparse2,status)
  if (status.ne.0) call outofmemory('reaxFFds_phon','d2is')
  call realloc(d2evaldthetadBO_neigh,maxsparse,maxneigh1a,status)
  if (status.ne.0) call outofmemory('reaxFFds_phon','d2evaldthetadBO_neigh')
  call realloc(d2etorsdthetaddeltaj_neigh,maxsparse,maxneigh,status)
  if (status.ne.0) call outofmemory('reaxFFds_phon','d2etorsdthetaddeltaj_neigh')
  call realloc(d2etorsdthetadBOpi_neigh,maxsparse,maxneigh,status)
  if (status.ne.0) call outofmemory('reaxFFds_phon','d2etorsdthetadBOpi_neigh')
!*************************************
...
!*************************************************************************
  ...
  if (status/=0) call outofmemory('reaxFFds_phon','nbotodoptr')
! 
  if (lextended) then
    nbotodo = numat
    do i = 1,numat
...
!  Sigma
!
            call reaxFFbo_sigma(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.true.,.true., &
...
!  Pi
!
              call reaxFFbo_pi(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.true.,.true., &
                               .not.lStrict)
              BOp_pi(ni,i) = BOpij
              deltap(i) = deltap(i) + BOpij
...
!  Pi_pi
!
              call reaxFFbo_pipi(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.true.,.true., &
                                 .not.lStrict)
              BOp_pipi(ni,i) = BOpij
              deltap(i) = deltap(i) + BOpij
...
!
  ...
  if (status/=0) call deallocate_error('reaxFFds_phon','nbotodoptr')
!
!  Set neighnoRptr
!
  do i = 1,numat
...
!
      ...
      if (status/=0) call outofmemory('reaxFFds_phon','StatMPI')
!
      do i = 1,numat
...
!
      ...
      if (status/=0) call deallocate_error('reaxFFds_phon','StatMPI')
    else
#endif
!
      call mpbarrier
...
!
      ...
      if (status/=0) call outofmemory('reaxFFds_phon','nntmp')
      allocate(botmp(2*ntmp),stat=status)
      if (status/=0) call outofmemory('reaxFFds_phon','botmp')
...
!
      ...
      if (status/=0) call deallocate_error('reaxFFds_phon','botmp')
      deallocate(nntmp,stat=status)
      if (status/=0) call deallocate_error('reaxFFds_phon','nntmp')
    else
#endif
      do i = 1,numat
        if (procid.eq.numattodonptr(i)) then
...
!***************************************
        call reaxFF_bos(i,j,nspeci,nspecj,ni,nj,deltapi,deltapj,mneigh,nneigh,BOp,BOp_pi,BOp_pipi, &
                        ...
                        nonzero_ij_i,nonzeroptr_ij,.true.,.true.)
!
...
!
          call d2addpdmsp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nboatomRptr,d1is,d2is, &
                          xkv,ykv,zkv,nonzero_ij,nonzeroptr_ij,.true.,.false.)
        endif
!
...
!
                    call reaxFF_f7(nspeci,nspecj,nspeck,nval3,BOij,BOik,f7,df7dBOij,df7dBOik, &
                                   d2f7dBOij2,d2f7dBOijdBOik,d2f7dBOik2,.true.,.true.)
                    call reaxFF_f8(nspeci,nspecj,nspeck,nval3,delta(i),f8,df8ddeltai,d2f8ddeltai2,.true.,.true.)
                    call reaxFF_theta0(nspeci,nspecj,nspeck,nval3,sbo,theta0,dtheta0dsbo,d2theta0dsbo2,.true.,.true.)
!
...
!
                  call reaxFF_f9(delta(i),f9,df9ddeltai,d2f9ddeltai2,.true.,.true.)
!
...
!
                          call reaxFF_f10(BOik,BOij,BOjl,f10,df10dBOik,df10dBOij,df10dBOjl, &
                                          ...
                                          d2f10dBOijdBOjl,d2f10dBOjl2,.true.,.true.)
                          call reaxFF_f11(nspeci,nspecj,delta(i),delta(j),f11,df11ddeltaij,d2f11ddeltaij2,.true.,.true.)
!
...
!
                          call reaxFF_f12(BOik,BOij,BOjl,f12,df12dBOik,df12dBOij,df12dBOjl, &
                                          ...
                                          d2f12dBOijdBOjl,d2f12dBOjl2,lStrict,.true.,.true.)
!
...
!***************************************
          call reaxFF_bos(i,j,nspeci,nspecj,ni,nj,deltapi,deltapj,mneigh,nneigh,BOp,BOp_pi,BOp_pipi, &
                          ...
                          nonzero_ij,nonzero_ij_i,nonzeroptr_ij,.true.,.true.)
!
!  Convert non-zero indices
!
          ind0 = nneigh(i) + nneigh(i)*(nneigh(i) + 1)/2 + (ni - 1)*(mneigh+1)*mneigh + ni*mneigh
...
!
      call d2addpdmsp(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nboatomRptr,d1is,d2is, &
                      xkv,ykv,zkv,nonzero,nonzeroptr,.true.,lextended)
!
...
!
                      call real1strterm(ndim,xji,yji,zji,0.0_dp,0.0_dp,0.0_dp,dr2ds(1,1), &
                        d2r2dx2(1,1),d2r2dsdx(1,1,1),d2r2ds2(1,1,1),.true.)
                      call real1strterm(ndim,xki,yki,zki,0.0_dp,0.0_dp,0.0_dp,dr2ds(1,2), &
                        d2r2dx2(1,2),d2r2dsdx(1,1,2),d2r2ds2(1,1,2),.true.)
                      call real1strterm(ndim,xkj,ykj,zkj,0.0_dp,0.0_dp,0.0_dp,dr2ds(1,3), &
                        d2r2dx2(1,3),d2r2dsdx(1,1,3),d2r2ds2(1,1,3),.true.)
!
...
!
                      ...
                      call add_drv2_1pd(lilocal,ljlocal,ix,iy,iz,ixf,iyf,izf,jx,jy,jz,jxf,jyf,jzf,xji,yji,zji, &
                                        dehbdrij,d2ehbdrij2,d2r2dx2(1,1))
!
!  i-j / i-k
!
...
!
                      ...
                      call add_drv2_1pd(lilocal,lklocal,ix,iy,iz,ixf,iyf,izf,kx,ky,kz,kxf,kyf,kzf,xki,yki,zki, &
                                        dehbdrik,d2ehbdrik2,d2r2dx2(1,2))
!
!  i-k / j-k
!
...
!
                      ...
                      call add_drv2_1pd(ljlocal,lklocal,jx,jy,jz,jxf,jyf,jzf,kx,ky,kz,kxf,kyf,kzf,xkj,ykj,zkj, &
                                        dehbdrjk,d2ehbdrjk2,d2r2dx2(1,3))
!
!  Derivatives of hydrogen bond energy with respect to bond orders -> add to eval terms
!
...
!
                      ...
                      enddo
                      call d2addmixdspp(lilocal,lilocal,ljlocal,i,ix,iy,iz,ixf,iyf,izf,maxneigh,mneigh,nneigh,neighno, &
...
!
                      ...
                      do nl = 1,nonzero_ij
                        d2mix(nl) = d2trm*d1BOi(nl)
                      enddo
                      call d2addmixdspp(lilocal,lilocal,lklocal,i,ix,iy,iz,ixf,iyf,izf,maxneigh,mneigh,nneigh,neighno, &
                                        xneigh,yneigh,zneigh,nbodummyptr,d2mix,nonzero_ij,nonzeroptr_ij,ix,iy,iz,ixf,iyf,izf, &
...
!
                      ...
                      do nl = 1,nonzero_ij
                        d2mix(nl) = d2trm*d1BOi(nl)
                      enddo
                      call d2addmixdspp(lilocal,ljlocal,lklocal,i,ix,iy,iz,ixf,iyf,izf,maxneigh,mneigh,nneigh,neighno, &
...
!***************************************
  ...
  if (lreaxFF_ecoul.and.lreaxFFQ) then
    eself = 0.0_dp
    call setreaxffQ(nboatom,nboatomRptr,nbos,nbosptr,qreaxFF,eself,.false.,.true.)
  else
...
!
              call d2chargeselfpd(ii,i,j,ix,iy,iz,jx,jy,jz,xkv,ykv,zkv,nbosptr)
              cycle jloop
            endif
!
...
!
              call p7reaxFFvdwtaper(rij,reaxFFcutoffVDW,tp,dtpdr,d2tpdr2,.true.,.true.)
              if (reaxFFcutoffQ.ne.reaxFFcutoffVDW) then
                call p7reaxFFqtaper(rij,reaxFFcutoffQ,tpQ,dtpQdr,d2tpQdr2,.true.,.true.)
              else
                tpQ      = tp
                dtpQdr   = dtpdr
                d2tpQdr2 = d2tpdr2
...
!  Compute f13
!
                call reaxFF_f13(rij,reaxFFgammaVDW(ind),f13,df13drij,d2f13drij2,.true.,.true.)
!
!  Energy terms
!
...
!*****************************************
          if (lreaxFFQ) then
            ...
            enddo
            call d2chargepd(ii,i,j,nor,ix,iy,iz,jx,jy,jz,xkv,ykv,zkv,d0i,d0j,d1ix,d1iy,d1iz, &
                            d1jx,d1jy,d1jz,d2i2,d2ij,d2j2,d2self,derive0selfi,derive0selfj, &
                            .true.,.true.,nbosptr)
          endif
!
!  End of loop over j
...
!
  ...
  if (status/=0) call deallocate_error('reaxFFds_phon','d2ebpendBO2_neigh')
  deallocate(d2ebpenddeltaidBO_neigh,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','d2ebpenddeltaidBO_neigh')
  deallocate(debpendBO_neigh,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','debpendBO_neigh')
  deallocate(d1BOp_pipi,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','d1BOp_pipi')
  ...
  if (status/=0) call deallocate_error('reaxFFds_phon','sum0')
  deallocate(sum,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','sum')
  deallocate(sumsij,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','sumsij')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','nneigh')
  deallocate(latomdone,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','latomdone')
  deallocate(nbosptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','nbosptr')
  deallocate(nbos,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','nbos')
  deallocate(nbodummyptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','nbodummyptr')
  deallocate(nboatomRptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','nboatomRptr')
  deallocate(nboatomptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','nboatomptr')
  deallocate(numattodoRptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','numattodoRptr')
  deallocate(numattodonptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','numattodonptr')
  deallocate(numattodoptr,stat=status)
  if (status/=0) call deallocate_error('reaxFFds_phon','numattodoptr')
!     
