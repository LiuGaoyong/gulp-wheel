...
!
  ...
  use g_constants
  use control,        only : lDoQDeriv1, lrigid
  use current
  use derivatives
...
!
                do kl = 1,nstrains
                  ks = nstrptr(kl)
                  rstrd(kl) = rstrd(kl) + ed11*dr2ds(ks,1)
                  rstrd(kl) = rstrd(kl) + ed12*dr2ds(ks,2)
                  rstrd(kl) = rstrd(kl) + ed13*dr2ds(ks,3)
                enddo
              endif
!*************************
