...
!
  ...
  use control
  use current,        only : numat, nstrains, xclat, yclat, zclat, ndim, radf, nbsmat, strain, rv, ncf, lbsmat
  use current,        only : nrelf2a
...
!
  ...
  real(dp)                              :: xib              ! Stores the original value of the variable being finite differenced inbox
...
!
        if (ix.eq.1) then
          ...
          xinbox(i) = xib + findiffc
        elseif (ix.eq.2) then
          yclat(i) = xci + findiffc
          yinbox(i) = xib + findiffc
        else
          zclat(i) = xci + findiffc
          zinbox(i) = xib + findiffc
        endif
