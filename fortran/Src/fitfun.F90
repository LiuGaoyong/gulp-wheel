...
!   7/12 finfo array added for mode observable
!  11/12 Unused argument removed
!   5/13 Volume observable added
!   7/13 Check added as to whether optimisations failed
!  10/13 Name of property changed to property3
!  12/13 Stresses added as a separate observable to strain derivatives
!   3/14 Call to potential changed to electrostatic_potential for the
!        benefit of ChemShell
!   3/15 Call to setup added if Gasteiger is being used, even if configuration
!        hasn't changed since this is needed to get charges right
!  11/16 Call to phonond added for the parallel case
!  12/16 fc_supercell algorithm added
!  12/16 Call to deffreq replaced with phonond/phonon for cluster case
!   3/17 Modifications made to allow for new variable order in iopt
!   8/17 lnumerical added
!   9/17 Absolute dipole moment added as an observable
...
!
  ...
  real(dp)                                  :: time2
  real(dp)                                  :: volume
  real(dp), dimension(:), allocatable, save :: xcfull
  real(dp)                                  :: xdv
  real(dp)                                  :: ydv
  real(dp)                                  :: zdv
  real(dp)                                  :: xji
...
!
  lusenumericd2 = (lnoanald2.or.lnumerical)
!
...
!
  call fitcon(xcfull)
  call putpar(xcfull,ncfold,dummy,.false.)
!
...
!
!  Setup is needed for Gasteiger even if configuration doesn't change
!
      if (lgasteiger) call setup(.true.)
...
!****************************************
    if (lrelax) then
      ...
      if (.not.loptsuccess) loptallsuccess = .false.
    else
      do i = 1,nasym
...
!
    if (.not.lstraincellprop.and.lfgrad) then
...
!
      ...
      do i = 1,nvar
        ...
        elseif (iopttype(i).eq.iopt_radius) then
          gc(i) = rada(ind)
        elseif (iopttype(i).eq.iopt_cell.or.iopttype(i).eq.iopt_strain) then
          if (ind.eq.1) then
            gc(i) = aa
...
!
      if (lsymopt.and.(.not.lsymderv.or.lfcprop(nc))) then
        do i = 1,nasym
          ...
          ydrv(i) = rneq*ydrv(nr)
          zdrv(i) = rneq*zdrv(nr)
          if (lbsmat(i)) raderv(i) = rneq*raderv(nr)
        enddo
        if (lrigid) then
!
...
!  Dot product with unit vectors, then scale the magnitude of vector to change from per angstrom per frac
!
...
!
          do kk = 1,nmolasym
            ...
            ydv = molTdrv(2,kk)
            zdv = molTdrv(3,kk)
            molTdrv(1,kk) = xdv*rvt(1,1) + ydv*rvt(2,1) + zdv*rvt(3,1)
            molTdrv(2,kk) = xdv*rvt(1,2) + ydv*rvt(2,2) + zdv*rvt(3,2)
            molTdrv(3,kk) = xdv*rvt(1,3) + ydv*rvt(2,3) + zdv*rvt(3,3)
          enddo
        endif
...
!
          do kk = 1,nmolasym
            xdv = molTdrv(1,kk)
            ydv = molTdrv(2,kk)
            molTdrv(1,kk) = xdv*rvt(1,1) + ydv*rvt(2,1)
            molTdrv(2,kk) = xdv*rvt(1,2) + ydv*rvt(2,2)
          enddo
        endif
...
!
      do i = ninternalmin,ninternalmax
        ind = ioptindex(i)
        if (iopttype(i).eq.iopt_xf) then
          if (lsumcoreshell(ncf).and.ncsptr(ind).gt.0) then
            gc(i) = xdrv(ind) + xdrv(ncsptr(ind))
          else
            gc(i) = xdrv(ind)
          endif
        elseif (iopttype(i).eq.iopt_yf) then
          if (lsumcoreshell(ncf).and.ncsptr(ind).gt.0) then
            gc(i) = ydrv(ind) + ydrv(ncsptr(ind))
          else
            gc(i) = ydrv(ind)
          endif
        elseif (iopttype(i).eq.iopt_zf) then
          if (lsumcoreshell(ncf).and.ncsptr(ind).gt.0) then
            gc(i) = zdrv(ind) + zdrv(ncsptr(ind))
          else
            gc(i) = zdrv(ind)
          endif
        elseif (iopttype(i).eq.iopt_radius) then
...
!
!  Internal x centre of mass derivatives
!
            nv = ncfixind(i)
            lfound = .false.
            j = ninternalmin - 1
...
!
!  Internal y centre of mass derivatives
!
            nv = ncfixind(i)
            lfound = .false.
            j = ninternalmin - 1
...
!
!  Internal z centre of mass derivatives
!
            nv = ncfixind(i)
            lfound = .false.
            j = ninternalmin - 1
...
!
!  Internal x quaternion derivatives
!
            nv = ncfixind(i)
            lfound = .false.
            j = ninternalmin - 1
...
!
!  Internal y quaternion derivatives
!
            nv = ncfixind(i)
            lfound = .false.
            j = ninternalmin - 1
...
!
!  Internal z quaternion derivatives
!
            nv = ncfixind(i)
            lfound = .false.
            j = ninternalmin - 1
...
!***********************
    ...
    if (lfcphon(nc)) then
      if (lfcsupercell.and.ndimen(nc).gt.0) then
        call phonon_fc(.false.,fc,nobsmodeptr0,nobsmodecfg(nc))
      else
        if (nprocs.gt.1) then
          call phonond(.false.,fc,nobsmodeptr0,nobsmodecfg(nc))
        else
    ...
    lsymderv2 = lsymd2save
    if (npotptcfg(nc).gt.0) call electrostatic_potential
    tim2 = g_cpu_time()
...
!
        elseif (nobt.eq.34) then
          fcalc(i) = fcalc(i) + freaction(ncf,i)*(fc + fhenergy)
!
!  Reaction equipartition free energy
!
        elseif (nobt.eq.35) then
          fcalc(i) = fcalc(i) + freaction(ncf,i)*(fc + fhenergy_eq)
!
!  Young's modulus
!
...
!
!  Absolute dipole moment - note that the value has to be converted to Debye here
!
        elseif (nobt.eq.28) then
          fcalc(i) = abs_dipole_moment*4.8_dp
!
!  Vibrational mode
!
        elseif (nobt.eq.29) then
          ...
          finfo(i) = fobsmodeover(ni)
!
!  Volume
!
        elseif (nobt.eq.30) then
          fcalc(i) = volume(rv)
!
!  Stresses
!
        elseif (nobt.eq.31) then
          ni = nobptr(i)
          fcalc(i) = stresses(ni)
!
!  Free energy
!
        elseif (nobt.eq.32) then
          fcalc(i) = fc + fhenergy
!
!  Free energy (equipartition)
!
        elseif (nobt.eq.33) then
          fcalc(i) = fc + fhenergy_eq
!
!  Structure
!
