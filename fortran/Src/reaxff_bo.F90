  subroutine reaxFF_bo(i,j,nspeci,nspecj,ni,nj,deltapi,deltapj,mneigh,nneigh,BOp,BOp_pi,BOp_pipi, &
                       d1BOp,d1BOp_pi,d1BOp_pipi,d2BOp,d2BOp_pi,d2BOp_pipi,BOij_s,BOij_pi,BOij_pipi, &
                       d1BOi_s,d1BOi_pi,d1BOi_pipi,d2BOi_s,d2BOi_pi,d2BOi_pipi,lgrad1,lgrad2)
!
...
!  12/22 Corrected for cases where f1 or f45 are needed
!   3/23 Correction to derivatives where f1 and f45 are needed
!   4/23 Sparsity used to speed up derivatives
!
!  Conditions of use:
!
...
!
  ...
  nneighi2i2 = nneighi2i*(nneighi2i + 1)/2 
!
!  Sizes for arrays where sparsity is used
!
...
!
    if (lgrad1) then
      allocate(nonzeroptr(nneighij),stat=status)
      if (status/=0) call outofmemory('reaxFF_bo','nonzeroptr')
...
!
    if (lgrad1) then
      d1BOi_s(1:nneighi2i) = 0.0_dp
      d1BOi_pi(1:nneighi2i) = 0.0_dp
      d1BOi_pipi(1:nneighi2i) = 0.0_dp
      df1dr(1:nneighij) = 0.0_dp
      df45dr(1:nneighij) = 0.0_dp
      if (lgrad2) then
        d2BOi_s(1:nneighi2i2) = 0.0_dp
        d2BOi_pi(1:nneighi2i2) = 0.0_dp
        d2BOi_pipi(1:nneighi2i2) = 0.0_dp
...
!
      ...
      enddo
      indk = nneigh(i) + nneigh(i)*(nneigh(i) + 1)/2 + (ni - 1)*(mneigh+1)*mneigh + ni*mneigh
      do k = 1,nneigh(j)
        indk = indk + 1
        nonzero = nonzero + 1
        nonzeroptr(nonzero) = indk
...
!
        do nk = 1,nonzero
          k = nonzeroptr(nk)
!
          d1BOi_s(k)    = d1BOi_s(k)    + fs*df1dr(nk)
          d1BOi_pi(k)   = d1BOi_pi(k)   + fpi*df1dr(nk)
          d1BOi_pipi(k) = d1BOi_pipi(k) + fpipi*df1dr(nk)
!
          d1BOi_s(k)    = d1BOi_s(k)    + BOpij_s*f1*df45dr(nk)
          d1BOi_pi(k)   = d1BOi_pi(k)   + BOp_pi(ni,i)*f1*f1*df45dr(nk)
          d1BOi_pipi(k) = d1BOi_pipi(k) + BOp_pipi(ni,i)*f1*f1*df45dr(nk)
        enddo
!
        if (lgrad2) then
!
...
!================================
          ...
          do nk = 1,nonzero
            ...
            do nl = 1,nk
              ...
              ind2 = ind2 + 1
!
              d2BOi_s(ind)    = d2BOi_s(ind)    + (BOpij_s - f1*BOpij_pi)*f4*f5*d2f1dr2(ind2)
              d2BOi_pi(ind)   = d2BOi_pi(ind)   + 2.0_dp*BOp_pi(ni,i)*f1*f4*f5*d2f1dr2(ind2)
              d2BOi_pipi(ind) = d2BOi_pipi(ind) + 2.0_dp*BOp_pipi(ni,i)*f1*f4*f5*d2f1dr2(ind2)
!
              d2BOi_s(ind)    = d2BOi_s(ind)    + BOpij_s*f1*d2f45dr2(ind2)
              d2BOi_pi(ind)   = d2BOi_pi(ind)   + BOp_pi(ni,i)*f1*f1*d2f45dr2(ind2)
              d2BOi_pipi(ind) = d2BOi_pipi(ind) + BOp_pipi(ni,i)*f1*f1*d2f45dr2(ind2)
!
...
!
        do nk = 1,nonzero
          k = nonzeroptr(nk)
!
          d1BOi_s(k)    = d1BOi_s(k)    + BOp(ni,i)*f4*f5*df1dr(nk)
          d1BOi_pi(k)   = d1BOi_pi(k)   + BOp_pi(ni,i)*f4*f5*df1dr(nk)
          d1BOi_pipi(k) = d1BOi_pipi(k) + BOp_pipi(ni,i)*f4*f5*df1dr(nk)
!
          d1BOi_s(k)    = d1BOi_s(k)    + BOp(ni,i)*f1*df45dr(nk)
          d1BOi_pi(k)   = d1BOi_pi(k)   + BOp_pi(ni,i)*f1*df45dr(nk)
          d1BOi_pipi(k) = d1BOi_pipi(k) + BOp_pipi(ni,i)*f1*df45dr(nk)
        enddo
!
        if (lgrad2) then
!
...
!================================
          ...
          do nk = 1,nonzero
            ...
            do nl = 1,nk
              ...
              ind2 = ind2 + 1
!
              d2BOi_s(ind)    = d2BOi_s(ind)    + BOp(ni,i)*f4*f5*d2f1dr2(ind2)
              d2BOi_pi(ind)   = d2BOi_pi(ind)   + BOp_pi(ni,i)*f4*f5*d2f1dr2(ind2)
              d2BOi_pipi(ind) = d2BOi_pipi(ind) + BOp_pipi(ni,i)*f4*f5*d2f1dr2(ind2)
!
              d2BOi_s(ind)    = d2BOi_s(ind)    + BOp(ni,i)*f1*d2f45dr2(ind2)
              d2BOi_pi(ind)   = d2BOi_pi(ind)   + BOp_pi(ni,i)*f1*d2f45dr2(ind2)
              d2BOi_pipi(ind) = d2BOi_pipi(ind) + BOp_pipi(ni,i)*f1*d2f45dr2(ind2)
!
...
!
    if (lgrad1) then
      ...
      if (status/=0) call deallocate_error('reaxFF_bo','nonzeroptr')
    endif
...
!
        do nk = 1,nonzero
          k = nonzeroptr(nk)
          d1BOi_s(k)    = d1BOi_s(k)    + (BOpij_s - f1*BOpij_pi)*df1dr(nk)
          d1BOi_pi(k)   = d1BOi_pi(k)   + 2.0_dp*f1*BOp_pi(ni,i)*df1dr(nk)
          d1BOi_pipi(k) = d1BOi_pipi(k) + 2.0_dp*f1*BOp_pipi(ni,i)*df1dr(nk)
        enddo
!
        if (lgrad2) then
!
...
!
              d2BOi_s(ind)    = d2BOi_s(ind)    + (BOpij_s - f1*BOpij_pi)*d2f1dr2(ind2)
              d2BOi_pi(ind)   = d2BOi_pi(ind)   + 2.0_dp*f1*BOp_pi(ni,i)*d2f1dr2(ind2)
              d2BOi_pipi(ind) = d2BOi_pipi(ind) + 2.0_dp*f1*BOp_pipi(ni,i)*d2f1dr2(ind2)
            enddo
          enddo
        endif
      else
...
!
        do nk = 1,nonzero
          k = nonzeroptr(nk)
          d1BOi_s(k)    = d1BOi_s(k)    + BOp(ni,i)*df1dr(nk)
          d1BOi_pi(k)   = d1BOi_pi(k)   + BOp_pi(ni,i)*df1dr(nk)
          d1BOi_pipi(k) = d1BOi_pipi(k) + BOp_pipi(ni,i)*df1dr(nk)
        enddo
!
        if (lgrad2) then
!
...
!
          ...
          do nk = 1,nonzero
            ...
            do nl = 1,nk
              ...
              ind2 = ind2 + 1
!
              d2BOi_s(ind)    = d2BOi_s(ind)    + BOp(ni,i)*d2f1dr2(ind2)
              d2BOi_pi(ind)   = d2BOi_pi(ind)   + BOp_pi(ni,i)*d2f1dr2(ind2)
              d2BOi_pipi(ind) = d2BOi_pipi(ind) + BOp_pipi(ni,i)*d2f1dr2(ind2)
            enddo
          enddo
        endif
      endif
...
!
      do nk = 1,nonzero
        k = nonzeroptr(nk)
        d1BOi_s(k)    = d1BOi_s(k)    + BOp(ni,i)*df45dr(nk)
        d1BOi_pi(k)   = d1BOi_pi(k)   + BOp_pi(ni,i)*df45dr(nk)
        d1BOi_pipi(k) = d1BOi_pipi(k) + BOp_pipi(ni,i)*df45dr(nk)
      enddo
!
      if (lgrad2) then
!
...
!
        ...
        do nk = 1,nonzero
          ...
          do nl = 1,nk
            ...
            ind2 = ind2 + 1
!
            d2BOi_s(ind)    = d2BOi_s(ind)    + d2f45dr2(ind2)*BOp(ni,i)
            d2BOi_pi(ind)   = d2BOi_pi(ind)   + d2f45dr2(ind2)*BOp_pi(ni,i)
            d2BOi_pipi(ind) = d2BOi_pipi(ind) + d2f45dr2(ind2)*BOp_pipi(ni,i)
          enddo
        enddo
!
        do nk = 1,nonzero
...
!  Derivatives of BO' 
!
      d1BOi_s(ni)    = d1BOi_s(ni)    + d1BOp(ni,i)
      d1BOi_pi(ni)   = d1BOi_pi(ni)   + d1BOp_pi(ni,i)
      d1BOi_pipi(ni) = d1BOi_pipi(ni) + d1BOp_pipi(ni,i)
!
      if (lgrad2) then
!
!  Second derivatives of BO' only
!
        ind = ni*(ni + 1)/2
        d2BOi_s(ind)    = d2BOi_s(ind)    + d2BOp(ni,i)
        d2BOi_pi(ind)   = d2BOi_pi(ind)   + d2BOp_pi(ni,i)
        d2BOi_pipi(ind) = d2BOi_pipi(ind) + d2BOp_pipi(ni,i)
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('reaxFF_bo')
#endif
!
  return
  end
