...
!
  do i = nasum,ninsert,-1
    natcfg(i+1) = natcfg(i)
    nregionnocfg(i+1) = nregionnocfg(i)
    ...
    oxcfg(i+1) = oxcfg(i)
    radcfg(i+1) = radcfg(i)
    lbsmatcfg(i+1) = lbsmatcfg(i)
    lqmatom(i+1) = lqmatom(i)
    lsliceatom(i+1) = lsliceatom(i)
    ltranat(i+1) = ltranat(i)
    ltranatminus(i+1) = ltranatminus(i)
    lfix(i+1) = lfix(i)
    extpotcfg(i+1) = extpotcfg(i)
