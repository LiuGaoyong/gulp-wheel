...
!   9/13 Bonded and non-bond potentials treated in separate sections
...
!
        if (.not.lmatch(ni,ntypi,nt1,ntyp1,.true.)) cycle ibloop
...
!
        if (QMMMmode(ncf).gt.0) then
          if (nregiontypi.eq.1) cycle ibloop
        endif
! 
!  Dreiding option handling                 
! 
        if (ltdreiding(n)) then               
          if (.not.bonded2donor(i)) cycle ibloop
        endif
!
        xc1 = xclat(i)
        yc1 = yclat(i)
...
!  Compute r12
!
          call mindtoijk(nbondind(jb,i),jxx,jyy,jzz)
          x21 = x21 + r1x*jxx + r2x*jyy + r3x*jzz
          y21 = y21 + r1y*jxx + r2y*jyy + r3y*jzz
          z21 = z21 + r1z*jxx + r2z*jyy + r3z*jzz
          r12 = x21*x21 + y21*y21 + z21*z21
          r12 = sqrt(r12)
!
          nbtypeij  = nbondedtype(1,jb,i)
          nbtypeij2 = nbondedtype(2,jb,i)
!
!  Inner loop over second site
!
          kbloop: do kb = jb+1,nbonds(i)
            k = nbonded(kb,i)
            nk = nat(k)
            ntypk = nftype(k)
...
!
            if (.not.lmatch(nk,ntypk,nto,ntypo,.true.)) cycle kbloop
!  
!  QM/MM handling
!           
            if (QMMMmode(ncf).gt.0) then
              if (nregiontypi.eq.1.and.nregiontypj.eq.1.and.nregiontypk.eq.1) cycle kbloop
            endif
!
!  Dreiding option handling
!
            if (ltdreiding(n)) then
              if (.not.bonded2donorJK(i,j,k)) cycle kbloop
            endif
!
            x31 = xclat(k) - xc1
            y31 = yclat(k) - yc1
...
!  Compute r13
!
            call mindtoijk(nbondind(kb,i),kxx,kyy,kzz)
            x31 = x31 + r1x*kxx + r2x*kyy + r3x*kzz
            y31 = y31 + r1y*kxx + r2y*kyy + r3y*kzz
            z31 = z31 + r1z*kxx + r2z*kyy + r3z*kzz
            r13 = x31*x31 + y31*y31 + z31*z31
            r13 = sqrt(r13)
!
!  Bond type checking
!  
...
!               
            lbondtypeOK = .true.
            nbtypeik  = nbondedtype(1,kb,i)
            nbtypeik2 = nbondedtype(2,kb,i)
!               
...
!                 
              if (.not.ldiff23typ.or..not.ldiff23bo) cycle kbloop
!  
!  Check i-j bond for correct order
!                 
              if (nbotyp21.gt.0.and.nbotyp21.ne.nbtypeij) cycle kbloop
              if (nbotyp22.gt.1.and.nbotyp22.ne.nbtypeij2) cycle kbloop
!               
!  Check i-k bond for correct order
!                 
              if (nbotyp11.gt.0.and.nbotyp11.ne.nbtypeik) cycle kbloop
              if (nbotyp12.gt.1.and.nbotyp12.ne.nbtypeik2) cycle kbloop
            endif
!
!  Check r23 is OK
!
...
!
            ...
            nthbptr(nlist3md) = n
            if (ndim.gt.0) then
              icell31(nlist3md) = nbondind(jb,i)
              icell32(nlist3md) = nbondind(kb,i)
            else
              icell31(nlist3md) = 0
              icell32(nlist3md) = 0
            endif
...
!  Distance checking
!
            if (r12.gt.ttr1.or.r12.lt.ttr1m) then
...
!
                if (r13.gt.ttr2.or.r13.lt.ttr2m) then
