...
!   7/20 eigtol initialised
!   7/20 ELPA chosen as default maths library in parallel if available
!   8/20 lzdipole added
...
!   3/22 radmax initialised to zero
!   5/22 Extra sub-option added for parallel
!   8/22 leigenvectors_original added
!  10/22 lfreeze initialised
!   4/23 reaxff_ffield added
!   5/23 Copying of vsip and zeta values between arrays added
!   6/23 ndenfncomp initialised
!   7/23 lupdateinbox added
!   8/23 Initialisation of cutoffmax values added
!
!  Conditions of use:
!
...
!
  ...
  use m_eht,         only : vsip_s, vsip_p, zeta_s, zeta_p, maxele_eht
  use m_eht,         only : vsip_sin, vsip_pin, zeta_sin, zeta_pin
  use gulp_files
  use gulp_gfnff
  use fitting
  use four
  ...
  use ksample
  use kspace,        only : maxindk, maxindk2, maxindk3, accf1D, eta, tweatpi, seta, radmax
  use library
  ...
  use m_three
#ifdef ELPA
  use maths,         only : lhess2D, lcosmo2D, eigtol, lelpa
#else
  use maths,         only : lhess2D, lcosmo2D, eigtol
#endif
...
!
  ...
  leig = .false.
  leigen = .false.
  leigenvectors_original = .true.
  leigenvectors_phase = .false.
  leprint = .false.
  lequipartition = .false.
  leregion = .false.
  lewald = .false.
...
!
  ...
  if (nprocs.gt.1) then
!
!  Make ELPA maths library the default for parallel runs, if present
!
    lelpa = .true.
  endif
#endif
...
!  EHT parameters
!
  vsip_s(1:maxele_eht) = vsip_sin(1:maxele_eht)
  vsip_p(1:maxele_eht) = vsip_pin(1:maxele_eht)
  zeta_s(1:maxele_eht) = zeta_sin(1:maxele_eht)
  zeta_p(1:maxele_eht) = zeta_pin(1:maxele_eht)
!
!  EDIP parameters
!
...
!
!  EAM/MEAM value initialisation
!
  ndenfncomp(1:maxeamspec) = 0
!  
...
!---------------------------
  fitlabel(1,1,10) ='Val bond r0  '
  fitlabel(2,1,10) ='Val bond m   '
  fitlabel(3,1,10) ='Val bond rho '
  fitlabel(4,1,10) ='Val ener K_v '
  fitlabel(5,1,10) ='Val ener v0  '
  fitlabel(6,1,10) ='Val ener K_w '
  fitlabel(7,1,10) ='Val ener w0  '
  fitlabelunits(1,1,10)='Ang       '
  fitlabelunits(3,1,10)='Ang       '
  fitlabelunits(4,1,10)='eV        '
  fitlabelunits(6,1,10)='eV*Ang^-4 '
  fitlabelunits(7,1,10)='Ang       '
#ifdef TRACE
