  subroutine gettorsions(iout,imode,nphitot)
!
!  Calculates torsion angles and prints them out for valid four-body terms
!
!  On entry:
!
...
!  10/12 Error in middle atom type handling corrected - when middle atom types are
!        swapped then we need to swap ljmatch2/3
!  10/12 Modified so that the routine can be used either to
!        perform the main GULP I/O task or to write out the
!        Lammps file.
!   7/13 Logic of checking pairs of atoms changed. In the old version
!        a pair would be valid if the distance is below the cutoff
!        even if the atoms were not bonded for a potential of bonded
!        type. Now bonding overrides the cutoff.
!   7/13 When the end atoms are swapped, the middle atoms are now also
!        swapped otherwise the order of atom types for checking can get
!        messed up.
!   7/13 Improper torsion type added 
!   7/13 Logic for handling improper torsions changed
!   7/13 Improper torsions moved to a separate routine
!   7/13 Trap for improper torsions in main pots loop added
!   3/14 Renamed from torsion for benefit of ChemShell
!   2/18 Trace added
!   9/19 nrel2 / nrelat changed to nrela2f / nrelf2a
!  11/19 lnotorXduplicates added
!  11/22 Handling of lnotorXduplicates corrected
...
!
  if (imode.lt.0.or.imode.gt.1) then
    call outerror('invalid mode passed to gettorsions subroutine',0_i4)
    call stopnow('gettorsions')
  endif
!
...
!
  ...
  enddo
!   
!  If there are no standard four-body potentials - don't bother printing the banner!
!   
  if (imode.eq.0) then
    if (noofp.ne.nfor) then
      write(iout,'(/,''  Analysis of four-body terms for primitive cell:'',/)')
      write(iout,'(''--------------------------------------------------------------------------------'')')
...
!*************************
  pots: do n = 1,nfor
    nphi = 0
    if (loutofplane(n).or.mmfexc(n).eq.2) cycle pots
    nt1 = nfspec1(n)
    nt2 = nfspec2(n)
    nt3 = nfspec3(n)
    nt4 = nfspec4(n)
    ...
    ltsymm_exact = lexactmatch(nt2,ntyp2,nt3,ntyp3)
    lbtyp = (mmfexc(n).ge.1)
...
!
!  Is this a potential with wildcards?
!
    nwild1 = 0
    if (nt1.eq.maxele) nwild1 = nwild1 + 1
    if (nt4.eq.maxele) nwild1 = nwild1 + 1
!
...
!
              ...
              tr3 = rtmp
!
!  ... and this includes for the central atoms too
!
              ntmp = nt2
              nt2 = nt3
              nt3 = ntmp
...
!
!  If this is a wildcard potential would it duplicate a non-wildcard potential?
!
                  if (lnotorXduplicates.and.nwild1.gt.0) then
                    lpotok = .true.
                    pots2: do n2 = 1,nfor
                      if (loutofplane(n2).or.mmfexc(n2).eq.2.or.n.eq.n2) cycle pots2
!
!  Matches for atoms without wildcards
!
                      limatch12 = lmatch(ni,ntypi,nfspec1(n2),nfptyp1(n2),.false.)
                      limatch42 = lmatch(ni,ntypi,nfspec4(n2),nfptyp4(n2),.false.)
                      ljmatch22 = lmatch(nj,ntypj,nfspec2(n2),nfptyp2(n2),.false.)
                      ljmatch32 = lmatch(nj,ntypj,nfspec3(n2),nfptyp3(n2),.false.)
                      lkmatch22 = lmatch(nk,ntypk,nfspec2(n2),nfptyp2(n2),.false.)
                      lkmatch32 = lmatch(nk,ntypk,nfspec3(n2),nfptyp3(n2),.false.)
                      llmatch12 = lmatch(nl,ntypl,nfspec1(n2),nfptyp1(n2),.false.)
                      llmatch42 = lmatch(nl,ntypl,nfspec4(n2),nfptyp4(n2),.false.)
!
                      if (limatch12.and.ljmatch22.and.lkmatch32.and.llmatch42) then
!
!  Exact match without wildcards - takes priority
!
                        lpotok = .false.
                        exit pots2
                      elseif (limatch42.and.ljmatch32.and.lkmatch22.and.llmatch12) then
!
!  Exact match without wildcards - takes priority
!
                        lpotok = .false.
                        exit pots2
                      elseif (ljmatch22.and.lkmatch32) then
!
!  Find out if potential has wildcards
!
                        nwild2 = 0
                        ...
                        if (nwild2.gt.0) then
!
!  Check for matches with wildcards
!
                          limatch12 = lmatch(ni,ntypi,nfspec1(n2),nfptyp1(n2),.true.)
                          llmatch42 = lmatch(nl,ntypl,nfspec4(n2),nfptyp4(n2),.true.)
!
                          if (limatch12.and.llmatch42) then
!
!  Decide which potential has fewer wildcards and so takes precedence
!
                            if (nwild2.lt.nwild1) then
                              lpotok = .false.
                              exit pots2
                            endif
                          endif
                        endif
                      elseif (ljmatch32.and.lkmatch22) then
!
!  Find out if potential has wildcards
!
                        nwild2 = 0
...
!  Check for matches with wildcards
!
                          limatch42 = lmatch(ni,ntypi,nfspec4(n2),nfptyp4(n2),.true.)
                          llmatch12 = lmatch(nl,ntypl,nfspec1(n2),nfptyp1(n2),.true.)
!
                          if (limatch42.and.llmatch12) then
!
!  Decide which potential has fewer wildcards and so takes precedence
!
                            if (nwild2.lt.nwild1) then
                    ...
                    enddo pots2
!
!  If another potential takes priority then skip this one
!
                    if (.not.lpotok) cycle llloop
                  endif
!*********************************
...
!
  ...
  if (imode.eq.0) then
    if (nphitot.gt.0) then
      write(iout,'(''  Total number of torsions = '',i8)') nphitot
      write(iout,'(''--------------------------------------------------------------------------------'')')
    endif   
    write(iout,'(/)')
  endif
...
!
  ...
  if (status/=0) call deallocate_error('fournos','xvec')
!
!  For now don't do out of planes for Lammps
!
  if (imode.eq.0) then
! 
...
! 
!  If improper potentials are present call routine for these
! 
  if (nimproper.gt.0) call improper(iout,imode,nphitot)
!
!  Timing
!
  time2 = g_cpu_time()
