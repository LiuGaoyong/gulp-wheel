...
!   5/17 Parallelised for imode = 2 case with no derivatives
!   2/18 Trace added
...
!
  ...
  elseif (imode.eq.2) then
    if (nprocs.gt.1.and..not.lgrad1) then
      n1 = procid + 1
      nstep = nprocs
    else
      n1 = 2
...
!
    ...
    if (imode.eq.3.or.imode.eq.4) then
      ...
      if (imode.eq.2.and.(nprocs.gt.1.and..not.lgrad1)) then
        nloop2 = nreg1old
      else
        nloop2 = i - 1
      endif
...
!
      call twobody1(eatom,ereal,ec6,lgrad1,lgrad2,.false.,nor,1_i4,dist,xcrd,ycrd,zcrd,d0i,d0j, &
                    ...
                    sctrm1,sctrm2,qli,qlj,1_i4,1_i4,lcspair,.false.,.false.,lbonded,l2bonds,l3bonds, &
...
!
  ereal = ereal + ec6
!
!  If imode = 2 and there are no derivatives perform global sum of energies
!  and apply double counting correction.
!
  if (imode.eq.2.and.nprocs.gt.1.and..not.lgrad1) then
    esum1(1) = 0.5_dp*ereal
    esum1(2) = 0.5_dp*eatom
    call sumall(esum1,esum2,2_i4,"real11","esum")
    ereal = esum2(1)
    eatom = esum2(2)
  endif
!
