...
!   7/20 ELPA added as an option
!   2/21 Taper added to QEq radius
!   8/21 lxyz_extended added
!  10/21 q0 input added
!  12/21 lnomottlittleton now set for lpolar
!  12/21 pGFNFF modifications added
!   2/22 JDFTx output added
!   3/22 Version checking added
!   5/22 Extra sub-option for parallel added
!   8/22 OpenMM taper added
!   4/23 Check added to ensure that lammps and lammps_pots are differentiated in output option
!   5/23 Inputs of eht_scale, vsip and zeta added
!   6/23 Further inputs added for EHT
!
!  Conditions of use:
!
...
!
  ...
  use element
  use eemdata
  use m_eht,                  only : scale_eht, vsip_s, vsip_p, zeta_s, zeta_p
  ...
  use maths,                  only : lhess2D, lcosmo2D, leig_2stage, leig_mrrr, eigtol, lblock2D
  use molecule
...
!
  ...
  if (index(word,'vers').eq.1) goto 105
  ...
  if (index(word,'q0').eq.1)   goto 445
  ...
  if (index(word,'vsip').eq.1) goto 445
  if (index(word,'zeta').eq.1) goto 445
  ...
  if (index(word,'gfnff_ac').eq.1) goto 980
  if (index(word,'gfnff_ta').eq.1) goto 990
  if (index(word,'gfnff_ver').eq.1) goto 995
  if (index(word,'gfnff_p').eq.1) goto 1010
  if (index(word,'gfnff_cn').eq.1) goto 1020
  if (index(word,'gfnff_to').eq.1) goto 1030
  if (index(word,'gfnff_qt').eq.1) goto 1040
  if (index(word,'gfnff_hi').eq.1) goto 1050
  if (index(word,'gfnff_ks').eq.1) goto 1060
  if (index(word,'gfnff_te').eq.1) goto 1070
  if (index(word,'gfnff_et').eq.1) goto 1080
  if (index(word,'gfnff_rt').eq.1) goto 1090
  if (index(word,'gfnff_ct').eq.1) goto 1100
  if (index(word,'gfnff_at').eq.1) goto 1110
  if (index(word,'gfnff_fr').eq.1) goto 1120
  if (index(word,'eht_s').eq.1) goto 1130
  if (index(word,'eht_o').eq.1) goto 1140
  if (index(word,'eht_k').eq.1) goto 1150
  if (index(word,'eht_g').eq.1) goto 1160
  if (index(word,'fc_r').eq.1) goto 1170
  return
!*************************************
...
!  Version checking  *
!*********************
105 if (nfloat.gt.0) then
    if (abs(floats(1)).gt.version) then
      call outerror('This library requires a more recent version of GULP',iline)
  ...
  if (nword.gt.1) then
    do ii = 2,nword
      ...
      elseif (index(words(ii),'p7').eq.1) then
        tapertype = 6
      elseif (index(words(ii),'op').eq.1) then
        tapertype = 7
      endif
    enddo
  endif
...
310 if (nfloat.gt.0) then
    rqeq = floats(1)
    if (nfloat.gt.1) then
      rqeqtaper = rqeq - abs(floats(2))
      if (rqeqtaper.lt.0.0_dp) then
        call outerror('rqeq taper range cannot be larger than rqeq',iline)
        call stopnow('genword')
      endif
    else
      rqeqtaper = max(rqeq-3.0_dp,0.0_dp)
    endif
  else
...
!
  ...
  elseif (index(word,'ioni').eq.1) then
    ...
    elseif (nfloat.ge.1) then
      ...
      elseif (nfloat.ge.2) then
        ...
        call outerror('insufficient input reading ionic radius',iline)
        call stopnow('genword')
      endif
    endif
    rion(na) = val
  ...
  elseif (index(word,'cova').eq.1) then
    ...
    rcov(na) = val
    goto 440
  elseif (index(word,'q0').eq.1) then
    if (nword.ge.2) then
      s1 = words(2)(1:2)
      na = istoan(s1)
      if (nfloat.ge.1) then
    ...
    elseif (nfloat.ge.1) then
      ...
      elseif (nfloat.ge.2) then
        ...
        call outerror('insufficient input reading element charge',iline)
        call stopnow('genword')
      endif
    endif
    q0(na) = val
...
!
  ...
  elseif (index(word,'symb').eq.1) then
    ...
    atsym(na) = cha
    goto 440
  elseif (index(word,'vsip').eq.1) then
    if (nword.ge.2) then
      s1 = words(2)(1:2)
      na = istoan(s1)
      if (nfloat.ge.2) then
      ...
      elseif (nfloat.eq.1) then
        ...
        call outerror('insufficient input reading element vsip values',iline)
        call stopnow('genword')
      endif
    elseif (nfloat.ge.2) then
      na = nint(floats(1))
    ...
    goto 440
  elseif (index(word,'zeta').eq.1) then
    if (nword.ge.2) then
      s1 = words(2)(1:2)
      na = istoan(s1)
      if (nfloat.ge.2) then
      ...
      elseif (nfloat.eq.1) then
        ...
        call outerror('insufficient input reading element zeta values',iline)
        call stopnow('genword')
      endif
    elseif (nfloat.ge.2) then
      na = nint(floats(1))
...
510 if (nword.lt.2) then
  ...
  do i = 2,nword
    ...
    lset = .false.
!
!  Trap options with a - flag
!
    if (index(word,'-').eq.1) cycle
!
...
!
    if (lnodot) then
      ...
      if (.not.lxyz) then
        if (index(word,'xxyz ').eq.1) then
          lxyz = .true.
          lxyz_extended = .true.
          iwc = 10
          lset = .true.
        elseif (index(word,'xyz ').eq.1) then
          lxyz = .true.
          iwc = 10
          lset = .true.
        endif
      ...
      if (.not.llammps) then
!
!  Check that this is lammps and not lammps_pots
!
        if (index(word,'lam').eq.1.and.index(word,'lammps_p').eq.0) then
          llammps = .true.
          iwc = 28
          lset = .true.
        endif
      ...
      if (.not.lcfg) then
        if (index(word,'cfg ').eq.1) then
          lcfg = .true.
          iwc = 35
          lset = .true.
        endif
        if (i.lt.nword) then
          if (index(words(i+1),'-').eq.1) then
!
!  Process options
!
            if (index(words(i+1),'e').gt.1) then
              lcfg_e = .true.
            endif
            if (index(words(i+1),'f').gt.1) then
              lcfg_f = .true.
            endif
            if (index(words(i+1),'x').gt.1) then
              lcfg_fx = .true.
            endif
            if (index(words(i+1),'y').gt.1) then
              lcfg_fy = .true.
            endif
            if (index(words(i+1),'z').gt.1) then
              lcfg_fz = .true.
            endif
            if (index(words(i+1),'q').gt.1) then
              lcfg_q = .true.
            endif
            if (index(words(i+1),'d').gt.1) then
              lcfg_dq = .true.
            endif
          endif
        endif
...
!
      ...
      elseif (iwc.eq.34) then
        ...
        lxsfsym = llocalsym
        lxsfseq = llocalseq
      elseif (iwc.eq.35) then
        cfgfile = words(i)
        if (index(cfgfile,'.cfg').eq.0) then
          call endstring(cfgfile,len(cfgfile),iend)
          if (iend.eq.0) iend = len(cfgfile) - 3
          cfgfile(iend:iend+3) = '.cfg'
        endif
      elseif (iwc.eq.36) then
        jdftxfile = words(i)
        if (index(jdftxfile,'.jdftx').eq.0) then
          call endstring(jdftxfile,len(jdftxfile),iend)
          if (iend.eq.0) iend = len(jdftxfile) - 5
          fdffile(iend:iend+5) = '.jdftx'
        endif
...
650 if (nword.ge.2) then
    ...
    if (index(words(2),'lap').eq.1) then
      ...
      do i = 3,nword
        call stolc(words(i),maxword)
        if (index(words(i),'div').eq.1) ldivide_and_conquer = .true.
        if (index(words(i),'two').eq.1) leig_2stage = .true.
        if (index(words(i),'mrr').eq.1) leig_mrrr = .true.
        if (index(words(i),'blo').eq.1) lblock2D = .true.
      enddo
    elseif (index(words(2),'eis').eq.1) then
      leispack_eigensolve = .true.
    elseif (index(words(2),'elp').eq.1) then
      lelpa = .true.
    endif
  endif
#ifndef ELPA
  if (lelpa) then
    call outerror('Maths library is specified as ELPA but GULP was not compiled with -e ',iline)
    call stopnow('genword')
  endif
#endif
  if (nfloat.gt.0) then
    if (lelpa) then
      elpatype = nint(abs(floats(1)))
      if (elpatype.le.0.or.elpatype.gt.2) then
        call outerror('Invalid ELPA type specified - must be 1 or 2',iline)
        call stopnow('genword')
      endif
    else
      eigtol = abs(floats(1))
...
660 if (nword.ge.2) then
    ...
    if (index(words(2),'avoi').eq.1) then
      lfastMPI = .false.
    elseif (index(words(2),'bett').eq.1) then
      lscaleMPI = .true.
    endif
  endif
  lwordok = .true.
...
970 if (nword.gt.1) then
  ...
  return
!******************************
!  Accuracy factor for GFNFF  *
!******************************
980 if (nfloat.gt.0) then
    if (nword.gt.1) then
      if (index(words(2),'dis').eq.1) then
        gfnff_accuracy_disp = abs(floats(1))
        if (gfnff_accuracy_disp.gt.max_gfnff_acc_disp) then
          call outerror('gfnff_accuracy_disp is too large and would lead to negative cutoffs',iline)
          call stopnow('genword')
        endif
      elseif (index(words(2),'rep').eq.1) then
        gfnff_accuracy_rep = abs(floats(1))
        if (gfnff_accuracy_rep.gt.max_gfnff_acc_rep) then
          call outerror('gfnff_accuracy_rep is too large and would lead to negative cutoffs',iline)
          call stopnow('genword')
        endif
      elseif (index(words(2),'cn').eq.1) then
        gfnff_accuracy_cn = abs(floats(1))
        if (gfnff_accuracy_cn.gt.max_gfnff_acc_cn) then
          call outerror('gfnff_accuracy_cn is too large and would lead to negative cutoffs',iline)
          call stopnow('genword')
        endif
      elseif (index(words(2),'hb1').eq.1) then
        gfnff_accuracy_hb1 = abs(floats(1))
        if (gfnff_accuracy_hb1.gt.max_gfnff_acc_hb1) then
          call outerror('gfnff_accuracy_hb1 is too large and would lead to negative cutoffs',iline)
          call stopnow('genword')
        endif
      elseif (index(words(2),'hb2').eq.1) then
        gfnff_accuracy_hb2 = abs(floats(1))
        if (gfnff_accuracy_hb2.gt.max_gfnff_acc_hb2) then
          call outerror('gfnff_accuracy_hb2 is too large and would lead to negative cutoffs',iline)
          call stopnow('genword')
        endif
      else
        gfnff_accuracy = abs(floats(1))
        gfnff_accuracy_disp = gfnff_accuracy
        gfnff_accuracy_rep  = gfnff_accuracy
        gfnff_accuracy_cn   = gfnff_accuracy
        gfnff_accuracy_hb1  = gfnff_accuracy
        gfnff_accuracy_hb2  = gfnff_accuracy
      endif
    else
      gfnff_accuracy = abs(floats(1))
      gfnff_accuracy_disp = gfnff_accuracy
    ...
    read(nru,*,err=99,end=99) gfnff_accuracy
    gfnff_accuracy = abs(gfnff_accuracy)
    iline = iline + 1
    gfnff_accuracy_disp = gfnff_accuracy
    gfnff_accuracy_rep  = gfnff_accuracy
    gfnff_accuracy_cn   = gfnff_accuracy
...
!  Sanity check input
!
  if (gfnff_accuracy.gt.max_gfnff_accuracy) then
    call outerror('gfnff_accuracy is too large and would lead to negative cutoffs',iline)
    call stopnow('genword')
  endif
  lwordok = .true.
  return
!***************************
!  Taper factor for GFNFF  *
!***************************
990 if (nfloat.gt.0) then
    gfnff_taper = floats(1)
  else
    read(nru,*,err=99,end=99) gfnff_taper
    iline = iline + 1
  endif
!
!  Sanity check input
!
  if (gfnff_taper.gt.1.0_dp.or.gfnff_taper.lt.0.0_dp) then
    call outerror('gfnff_taper must be between 0 and 1',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
...
995 continue
  if (nword.gt.1) then
    gfnff_version = words(2)(1:6)
  else
    call outerror('gfnff_version has no version string',iline)
    call stopnow('genword')
  endif
!
!  Sanity check input
!
  if (gfnff_version.ne.'v6.3.2'.and.gfnff_version.ne.'v6.3.3') then
    call outerror('unknown gfnff version string',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
...
1010 continue
  if (nword.gt.1) then
    if (index(words(2),'in').eq.1) gfnff_pi_change = 1
    if (index(words(2),'de').eq.1) gfnff_pi_change = -1
  endif
!
  lwordok = .true.
  return
!***************************************************************
!  GFNFF C6 coordination number tolerance for 2nd derivatives  *
!***************************************************************
1020 continue
  if (nfloat.gt.0) then
    val = abs(floats(1))
    if (val.lt.1.0_dp) then
      gfnff_cnc6tol = val
    else
      gfnff_cnc6tol = 10.0_dp**(-val)
    endif
  else
    call outerror('value missing at gfnff_cnc6tol input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!************************************
!  GFNFF topological distance flag  *
!************************************
1030 continue
  if (nword.gt.1) then
    do i = 2,nword
      if (index(words(i),'ol').ne.0) lgfnff_newtopo = .false.
      if (index(words(i),'xt').ne.0) lgfnff_xtbtopo = .true.
    enddo
  endif
  if (nfloat.ge.2) then
    maxtoposhell  = nint(abs(floats(1)))
    maxtoposhell1 = nint(abs(floats(2)))
    if (maxtoposhell.lt.1.or.maxtoposhell1.lt.1) then
      call outerror('number of shells for GFNFF topology must be at least one',iline)
      call stopnow('genword')
    endif
    if (maxtoposhell1.gt.maxtoposhell) then
      call outwarning('number of shells for GFNFF topology in first loop is greater than for second',iline)
    endif
  elseif (nfloat.ge.1) then
    maxtoposhell = nint(abs(floats(1)))
    if (maxtoposhell.lt.1) then
      call outerror('number of shells for GFNFF topology must be at least one',iline)
      call stopnow('genword')
    endif
  endif
...
1040 continue
  if (nfloat.gt.0) then
    gfnff_q_trap = floats(1)
    if (gfnff_q_trap.gt.-1.0_dp) then
      call outwarning('qmin for gfnff_qtrap is not particular negative and may cause problems',iline)
    endif
  else
    call outerror('value missing at gfnff_qtrap input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!**************************************
!  GFNFF high coordination trap flag  *
!**************************************
1050 continue
  if (nword.gt.1) then
    lgfnff_highcn_trap = (index(words(2),'tr').ne.0) 
  endif
!
  lwordok = .true.
  return
!*************************************
!  GFNFF K space sampling parameter  *
!*************************************
1060 continue
  if (nfloat.gt.0) then
    gfnff_ks = abs(floats(1))
  else
    call outerror('value missing at gfnff_ks input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!********************************
!  GFNFF pi system temperature  *
!********************************
1070 continue
  if (nfloat.ge.2) then
    gfnff_pi_temp1 = abs(floats(1))
    gfnff_pi_temp2 = abs(floats(2))
  elseif (nfloat.eq.1) then
    gfnff_pi_temp1 = abs(floats(1))
    gfnff_pi_temp2 = gfnff_pi_temp1
  else
    call outerror('value missing at gfnff_temp input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!*****************************
!  GFNFF Wolf sum eta value  *
!*****************************
1080 continue
  if (nfloat.ge.1) then
    gfnff_wolf_eta = abs(floats(1))
  else
    call outerror('value missing at gfnff_eta input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!********************************************
!  GFNFF topological distance scale factor  *
!********************************************
1090 continue
  if (nfloat.ge.1) then
    gfnff_rtopo_scale = abs(floats(1))
  else
    call outerror('value missing at gfnff_rtopo input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!**************************************
!  GFNFF topological distance cutoff  *
!**************************************
1100 continue
  if (nfloat.ge.1) then
    tdist_thr = abs(floats(1))
    if (tdist_thr.ne.12.0_dp) lgfnff_newtopo = .true.
  else
    call outerror('value missing at gfnff_ctopo input',iline)
    call stopnow('genword')
  endif
!
  lwordok = .true.
  return
!*********************************
!  GFNFF ATM dispersion damping  *
!*********************************
1110 continue
  if (nword.ge.2) then
    word = words(2)
  endif
  if (index(word,'und').eq.1) atm_alpha1 = 0.0_dp
!
  lwordok = .true.
  return
!***************************
!  GFNFF fragment control  *
!***************************
1120 continue
  if (nword.ge.2) then
    word = words(2)
  endif
  if (index(word,'bon').eq.1) lgfnff_fragment_bond = .true.
!
  lwordok = .true.
  return
!*********************************
!  EHT Hamiltonian scale factor  *
!*********************************
1130 continue
  if (nfloat.ge.1) then
    scale_eht = abs(floats(1))
  else
    call outerror('value missing at eht_scale input',iline)
    call stopnow('genword')
  endif
!     
  lwordok = .true.
  return
!***********************************
!  EHT Hamiltonian overlap cutoff  *
!***********************************
1140 continue                  
  if (nfloat.ge.1) then        
    rmax_overlap = abs(floats(1)) 
  else
    call outerror('value missing at eht_overlap input',iline)
    call stopnow('genword')    
  endif
!     
  lwordok = .true.             
  return
!**********************************
!  EHT Hamiltonian kspace cutoff  *
!**********************************
1150 continue                  
  if (nfloat.ge.1) then        
    kspace_eht = abs(floats(1)) 
  else
    call outerror('value missing at eht_kspace input',iline)
    call stopnow('genword')    
  endif
!     
  lwordok = .true.             
  return
!*******************************
!  EHT Hamiltonian gamma flag  *
!*******************************
1160 continue                  
  if (nword.gt.1) then
    call stolc(words(2),maxword)
    if (index(words(2),'tr').eq.1) lgamma_eht = .true.
    if (index(words(2),'fa').eq.1) lgamma_eht = .false.
  endif
!     
  lwordok = .true.             
  return
!*************************
!  FC interaction range  *
!*************************
1170 continue                  
  if (nfloat.ge.1) then        
    rfcinteraction_in = abs(floats(1)) 
  else
    call outerror('value missing at fc_range input',iline)
    call stopnow('genword')    
  endif
  if (rfcinteraction_in.lt.1.0_dp) then
    call outerror('fc_range value is too small',iline)
    call stopnow('genword')
  endif
!     
  lwordok = .true.             
