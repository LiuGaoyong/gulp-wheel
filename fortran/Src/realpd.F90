...
!
        call twobody(eatom,ereal,ec6,.true.,.true.,.false.,nor,1_i4,npots,npotl,cut2r,cut2q,cut2s, &
                     nmolonly,factor,ofct,ospfct,radsum,sctrm1,sctrm2,qli,qlj,nspeci,nspecj,lcspair, &
                     lewaldtype,.false.,.false.,.false.)
        if ((leem.or.lgfnff).or.lDoQDeriv2) then
          do k = 1,nor
        ...
        if (leem.or.lgfnff) then
          ...
          elseif (lgfnff) then
            ...
            call gfnffbody1(eqeq,zetaij,.true.,.true.,nor,1_i4,dist,deriv,deriv2, &
...
!
          call d2chargepd(iloc,i,j,nor,ix,iy,iz,jx,jy,jz,xkv,ykv,zkv,d0i,d0j,d1ix,d1iy,d1iz, &
