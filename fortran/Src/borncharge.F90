...
!   8/23 Variable charge method extended to pGFN-FF and ReaxFF
!  10/23 Correction to parallel variable charge case
!
!  Conditions of use:
!
...
!
  use control,        only : leem, lrigid, lgfnff, lreaxFF
  use current
  use derivatives
  use element
...
!
  t1p = g_cpu_time()
  lvaryq = (leem.or.lreaxFF.or.lgfnff)   ! Variable charge flag
!
...
!
  do i = 1,numat
    if (nregionno(nrelf2a(i)).eq.1.and.nat(i).le.maxele) then
...
!
  if (nshell.gt.0) then
!#################################################
!  Shell contribution to Born effective charges  #
!#################################################
!
...
!
  ...
  elseif (lvaryq) then
!###########################################################
!  Variable charge contribution to Born effective charges  #
!###########################################################
...
!
        ...
        do k = 1,numatr1
          ...
          bornq(2,1,k) = bornq(2,1,k) + dqdxyz(ky,i)*xi
          bornq(3,1,k) = bornq(3,1,k) + dqdxyz(kz,i)*xi
          bornq(1,2,k) = bornq(1,2,k) + dqdxyz(kx,i)*yi
          bornq(2,2,k) = bornq(2,2,k) + dqdxyz(ky,i)*yi
          bornq(3,2,k) = bornq(3,2,k) + dqdxyz(kz,i)*yi
          bornq(1,3,k) = bornq(1,3,k) + dqdxyz(kx,i)*zi
          bornq(2,3,k) = bornq(2,3,k) + dqdxyz(ky,i)*zi
...
!**********************
  if (lprint.and.ioproc) then
    ...
    if (ndim.eq.2) then
      do i = 1,numat
        inat = nat(i)  
        if (nregionno(nrelf2a(i)).eq.1.and.inat.le.maxele) then
          n = n + 1
          itype = nftype(i)
