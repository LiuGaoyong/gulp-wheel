...
!  11/12 Unused argument to lmbfgssub removed
!   1/18 Trace added
...
!   2/22 Correction to check on fixed variables
!   4/23 Correction to referencing of rcell 
!
!  Conditions of use:
!
...
!
  ...
  do i = 1,nvar
    if (iopttype(i).eq.iopt_xf) then
      lopttmp(3*ioptindex(i)-2) = .true.
    elseif (iopttype(i).eq.iopt_yf) then
      lopttmp(3*ioptindex(i)-1) = .true.
    elseif (iopttype(i).eq.iopt_zf) then
      lopttmp(3*ioptindex(i)) = .true.
    endif
  enddo
!
  do i = 1,nasym
    ...
    if (.not.lopttmp(ix)) then
      diff = abs(xafrac(i) - nebfinalxyz(1,i,ncf))
    ...
    if (.not.lopttmp(iy)) then
      diff = abs(yafrac(i) - nebfinalxyz(2,i,ncf))
      if (diff.gt.1.0d-4) then
        call outerror('fixed variables must be the same for initial and final structures',0_i4)
        call stopnow('runneb')
      endif
    ...
    if (.not.lopttmp(iz)) then
      diff = abs(zafrac(i) - nebfinalxyz(3,i,ncf))
      if (diff.gt.1.0d-4) then
        call outerror('fixed variables must be the same for initial and final structures',0_i4)
        call stopnow('runneb')
      endif
...
!
      do n = 1,nvar
        ...
        elseif (iopttype(n).eq.iopt_zf) then
          ...
          if (lstraincell) then
            if (ioptindex(n).le.ndim) then
              xcrep(n,nrp+1) = strain(ioptindex(n)) + rcell(ioptindex(n))*GULP_random(iseed,2_i4)
            else
              xcrep(n,nrp+1) = strain(ioptindex(n))
            endif
          else
            if (ioptindex(n).le.ndim) then
              xcrep(n,nrp+1) = rcell(ioptindex(n))*GULP_random(iseed,2_i4)
            else
              xcrep(n,nrp+1) = 0.0_dp 
            endif
          endif
        endif
      enddo
...
!
  do n = 1,nvar
    ...
    elseif (iopttype(n).eq.iopt_zf) then
      ...
      if (lstraincell) then
        xcrep(n,nrep) = strain(ioptindex(n))
      else
        xcrep(n,nrep) = 0.0_dp
      endif
    endif
...
!
    call lmbfgssub(niterlocal,nvar*nrep,5_i4,stp,gcrep,diag,hessian,maxhess,pcrep)
!
...
!*************************************
  ...
  do ii = 1,nnebreplicatot
    if (nebreplicacfgptr(ii).eq.ncf) then
      nrp = nrp + 1
      if (lstraincell) then
        xstr(1:nstrains) = strain(1:nstrains)
      else
        xstr(1:nstrains) = 0.0_dp
      endif
      do n = 1,nvar
        if (iopttype(n).eq.iopt_radius) then
          i = ioptindex(n)
        ...
        elseif (iopttype(n).eq.iopt_zf) then
          ...
          xstr(ioptindex(n)) = xcrep(n,nrp+1)
        endif
      enddo
      rvloc = 0.0_dp
      if (ndim.eq.3) then
