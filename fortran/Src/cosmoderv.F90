...
!   3/14 Pointers nullified
!   9/14 Trap for npts = 0 added
!   4/17 Modified so that cosmoA appears to be 2D
!   4/17 New arguments added to subroutines when passing fullA
!   2/18 Trace added
...
!
  ...
  real(dp), dimension(:,:),   pointer,     save :: fullA => null()
...
!
    ...
    do ipts = 1,npts
      do jpts = 1,ipts
        kk = kk + 1
        fullA(ipts,jpts) = - 2.0_dp*cosmoA(kk,1)*fact
        fullA(jpts,ipts) = - 2.0_dp*cosmoA(kk,1)*fact
      enddo
    enddo
!
...
!****************************
  call setdcosmoamat(ldqneeded,lcosmicd2,lgrad2,dcosmoA,dcosmoAA,fullA,npts,d2qfct,dsegweight,d2segweight, &
    nvec,nmid,nearsas,nearsasptr,nearsasrptr,lnearsas,lanynonunit,xvec,yvec,zvec,dcosmoA2)
!****************************
!  Derivatives of B matrix  *
!****************************
  call setdcosmobmat(ldqneeded,lcosmicd2,lgrad2,dcosmoB,fullA,npts,d2qfct,dsegweight,d2segweight)
  if (ldqneeded) then
!*********************************************************
