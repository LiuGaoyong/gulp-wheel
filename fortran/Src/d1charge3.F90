...
!   7/17 Typo in atomic stress for atom k corrected
!   2/18 Trace added
...
!
  ...
  use current,        only : nstrains, nrelf2a, nregionno
  use derivatives
...
!
  do n = 1,nqatoms(i)
    m = nqatomptr(n,i)
    lopm = (.not.lfreeze.or.lopf(nrelf2a(m)))
    if (lopm) then
      mx = 3*(m - 1) + 1
      my = mx + 1
      mz = my + 1
  ...
  if (lstr) then
    ...
    if (latomicstress) then
      do kl = 1,nstrains
        ...
        atomicstress(kl,k) = atomicstress(kl,k) + qpotsumk*dqds(kl,k)
      enddo
    endif
  endif
#ifdef TRACE
  call trace_out('d1charge3')
#endif
!
  return
  end
