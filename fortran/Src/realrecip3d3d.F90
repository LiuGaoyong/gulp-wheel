  subroutine realrecip3d3d(nkp,mcvminloc,mcvmaxloc,eigr,eigi,dsqh,maxd2l,lfirstkpt)
!
!  Supplies the first derivatives of the free energy for a solid
!  Distributed memory parallel version. 
!
!  NOTE : Needs finishing for BSM and partial occupancy
!
!   5/17 Created from realrecip3d3
...
!   9/20 Correction to ShengBTE output of ghostcell vectors
...
!
  ...
  use gulp_files,     only : lshengBTE, iout_shengBTE
  use four
  use general
  use iochannels
  use ksample
...
!
!  Divide derivatives by number of processors since they will be summed at the end
!
  xdrv(1:numat) = xdrv(1:numat)/dble(nprocs)
  ydrv(1:numat) = ydrv(1:numat)/dble(nprocs)
  zdrv(1:numat) = zdrv(1:numat)/dble(nprocs)
  strderv(1:nstrains) = strderv(1:nstrains)/dble(nprocs)
!
!  Local variables
!
  small = 1.0d-12
  ...
  lmany = (lthb.or.lfor)
!
!  Initialise number of force constant derivatives
!
  nfc3 = 0
!
!  Open file for shengBTE if requested
!
  if (lshengBTE) then
    open(iout_shengBTE,file='FORCE_CONSTANTS_3RD_tmp',status='unknown',form='formatted')
!
!  Create cell vectors for original ghost cell
!
    rvgc(1:3,1) = rv(1:3,1)/dble(nsuperghost(1,ncf))
    rvgc(1:3,2) = rv(1:3,2)/dble(nsuperghost(2,ncf))
    rvgc(1:3,3) = rv(1:3,3)/dble(nsuperghost(3,ncf))
  endif
!  
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3d','npotl')
!**********************************************************
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3d','ktrm0')
  allocate(ktrm20(nwords),stat=status)
  if (status/=0) call outofmemory('realrecip3d3d','ktrm20')
  if (lc6loc) then
    allocate(ktrm6(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3d','ktrm6')
    allocate(ktrm60(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3d','ktrm60')
    allocate(ktrm62(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3d','ktrm62')
    allocate(ktrm620(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3d','ktrm620')
  else
...
!
    ...
    qli = qf(i)
    oci = occuf(i)
    if (lcorei) rmassi = rfmass(3*(i-1)+1)
    if (lbsmat(nrelf2a(i))) then
      radi = radf(i)
    else
      radi = 0.0_dp
    endif
    ...
    iz = indi + 3
!
!  Get ghostcell atom index for ShengBTE
!
    if (lshengBTE) then
      call getgcindex(i,igc,igcx,igcy,igcz)
    endif
!
!  Molecule handling
!
...
!
      ...
      qlj = qf(j)
      ocj = occuf(j)
      if (lcorej) rmassj = rfmass(3*(j-1)+1)
      if (lbsmat(nrelf2a(j))) then
        radj = radf(j)
      else
        radj = 0.0_dp
      endif
...
!
      if (lshengBTE) then
        call getgcindex(j,jgc,jgcx,jgcy,jgcz)
      endif
!
...
!
      call twobody(eatom,ereal,ec6,.true.,.true.,.true.,nor,1_i4,npots,npotl,cut2,cut2q, &
                   ...
                   nspeci,nspecj,lcspair,lewaldtype,.false.,.false.,.false.)
!*****************************************************************
...
!
          if (lshengBTE) then
            if (i.le.ncore.and.j.le.ncore) then
              nfc3 = nfc3 + 1
              write(iout_shengBTE,'(/,i6)') nfc3
!
!  Compute ghostcell vectors
!
              jigcx = jgcx - igcx
              jigcy = jgcy - igcy
              jigcz = jgcz - igcz
              xgc = rvgc(1,1)*dble(jigcx) + rvgc(1,2)*dble(jigcy) + rvgc(1,3)*dble(jigcz)
              ygc = rvgc(2,1)*dble(jigcx) + rvgc(2,2)*dble(jigcy) + rvgc(2,3)*dble(jigcz)
              zgc = rvgc(3,1)*dble(jigcx) + rvgc(3,2)*dble(jigcy) + rvgc(3,3)*dble(jigcz)
!
              write(iout_shengBTE,'(3f12.6)') xgc,ygc,zgc
              write(iout_shengBTE,'(3f12.6)') 0.0_dp,0.0_dp,0.0_dp
              write(iout_shengBTE,'(3i6)') igc,jgc,igc
!
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,1,1,d3k(1,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,1,2,d3k(2,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,1,3,d3k(3,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,2,1,d3k(2,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,2,2,d3k(2,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,2,3,d3k(3,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,3,1,d3k(3,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,3,2,d3k(3,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,3,3,d3k(3,3,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,1,1,d3k(2,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,1,2,d3k(2,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,1,3,d3k(3,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,2,1,d3k(2,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,2,2,d3k(2,2,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,2,3,d3k(3,2,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,3,1,d3k(3,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,3,2,d3k(3,2,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 2,3,3,d3k(3,3,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,1,1,d3k(3,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,1,2,d3k(3,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,1,3,d3k(3,3,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,2,1,d3k(3,2,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,2,2,d3k(3,2,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,2,3,d3k(3,3,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,3,1,d3k(3,3,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,3,2,d3k(3,3,2)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 3,3,3,d3k(3,3,3)
!
              nfc3 = nfc3 + 1
              write(iout_shengBTE,'(/,i6)') nfc3
              write(iout_shengBTE,'(3f12.6)') xgc,ygc,zgc
              write(iout_shengBTE,'(3f12.6)') xgc,ygc,zgc
              write(iout_shengBTE,'(3i6)') igc,jgc,jgc
!
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,1,1,d3k(1,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,1,2,d3k(2,1,1)
              write(iout_shengBTE,'(3i2,1x,f20.12)') 1,1,3,d3k(3,1,1)
...
!*********************************************
      t1 = g_cpu_time()
      call projd3ad(mcvminloc,mcvmaxloc,d3r,d3i,d3,d3rs,d3is,d3s,lstr,i,ix,iy,iz,j,jx,jy,jz, &
        ...
        nforkl,nptrfork,nptrforl,maxd2l,dsqh,lzsisa)
      t2 = g_cpu_time()
      tproj = tproj + t2 - t1
!
...
!************************
  ...
  if (status/=0) call deallocate_error('realrecip3d3d','ktrm620')
  deallocate(ktrm62,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','ktrm62')
  deallocate(ktrm60,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','ktrm60')
  deallocate(ktrm6,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','ktrm6')
  deallocate(ktrm20,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','ktrm20')
  deallocate(ktrm0,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','ktrm0')
!
!  Free local memory
!
  deallocate(npotl,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','npotl')
!
!  Global sum of derivatives
!
  allocate(dtmp(max(numat,nstrains)),stat=status)
  if (status/=0) call outofmemory('realrecip3d3d','dtmp')
!
  call sumall(xdrv,dtmp,numat,"realrecip3d3d","xdrv")
  xdrv(1:numat) = dtmp(1:numat)
  call sumall(ydrv,dtmp,numat,"realrecip3d3d","ydrv")
  ydrv(1:numat) = dtmp(1:numat)
  call sumall(zdrv,dtmp,numat,"realrecip3d3d","zdrv")
  zdrv(1:numat) = dtmp(1:numat)
  call sumall(strderv,dtmp,nstrains,"realrecip3d3d","strderv")
  strderv(1:nstrains) = dtmp(1:nstrains)
!
  deallocate(dtmp,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3d','dtmp')
!*********************************************************
...
!
!  Perform post-processing tasks on third-order derivatives to create a file for ShengBTE if requested
!
  if (lshengBTE) then
    call outshengBTEfc3(iout_shengBTE,nfc3)
!
!  Delete temporary file
!
    close(iout_shengBTE,status='delete')
  endif
!
