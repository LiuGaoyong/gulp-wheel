...
!
  ...
  character(len=maxwordlength), intent(in) :: action
!
!  Internal variables
!
  character(len=maxwordlength)             :: actionloc
  integer(i4)                              :: i
  integer(i4)                              :: iin
  integer(i4)                              :: iout
  integer(i4)                              :: j
...
!*******************
    ...
    ncellmaxcfg(ncfout) = ncellmaxcfg(ncfin)
    ncellmincfg(ncfout) = ncellmincfg(ncfin)
    ndimen(ncfout) = ndimen(ncfin)
    nfixatomcfg(ncfout) = nfixatomcfg(ncfin)
    nfixatomtypecfg(ncfout) = nfixatomtypecfg(ncfin)
    ninternalmaxcfg(ncfout) = ninternalmaxcfg(ncfin)
    ninternalmincfg(ncfout) = ninternalmincfg(ncfin)
    nregions(ncfout) = nregions(ncfin)
    nspcg(ncfout) = nspcg(ncfin)
    nsregion2(ncfout) = nsregion2(ncfin)
    nsuper(1:3,ncfout) = nsuper(1:3,ncfin)
    nsuperghost(1:3,ncfout) = nsuperghost(1:3,ncfin)
...
!
  ...
  if (index(actionloc,'all').eq.1.or.index(actionloc,'con').eq.1) then
!********************
!  Conditions copy  *
!********************
    ntempramp(ncfout) = ntempramp(ncfin)
    ntempstp(1:ntempramp(ncfout),ncfout) = ntempstp(1:ntempramp(ncfin),ncfin)
    ntempstpstart(1:ntempramp(ncfout),ncfout) = ntempstpstart(1:ntempramp(ncfin),ncfin)
    presscfg(ncfout) = presscfg(ncfin)
    stresscfg(1:6,ncfout) = stresscfg(1:6,ncfin)
    tempcfg(ncfout) = tempcfg(ncfin)
    tempstp(1:ntempramp(ncfout),ncfout) = tempstp(1:ntempramp(ncfin),ncfin)
  endif
  if (index(actionloc,'all').eq.1.or.index(actionloc,'sol').eq.1) then
!***********************
!  Solvent/COSMO copy  *
!***********************
  ...
  if (index(actionloc,'all').eq.1.or.index(actionloc,'md').eq.1) then
!****************************
...
!****************************
    ...
    tmdfieldstart(ncfout) = tmdfieldstart(ncfin)
    tmdfieldstop(ncfout) = tmdfieldstop(ncfin)
