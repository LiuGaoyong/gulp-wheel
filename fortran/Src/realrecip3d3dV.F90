...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3dV','npotl')
!**********************************************************
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3dV','ktrm0')
  allocate(ktrm20(nwords),stat=status)
  if (status/=0) call outofmemory('realrecip3d3dV','ktrm20')
  if (lc6loc) then
    allocate(ktrm6(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dV','ktrm6')
    allocate(ktrm60(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dV','ktrm60')
    allocate(ktrm62(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dV','ktrm62')
    allocate(ktrm620(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dV','ktrm620')
  else
...
!******************************************************************************
      t1 = g_cpu_time()
      call projd3dV(mcv,d3rs,d3is,d3s,i,ix,iy,iz,j,jx,jy,jz,lcorei,lcorej, &
                    rmassi,rmassj,derv2,dervi,eigr,eigi,maxd2l,grueneisen,d33s,d33rs,d33is, &
                    lmany,nmanyk,d34s,d34rs,d34is,nforkl)
      t2 = g_cpu_time()
      tproj = tproj + t2 - t1
1110  continue
    enddo
...
!************************
  ...
  if (status/=0) call deallocate_error('realrecip3d3dV','ktrm620')
  deallocate(ktrm62,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dV','ktrm62')
  deallocate(ktrm60,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dV','ktrm60')
  deallocate(ktrm6,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dV','ktrm6')
  deallocate(ktrm20,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dV','ktrm20')
  deallocate(ktrm0,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dV','ktrm0')
!
!  Free local memory
!
  deallocate(npotl,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dV','npotl')
!
