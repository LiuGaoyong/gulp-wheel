  subroutine four12d(efor,lgrad1,lgrad2,imode)
!
...
!
!   6/17 Created from four12
!   7/17 Error in setting lbondedjk corrected
!   7/17 Modified so that niltor is correct for Dreiding potentials
!   2/18 Trace added
...
!  11/21 Duplicate wildcard torsion checking corrected
!  11/22 Duplicate wildcard torsion checking further corrected
!
!  Conditions of use:
!
...
!
  use g_constants
  use control,         only : keyword, ld1sym, ld2sym, lnotorXduplicates
  use current
  use defects
  use derivatives
...
!
  ...
  if (status/=0) call outofmemory('four12d','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('four12d','ntypmiddle')
  allocate(nptrnfornonoop(nfor),stat=status)
  if (status/=0) call outofmemory('four12d','nptrnfornonoop')
  allocate(xderv(nr1),stat=status)
  if (status/=0) call outofmemory('four12d','xderv')
  allocate(yderv(nr1),stat=status)
  if (status/=0) call outofmemory('four12d','yderv')
  allocate(zderv(nr1),stat=status)
  if (status/=0) call outofmemory('four12d','zderv')
!
!  Initialisation
!
  efor = 0.0_dp
...
!
  ...
  do n = 1,nfor
    ...
    elseif (mmfexc(n).eq.2) then
      ...
      if (lfdreiding(n)) lanydreiding = .true.
    endif
  enddo
!
...
!
  ...
  jloop: do jloc = lpmin,lpmax,nproclp
    if (imode.eq.1) then
      j = node2reg1(jloc)
    else
      j = jloc
        ...
        if (lmolokjk.and.lanybtyp) then
          if (imode.eq.1) then
            if (lregion1j.and.lregion1k) then
              icm = 1
              lbondedjk = .false.
              do while (icm.le.nbondsdef(j).and..not.lbondedjk)
                imm = nbondeddef(icm,j)
                lbondedjk = (imm.eq.k)
                icm = icm + 1
              enddo
!
!  Set defaults for bond types
!
              nbtypejk = 0
              nbtypejk2 = 1
              lsamemoljk = lbondedjk
            else
...
!
            do nil2 = 1,niltor
              if (nil.eq.nil2) cycle
              if (lkeeptor(nil2)) then
...
!
                  if (nwild1.gt.0.or.nwild2.gt.0) then
                    if (nwild1.gt.nwild2) then
                      lkeeptor(nil) = .false.
                    else
                      lkeeptor(nil2) = .false.
                    endif
...
!
!  Handle energy being calculated differently for region 1/2 jk pairs
!
        if (.not.lregion1k) then
          efor = efor + eterm
        else
          efor = efor + 0.5_dp*eterm
        endif
!         
...
!********************************
  jloop2:  do j = 1,ntot-1
    ...
    if (.not.lmatchany(nj,ntypj,nmiddle,natmiddle,ntypmiddle,.true.)) cycle jloop2
!***************************************
!  Loop over second middle site 3 / k  *
!***************************************
    kloop2: do k = j+1,ntot
      lregion1k = (k.le.nr1)
      if (lregion1k) then
        if (imode.eq.1) then
          nk = natdefe(k)
      ...
      if (.not.lmatchany(nk,ntypk,nmiddle,natmiddle,ntypmiddle,.true.)) cycle kloop2
      if (.not.lmatchpair(nj,ntypj,nk,ntypk,nfor,nfspec2,nfptyp2,nfspec3,nfptyp3,.true.)) cycle kloop2
!
      x32 = xc3 - xc2
      y32 = yc3 - yc2
      z32 = zc3 - zc2
...
!  Distance checking
!     
      if (r322.gt.tr2max.and.(.not.lanybtyp.or..not.lbondedjk)) cycle kloop2
!***********************************
!  Loop over four-body potentials  *
!***********************************
      pots2: do nn = 1,nfornonoop
        n = nptrnfornonoop(nn)
        nfortype = nforty(n)
        nt1 = nfspec1(n)
        nt2 = nfspec2(n)
...
!  If no pair of matches can be found then cycle
!
        if (.not.ljkmatch.and..not.lkjmatch) cycle pots2
        lswitchil = .false.
        if (.not.ljkmatch) then
!
!  If j-k doesn't match, but k-j does then swap terms
...
!  Distance checking for j-k
!
        if (r322.gt.tr2.and.(.not.lbtyp.or..not.lbondedjk)) cycle pots2
!       
!  Check for intra and but not in same molecule
!       
...
!  Loop over end site 1 / i  *
!*****************************
        iloop2: do iloc = lpmin,lpmax,nproclp
          if (imode.eq.1) then
            i = node2reg1(iloc)
          else
            i = iloc
...
!
          ...
          if (lmol.and.lneedmol) then
            ...
            if (lmolok.and.linter_only) cycle iloop2
            if (.not.lmolok.and.lintra_only) cycle iloop2
            if (lmolok.and.lbtyp) then
              if (imode.eq.1) then
                if (lregion1j) then
                  lbondedij = .false.
...
!  Check for intra and but not in same molecule
!
          if (lintra_only.and..not.lmolok) cycle iloop2
          if (lbtyp.and..not.lmolok) cycle iloop2
          x21 = xc2 - xc1
          y21 = yc2 - yc1
          z21 = zc2 - zc1
!
...
!
          r212 = x21*x21 + y21*y21 + z21*z21
          if (r212.lt.1.0d-12) cycle iloop2
          if (r212.gt.tr1.and.(.not.lbtyp.or..not.lbondedij)) cycle iloop2
!
!  Check r31 is OK
!
          x31 = x32 + x21
          ...
          z31 = z32 + z21
          r312 = x31*x31 + y31*y31 + z31*z31
          if (r312.lt.1.0d-10) cycle iloop2
!**********************************
!  Loop over last end site 4 / l  *
!**********************************
          lloop2: do l = 1,ntot
...
!           
            ...
            if (lmol.and.lneedmol) then
              ...
              if (lmolok.and.linter_only) cycle lloop2
              if (.not.lmolok.and.lintra_only) cycle lloop2
              if (lmolok.and.lbtyp) then
                if (imode.eq.1) then
                  if (lregion1k.and.lregion1l) then
                    icm = 1
...
!
            r432 = x43*x43 + y43*y43 + z43*z43
            if (r432.lt.1d-12) cycle lloop2
            if (r432.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop2
!
!  Check r41 is OK
!
            x41 = x43 + x32 + x21
            ...
            z41 = z43 + z32 + z21
            r412 = x41*x41 + y41*y41 + z41*z41
            if (r412.lt.1.0d-12) cycle lloop2
            if (r412.gt.tr4.and.tr4.gt.0.0_dp.and..not.lbtyp) cycle lloop2
!
!  Check r42 is OK
!
            x42 = x32 + x43
            ...
            z42 = z32 + z43
            r422 = x42*x42 + y42*y42 + z42*z42
            if (r422.lt.1.0d-12) cycle lloop2
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!           
        ...
        enddo iloop2
!
!  Need to perform global sum of niltor if this is a Dreiding potential
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"four12d","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!
          if (lfdreiding(n)) then
            rkforloc = rkforloc/dble(niltortot)
          endif
          npha = 0
          nfortype = nforty(n)
          if (nfortype.eq.1) then
...
!
          call fourbody(n,nfortype,r21,r31,r41,r32,r42,r43,eterm,e1d,e2d,e3d, &
                        rko,rn,phi0o,isgn,fpoly,lgrad1,lgrad2,.false.)
!
!  Only need to add energy here if both j and k are in region 2
!
          if (.not.lregion1j.and..not.lregion1k) then
            efor = efor + eterm
          endif
!         
!  Check that at least one species is in the asymmetric unit
...
!
            ...
            if (lmol.and.lneedmol) then
              ...
              if (lmolok.and.linter_only) cycle iloop3
              if (.not.lmolok.and.lintra_only) cycle iloop3
              if (lmolok.and.lbtyp) then
                if (imode.eq.1) then
                  if (lregion1i.and.lregion1j) then
                    lbondedij = .false.
...
!  Check for intra and but not in same molecule
!
            if (lintra_only.and..not.lmolok) cycle iloop3
            if (lbtyp.and..not.lmolok) cycle iloop3
            x21 = xc2 - xc1
            y21 = yc2 - yc1
            z21 = zc2 - zc1
!
...
!
            r212 = x21*x21 + y21*y21 + z21*z21
            if (r212.lt.1.0d-12) cycle iloop3
            if (r212.gt.tr1.and.(.not.lbtyp.or..not.lbondedij)) cycle iloop3
!
!  Check r31 is OK
!
            x31 = x32 + x21
            ...
            z31 = z32 + z21
            r312 = x31*x31 + y31*y31 + z31*z31
            if (r312.lt.1.0d-10) cycle iloop3
!**********************************
!  Loop over last end site 4 / l  *
!**********************************
            lloop3: do lloc = lpmin,lpmax,nproclp
              if (imode.eq.1) then
                l = node2reg1(lloc)
              else
                l = lloc
...
!  Check l is allowed for n
!           
              if (.not.lmatch(nl,ntypl,nt4,ntyp4,.true.)) cycle lloop3
!  
!  Prevent atoms j and l being the same atom
!           
              if (l.eq.j) cycle lloop3
!  
!  Molecularity check
!           
              lbondedkl = .false.
              if (lmol.and.lneedmol) then
                ...
                if (lmolok.and.linter_only) cycle lloop3
                if (.not.lmolok.and.lintra_only) cycle lloop3
                if (lmolok.and.lbtyp) then
                  if (imode.eq.1) then
                    if (lregion1k) then
                      icm = 1
...
!
                  ...
                  if (.not.lbondedkl) cycle lloop3
                endif
              else
                lmolok = .false.
              endif
              if (lbtyp.and..not.lmolok) cycle lloop3
              x43 = xc4 - xc3
              y43 = yc4 - yc3
              z43 = zc4 - zc3
!
...
!
              r432 = x43*x43 + y43*y43 + z43*z43
              if (r432.lt.1d-12) cycle lloop3
              if (r432.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop3
!
!  Check r41 is OK
!
              x41 = x43 + x32 + x21
              ...
              z41 = z43 + z32 + z21
              r412 = x41*x41 + y41*y41 + z41*z41
              if (r412.lt.1.0d-12) cycle lloop3
              if (r412.gt.tr4.and.tr4.gt.0.0_dp.and..not.lbtyp) cycle lloop3
!
!  Check r42 is OK
!
              x42 = x32 + x43
              ...
              z42 = z32 + z43
              r422 = x42*x42 + y42*y42 + z42*z42
              if (r422.lt.1.0d-12) cycle lloop3
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!****************************
  if (noofp.gt.0) call fouroop12d(efor,lgrad1,lgrad2,imode,xderv,yderv,zderv)
!************************
!  Improper potentials  *
!************************
  if (nimproper.gt.0) call fourimp12d(efor,lgrad1,lgrad2,imode,xderv,yderv,zderv)
!
  if (lgrad1) then
    if (ldsl) then
!
...
!  Globalise energy
!
  call sumone(efor,eterm,"four12d","efor")
  efor = eterm
!
!  Free local memory
!
  deallocate(zderv,stat=status)
  if (status/=0) call deallocate_error('four12d','zderv')
  deallocate(yderv,stat=status)
  if (status/=0) call deallocate_error('four12d','yderv')
  deallocate(xderv,stat=status)
  if (status/=0) call deallocate_error('four12d','xderv')
  deallocate(nptrnfornonoop,stat=status)
  if (status/=0) call deallocate_error('four12d','nptrnfornonoop')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('four12d','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('four12d','natmiddle')
!
!  Timing
!
  time2 = g_cpu_time()
