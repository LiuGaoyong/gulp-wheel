...
!   9/12 Error in position of adding esum to region energies fixed
...
!
  use datatypes
  use bondorderdata
  use configurations, only : nregions
...
!       
    ...
    if (nregions(ncf).ge.2) then
      lreg2pair = (nregionno(nrelf2a(ii)).gt.1)
    endif
    do m = 1,nboQ0
      esum = 0.0_dp
