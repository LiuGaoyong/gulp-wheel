...
!
  use datatypes
  use bondorderdata
  use configurations, only : nregiontype, QMMMmode
...
#ifdef TRACE
  call trace_in('bondorderfcd')
#endif
!
  t1 = g_cpu_time()
  allocate(nboatomRptr(numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','nboatomRptr')
! 
...
!
  ...
  if (status/=0) call outofmemory('bondorderfcd','latomdone')
  allocate(nneigh(numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','nneigh')
  allocate(rBOcutmax(numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','rBOcutmax')
  allocate(Zcn(numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','Zcn')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('bondorderfcd','d2BtotiR')
    deallocate(d2BtotiA,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','d2BtotiA')
    deallocate(d2i,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','d2i')
    deallocate(d1BtotiR,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','d1BtotiR')
    deallocate(d1BtotiA,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','d1BtotiA')
    deallocate(d1i,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','d1i')
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','rneigh')
    deallocate(ineigh,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','ineigh')
    deallocate(nbopotptr,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','nbopotptr')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('bondorderfcd','neighno')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('bondorderfcd','neighno')
  allocate(nbopotptr(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','nbopotptr')
  allocate(ineigh(3_i4,maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','ineigh')
  allocate(rneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','rneigh')
  allocate(xneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','xneigh')
  allocate(yneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','yneigh')
  allocate(zneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','zneigh')
  allocate(d1i(maxneigh2),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','d1i')
  allocate(d1BtotiA(maxneigh2),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','d1BtotiA')
  allocate(d1BtotiR(maxneigh2),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','d1BtotiR')
  allocate(d2i(maxneigh22),stat=status)   
  if (status/=0) call outofmemory('bondorderfcd','d2i')
  allocate(d2BtotiA(maxneigh22),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','d2BtotiA')
  allocate(d2BtotiR(maxneigh22),stat=status)
  if (status/=0) call outofmemory('bondorderfcd','d2BtotiR')
!*************************************
...
!
              ...
              elseif (nBOtapertype(npki).eq.3) then
                call murtytaper(rik,rBOmin(npki),rBOmax(npki),fik,dfikdr,d2fikdr2,d3fikdr3,.true.,.true.,.false.)
              else
                ...
                if (nboRik.gt.0) then
                  if (nBOtypeR(nboRik).ne.1) then
                    call GthetaBO(xji,yji,zji,xki,yki,zki,nBOtypeR(nboRik),BOccoeffR(1,nboRik), &
                      BOhcoeffR(nboRik),Gijk,dGijkdr,d2Gijkdr2,d3Gijkdr3,.true.,.true.,.false.)
                  else
                    Gijk = 1.0_dp
                    dGijkdr = 0.0_dp
                    d2Gijkdr2 = 0.0_dp
                ...
                if (nboAik.gt.0) then
                  if (nBOtypeA(nboAik).ne.1) then
                    call GthetaBO(xji,yji,zji,xki,yki,zki,nBOtypeA(nboAik),BOccoeffA(1,nboAik), &
                      BOhcoeffA(nboAik),Gijk,dGijkdr,d2Gijkdr2,d3Gijkdr3,.true.,.true.,.false.)
                  else
                    Gijk = 1.0_dp
                    dGijkdr = 0.0_dp
                    d2Gijkdr2 = 0.0_dp
...
!
    call d2addfcd(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,ineigh,nboatomRptr,d1i,d2i,.false.)
  enddo
  if (nboZ.gt.0) then
!
...
!
    ...
    if (status/=0) call outofmemory('bondorderfcd','d1ZiCN')
!**************************************
...
!
    ...
    if (status/=0) call deallocate_error('bondorderfcd','d1ZiCN')
  endif
!
!  Free local memory
!
  ...
  if (status/=0) call deallocate_error('bondorderfcd','Zcn')
  deallocate(rBOcutmax,stat=status)
  if (status/=0) call deallocate_error('bondorderfcd','rBOcutmax')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('bondorderfcd','nneigh')
  deallocate(latomdone,stat=status)
  if (status/=0) call deallocate_error('bondorderfcd','latomdone')
  deallocate(nboatomRptr,stat=status)
  if (status/=0) call deallocate_error('bondorderfcd','nboatomRptr')
!
  t2 = g_cpu_time()
  tbondorder = tbondorder + t2 - t1
#ifdef TRACE
  call trace_out('bondorderfcd')
#endif
!
  return
  end
