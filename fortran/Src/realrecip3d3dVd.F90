  subroutine realrecip3d3dVd(nkp,mcv,mcvloc,mcvptr,eigr,eigi,maxd2l,grueneisen)
!
...
!
!   2/18 Created from realrecip3d3dV
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3dVd','npotl')
!**********************************************************
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3dVd','ktrm0')
  allocate(ktrm20(nwords),stat=status)
  if (status/=0) call outofmemory('realrecip3d3dVd','ktrm20')
  if (lc6loc) then
    allocate(ktrm6(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVd','ktrm6')
    allocate(ktrm60(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVd','ktrm60')
    allocate(ktrm62(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVd','ktrm62')
    allocate(ktrm620(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVd','ktrm620')
  else
...
!
      ...
      d3rs(1:3,1:3,1:3) = 0.0_dp
      d3is(1:3,1:3,1:3) = 0.0_dp
!
...
!*****************************************************************
      if (ndim.gt.1) then
        call reciptrmd3dV(i,j,nkp,c6tot,nkvec0,ktrm0,ktrm20,ktrm6,ktrm60,ktrm62, &
          ktrm620,d3s,d3rs,d3is)
      endif
...
!****************************
      if (ndim.eq.1) then
        call real1d3dV(nkp,i,j,nati,natj,d3s,d3rs,d3is,xcrd,ycrd,zcrd)
      endif
!***************************
...
!****************************
      ...
      nforkl = 0
      if (lthb) then
        call three3d3dV(nkp,i,j,nati,ntypi,natj,ntypj,d3s,d3rs,d3is, &
...
!***************************
      if (lfor) then
        call four3d3dV(nkp,i,j,d3s,d3rs,d3is,nmanyk,nforkl)
      endif
!******************************************************************************
!  Project contributions on to phonon modes to compute Grueneisen parameters  *
!******************************************************************************
      t1 = g_cpu_time()
      call projd3dVd(mcvloc,d3rs,d3is,d3s,i,ix,iy,iz,j,jx,jy,jz,lcorei,lcorej, &
                     rmassi,rmassj,derv2,dervi,eigr,eigi,maxd2l,grueneisen,d33s, &
                     d33rs,d33is,lmany,nmanyk,d34s,d34rs,d34is,nforkl)
      t2 = g_cpu_time()
      tproj = tproj + t2 - t1
1110  continue
    enddo
...
!************************
  ...
  if (status/=0) call deallocate_error('realrecip3d3dVd','ktrm620')
  deallocate(ktrm62,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','ktrm62')
  deallocate(ktrm60,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','ktrm60')
  deallocate(ktrm6,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','ktrm6')
  deallocate(ktrm20,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','ktrm20')
  deallocate(ktrm0,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','ktrm0')
!
!  Free local memory
!
  deallocate(npotl,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','npotl')
!
!  Global sum of Grueneisen parameters
!
  allocate(dtmp(mcv),stat=status)
  if (status/=0) call outofmemory('realrecip3d3dVd','dtmp')
!
!  Move values to correct global location
!
  dtmp(1:mcv) = 0.0_dp
  do i = 1,mcvloc
    dtmp(mcvptr(i)) = grueneisen(i)
  enddo
!
  call sumall(dtmp,grueneisen,mcv,"realrecip3d3dVd","grueneisen")
!
  deallocate(dtmp,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVd','dtmp')
!
...
#ifdef TRACE
  call trace_out('realrecip3d3dVd')
#endif
!
  return
  end
