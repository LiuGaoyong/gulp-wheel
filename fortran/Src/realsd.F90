...
!***********************
        if (ndim.eq.3) then
          call rsearch3D(xcrd,ycrd,zcrd,lmolok,lcspair,nreli,j,nmi,ixj,iyj,izj,nor,nmolonly,lself,cut2,cut2q)
        elseif (ndim.eq.2) then
          call rsearch2D(xcrd,ycrd,zcrd,lmolok,lcspair,nreli,j,nmi,ixj,iyj,izj,nor,nmolonly,lself,cut2,cut2q)
        elseif (ndim.eq.1) then
          call rsearch1D(xcrd,ycrd,zcrd,lmolok,lcspair,nreli,j,nmi,ixj,iyj,izj,nor,nmolonly,lself,cut2,cut2q)
        endif
!
        if (lself) then
!**************
...
!**************
          ...
          derive0self = 0.0_dp
          if (lfreeze) then
            call selfterm(ereall,erecipl,ec6l,derive0self,factor,fct,ofct,ospfct,dfct,npotl,npots,c6tot,d2self, &
...
!
        ...
        if (leem.or.lgfnff) then
          ...
          elseif (lgfnff) then
            ...
            call gfnffbody(eqeql,zetaij,lgrad1,.false.,nor,1_i4,fct,qli,qlj)
          endif
          eqeq = eqeq + ffct*eqeql
        endif
        esum = ffct*(eatoml+ ereall + ec6l + eqeql)
!
...
!
    if (lbsmat(i).and.(lopf(i).or..not.lfreeze)) then
      nati = iatn(i)
      ntypi = natype(i)
