...
!  10/12 Call to potwordk added
!   5/13 Removal of duplicate library names added
!   3/14 Unused arguments passed to routines removed
!   9/14 Tracking of EAM species in library added
!   1/15 lint, linr added back to boword argument list
!   2/15 nlibnobo added
!   3/15 nlibbondtype added
!   1/18 Trace added
!   1/19 maxlinelength changes added
!  11/20 Tersoff reorganised
!   7/21 Bond valence added
!   4/23 Option to read ReaxFF format library files added
!
!  Conditions of use:
!
...
!
  ...
  use eam,            only : neamspec, nlibeamspec, neamfnspec, nlibeamfnspec
  use four
  use gulpinput
  use gulp_lengths
  use iochannels,     only : ioin, iotmp
  use kim_models,     only : nkimmodel, nlibnkimmodel
  use library
  use m_pdfneutron,   only : pdfword
  use m_three
  use molecule,       only : nlibnobo, nnobo, nlibbondtype, nbondtype
  use one,            only : none
  use parallel
  use reaxFFdata
  use six
...
!
  ...
  if (l1000) goto 1000
  if (lwordok) goto 50
  call potwordm(iotmp,word,lwordok,iline,line,l55,l1000,.false.,lfflags)
  if (l55) goto 55
  if (l1000) goto 1000
  if (lwordok) goto 50
  call potwordk(iotmp,word,lwordok,iline,line,l55,l1000)
  if (l55) goto 55
  if (l1000) goto 1000
  if (lwordok) goto 50
...
100 if (nword.eq.1) then
  ...
  do i = 2,nword
    if (index(words(i),'nodu').eq.1) then
      ...
      lspace = .false.
      ind2 = ind1
      do while (.not.lspace.and.ind2.lt.maxlinelength)
        ind2 = ind2 + 1
...
!
        lduplicate = .false.
        ilib = 1
        do while (ilib.lt.nlib.and..not.lduplicate) 
          lduplicate = (libname(ilib).eq.libname(nlib))
          ilib = ilib + 1
        enddo
!
!  If duplicate found then remove
!
        if (lduplicate) then
          nlib = nlib - 1
        endif
...
!
  ...
  nlibsp = nspec
  nlibseps = nseps
  nlibnkimmodel = nkimmodel
  nlibeamfnspec = neamfnspec
  nlibeamspec = neamspec
  nlibnboA = nboA
  nlibnboO = nboO
  nlibnboR = nboR
  nlibnboQ = nboQ
  ...
  nlibnobo = nnobo
  nlibbondtype = nbondtype
  nlibvalbond = nvalbond
  nlibvalener = nvalener
!****************************
!  Potential library input  *
!****************************
  if (nlib.gt.0) call setlib(ncurr)
!
!  If a library file has been specified for ReaxFF then read
!
  if (lreaxff_ffield) then
    call readreaxfflib
  endif
!
