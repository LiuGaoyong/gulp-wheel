...
!    2 => bonded only improper potential
!
...
!  11/12 Defaults for potentials explicitly reinitialised to solve
!        bug with mmfexc taking the wrong value.
!   5/13 Custom bond order added
!   7/13 Sub-option to specify bonded improper torsion
!   7/13 Modified so that bond sub-option doesn't overwrite improper
!   3/14 Fractional bond orders added
!   6/14 Bug in setting of fitting flags for Ryckeart-B. potential fixed
!   1/17 Fourth cutoff distance added for torharm potential
!   3/17 degtorad factor removed here to match new style output handling
!   1/19 Worsy changed to use word length parameter
!   2/19 Protection added again overwrite of improper specification by molmec
!  11/21 Modifications for TI added
!   3/22 Handling of multiple species matches added for selected potentials
!   7/23 Uninitialised use of norder corrected for ESFF and standard potential
!
!  Conditions of use:
!
...
!
  if (llibrary) then
    allocate(nvars(4,maxqrt))
    allocate(itypes(4,maxqrt))
    allocate(syms(4,maxqrt))
  else
    allocate(nvars(4,1))
    allocate(itypes(4,1))
    allocate(syms(4,1))
...
!
!  If improper has already been specified then do not overwrite
!
        if (mmfexc(nfor+1).ne.2) then
          mmfexc(nfor+1) = 1
        endif
        lfintra(nfor+1) = .true.
        lfinter(nfor+1) = .false.
        lfound = .true.
      elseif (index(words(i),'imp').eq.1) then
        mmfexc(nfor+1) = 2
        lfintra(nfor+1) = .true.
        lfinter(nfor+1) = .false.
        lfound = .true.
...
!
    if (llibrary) then
      call getpotsymbols4(iline,llibrary,maxqrt,nqrt,nvars,itypes,syms,nbeg,lvalidpot)
!
!  Check dimension maxqrt
!
      if (nqrt.gt.maxqrt) then
        maxqrt = nqrt
!
        deallocate(syms)
        deallocate(itypes)
...
!
      if (lvalidpot) then
        ...
        itypes(3,1) = itype3
        itypes(4,1) = itype4
        syms(1,1) = symbol4(1,nfor)
        syms(2,1) = symbol4(2,nfor)
        syms(3,1) = symbol4(3,nfor)
        syms(4,1) = symbol4(4,nfor)
      endif
    endif
!
...
!
    ...
    ltib4body(nfor) = ltib
    ltif4body(nfor) = ltif
    if (mmfexc(nfor).eq.1.or.mmfexc(nfor).eq.2) then
      if (nfloat.ge.1) then
        fork(nfor) = floats(1+nbeg)*units
      else
        call outerror('error in force constant input for torsion',iline)
        call stopnow('potword4')
      endif
    else
      if (nfloat.ge.5) then
...
!
    if (nfor+nqrt-1.gt.maxfor) then
      maxfor = nfor + nqrt
      call changemaxfor
    endif
!****************************************
!  Loop over valid quartets of species  *
!****************************************
    do nf = 1,nqrt
      if (nf.gt.1) then
        nfor = nfor + 1
...
!
        mmfexc(nfor) = mmfexc(npot0)
        n4botype(1:2,nfor) = n4botype(1:2,npot0)
        nforty(nfor) = nforty(npot0)
        npfor(nfor) = npfor(npot0)
        lfdreiding(nfor) = lfdreiding(npot0)
        loutofplane(nfor) = loutofplane(npot0)
        ltib4body(nfor) = ltib
        ltif4body(nfor) = ltif
        if (norder.gt.0) then
          forpoly(1:norder,nfor) = forpoly(1:norder,npot0)
        endif
        fork(nfor) = fork(npot0)
        for1(nfor) = for1(npot0)
        for2(nfor) = for2(npot0)
        for3(nfor) = for3(npot0)
        for4(nfor) = for4(npot0)
      endif
!
      nfspec1(nfor) = nvars(1,nf)
      nfspec2(nfor) = nvars(2,nf)
      nfspec3(nfor) = nvars(3,nf)
      nfspec4(nfor) = nvars(4,nf)
      nfptyp1(nfor) = itypes(1,nf)
      nfptyp2(nfor) = itypes(2,nf)
      nfptyp3(nfor) = itypes(3,nf)
      nfptyp4(nfor) = itypes(4,nf)
      symbol4(1,nfor) = syms(1,nf)
      symbol4(2,nfor) = syms(2,nf)
      symbol4(3,nfor) = syms(3,nf)
      symbol4(4,nfor) = syms(4,nf)
    enddo
    goto 102
  elseif (lesff) then
!***************************
...
!
        ...
        ltib4body(nfor) = ltib
        ltif4body(nfor) = ltif
        forpoly(1,nfor) = forpoly(1,npot0)
        fork(nfor) = fork(npot0)
        for1(nfor) = for1(npot0)
        for2(nfor) = for2(npot0)
        for3(nfor) = for3(npot0)
