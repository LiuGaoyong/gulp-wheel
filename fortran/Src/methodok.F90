...
!   9/20 Trap EEM methods and GFNFF in the same run
!  10/21 lgfnff moved to control
!   1/22 Variable charge second derivatives enabled for new algorithm
!   4/22 Trap for GFNFF with solvation added
!   7/22 Trap for GFNFF with rigid molecules added
!   5/23 Parallel second derivatives enabled for ReaxFF
!   7/23 lbsmatcfg added
!   8/23 Group velocities and omega trapped for EEM/ReaxFF/pGFN-FF
!   9/23 Check added to prevent EEM with ReaxFF
!  10/23 Variable charge frequency-dependent properties enabled
!
!  Conditions of use:
!
...
!
  ...
  use configurations, only : lbsmatcfg, nasum, occucfg, lomega
  use control
...
!  Variable charges
!
    if (lneed2nd.and.lDoQDeriv2.and.loldd2q) then
      if (ioproc) then
        call outerror('charge second derivatives unavailable in parallel with old algorithm',0_i4)
      endif
      call stopnow('methodok')
    endif
    if (lneed2nd.and.lDoQDeriv2.and.(.not.leem.and..not.lgfnff.and..not.lreaxFF)) then
      if (ioproc) then
        call outerror('bond order charge second derivatives unavailable in parallel',0_i4)
      endif
      call stopnow('methodok')
    endif
!
!  Breathing shells
!
    if (lneed2nd.and.any(lbsmatcfg)) then
      if (ioproc) then
...
!
  if (lgfnff.and.leem) then
!
!  EEM & GFNFF
!
    if (ioproc) then
      call outerror('Variable charges (EEM,QEq,SM) and GFNFF are mutually exclusive',0_i4)
    endif
    call stopnow('methodok')
  endif
!
  if (lreaxff.and.leem) then
!
!  EEM & ReaxFF
!
    if (ioproc) then
      call outerror('Variable charges (EEM,QEq,SM) and ReaxFF are mutually exclusive',0_i4)
    endif
    call stopnow('methodok')
  endif
!
!  K point incompatibility with variable charge methods
!
  if (leem.or.lreaxFF.or.lgfnff) then
    if (lgroupvelocity) then
      if (ioproc) then
        call outerror('group velocities unavailable with variable charges',0_i4)
      endif
      call stopnow('methodok')
    endif
  endif
...
!*****************************************************************************
  if (lcosmo) then
    ...
    elseif (lgfnff) then
      if (ioproc) then
        call outerror('Solvation not yet compatible with pGFNFF variable charges',0_i4)
      endif
      call stopnow('methodok')
    endif
  endif
...
!********************
  if (lrigid) then
    ...
    if (lgfnff) then
      if (ioproc) then
        call outerror('pGFNFF not yet implemented for rigid molecules',0_i4)
      endif
      call stopnow('methodok')
    endif
    if (lbrenner) then
