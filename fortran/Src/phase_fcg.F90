  subroutine phase_fcg(xk,yk,zk)
!
!  Calculates the phased dynamical matrix for a k point from
!  the second derivatives stored for supercells. This version
!  is for case where the phonons are to be computed for a 
!  ghost cell that is a sub-cell of the present supercell.
!
!  Note that at present the reciprocal space and 1-D real space
!  Coulomb sums are computed using the standard algorithm.
!
!   1/18 Created from phase_fc
!   2/18 Trace added
...
!
    ...
    enddo
  enddo
! DEBUG - needs BSM adding to below
!
!  Now compress supercell down to ghost cell dynamical matrices 
!
  ix = -2
  iy = -1
  iz =  0
  ...
  do i = 1,numat,nghostcell
    ...
    jyg = -1
    jzg =  0
    do j = 1,numat,nghostcell
      jx = jx + 3
      jy = jy + 3
      jz = jz + 3
!
      d2locr(1:3,1:3) = 0.0_dp
      d2loci(1:3,1:3) = 0.0_dp
!
      do k = 1,nghostcell
        jxg = jxg + 3
        jyg = jyg + 3
        jzg = jzg + 3
!
        d2locr(1,1) = d2locr(1,1) + derv2(jxg,ixg)
        d2locr(2,1) = d2locr(2,1) + derv2(jyg,ixg)
        d2locr(3,1) = d2locr(3,1) + derv2(jzg,ixg)
        d2locr(1,2) = d2locr(1,2) + derv2(jxg,iyg)
        d2locr(2,2) = d2locr(2,2) + derv2(jyg,iyg)
        d2locr(3,2) = d2locr(3,2) + derv2(jzg,iyg)
        d2locr(1,3) = d2locr(1,3) + derv2(jxg,izg)
        d2locr(2,3) = d2locr(2,3) + derv2(jyg,izg)
        d2locr(3,3) = d2locr(3,3) + derv2(jzg,izg)
!
        d2loci(1,1) = d2loci(1,1) + dervi(jxg,ixg)
        d2loci(2,1) = d2loci(2,1) + dervi(jyg,ixg)
        d2loci(3,1) = d2loci(3,1) + dervi(jzg,ixg)
        d2loci(1,2) = d2loci(1,2) + dervi(jxg,iyg)
        d2loci(2,2) = d2loci(2,2) + dervi(jyg,iyg)
        d2loci(3,2) = d2loci(3,2) + dervi(jzg,iyg)
        d2loci(1,3) = d2loci(1,3) + dervi(jxg,izg)
        d2loci(2,3) = d2loci(2,3) + dervi(jyg,izg)
        d2loci(3,3) = d2loci(3,3) + dervi(jzg,izg)
      enddo
!
      derv2(jx,ix) = d2locr(1,1)
      derv2(jy,ix) = d2locr(2,1)
      derv2(jz,ix) = d2locr(3,1)
      derv2(jx,iy) = d2locr(1,2)
      derv2(jy,iy) = d2locr(2,2)
      derv2(jz,iy) = d2locr(3,2)
      derv2(jx,iz) = d2locr(1,3)
      derv2(jy,iz) = d2locr(2,3)
      derv2(jz,iz) = d2locr(3,3)
!
      dervi(jx,ix) = d2loci(1,1)
      dervi(jy,ix) = d2loci(2,1)
      dervi(jz,ix) = d2loci(3,1)
      dervi(jx,iy) = d2loci(1,2)
      dervi(jy,iy) = d2loci(2,2)
      dervi(jz,iy) = d2loci(3,2)
      dervi(jx,iz) = d2loci(1,3)
      dervi(jy,iz) = d2loci(2,3)
      dervi(jz,iz) = d2loci(3,3)
    enddo
    ixg = ixg + 3*nghostcell
    iyg = iyg + 3*nghostcell
    izg = izg + 3*nghostcell
  enddo
#ifdef TRACE
  call trace_out('phase_fcg')
#endif
!
  return
  end
