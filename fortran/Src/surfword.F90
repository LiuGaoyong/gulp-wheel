...
!  10/18 Restriction on translation atoms only in region 1 removed
!   1/19 Worsy changed to use word length parameter
!   7/23 lbsmatcfg added
!   7/23 nregionnocfg added
!   9/23 Electric fields enabled for ReaxFF
!
!  Conditions of use:
!
...
!
  ...
  integer(i4)                              :: ind
  integer(i4)                              :: inat
  integer(i4), dimension(:), pointer, save :: itmp => null()
  integer(i4)                              :: itype
  integer(i4)                              :: j
  integer(i4)                              :: nai
  integer(i4)                              :: nbeg
...
!
  ...
  if (index(word,'td_f').eq.1) goto 320
!
  ...
  if (index(word,'qons').eq.1) goto 330
!
  return
!********************************************
...
!         
      ...
      if (lregionrigid(nregioncurr,ncurr)) then
        ...
        if (nword.gt.2) then
          do nw = 3,nword
            if (index(words(nw),'x').ne.0) lopfreg(3*(nregioncurr-1)+1,ncurr) = .true.
            if (index(words(nw),'y').ne.0) lopfreg(3*(nregioncurr-1)+2,ncurr) = .true.
            if (index(words(nw),'z').ne.0) lopfreg(3*(nregioncurr-1)+3,ncurr) = .true.
          enddo
        endif
      endif
    endif
...
!     
      do i = 2,nword
        if (index(words(i),'t').eq.1.or.index(words(i),'T').eq.1) &
          ltranat(nasum+nasym) = .true.
        if (index(words(i),'t-').eq.1.or.index(words(i),'T-').eq.1) &
          ltranatminus(nasum+nasym) = .true.
...
!
  if (lmodco) then
    do i = 1,nasym
      xold = xfrac(i)
      yold = yfrac(i)
      xfrac(i) = dmod(xfrac(i)+1000.0_dp,1.0_dp)
      yfrac(i) = dmod(yfrac(i)+1000.0_dp,1.0_dp)
!
...
320 units = 1.0_dp
  if (nfloat.eq.0) then
    call outerror('TD-electric field value missing',iline)
    call stopnow('surfword')
  else
    ntdfieldcfg(ncurr) = ntdfieldcfg(ncurr) + 1
!
!  Check maxtdfield
!
    if (ntdfieldcfg(ncurr).gt.maxtdfield) then
      maxtdfield = ntdfieldcfg(ncurr)
      call changemaxtdfield
    endif
    td_fieldcfg(1,ntdfieldcfg(ncurr),ncurr) = floats(1)*units
    if (ndimen(ncurr).eq.3) then
!
!  If field is specified then this will break the symmetry and so symmetry must be turned off
!
      ...
      if (nword.gt.1) then
        if (index(words(2),'cart').eq.1) then
          if (nfloat.ge.6) then
            ...
            yf = floats(3)
            zf = floats(4)
            call cart2frac(ndimen(ncfg),xf,yf,zf,rvcfg(1,1,ncfg), &
                           td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr), &
                           td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr), &
                           td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr), &
                           icx,icy,icz)
            norm = td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr)**2 + &
                   td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr)**2 + &
                   td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr)**2
            if (abs(norm).lt.1.0d-12) then
              call outerror('Norm of TD-electric field direction is too close to zero',iline)
              call stopnow('surfword')
            endif
            td_fieldcfg(2,ntdfieldcfg(ncurr),ncurr) = floats(5)
            td_fieldcfg(3,ntdfieldcfg(ncurr),ncurr) = floats(6)
          else
            call outerror('Missing input for TD-electric field',iline)
            call stopnow('surfword')
          endif
        elseif (index(words(2),'c').eq.1) then
          td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr) = 0.0_dp
          td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr) = 0.0_dp
          td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr) = 1.0_dp
          if (nfloat.ge.3) then
            td_fieldcfg(2,ntdfieldcfg(ncurr),ncurr) = floats(2)
            td_fieldcfg(3,ntdfieldcfg(ncurr),ncurr) = floats(3)
          else
            call outerror('Missing input for TD-electric field',iline)
            call stopnow('surfword')
          endif
        ...
        elseif (index(words(2),'b').eq.1) then
          td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr) = 0.0_dp
          td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr) = 1.0_dp
          td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr) = 0.0_dp
          if (nfloat.ge.3) then
            td_fieldcfg(2,ntdfieldcfg(ncurr),ncurr) = floats(2)
            td_fieldcfg(3,ntdfieldcfg(ncurr),ncurr) = floats(3)
          else
        ...
        elseif (index(words(2),'a').eq.1) then
          td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr) = 1.0_dp
          td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr) = 0.0_dp
          td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr) = 0.0_dp
          if (nfloat.ge.3) then
            td_fieldcfg(2,ntdfieldcfg(ncurr),ncurr) = floats(2)
      ...
      elseif (nfloat.ge.6) then
        td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr) = floats(2)
        td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr) = floats(3)
        td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr) = floats(4)
        norm = td_fielddirectioncfg(1,ntdfieldcfg(ncurr),ncurr)**2 + &
               td_fielddirectioncfg(2,ntdfieldcfg(ncurr),ncurr)**2 + &
               td_fielddirectioncfg(3,ntdfieldcfg(ncurr),ncurr)**2
        if (abs(norm).lt.1.0d-12) then
  ...
  if (abs(td_fieldcfg(1,ntdfieldcfg(ncurr),ncurr)).gt.0.0_dp) then
!
!  If this is a variable charge model then disable second derivatives
!
    if (leem) lnoanald2 = .true.
    if (lreaxff) then
      call outerror('TD-electric field cannot be used with ReaxFF at present',iline)
      call stopnow('surfword')
    endif
  endif
  lwordok = .true.
  return
!*************************
!  Charge on SAS option  *
!*************************
330 if (nfloat.ge.1) then
    qonsastarget(ncurr) = floats(1)
  endif
  lwordok = .true.
  return
!
