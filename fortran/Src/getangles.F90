  subroutine getangles(iout,imode,nangtot)
!
...
!  On exit:
!
!  nangtot   = total number of angle terms over all potentials
!
...
!        Lammps file.
!  11/12 Bug in LAMMPS output corrected
!  11/12 Modified so that symmetry is ignored for imode = 1
!        since LAMMPS needs all angles
!  11/12 Corrected to handle case where maxangle gets changed during
!        run while writing values to an external file.
!   3/14 Renamed from angle for benefit of ChemShell
!   1/18 Calls to mindtoijk introduced for nmolind handling
!   2/18 Trace added
...
!
  if (imode.lt.0.or.imode.gt.1) then
    call outerror('invalid mode passed to getangles subroutine',0_i4)
    call stopnow('getangles')
  endif
!
  if (imode.eq.0) then
    if (nasym.ne.numat) then
      write(iout,'(/,''  Analysis of three-body terms for asymmetric unit :'',/)')
    else
      write(iout,'(/,''  Analysis of three-body terms :'',/)')
    endif
    write(iout,'(''--------------------------------------------------------------------------------'')')
    write(iout,'(''  Potential   Atom 1  Atom 2  Atom 3   Angle   Atom 1  Atom 2  Atom 3   Angle  '')')
    write(iout,'(''--------------------------------------------------------------------------------'')')
  endif
!
  lredimension = .false.
...
!
  ...
  if (status/=0) call outofmemory('getangles','xvec')
  allocate(yvec(maxvector),stat=status)
  if (status/=0) call outofmemory('getangles','yvec')
  allocate(zvec(maxvector),stat=status)
  if (status/=0) call outofmemory('getangles','zvec')
  allocate(ang(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ang')
  allocate(nat1(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','nat1')
  allocate(nat2(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','nat2')
  allocate(nat3(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','nat3')
  allocate(ntp1(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ntp1')
  allocate(ntp2(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ntp2')
  allocate(ntp3(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ntp3')
  allocate(ind1(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ind1')
  allocate(ind2(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ind2')
  allocate(ind3(maxangle),stat=status)
  if (status/=0) call outofmemory('getangles','ind3')
!
...
!
        ...
        if (status/=0) call deallocate_error('getangles','zvec')
        deallocate(yvec,stat=status)
        if (status/=0) call deallocate_error('getangles','yvec')
        deallocate(xvec,stat=status)
        if (status/=0) call deallocate_error('getangles','xvec')
...
!
                ...
                if (status/=0) call deallocate_error('getangles','ind3')
                deallocate(ind2,stat=status)
                if (status/=0) call deallocate_error('getangles','ind2')
                deallocate(ind1,stat=status)
                if (status/=0) call deallocate_error('getangles','ind1')
                deallocate(ntp3,stat=status)
                if (status/=0) call deallocate_error('getangles','ntp3')
                deallocate(ntp2,stat=status)
                if (status/=0) call deallocate_error('getangles','ntp2')
                deallocate(ntp1,stat=status)
                if (status/=0) call deallocate_error('getangles','ntp1')
                deallocate(nat3,stat=status)
                if (status/=0) call deallocate_error('getangles','nat3')
                deallocate(nat2,stat=status)
                if (status/=0) call deallocate_error('getangles','nat2')
                deallocate(nat1,stat=status)
                if (status/=0) call deallocate_error('getangles','nat1')
                deallocate(ang,stat=status)
                if (status/=0) call deallocate_error('getangles','ang')
!
!  Rewind nangtot...
!
                nangtot = nangtot - nang
!
!  ...and start again
!
                lredimension = .true.
...
!
      ...
      nline2 = nline - 2
      if (nang.eq.0) then
        write(iout,'(1x,i6,5x,3x,''No angles found'')') n
      elseif (nang.eq.1) then
        ...
        write(iout,'(1x,i6,5x,1x,3(1x,a5,1x,a1),1x,f7.3)') n,lab1,cstyp1,lab2,cstyp2,lab3,cstyp3,ang(1)
      else
        call label(nat1(1),ntp1(1),lab1)
        call label(nat2(1),ntp2(1),lab2)
        call label(nat3(1),ntp3(1),lab3)
        ...
        write(iout,'(1x,i6,5x,2(1x,3(1x,a5,1x,a1),1x,f7.3))') &
        ...
        if (nline2.ge.1) then
          ...
          do m = 1,nline2
            ...
            write(iout,'(12x,2(1x,3(1x,a5,1x,a1),1x,f7.3))') &
              lab1,cstyp1,lab2,cstyp2,lab3,cstyp3,ang(ind+1),lab4,cstyp4,lab5,cstyp5,lab6, &
              cstyp6,ang(ind+2)
          enddo
        endif
        if (nang.gt.2) then
        ...
        elseif (nliner.eq.2) then
          ...
          if (nat2(ind+2).gt.maxele) cstyp5 = 's'
          if (nat3(ind+2).gt.maxele) cstyp6 = 's'
          write(iout,'(12x,2(1x,3(1x,a5,1x,a1),1x,f7.3))') lab1,cstyp1, &
...
!********************
      ...
      do iang = 1,nang
        ...
        j = ind2(iang)
        k = ind3(iang)
        write(iout,'(5i8)') ind+iang,n,j,i,k
      enddo
    endif
!
!  End of outer loops
!
  ...
  if (imode.eq.0) then
    if (nangtot.gt.0) then
      write(iout,'(''  Total number of angles = '',i8)') nangtot
      write(iout,'(''--------------------------------------------------------------------------------'')')
    endif
    write(iout,'(/)')
  endif
