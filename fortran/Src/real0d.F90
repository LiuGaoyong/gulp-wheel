...
!  12/22 Call to d2charged changed so that zero is passed for strain terms
...
!*************************************************
      ...
      call twobody1(eatom,ereal,ec6,lgrad1p,lgrad2,.false.,nor,1_i4,dist,xcrd,ycrd,zcrd,d0i,d0j, &
                    ...
                    cut2q,cut2s,lptrmol,0_i4,factor,ofct,ospfct,radsum,rtrm1,rtrm2,rtrm3,rtrm32, &
...
!
      if ((lDoQDeriv1.or.lDoQDeriv2).and.lgrad1) then
        d0i(1) = d0i(1) + derive0*qlj
        d0j(1) = d0j(1) + derive0*qli
      endif
      if (leem.or.lgfnff) then
        ...
        elseif (lgfnff) then
          ...
          call gfnffbody1(eqeq,zetaij,lgrad1,lgrad2,nor,1_i4,dist,deriv,deriv2, &
                          fct,qli,qlj,d1i,d1j,d2i2,d2ij,d2j2)
        endif
        esum = eqeq - eqeq_before
        eregion2region(nregionj,nregioni) = eregion2region(nregionj,nregioni) + esum
!
        siteenergy(i) = siteenergy(i) + 0.5_dp*esum
        siteenergy(j) = siteenergy(j) + 0.5_dp*esum
...
!*************************************
      if (lpolar) then
        ...
        vx(j) = vx(j) + qli*derivqd(1)*xcrd
        vy(j) = vy(j) + qli*derivqd(1)*ycrd
        vz(j) = vz(j) + qli*derivqd(1)*zcrd
      endif
!**************************
...
!*******************************************************
        if (lDoQDeriv2) then
          ...
          call d2charge(i,j,1_i4,ix,iy,iz,jx,jy,jz,lopi,lopj,d0i,d0j,d1ix,d1iy,d1iz,d1jx,d1jy,d1jz, &
                        d1is,d1js,d2i2,d2ij,d2j2,d2self,0.0_dp,0.0_dp,.true.,.true.)
        endif
      endif
!
