...
!***************************************************
                deriv(1:3) = 0.0_dp
                if (lsg1) derivs(1:nstrains) = 0.0_dp
...
!**********************************
                          call meamrho(nati,ntypi,natj,ntypj,r,rpot(npot),xji,yji,zji,rhoij,rhoji,drhoij,drhoji, &
                                       ...
                                       1.0_dp,1.0_dp,.true.,lsg1,.true.,.false.,twopot(1,npot))
                        endif
                      endif
                    enddo
!*******************************
...
!
                        if (nptype(npot).eq.45.or.nptype(npot).eq.55) then
                          if (r.gt.rpot2(npot).and.r.le.rpot(npot)) then
                            ...
                            ebas  = ebas*ofct
                            d1bas = rk*d1bas*ofct
                            deriv(1) = deriv(1) + d1bas*xji*Sij
                            deriv(2) = deriv(2) + d1bas*yji*Sij
                            deriv(3) = deriv(3) + d1bas*zji*Sij
                            if (lstr) then
                              call real1strterm(ndim,xji,yji,zji,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                              do is = 1,nstrains
                                ns = nstrptr(is)
...
!
                            call psiscreenderv(i,j,npartial,partial,xji,yji,zji,ebas,Sij,lstr)
                          endif
                        endif
!
...
!
                                ...
                                if (lstr) then
                                  do kl = 1,nstrains
                                    rstrd(kl) = rstrd(kl) + partial%sa_drhotots(kl,np)*ofct
                                  enddo
                                  if (latomicstress) then
                                    do kl = 1,nstrains
                                      atomicstress(kl,i) = atomicstress(kl,i) + third*partial%sa_drhotots(kl,np)*ofct
                                      atomicstress(kl,j) = atomicstress(kl,j) + third*partial%sa_drhotots(kl,np)*ofct
                                      atomicstress(kl,k) = atomicstress(kl,k) + third*partial%sa_drhotots(kl,np)*ofct
                                    enddo
                                  endif
                                endif
                              enddo
...
!
                  if (QMMMmode(ncf).gt.0) then
                    ...
                    if (lsg1) then
                      derivs(1:nstrains) = derivs(1:nstrains) + (rscrhoi*drhototijs(1:nstrains) &
                                                              + rscrhoj*drhototjis(1:nstrains))*ofct
                    endif
                  endif
                endif
                if (lvalidij) then
...
!*****************************
                  if (lstr) then
                    do kl = 1,nstrains
                      rstrd(kl) = rstrd(kl) + derivs(kl)
                    enddo
                    if (latomicstress) then
                      do kl = 1,nstrains
                        atomicstress(kl,i) = atomicstress(kl,i) + 0.5_dp*derivs(kl)
                        atomicstress(kl,j) = atomicstress(kl,j) + 0.5_dp*derivs(kl)
                      enddo
                    endif
                  endif
                endif
