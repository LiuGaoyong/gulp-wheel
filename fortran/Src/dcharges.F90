  subroutine dcharges(lprint,emat,maxe)
!
!  Calculates the first derivative of the charge with respect to the coordinates of the atoms. 
!  Symmetry adapted version of dcharge
!
...
!   7/21 Calls to lapack replaced with call to matrix_inversion_library
!  10/21 lgfnff moved to control
!  10/23 Initialisation of dqds corrected
!  10/23 dqdVxyz added
!
!  On entry:
!
...
!
    call sympot(emat,maxe,chitmp,2_i4)
!
...
!
    do ieem = 1,neem
      ...
      if (ni.ne.1) then
        emat(ieem,ieem) = emat(ieem,ieem) + 2.0_dp*murange(nqr,ni,neemtype)*occua(i)
      else
!
...
!
        ...
        emat(ieem,ieem) = emat(ieem,ieem) + 2.0_dp*murange(nqr,1,neemtype)*occua(i)*rjfac
      endif
    enddo
!
...
!
    do ieem = 1,neem
      i = neemptr(ieem)
      emat(ieem,neem+1) = dble(neqv(i))*occua(i)
      emat(neem+1,ieem) = 1.0_dp
    enddo
...
!  Factorise matrix
!
    call matrix_inversion_library(n,1_i4,maxe,nblocksize,emat,0_i4,.false.,ifail)
!
!  Was inversion successful?
!
    if (ifail.ne.0) then
      call outerror('matrix inversion failed in dcharges',0_i4)
      call stopnow('dcharges')
    endif
!
!  Free workspace
!
...
!
  ...
  allocate(dqs(nasym,6),stat=status)
  if (status/=0) call outofmemory('dcharges','dqs')
  if (lSandM) then
...
!
!  Set up reciprocal space strain terms
!
...
!
      do jeem = 1,neem
        ...
        do k = 1,neem
          indk = 3*(neemptr(k) - 1)
          dqdxyz(indk+1,j) = dqdxyz(indk+1,j) - 2.0_dp*dchix(k)*emat(jeem,ieem)
          dqdxyz(indk+2,j) = dqdxyz(indk+2,j) - 2.0_dp*dchiy(k)*emat(jeem,ieem)
          dqdxyz(indk+3,j) = dqdxyz(indk+3,j) - 2.0_dp*dchiz(k)*emat(jeem,ieem)
        enddo
      enddo
    endif
!
...
!
  ...
  if (status/=0) call deallocate_error('dcharges','dqs')
