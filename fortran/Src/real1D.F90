...
!  12/18 Charge second derivatives at finite strain corrected
...
!
            if (lgrad1) then
              ...
              if (lDoQDeriv1) then
                d0i(1) = qj*rr*angstoev
                d0j(1) = qi*rr*angstoev
...
!
              if (lgrad2) then
                ...
                if (lstr) then
                  ...
                  derv3(jx,1) = derv3(jx,1) + d2*x*dr2ds(1) + d1*d2r2dsdx(1,1)
                  derv3(jy,1) = derv3(jy,1) + d2*y*dr2ds(1) + d1*d2r2dsdx(1,2)
                  derv3(jz,1) = derv3(jz,1) + d2*z*dr2ds(1) + d1*d2r2dsdx(1,3)
                  sderv2(1,1) = sderv2(1,1) + d1*d2r2ds2(1,1) + d2*dr2ds(1)**2
                endif
!*******************************************************
...
!
          eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + d0
!
          siteenergy(i) = siteenergy(i) + d0
!
          if (lgrad1.and.lstr) then
            call real1strterm(ndim,acell,0.0_dp,0.0_dp,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,lgrad2)
            d1 = - d0*rr*rr
            rstrd(1) = rstrd(1) + d1*dr2ds(1)
            if (latomicstress) then
...
!
            ...
            if (lstr) then
              ...
              derv3(jx,1) = derv3(jx,1) - d2*(d2h1m(1) + d2h2m(1)) + d2*(dh1(1) + dh2(1))*dads/a
              derv3(jy,1) = derv3(jy,1) - d2*(d2h1m(2) + d2h2m(2)) + d2*(dh1(2) + dh2(2))*dads/a
              derv3(jz,1) = derv3(jz,1) - d2*(d2h1m(3) + d2h2m(3)) + d2*(dh1(3) + dh2(3))*dads/a
!
...
!*******************************************************
            if (lDoQDeriv2) then
              ...
              if (lstr) then
                d1is(1) = d1is(1) + (dh1s + dh2s)*qj*angstoev
                d1js(1) = d1js(1) + (dh1s + dh2s)*qi*angstoev
              endif
              call d2charge(i,j,1_i4,ix,iy,iz,jx,jy,jz,lopi,lopj,d0i,d0j,d1ix,d1iy,d1iz, &
                            d1jx,d1jy,d1jz,d1is,d1js,d2i2,d2ij,d2j2,0.0_dp,0.0_dp,0.0_dp, &
                            .true.,.false.)
...
!
        ...
        if (lstr) then
          ...
          if (lgrad2) then
            ...
            sderv2(1,1) = sderv2(1,1) + qii*d2s*angstoev
          endif
        endif
!
!  Compute variable charge term = E without charges
...
!*******************************************************
        if (lgrad2.and.lDoQDeriv2) then
          ...
          if (lstr) then
            d1is(1) = d1is(1) + dh1s*qi*angstoev
          endif
          call d2charge(i,i,1_i4,ix,iy,iz,ix,iy,iz,lopi,lopi,d0i,d0i,d1ix,d1iy,d1iz, &
                        d1ix,d1iy,d1iz,d1is,d1is,d2i2,d2ij,d2j2,0.0_dp,0.0_dp,0.0_dp, &
                        .true.,.false.)
        endif
      enddo iloop3
    endif
