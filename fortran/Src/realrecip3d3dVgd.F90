  subroutine realrecip3d3dVgd(nkp,mcv,mcvloc,mcvptr,eigr,maxd2l,grueneisen)
!
!  Supplies the first derivatives of the phonon frequencies for a solid with respect to volume
!  Gamma point version. Distributed memory version.
!
!  NOTE : Needs finishing for BSM 
!
!   2/18 Created from realrecip3d3dVg
...
!
  ...
  real(dp)                                     :: zd3
#ifdef TRACE
  call trace_in('realrecip3d3dVgd')
#endif
!
  time1 = g_cpu_time()
  tproj0 = tproj
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3dVgd','npotl')
!**********************************************************
...
!
  ...
  if (status/=0) call outofmemory('realrecip3d3dVgd','ktrm0')
  allocate(ktrm20(nwords),stat=status)
  if (status/=0) call outofmemory('realrecip3d3dVgd','ktrm20')
  if (lc6loc) then
    allocate(ktrm6(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVgd','ktrm6')
    allocate(ktrm60(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVgd','ktrm60')
    allocate(ktrm62(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVgd','ktrm62')
    allocate(ktrm620(nwords),stat=status)
    if (status/=0) call outofmemory('realrecip3d3dVgd','ktrm620')
  else
...
!******************************************************************************
      t1 = g_cpu_time()
      call projd3dVgd(mcvloc,d3s,i,ix,iy,iz,j,jx,jy,jz,lcorei,lcorej, &
...
!************************
  ...
  if (status/=0) call deallocate_error('realrecip3d3dVgd','ktrm620')
  deallocate(ktrm62,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','ktrm62')
  deallocate(ktrm60,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','ktrm60')
  deallocate(ktrm6,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','ktrm6')
  deallocate(ktrm20,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','ktrm20')
  deallocate(ktrm0,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','ktrm0')
!
!  Free local memory
!
  deallocate(npotl,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','npotl')
!
!  Global sum of Grueneisen parameters
!
  allocate(dtmp(mcv),stat=status)
  if (status/=0) call outofmemory('realrecip3d3dVgd','dtmp')
!
...
!
  ...
  enddo
!
  call sumall(dtmp,grueneisen,mcv,"realrecip3d3dVgd","grueneisen")
!
  deallocate(dtmp,stat=status)
  if (status/=0) call deallocate_error('realrecip3d3dVgd','dtmp')
!
...
#ifdef TRACE
  call trace_out('realrecip3d3dVgd')
#endif
!
  return
  end
