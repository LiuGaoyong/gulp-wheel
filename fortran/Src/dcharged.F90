  subroutine dcharged(lprint,emat,maxe,lrecalcA,lstrainin,lgrad2)
!
!  Calculates the first derivative of the charge with respect to the coordinates of the atoms/strains.
...
!  11/20 Modifications for GFNFF added
...
#endif
  use m_strain,       only : gstrterms, real1strterm, cartstrterm
  use parallel
  use qmedata
  use shells
#ifdef TRACE
...
!
  if (lSandM) then
    allocate(dsmtrm(n3a,numat),stat=status)
    if (status/=0) call outofmemory('dcharged','dsmtrm')
    allocate(dtmp(n3a),stat=status)
    if (status/=0) call outofmemory('dcharged','dtmp')
!
    dsmtrm(1:n3a,1:numat) = 0.0_dp
  endif
!
...
!*************************************
    allocate(z(numat),stat=status)
    if (status/=0) call outofmemory('dcharged','z')
!
...
!
    ...
    if (lnoqeem) then
      ...
      if (lqeq.and.lhpresent) then
        call genpotdm(emat,maxe,z,2_i4)
      else
        call genpotdm(emat,maxe,z,1_i4)
      endif
    endif
!
...
!
    ...
    enddo
    emat(1:neemfull,neemfullloc+1) = 1.0_dp
...
!
    ...
    if (status/=0) call deallocate_error('dcharged','z')
!**************************
...
!       
      ...
      do j = 1,numat
        ...
        if (lelementOK(nj).and.nregionno(nrelf2a(j)).eq.1) then
          jeem = jeem + 1
          dqdVxyz(1,iloc) = dqdVxyz(1,iloc) + xclat(j)*emat(jeem,ieem)
          dqdVxyz(2,iloc) = dqdVxyz(2,iloc) + yclat(j)*emat(jeem,ieem)
          dqdVxyz(3,iloc) = dqdVxyz(3,iloc) + zclat(j)*emat(jeem,ieem)
        endif
      enddo
!
          ...
          if (lelementOK(nj).and.nregionno(nrelf2a(j)).eq.1) then
            ...
            do iloc = 1,natomsonnode
              ...
              if (lelementOK(ni).and.nregionno(nrelf2a(i)).eq.1) then
                ieem = ieem + 1
                do kl = 1,nstrains
                  dqds(kl,iloc) = dqds(kl,iloc) + fieldx*dxyzds(kl,1)*emat(jeem,ieem) + &
...
!
      do i = 1,numat
        call sumall(dsmtrm(1,i),dtmp,n3a,"dcharged","dsmtrm")
        dsmtrm(1:n3a,i) = dtmp(1:n3a)
      enddo
!
!  For S & M, multiply dchix/dchiy/dchiz by inverse matrix
!
...
!
    do i = numat,1,-1
      if (procid.eq.atom2node(i)) then
        ...
        dtmp(n3a+1:n3a+3) = dqdVxyz(1:3,iloc)
        if (lstrain) then
          dtmp(n3a+4:n3a+3+nstrains) = dqds(1:nstrains,iloc)
        endif
      endif
      call sendall(dtmp,ntmp,atom2node(i),"dcharged","dtmp")
      dqdxyz(1:n3a,i) = dtmp(1:n3a)
      dqdVxyz(1:3,i) = dtmp(n3a+1:n3a+3)
      if (lstrain) then
        dqds(1:nstrains,i) = dtmp(n3a+4:n3a+3+nstrains)
      endif
    enddo
!
    deallocate(dtmp,stat=status)
...
!
      allocate(dtmp(numat),stat=status)
      if (status/=0) call outofmemory('dcharged','dtmp')
      allocate(dtmp2(numat),stat=status)
      if (status/=0) call outofmemory('dcharged','dtmp2')
!
      dtmp(1:numat) = 0.0_dp
      do iloc = 1,natomsonnode
        i = node2atom(iloc)
        dtmp(i) = dqdVxyz(1,iloc)
      enddo
!
      call sumall(dtmp,dtmp2,numat,"dcharged","dqdVxyz")
!
      do i = 1,numat
        dqdVxyz(1,i) = dtmp2(i)
      enddo
