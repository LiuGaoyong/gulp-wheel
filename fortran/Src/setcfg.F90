...
!   3/21 Einstein output corrected
!  11/21 Trap for nfixatom = 0 added
!  12/21 lphonfit set to be true if free energy is to be fitted
!   2/22 lsync added
!   7/23 lbsmatcfg added
!   7/23 nregionnocfg added
!   7/23 lkptgammaonlycfg added
!   8/23 Outkpt now called for all processors
!
!  Conditions of use:
!
...
!
  ...
  if (lfit) then
    do i = 1,nobs
      if (nobtyp(i).eq.9.or.nobtyp(i).eq.13.or.nobtyp(i).eq.14.or.nobtyp(i).eq.29.or. &
          nobtyp(i).eq.32.or.nobtyp(i).eq.33) then
        lphonfit = .true.
      endif
    enddo
  endif
...
!
    ...
    loptz(1:nasym) = .false.
!
    if ((lopt.or.lharmrelax.or.lgrad.or.lfit.or.lmc.or.lmd.or.lneb.or.lsync).and..not.lbulknoopt) then
!*******************
!  Unit cell flags *
!*******************
...
!
      if (.not.lsymopt.and..not.lshello.and..not.lflags.and..not.lalleinstein.and..not.lneb.and..not.lsync &
...
!
      if (lmd.and..not.lflags.and.numat.gt.0.and.nfixatom.gt.0) then
        if (nasymnomol.gt.0) then
          loptx(nfixatom) = .true.
          lopty(nfixatom) = .true.
          loptz(nfixatom) = .true.
      ...
      do i = 1,nasym
        if (leinsteinat(nsft+i)) then
          if (lfirstout) then
            ...
            elseif (ndimen(icfg).eq.2) then
              write(ioout,'(''        Label        (frac)        (frac)       (Ang)        (eV/Angs**2)'')')
            elseif (ndimen(icfg).eq.1) then
              write(ioout,'(''        Label        (frac)        (Ang)        (Ang)        (eV/Angs**2)'')')
            else
              write(ioout,'(''        Label        (Ang)         (Ang)        (Ang)        (eV/Angs**2)'')')
            endif
            write(ioout,'(''--------------------------------------------------------------------------------'')')
          endif
          inat = iatn(i)
...
!
          if (inat.le.maxele.or..not.lhideshells) then
            ...
            write(ioout,'(i7,1x,a5,1x,a2,4f14.6)') &
              i,lab,cstype,xfeinsteinat(nsft+i),yfeinsteinat(nsft+i),zfeinsteinat(nsft+i),keinsteinat(nsft+i)
          endif
        endif
      enddo
      if (.not.lfirstout) then
...
!
!  K point output and related setup
!
    if (nkpt.gt.0) call outkpt
!**********************************************************
