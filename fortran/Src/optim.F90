...
!   9/21 Freezing turned off for valence bond potentials
!  12/21 pGFNFF modifications added
!   7/23 lusefcalg added
!   8/23 Calls to phonond_newfc added
!   8/23 Born effective charges called for ReaxFF
!
!  Conditions of use:
!
...
!
  ...
  lfreezeok = (.not.lnoexclude.and.ncell.eq.0.and..not.lfree.and. &
    ...
    .and..not.lrigid.and..not.lgfnff.and.(nvalener.eq.0))
...
!
    ...
    if (.not.lfree.and..not.loptiloc.and..not.lharmloc) then
      if ((lprop.or.lborn.or.lphon).and.(lewald.or.lwolf.or.lreaxff.or.leem)) then
      ...
      if (lphonloc) then
        if (ndim.gt.0) then
          if (lfcsupercell) then
            ...
            if (nprocs.gt.1) then
              if (lusefcalg) then
                call phonond_newfc(.true.,fc,0_i4,0_i4)
              else
                call phonond(.true.,fc,0_i4,0_i4)
              endif
            else
              if (lusefcalg) then
                call phonon_newfc(.true.,fc,0_i4,0_i4)
              else
