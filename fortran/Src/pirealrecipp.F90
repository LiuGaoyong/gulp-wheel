  subroutine pirealrecipp
!
!  Calculates the second derivatives of the polarisation energy
!  for phonon calculations at gamma ONLY.
!
...
!
  ...
  if (status/=0) call outofmemory('pirealrecipp','ld2ok')
  allocate(npotl(npote),stat=status)
  if (status/=0) call outofmemory('pirealrecipp','npotl')
  allocate(d2a(3,3,numat),stat=status)
  if (status/=0) call outofmemory('pirealrecipp','d2a')
!
...
!*****************************************************************
      if (lewald.and..not.lnorecip) then
        call reciptrmdp(xcrd,ycrd,zcrd,.true.,lstr,ofct,d2,d2s,d3,d3s,d3ss)
      endif
!****************************
...
!
  ...
  if (status/=0) call deallocate_error('pirealrecipp','d2a')
  deallocate(npotl,stat=status)
  if (status/=0) call deallocate_error('pirealrecipp','npotl')
  deallocate(ld2ok,stat=status)
  if (status/=0) call deallocate_error('pirealrecipp','ld2ok')
!
!  Timing
!
  time2 = g_cpu_time()
