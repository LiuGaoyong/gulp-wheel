...
!   4/23 Handling of incomplete data for odirection changed
!
!  Conditions of use:
!
...
!
  ...
  if (index(word,'eigv').eq.1) goto 820
  return
!************************
!  FC_supercell option  *
!************************
...
820 continue
  if (nword.gt.1) then
    do i = 2,nword
      if (index(words(i),'mass').eq.1) then
        leigenvectors_original = .false.
      elseif (index(words(i),'orig').eq.1) then
        leigenvectors_original = .true.
      elseif (index(words(i),'phas').eq.1) then
        leigenvectors_phase = .true.
      elseif (index(words(i),'unph').eq.1) then
