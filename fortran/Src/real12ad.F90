  subroutine real12ad(eatom1,ereal1,erecip1,ec61,lgrad1,lgrad2,imode)
!
...
!
!  imode = 2 - shouldn't be called for derivatives 
!
!  NB: BSM not currently implemented
!
!   5/17 Created from real12a
!   6/17 ec6loc initialised to zero
!   2/18 Trace added
...
!
  use configurations, only : radcfg
  use g_constants
  use control
  use current
  use defects
...
!
  if (imode.eq.2.and.lgrad1) then
    call outerror('real12ad called with imode 2 - should use real12a',0_i4)
    call stopnow('real12ad')
  endif
!
!  Local variables
!
...
!
  ...
  if (status/=0) call outofmemory('real12ad','npotl')
!
  eatomloc = 0.0_dp
  erealloc = 0.0_dp
  ereciploc = 0.0_dp
...
!
                  ...
                  ereciploc = ereciploc - setrm
                  if (lc6) then
                    c6prod = ofct*c6tot
                    ec6loc = ec6loc + c6self1*c6prod
                  endif
                  if (lgrad2) then
                    setrm = setrm*twoeta3
...
!
                  dist = sqrt(r)
                  call twobody1(eatomloc,erealloc,ec6loc,lgrad1,lgrad2,.false.,1_i4,1_i4,dist,xcd,ycd,zcd,d0i,d0j, &
                                deriv,deriv2,deriv3,derive0,derive,derive2,derive3,rderiv,npots,npotl,cut2r, &
                                cut2q,cut2s,lptrmol,nmolonly,factor,ofct,ospfct,radsum,rtrm1,rtrm2,rtrm3,rtrm32, &
                                sctrm1,sctrm2,qli,qlj,1_i4,1_i4,lcspair,.true.,ljinregion1,lbonded,l2bonds,l3bonds, &
                                nbtypeij,nbtypeij2,ljinregion1,.false.,d1i,d1j,d2i2,d2ij,d2j2)
                  if (lgrad1) then
                    call rpaird(i,j,indri,indrif,ix,iy,iz,jx,jy,jz,lgrad2,xcd,ycd,zcd,deriv,deriv2,rderiv, &
                        ...
                        do m = 1,npote
                          if (nptype(m).eq.14) then
                            if (lmatch(nati,ntypi,nspec1(m),nptyp1(m),.true.)) then
                              ...
                              if (lgrad1) then
                                raderv(i) = raderv(i) + apt*rdiff
                                if (lgrad2) derv2(indrif,indri) = derv2(indrif,indri) + apt
                              endif
                            endif
                          elseif (nptype(m).eq.17) then
                            if (lmatch(nati,ntypi,nspec1(m),nptyp1(m),.true.)) then
...
!
                      ...
                      call twobody1(eatomloc,erealloc,ec6loc,lgrad1,lgrad2,.false.,1_i4,1_i4,dist,xcd,ycd,zcd,d0i,d0j, &
                                    ...
                                    cut2r,cut2q,cut2s,lptrmol,nmolonly,factor,ofct,ospfct,radsum,rtrm1,rtrm2, &
                                    rtrm3,rtrm32,sctrm1,sctrm2,qli,qlj,1_i4,1_i4,lcspair,.true.,ljinregion1,lbonded, &
                                    l2bonds,l3bonds,nbtypeij,nbtypeij2,ljinregion1,.false.,d1i,d1j,d2i2,d2ij,d2j2)
                      if (lgrad1) then
                        call rpaird(i,j,indri,indrif,ix,iy,iz,jx,jy,jz,lgrad2,xcd,ycd,zcd,deriv,deriv2,rderiv, &
                                    rtrm1,rtrm2,radi,radj)
                      endif
...
!
                      dist = sqrt(r2)
                      call twobody1(eatomloc,erealloc,ec6loc,lgrad1,lgrad2,.false.,1_i4,1_i4,dist,rx,ry,rz,d0i,d0j, &
                                    deriv,deriv2,deriv3,derive0,derive,derive2,derive3,rderiv,npots,npotl,cut2r, &
                                    cut2q,cut2s,lptrmol,nmolonly,factor,ofct,ospfct,radsum,rtrm1,rtrm2,rtrm3,rtrm32, &
                                    sctrm1,sctrm2,qli,qlj,1_i4,1_i4,lcspair,.true.,ljinregion1,lbonded,l2bonds,l3bonds, &
                                    nbtypeij,nbtypeij2,ljinregion1,.false.,d1i,d1j,d2i2,d2ij,d2j2)
                      if (lgrad1) then
                        call rpaird(i,j,indri,indrif,ix,iy,iz,jx,jy,jz,lgrad2,rx,ry,rz,deriv,deriv2, &
...
!
  ...
  enddo
!
!  End of real space part - sum energies
!
  esum1(1) = eatomloc
  esum1(2) = erealloc
  esum1(3) = ereciploc
  esum1(4) = ec6loc
  call sumall(esum1,esum2,4_i4,"real12ad","esum")
  eatom1 = eatom1 + esum2(1)
  ereal1 = ereal1 + esum2(2)
  erecip1 = erecip1 + esum2(3)
  ec61 = ec61 + esum2(4)
!****************************************
...
!
  ...
  if (status/=0) call deallocate_error('real12ad','npotl')
!
!  Timing
!
  time2 = g_cpu_time()
