...
!   6/15 Checks added for cross terms in Baskes potentials
!   2/18 Trace added
...
!
      ...
      enddo
!
!  Check that all combination rule potentials have species sigma and epsilon values in place.
!
      do i = 1,npote
        if (lcombine(i)) then
          nat1 = nspec1(i)
      ...
      enddo
!
!  Check that Baskes cross terms are not missing
!
      lok = .true.
      if (neamfn.eq.9) then
        do n = 1,neamfnspec
!
!  Only do for Baskes potentials
!
          natn = neamfnnat(n)
          itypen = neamfntyp(n)
!
!  Find species for neamfn
!
          i = 0
          lfound = .false.
          do while (.not.lfound.and.i.lt.nspec) 
            i = i + 1
            na = natspec(i)
            ntyp = ntypspec(i)
            lfound = (natn.eq.na.and.(itypen.eq.ntyp.or.ntyp.eq.0))
          enddo
!
!  Find if there is a potential for cross terms
!
          do j = 1,nspec
            if (j.ne.i) then
              na = natspec(j)
              ...
              lfoundp = .false.
              np = 0
              do while (.not.lfoundp.and.np.lt.npote)
                np = np + 1
                if (nptype(np).eq.45.or.nptype(np).eq.55) then
!
!  Baskes two-body potential
!
                  lfoundp = (natn.eq.nspec1(np).and.(nptyp1(np).eq.itypen.or.nptyp1(np).eq.0) &
                         .and.na.eq.nspec2(np).and.(nptyp2(np).eq.ntyp.or.nptyp2(np).eq.0))
                  if (.not.lfoundp) then
                    lfoundp = (natn.eq.nspec2(np).and.(nptyp2(np).eq.itypen.or.nptyp2(np).eq.0) &
                           .and.na.eq.nspec1(np).and.(nptyp1(np).eq.ntyp.or.nptyp1(np).eq.0))
                  endif
                endif
              enddo
              if (.not.lfoundp) lok = .false.
            endif
          enddo
        enddo
!
!  If no cross term found then print warning
!
        if (.not.lok) then
          call outwarning('Baskes twobody cross term may be missing for a pair of species',0_i4)
        endif
      endif
!
      lfrst = .false.
