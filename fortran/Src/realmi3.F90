...
!   2/15 MM3buck added
!   7/17 Sign of setrm addition corrected
!   2/18 Trace added
...
!
            dUdl_f = 0.0_dp
            if (ltifqspec(nspeci)) then
              dUdl_f = dUdl_f + factor*sf_j
            elseif (ltibqspec(nspeci)) then
              dUdl_f = dUdl_f - factor*sf_j
            endif
            if (ltifqspec(nspecj)) then
              dUdl_f = dUdl_f + factor*sf_i
            elseif (ltibqspec(nspecj)) then
              dUdl_f = dUdl_f - factor*sf_i
            endif
...
!
            ...
            dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - tweatpi*dUdl_f
          endif
!
!  Remove self energy
!
...
!*************************************************
      ...
      call twobody1(eatom2,ereal2,ec62,lgrad1p,.false.,.false.,1_i4,1_i4,dist,xtmp,ytmp,ztmp,d0i,d0j, &
                    ...
                    sctrm1,sctrm2,qli,qlj,nspeci,nspecj,lcspair,lewaldtype,.false.,lbonded,l2bonds, &
                    l3bonds,nbtypeij,nbtypeij2,.false.,lQMMMelectro,d1i,d1j,d2i2,d2ij,d2j2)
      ...
      if (leem.or.lgfnff) then
        if (lqeq.or.lSandM.or.lgfnff) then
          ...
          elseif (lgfnff) then
            ...
            call gfnffbody1(eqeq2,zetaij,lgrad1,.false.,nor,1_i4,dist,deriv,deriv2, &
...
!*************************************
      if (lpolar) then
        ...
        vx(j) = vx(j) + qli*derivqd(1)*xtmp
        vy(j) = vy(j) + qli*derivqd(1)*ytmp
        vz(j) = vz(j) + qli*derivqd(1)*ztmp
        if (lattach) then
          vx12(i) = vx12(i) - qlj*derivqd(1)*xtmp
          vy12(i) = vy12(i) - qlj*derivqd(1)*ytmp
          vz12(i) = vz12(i) - qlj*derivqd(1)*ztmp
          vx12(j) = vx12(j) + qli*derivqd(1)*xtmp
          vy12(j) = vy12(j) + qli*derivqd(1)*ytmp
          vz12(j) = vz12(j) + qli*derivqd(1)*ztmp
        endif
      endif
!***********************
!  Radial derivatives  *
...
!
      dUdl_f = 0.0_dp
      if (ltifqspec(nspeci)) then
        dUdl_f = dUdl_f + 2.0_dp*factor*sf_i
      elseif (ltibqspec(nspeci)) then
        dUdl_f = dUdl_f - 2.0_dp*factor*sf_i
      endif
      factor = factor*sf_i*sf_i
      dUdl_f = dUdl_f*rtistep
!
      dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - tweatpi*dUdl_f
    endif
