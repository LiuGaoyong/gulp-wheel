...
!  12/21 Background call changed to specific MD routine and TI
!        handled through ltianyqspec flag
!  12/21 pGFNFF modifications added
!   3/23 Apply lmodco here
...
!
    do i = 1,numat
      if (lopf(i)) then
        ...
        if (.not.lmodco.or.abs(ri).lt.1.0d-12) then
          icos1 = 0
          icos2 = 0
          icos3 = 0
          xclat(i) = xal
...
!***********************************************************************************
!  GFNFF forcefield - must be called before electrostatics as it sets the charges  *
!***********************************************************************************
  if (lgfnff) then
...
!*******************
  if (lfieldcfg(ncf).or.(ntdfieldcfg(ncf).gt.0)) then
    call electricfield(efield,lgrad1,.false.)
  endif
!*******************
!  Einstein model  *
!*******************
...
!****************************
  if (ndim.gt.0.and.(abs(totalcharge).gt.1.0d-6.or.(lti.and.ltianyqspec))) call backgroundmd(ebgd,emad,lgrad1)
!*****************************
...
!********************************
  if (nprocs.gt.1) then
    call psumall(eatom,ereal,erecip,ec6,eqeq,eattach,esregion12,esregion2,ethb,efor, &
                 ...
                 eplumed,eboQself,eval,egfnff,lgrad1,.false.,.false.)
  endif
!
...
!
  ...
  etot = erecip + eatom + sft + ethb + ereal + efor + epv + ecmm + ec6 + eoop + emany + &
         ...
         eone + eedip + emad + eimp + ekim + eplumed + eval + egfnff
!
