  subroutine valencebond(eval,lgrad1,lgrad2)
!
...
!
!   8/21 Created from bondorder
!   9/21 Derivatives added
!   9/21 Second derivatives now added to upper triangle
!  12/21 Phonon calculation moved to a separate routine
...
!
  use datatypes
  use bondvalence
  use configurations, only : nregions, lsliceatom, nregiontype
...
!
  ...
  if (status/=0) call outofmemory('valencebond','nvbijptr')
  allocate(bv(maxvbnbr),stat=status)
  if (status/=0) call outofmemory('valencebond','vb')
  allocate(bvv(3,maxvbnbr),stat=status)
  if (status/=0) call outofmemory('valencebond','vbv')
!
  if (lgrad1) then
    allocate(d1i(maxvbnbr),stat=status)
    if (status/=0) call outofmemory('valencebond','d1i')
  else
  ...
  if (lgrad2) then
    ...
    if (status/=0) call outofmemory('valencebond','d2i')
  else
...
!
          ...
          call add_drv2_1ut(ix,iy,iz,jx,jy,jz,xji,yji,zji,deijdr,d2eidr2,drij2ds,d2rij2dx2,d2rij2ds2,d2rij2dsdx)
!
...
!
            call add_drv2_2ut(ix,iy,iz,jx,jy,jz,ix,iy,iz,kx,ky,kz,xji,yji,zji,xki,yki,zki,d2eidr2,drij2ds,drik2ds)
!
            rrik3 = rrik**3
...
!
            if (i.ge.k) then
              ...
              derv2(ix,kx) = derv2(ix,kx) - deidwi2*d2wkj2(1,1) - d2eidwi22*dwji2(1)*dwki2(1)
              derv2(iy,kx) = derv2(iy,kx) - deidwi2*d2wkj2(1,2) - d2eidwi22*dwji2(2)*dwki2(1)
              derv2(iz,kx) = derv2(iz,kx) - deidwi2*d2wkj2(1,3) - d2eidwi22*dwji2(3)*dwki2(1)
              derv2(ix,ky) = derv2(ix,ky) - deidwi2*d2wkj2(2,1) - d2eidwi22*dwji2(1)*dwki2(2)
              derv2(iy,ky) = derv2(iy,ky) - deidwi2*d2wkj2(2,2) - d2eidwi22*dwji2(2)*dwki2(2)
              derv2(iz,ky) = derv2(iz,ky) - deidwi2*d2wkj2(2,3) - d2eidwi22*dwji2(3)*dwki2(2)
              derv2(ix,kz) = derv2(ix,kz) - deidwi2*d2wkj2(3,1) - d2eidwi22*dwji2(1)*dwki2(3)
              derv2(iy,kz) = derv2(iy,kz) - deidwi2*d2wkj2(3,2) - d2eidwi22*dwji2(2)*dwki2(3)
              derv2(iz,kz) = derv2(iz,kz) - deidwi2*d2wkj2(3,3) - d2eidwi22*dwji2(3)*dwki2(3)
            endif
!
            if (j.ge.k) then
              ...
              derv2(jx,kx) = derv2(jx,kx) + deidwi2*d2wkj2(1,1) + d2eidwi22*dwji2(1)*dwki2(1)
              derv2(jy,kx) = derv2(jy,kx) + deidwi2*d2wkj2(1,2) + d2eidwi22*dwji2(2)*dwki2(1)
              derv2(jz,kx) = derv2(jz,kx) + deidwi2*d2wkj2(1,3) + d2eidwi22*dwji2(3)*dwki2(1)
              derv2(jx,ky) = derv2(jx,ky) + deidwi2*d2wkj2(2,1) + d2eidwi22*dwji2(1)*dwki2(2)
              derv2(jy,ky) = derv2(jy,ky) + deidwi2*d2wkj2(2,2) + d2eidwi22*dwji2(2)*dwki2(2)
              derv2(jz,ky) = derv2(jz,ky) + deidwi2*d2wkj2(2,3) + d2eidwi22*dwji2(3)*dwki2(2)
              derv2(jx,kz) = derv2(jx,kz) + deidwi2*d2wkj2(3,1) + d2eidwi22*dwji2(1)*dwki2(3)
              derv2(jy,kz) = derv2(jy,kz) + deidwi2*d2wkj2(3,2) + d2eidwi22*dwji2(2)*dwki2(3)
              derv2(jz,kz) = derv2(jz,kz) + deidwi2*d2wkj2(3,3) + d2eidwi22*dwji2(3)*dwki2(3)
            endif
!
...
!
                derv3(kx,kk) = derv3(kx,kk) + d2eidwi22*dwji2s(kk)*dwki2(1) + deidwi2*d2wki2sx(1)
                derv3(ky,kk) = derv3(ky,kk) + d2eidwi22*dwji2s(kk)*dwki2(2) + deidwi2*d2wki2sx(2)
                derv3(kz,kk) = derv3(kz,kk) + d2eidwi22*dwji2s(kk)*dwki2(3) + deidwi2*d2wki2sx(3)
              enddo
!
              do kk = 1,nstrains
                do kl = 1,nstrains
...
!
  ...
  if (status/=0) call deallocate_error('valencebond','d2i')
  deallocate(d1i,stat=status)
  if (status/=0) call deallocate_error('valencebond','d1i')
  deallocate(bvv,stat=status)
  if (status/=0) call deallocate_error('valencebond','vbv')
  deallocate(bv,stat=status)
  if (status/=0) call deallocate_error('valencebond','vb')
  deallocate(nvbijptr,stat=status)
  if (status/=0) call deallocate_error('valencebond','nvbijptr')
!
  t2 = g_cpu_time()
  tval = tval + t2 - t1
#ifdef TRACE
  call trace_out('valencebond')
#endif
!
  return
  end
