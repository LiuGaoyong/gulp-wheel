  subroutine brenner12d(ebrenner,imode,lgrad1,lgrad2)
!
...
!
!   5/17 Created from brenner12
!   5/17 Bug in setting of atomic number for some cases fixed
...
!
  ...
  integer(i4), dimension(:,:), allocatable       :: natneighno
...
! 
  ...
  if (status/=0) call outofmemory('brenner12d','nREBOatomptr')
  allocate(nREBOatomRptr(ntot),stat=status)
  if (status/=0) call outofmemory('brenner12d','nREBOatomRptr')
! 
...
!
  ...
  if (status/=0) call outofmemory('brenner12d','lopanyneigh')
  allocate(nauatom(ntot),stat=status)
  if (status/=0) call outofmemory('brenner12d','nauatom')
  allocate(nfreeatom(ntot),stat=status)
  if (status/=0) call outofmemory('brenner12d','nfreeatom')
  allocate(nfreeatomau(ntot),stat=status)
  if (status/=0) call outofmemory('brenner12d','nfreeatomau')
  allocate(nneigh(nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','nneigh')
  allocate(nsneigh(nREBOspecies,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','nsneigh')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('brenner12d','d2Ndr2')
    deallocate(d2Nconjdj2,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d2Nconjdj2')
    deallocate(d2Nconjdi2,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d2Nconjdi2')
    deallocate(d2Btotj,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d2Btotj')
    deallocate(d2Btoti,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d2Btoti')
    deallocate(d2j,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d2j')
    deallocate(d2i,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d2i')
!       
    deallocate(dNdr,stat=status)
    if (status/=0) call deallocate_error('brenner12d','dNdr')
    deallocate(dNconjdj,stat=status)
    if (status/=0) call deallocate_error('brenner12d','dNconjdj')
    deallocate(dNconjdi,stat=status)
    if (status/=0) call deallocate_error('brenner12d','dNconjdi')
    deallocate(d1Btotj,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d1Btotj')
    deallocate(d1Btoti,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d1Btoti')
    deallocate(d1j,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d1j')
    deallocate(d1i,stat=status)
    if (status/=0) call deallocate_error('brenner12d','d1i')
!       
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('brenner12d','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('brenner12d','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('brenner12d','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('brenner12d','rneigh')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('brenner12d','neighno')
    deallocate(natneighno,stat=status)
    if (status/=0) call deallocate_error('brenner12d','natneighno')
    deallocate(nREBObond,stat=status)
    if (status/=0) call deallocate_error('brenner12d','nREBObond')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('brenner12d','nREBObond')
  allocate(natneighno(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','natneighno')
  allocate(neighno(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','neighno')
  allocate(rneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','rneigh')
  allocate(xneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','xneigh')
  allocate(yneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','yneigh')
  allocate(zneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brenner12d','zneigh')
  if (lgrad1) then
    allocate(d1i(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brenner12d','d1i')
    allocate(d1j(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brenner12d','d1j')
    allocate(d1Btoti(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brenner12d','d1Btoti')
    allocate(d1Btotj(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brenner12d','d1Btotj')
    allocate(dNconjdi(maxneigh),stat=status)
    if (status/=0) call outofmemory('brenner12d','dNconjdi')
    allocate(dNconjdj(maxneigh),stat=status)
    if (status/=0) call outofmemory('brenner12d','dNconjdj')
    allocate(dNdr(maxneigh,ntot),stat=status)
    if (status/=0) call outofmemory('brenner12d','dNdr')
  else
  ...
  if (lgrad2) then
    ...
    if (status/=0) call outofmemory('brenner12d','d2i')
    allocate(d2j(maxneigh22),stat=status)   
    if (status/=0) call outofmemory('brenner12d','d2j')
    allocate(d2Btoti(maxneigh22),stat=status)
    if (status/=0) call outofmemory('brenner12d','d2Btoti')
    allocate(d2Btotj(maxneigh22),stat=status)
    if (status/=0) call outofmemory('brenner12d','d2Btotj')
    allocate(d2Nconjdi2(maxneigh*(maxneigh+1)/2),stat=status)
    if (status/=0) call outofmemory('brenner12d','d2Nconjdi2')
    allocate(d2Nconjdj2(maxneigh*(maxneigh+1)/2),stat=status)
    if (status/=0) call outofmemory('brenner12d','d2Nconjdj2')
    allocate(d2Ndr2(maxneigh,ntot),stat=status)
    if (status/=0) call outofmemory('brenner12d','d2Ndr2')
  else
      ...
      if (i.ne.j) then
        ...
        if (r2 .lt. bR22(nREBObo)) then
          ...
          if (nneigh(nri).ge.maxneigh.or..not.lmaxneighok) then
            ...
            natneighno(nneigh(nri),nri) = natj
...
!
      ...
      if (lgrad1) then
        do nn = 1,nneigh(nri)
          if (nn.ne.ni) then
            ns1 = nat2REBOspecies(natneighno(nn,nri))
        ...
        if (lgrad2) then
          do nn1 = 1,nneigh(nri)
            if (nn1.ne.ni) then
              ns1 = nat2REBOspecies(natneighno(nn1,nri))
              do nn2 = 1,nn1
                if (nn2.ne.ni) then
                  nn = nn1*(nn1 - 1)/2 + nn2
                  ns2 = nat2REBOspecies(natneighno(nn2,nri))
                  if (ns1.gt.ns2) then
...
!
      ...
      if (lgrad1) then
        do nn = 1,nneigh(nrj)
          if (nn.ne.nj) then
            ns1 = nat2REBOspecies(natneighno(nn,nrj))
        ...
        if (lgrad2) then
          do nn1 = 1,nneigh(nrj)
            if (nn1.ne.nj) then
              ns1 = nat2REBOspecies(natneighno(nn1,nrj))
              do nn2 = 1,nn1
                if (nn2.ne.nj) then
                  nn = nn1*(nn1 - 1)/2 + nn2
                  ns2 = nat2REBOspecies(natneighno(nn2,nrj))
                  if (ns1.gt.ns2) then
                    indns = ns1*(ns1 - 1)/2 + ns2
                  else
                    indns = ns2*(ns2 - 1)/2 + ns1
...
!
      ...
      do k = 1,nneigh(nri)
        if (k.ne.ni.and.natneighno(k,nri).eq.6) then
...
! Hard coded for C/H
      ...
      enddo
      do k = 1,nneigh(nrj)
        if (k.ne.nj.and.natneighno(k,nrj).eq.6) then
...
!
      if (lgrad1) then
        if (ldsl) then
          call d1adds(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,nauatom,ndeqv,d1j, &
                      .true.,.false.)
        else
          if (lilocal) then
            call d1add(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,d1j,.true.,.false.,.false.)
          endif
        endif
        if (lgrad2) then
          if (ldsl) then
            call d2addds(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nfreeatomau,nREBOatomRptr,nauatom, &
                         ndeqv,nr1,d1j,d2j,.true.)
          else
            call d2addddm(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,nr1,d1j,d2j,.true.)
          endif
        endif
      endif
!
...
!
    if (lgrad1) then
      if (ldsl) then
        call d1adds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,nauatom,ndeqv,d1i, &
                    .true.,.false.)
      else
        if (lilocal) then
          call d1add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,d1i,.true.,.false.,.false.)
        endif
      endif
      if (lgrad2) then
        if (ldsl) then
          call d2addds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nfreeatomau,nREBOatomRptr,nauatom, &
                       ndeqv,nr1,d1i,d2i,.true.)
        else
          call d2addddm(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,nr1,d1i,d2i,.true.)
        endif
      endif
    endif
  enddo iloop
...
!
  if (.not.lgrad2) then
    call sumone(ebrenner,esum,"brenner12d","ebrenner")
    ebrenner = esum
  endif
!
!  Free local memory
!
  ...
  if (status/=0) call deallocate_error('brenner12d','nsneigh')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('brenner12d','nneigh')
  deallocate(nfreeatomau,stat=status)
  if (status/=0) call deallocate_error('brenner12d','nfreeatomau')
  deallocate(nfreeatom,stat=status)
  if (status/=0) call deallocate_error('brenner12d','nfreeatom')
  deallocate(nauatom,stat=status)
  if (status/=0) call deallocate_error('brenner12d','nauatom')
  deallocate(lopanyneigh,stat=status)
  if (status/=0) call deallocate_error('brenner12d','lopanyneigh')
  deallocate(nREBOatomRptr,stat=status)
  if (status/=0) call deallocate_error('brenner12d','nREBOatomRptr')
  deallocate(nREBOatomptr,stat=status)
  if (status/=0) call deallocate_error('brenner12d','nREBOatomptr')
!
  t2 = g_cpu_time()
  tbrenner = tbrenner + t2 - t1
#ifdef TRACE
  call trace_out('brenner12d')
#endif
!
  return
  end
