  subroutine peigend(mtv,mtvloc,mtvptr,mtvnptr,mtvrptr,freq,ncore,ncfoc,nsfoc, &
                     iocptr,maxd2,eigc,oscstrength,ir,wk)
!
...
!   6/20 nphonatc passed via module rather than arguments
!   6/20 nmolcore changes added
!   6/20 Shell model corrections for rigid molecules added
!   8/20 Bug in calling of ramantrm for rigid molecules in new algorithm fixed
!  12/20 Array ir now always initialised on return
!   8/22 Choice between mass-weighted and/or phased eigenvectors or not added
!
!  Conditions of use:
!
...
!
  ...
  complex(dpc)                                 :: ceigz
  complex(dpc)                                 :: craman
  complex(dpc), dimension(:),   allocatable    :: egc
  complex(dpc)                                 :: egcl(6,6)
  complex(dpc), dimension(:),   allocatable    :: enorm
  complex(dpc), dimension(:,:), allocatable    :: etmp
  complex(dpc)                                 :: xir
  complex(dpc)                                 :: yir
  complex(dpc)                                 :: zir
  real(dp)                                     :: drQ(3,3)
...
!
!  Allocate array for eigenvector correction factors
!
  allocate(egc(mtv),stat=status)
  if (status/=0) call outofmemory('peigend','egc')
  allocate(enorm(mtvloc),stat=status)
  if (status/=0) call outofmemory('peigend','enorm')
!
!  Allocate memory for intensities
!
  allocate(irx(mtv),stat=status)
  if (status/=0) call outofmemory('peigend','irx')
  allocate(iry(mtv),stat=status)
  if (status/=0) call outofmemory('peigend','iry')
  allocate(irz(mtv),stat=status)
  if (status/=0) call outofmemory('peigend','irz')
  allocate(raman(mtv),stat=status)
  if (status/=0) call outofmemory('peigend','raman')
!************************************************************************
!  Compute renormalisation factors for eigenvectors if mass-unweighted  *
!************************************************************************
  if (.not.leigenvectors_original) then
    do m = 1,mtvloc
      enorm(m) = 0.0_dp
      do i = 1,mtv
         enorm(m) = enorm(m) + eigc(i,m)*conjg(eigc(i,m))*dcmplx(rfmass(i)**2,0.0_dp)
      enddo
      enorm(m) = 1.0_dp/sqrt(enorm(m))
    enddo
  endif
!***************************
...
!
    ...
    if (status/=0) call outofmemory('peigend','etmp')
    allocate(StatMPI(MPI_Status_Size),stat=status)
    if (status/=0) call outofmemory('peigend','StatMPI')
    call mpbarrier
!
    do m = 1,mtv
      mm = mtvrptr(m)
...
!
        if (ioproc) then
          nnode = mtvnptr(m)
          call MPI_IRecv(etmp,ntmp,MPI_double_complex,nnode, &
...
!
        if (mm.gt.0) then
          if (leigenvectors_original) then
            etmp(1:ntmp,1) = eigc(1:ntmp,mm)
          else
            do n = 1,ntmp
              etmp(n,1) = eigc(n,mm)*enorm(mm)*dcmplx(rfmass(n),0.0_dp)
            enddo
          endif
          if (.not.leigenvectors_phase) then
            do n = 1,ntmp
              etmp(n,1) = etmp(n,1)*fphase(n)
            enddo
          endif
!
!  Post send
!
          call MPI_ISend(etmp,ntmp,MPI_double_complex,0, &
                         ntag,MPI_Comm_World,Request,MPIerror)
        endif
...
!
          ...
          ii = 0
          do i = 1,nphonatc
            write(53,'(3f10.6,4x,3f10.6)') (dble(etmp(ii+j,1)),j=1,3),(aimag(etmp(ii+k,1)),k=1,3)
            ii = ii + 3
          enddo
          if (lrigid) then
            do i = 1,nphonatm
        ...
        if (mm.gt.0) then
          ...
          write(53,'(f15.6)') freq(m)
          if (leigenvectors_original) then
            egc(1:mtv) = eigc(1:mtv,mm)
          else
            do n = 1,mtv
              egc(n) = eigc(n,mm)*enorm(mm)*dcmplx(rfmass(n),0.0_dp)
            enddo
          endif
          if (.not.leigenvectors_phase) then
            do n = 1,mtv
              egc(n) = egc(n)*fphase(n)
            enddo
          endif
          ii = 0
          do i = 1,nphonatc
            write(53,'(3f10.6,4x,3f10.6)') (dble(egc(ii+j)),j=1,3),(aimag(egc(ii+k)),k=1,3)
            ii = ii + 3
          enddo
          if (lrigid) then
            do i = 1,nphonatm
...
!
    ...
    if (status/=0) call deallocate_error('peigend','StatMPI')
    deallocate(etmp,stat=status)
    if (status/=0) call deallocate_error('peigend','etmp')
  endif
!
...
!
          ...
          do i = 1,nphonatc
            ...
            write(54,'(2i5,2f16.12,4x,2f16.12,4x,2f16.12)') m,ig, &
              etmp(ii+1,1),etmp(ii+2,1),etmp(ii+3,1)
            ii = ii + 3
          enddo
          if (lrigid) then
            do i = 1,nphonatm
...
!
          do i = 1,nphonatc
            ig = ig + 1
            write(54,'(2i5,2f16.12,4x,2f16.12,4x,2f16.12)') m,ig,egc(ii+1),egc(ii+2),egc(ii+3)
            ii = ii + 3
          enddo
          if (lrigid) then
            do i = 1,nphonatm
...
!
    ...
    if (status/=0) call outofmemory('peigend','itmp')
    allocate(w1(mtv),stat=status)
    if (status/=0) call outofmemory('peigend','w1')
    allocate(w2(mtv),stat=status)
    if (status/=0) call outofmemory('peigend','w2')
!
              ...
              do while (l.le.ncore.and..not.lfound)
                if (iocptr(l).eq.k) then
                  ...
                  if (itmp(nrell).eq.1) then
                    lfound = .true.
                    w1(jj) = w1(jj) + dble(conjg(eigc(ind+1,j))*eigc(ind+1,j) + &
                                           conjg(eigc(ind+2,j))*eigc(ind+2,j) + &
                                           conjg(eigc(ind+3,j))*eigc(ind+3,j))
                  endif
                endif
                l = l + 1
              enddo
...
!
            do km = 1,nmol
              wperatom = dble(conjg(eigc(ind+1,j))*eigc(ind+1,j) + &
                              conjg(eigc(ind+2,j))*eigc(ind+2,j) + &
                              conjg(eigc(ind+3,j))*eigc(ind+3,j) + &
                              conjg(eigc(ind+4,j))*eigc(ind+4,j) + &
                              conjg(eigc(ind+5,j))*eigc(ind+5,j) + &
                              conjg(eigc(ind+6,j))*eigc(ind+6,j))
...
!
          call sumall(w1,w2,mtv,"peigend","w1")
          if (ioproc) then
            do j = 1,mtv
              write(59) w2(j)
            enddo
...
!
    ...
    if (status/=0) call deallocate_error('peigend','w2')
    deallocate(w1,stat=status)
    if (status/=0) call deallocate_error('peigend','w1')
    deallocate(itmp,stat=status)
    if (status/=0) call deallocate_error('peigend','itmp')
  endif
!***********************************
...
!
    ...
    if (status/=0) call outofmemory('peigend','w3')
    if (lrigid.and.loldinten) then
      allocate(w1r(3,mtv),stat=status)
      if (status/=0) call outofmemory('peigend','w1r')
    endif
!
    if (loldinten) then
!***************************************************
...
!
            xir = xir + w1(ind+1)*eigc(ind+1,ii)
            yir = yir + w1(ind+2)*eigc(ind+2,ii)
            zir = zir + w1(ind+3)*eigc(ind+3,ii)
!
...
!
            ...
            esqz = dble(conjg(ceigz)*ceigz)
!
            msdx(ncorenomol+j) = msdx(ncorenomol+j) + w3(ind+1)*esqx*ffact2
            msdy(ncorenomol+j) = msdy(ncorenomol+j) + w3(ind+2)*esqy*ffact2
            msdz(ncorenomol+j) = msdz(ncorenomol+j) + w3(ind+3)*esqz*ffact2
!
            xir = xir + w1(ind+1)*eigc(ind+1,ii)
            yir = yir + w1(ind+2)*eigc(ind+2,ii)
            zir = zir + w1(ind+3)*eigc(ind+3,ii)
!
            craman = craman + dcmplx(w2(ind+1),0.0_dp)*ceigx + &
                              ...
                              dcmplx(w2(ind+3),0.0_dp)*ceigz
!
            xir = xir + w1r(1,ind+4)*eigc(ind+4,ii) + &
                        w1r(2,ind+4)*eigc(ind+5,ii) + &
                        w1r(3,ind+4)*eigc(ind+6,ii)
            yir = yir + w1r(1,ind+5)*eigc(ind+4,ii) + &
                        w1r(2,ind+5)*eigc(ind+5,ii) + &
                        w1r(3,ind+5)*eigc(ind+6,ii)
            zir = zir + w1r(1,ind+6)*eigc(ind+4,ii) + &
                        w1r(2,ind+6)*eigc(ind+5,ii) + &
                        w1r(3,ind+6)*eigc(ind+6,ii)
!
            ind = ind + 6
          enddo
        else
...
!
            ...
            zir = zir + w1(ind+3)*eigc(ind+3,ii)
!
            craman = craman + dcmplx(w2(ind+1),0.0_dp)*eigc(ind+1,ii) + &
                              dcmplx(w2(ind+2),0.0_dp)*eigc(ind+2,ii) + &
                              dcmplx(w2(ind+3),0.0_dp)*eigc(ind+3,ii)
            ind = ind + 3
          enddo
        endif
!
        irx(i) = dble(conjg(xir)*xir)
        iry(i) = dble(conjg(yir)*yir)
        irz(i) = dble(conjg(zir)*zir)
...
!  Conversion for MSD : 10^23 x h / (4*pi*pi*c)
!
          w3(i) = wk*0.25_dp*(1.0d23*planck*avogadro*rfmass(ix)**2)/(pi*pi*speedl)
          do j = 1,ncore
            if (iocptr(j).eq.i) then
              nsj = nspecptr(nrelf2a(j))
              trmj = occuf(j)/sqrt(massspec(nsj))
...
!
          ...
          do j = 1,nmol
            ...
            ceigz = eigc(ind+3,ii)
!
            msdx(ncorenomol+j) = msdx(ncorenomol+j) + w3(ind+1)*dble(conjg(ceigx)*ceigx)*ffact2
            msdy(ncorenomol+j) = msdy(ncorenomol+j) + w3(ind+2)*dble(conjg(ceigy)*ceigy)*ffact2
            msdz(ncorenomol+j) = msdz(ncorenomol+j) + w3(ind+3)*dble(conjg(ceigz)*ceigz)*ffact2
!
            craman = craman + dcmplx(w2(ind+1),0.0_dp)*ceigx + &
                              dcmplx(w2(ind+2),0.0_dp)*ceigy + &
                              dcmplx(w2(ind+3),0.0_dp)*ceigz
...
!
    ...
    if (lrigid.and.loldinten) then
      ...
      if (status/=0) call deallocate_error('peigend','w1r')
    endif
    deallocate(w3,stat=status)
    if (status/=0) call deallocate_error('peigend','w3')
    deallocate(w2,stat=status)
    if (status/=0) call deallocate_error('peigend','w2')
    deallocate(w1,stat=status)
    if (status/=0) call deallocate_error('peigend','w1')
...
!
    ...
    if (status/=0) call outofmemory('peigend','sum1')
    allocate(sum2(5*mtv),stat=status)
    if (status/=0) call outofmemory('peigend','sum2')
!
    do i = 1,mtv
      sum1(i) = ir(i)
      sum1(mtv+i) = irx(i)
    ...
    enddo
!
    call sumall(sum1,sum2,5_i4*mtv,"peigend","sum1")
!
    do i = 1,mtv
      ir(i) = sum2(i)
      irx(i) = sum2(mtv+i)
...
!
    ...
    if (status/=0) call deallocate_error('peigend','sum2')
    deallocate(sum1,stat=status)
    if (status/=0) call deallocate_error('peigend','sum1')
  endif
!************************
!  Output eigenvectors  *
!************************
  ...
  if (lout) then
    if (ioproc) then
      ...
      if (.not.lrigid.and.ncfoc.ne.nphonatc) then
...
!
          if (mtvrptr(indi+1).gt.0) then
            do k = 1,3
              if (leigenvectors_original) then
                etmp(1:mtv,k) = eigc(1:mtv,mtvrptr(indi+k))
              else
                do n = 1,mtv
                  etmp(n,k) = eigc(n,mtvrptr(indi+k))*enorm(mtvrptr(indi+k))*dcmplx(rfmass(n),0.0_dp)
                enddo
              endif
!
              if (.not.leigenvectors_phase) then
                do n = 1,mtv
                  etmp(n,k) = etmp(n,k)*fphase(n)
                enddo
              endif
            enddo
!
...
!
            indj = 0
            do j = 1,njloop
              write(ioout,'(i6,'' x '',4x,6f10.6)') j,(etmp(indj+1,k),k=1,3)
              write(ioout,'(i6,'' y '',4x,6f10.6)') j,(etmp(indj+2,k),k=1,3)
              write(ioout,'(i6,'' z '',4x,6f10.6)') j,(etmp(indj+3,k),k=1,3)
              indj = indj + 3
            enddo
            if (lrigid) then
              do j = 1,nphonatm
                write(ioout,'(i6,'' Tx'',4x,6f10.6)') j,(etmp(indj+1,k),k=1,3)
                write(ioout,'(i6,'' Ty'',4x,6f10.6)') j,(etmp(indj+2,k),k=1,3)
                write(ioout,'(i6,'' Tz'',4x,6f10.6)') j,(etmp(indj+3,k),k=1,3)
                write(ioout,'(i6,'' R1'',4x,6f10.6)') j,(etmp(indj+4,k),k=1,3)
                write(ioout,'(i6,'' R2'',4x,6f10.6)') j,(etmp(indj+5,k),k=1,3)
                write(ioout,'(i6,'' R3'',4x,6f10.6)') j,(etmp(indj+6,k),k=1,3)
                indj = indj + 6
              enddo
            endif
          endif
          ...
          if (mtvrptr(indi+1).gt.0) then
            ...
            do j = 1,njloop
              do k = 1,3
                if (leigenvectors_original) then
                  egcl(1:3,k) = eigc(indj+1:indj+3,mtvrptr(indi+k))
                else
                  do n = 1,3
                    egcl(n,k) = eigc(indj+n,mtvrptr(indi+k))*enorm(mtvrptr(indi+k))*dcmplx(rfmass(indj+n),0.0_dp)
                  enddo
                endif
!
                if (.not.leigenvectors_phase) then
                  do n = 1,3
                    egcl(n,k) = egcl(n,k)*fphase(indj+n)
                  enddo
                endif
              enddo
!
              write(ioout,'(i6,'' x '',4x,6f10.6)') j,(egcl(1,k),k=1,3)
              write(ioout,'(i6,'' y '',4x,6f10.6)') j,(egcl(2,k),k=1,3)
              write(ioout,'(i6,'' z '',4x,6f10.6)') j,(egcl(3,k),k=1,3)
              indj = indj + 3
            enddo
            if (lrigid) then
              do j = 1,nphonatm
...
!
                ...
                enddo
! 
                write(ioout,'(i6,'' Tx'',4x,6f10.6)') j,(egcl(1,k),k=1,3)
                write(ioout,'(i6,'' Ty'',4x,6f10.6)') j,(egcl(2,k),k=1,3)
                write(ioout,'(i6,'' Tz'',4x,6f10.6)') j,(egcl(3,k),k=1,3)
                write(ioout,'(i6,'' R1'',4x,6f10.6)') j,(egcl(4,k),k=1,3)
                write(ioout,'(i6,'' R2'',4x,6f10.6)') j,(egcl(5,k),k=1,3)
                write(ioout,'(i6,'' R3'',4x,6f10.6)') j,(egcl(6,k),k=1,3)
                indj = indj + 6
              enddo
            endif
          endif
...
!
          indj = 0
          do j = 1,njloop
            write(ioout,'(i6,'' x '',4x,6f10.6)') j,(etmp(indj+1,k),k=1,iresid)
            write(ioout,'(i6,'' y '',4x,6f10.6)') j,(etmp(indj+2,k),k=1,iresid)
            write(ioout,'(i6,'' z '',4x,6f10.6)') j,(etmp(indj+3,k),k=1,iresid)
            indj = indj + 3
          enddo
          if (lrigid) then
            do j = 1,nphonatm
              write(ioout,'(i6,'' Tx'',4x,6f10.6)') j,(etmp(indj+1,k),k=1,iresid)
              write(ioout,'(i6,'' Ty'',4x,6f10.6)') j,(etmp(indj+2,k),k=1,iresid)
              write(ioout,'(i6,'' Tz'',4x,6f10.6)') j,(etmp(indj+3,k),k=1,iresid)
              write(ioout,'(i6,'' R1'',4x,6f10.6)') j,(etmp(indj+4,k),k=1,iresid)
              write(ioout,'(i6,'' R2'',4x,6f10.6)') j,(etmp(indj+5,k),k=1,iresid)
              write(ioout,'(i6,'' R3'',4x,6f10.6)') j,(etmp(indj+6,k),k=1,iresid)
              indj = indj + 6
            enddo
          endif
        endif
...
!
            ...
            enddo
!
            write(ioout,'(i6,'' x '',4x,6f10.6)') j,(egcl(1,k),k=1,iresid)
            write(ioout,'(i6,'' y '',4x,6f10.6)') j,(egcl(2,k),k=1,iresid)
            write(ioout,'(i6,'' z '',4x,6f10.6)') j,(egcl(3,k),k=1,iresid)
            indj = indj + 3
          enddo
          if (lrigid) then
            do j = 1,nphonatm
...
!
              ...
              enddo
!
              write(ioout,'(i6,'' Tx'',4x,6f10.6)') j,(egcl(1,k),k=1,iresid)
              write(ioout,'(i6,'' Ty'',4x,6f10.6)') j,(egcl(2,k),k=1,iresid)
              write(ioout,'(i6,'' Tz'',4x,6f10.6)') j,(egcl(3,k),k=1,iresid)
              write(ioout,'(i6,'' R1'',4x,6f10.6)') j,(egcl(4,k),k=1,iresid)
              write(ioout,'(i6,'' R2'',4x,6f10.6)') j,(egcl(5,k),k=1,iresid)
              write(ioout,'(i6,'' R3'',4x,6f10.6)') j,(egcl(6,k),k=1,iresid)
              indj = indj + 6
            enddo
          endif
        endif
...
! DEBUG
!    call makeeigenarraysd(eigc,freq,maxd2)
  endif
!
  deallocate(raman,stat=status)
  if (status/=0) call deallocate_error('peigend','raman')
  deallocate(irz,stat=status)
  if (status/=0) call deallocate_error('peigend','irz')
  deallocate(iry,stat=status)
  if (status/=0) call deallocate_error('peigend','iry')
  deallocate(irx,stat=status)
  if (status/=0) call deallocate_error('peigend','irx')
  deallocate(enorm,stat=status)
  if (status/=0) call deallocate_error('peigend','enorm')
  deallocate(egc,stat=status)
  if (status/=0) call deallocate_error('peigend','egc')
#ifdef TRACE
  call trace_out('peigend')
#endif
#else
!
!  Dummy assignment of value to ir to avoid compiler warning
!
  ir(1:mtvloc) = 0.0_dp
!
  call outerror('peigend called when not compiled with MPI',0_i4)
  call stopnow('peigend')
#endif
!
  return
  end
