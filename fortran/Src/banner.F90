...
!   7/20 Changed to GULP 6.1
!   7/22 Changed to GULP 6.2
!
!  Conditions of use:
!
...
!******************
  if (ioproc) then
#ifdef ACCELRYS
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                       GENERAL UTILITY LATTICE PROGRAM  v6.2                  *'')')
    ...
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                     BIOVIA Materials Studio 2023 Release                     *'')')
    write(ioout,'(''********************************************************************************'')')
#else
    write(ioout,'(''********************************************************************************'')')
    ...
    write(ioout,'(''*                    Curtin University, Western Australia                      *'')')
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''* Version = 6.2.0 * Last modified =  19th December 2023                        *'')')
    write(ioout,'(''********************************************************************************'')')
#endif
  endif
#ifdef TRACE
