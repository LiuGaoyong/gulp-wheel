...
!
  ...
  if (status/=0) call outofmemory('fourfcd','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourfcd','ntypmiddle')
  allocate(nptrnfornonoop(nfor),stat=status)
  if (status/=0) call outofmemory('fourfcd','nptrnfornonoop')
  allocate(nuniqueiptr(numat),stat=status)
  if (status/=0) call outofmemory('fourfcd','nuniqueiptr')
  allocate(nuniquekptr(numat),stat=status)
  if (status/=0) call outofmemory('fourfcd','nuniquekptr')
  allocate(nuniquelptr(numat),stat=status)
  if (status/=0) call outofmemory('fourfcd','nuniquelptr')
  if (ndim.gt.0) then
    allocate(ivec(3,maxvector),stat=status)
    if (status/=0) call outofmemory('fourfcd','ivec')
    allocate(xvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fourfcd','xvec')
    allocate(yvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fourfcd','yvec')
    allocate(zvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fourfcd','zvec')
  else
...
!
      ...
      if (status/=0) call deallocate_error('fourfcd','zvec')
      deallocate(yvec,stat=status)
      if (status/=0) call deallocate_error('fourfcd','yvec')
      deallocate(xvec,stat=status)
      if (status/=0) call deallocate_error('fourfcd','xvec')
      deallocate(ivec,stat=status)
      if (status/=0) call deallocate_error('fourfcd','ivec')
...
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"fourfcd","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!****************************
  if (noofp.gt.0) call fouroopfcd
!************************
!  Improper potentials  *
!************************
  if (nimproper.gt.0) call fourimpfcd
!
!  Free local memory
!
  deallocate(zvec,stat=status)
  ...
  if (status/=0) call deallocate_error('fourfcd','nuniquelptr')
  deallocate(nuniquekptr,stat=status)
  if (status/=0) call deallocate_error('fourfcd','nuniquekptr')
  deallocate(nuniqueiptr,stat=status)
  if (status/=0) call deallocate_error('fourfcd','nuniqueiptr')
  deallocate(nptrnfornonoop,stat=status)
  if (status/=0) call deallocate_error('fourfcd','nptrnfornonoop')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourfcd','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourfcd','natmiddle')
!
!  Timing
!
  time2 = g_cpu_time()
