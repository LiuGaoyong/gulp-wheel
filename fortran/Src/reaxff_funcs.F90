  subroutine reaxFFbo_sigma(nboij,nspeci,nspecj,rij,BO,dBOdr,d2BOdr2,lgrad1,lgrad2,ltaper,lnonzero)
!
...
!  d2BOdr2         = second derivative of BO w.r.t. r if lgrad2
    ...
    if (eTrm1.gt.reaxFFtol) then
      ...
      if (lTaper.and.eTrm1.lt.reaxFFtaperscale*reaxFFtol) then
        ...
        if (lgrad1) then
          ...
          if (lgrad2) then
            dTrm2 = dTrm1*(dTrm1 + (reaxFFpbo(2,nboij) - 1.0_dp)/rij)
            d2BOdr2 = d2BOdr2 + (ft + dftdBO*(eTrm1 - reaxFFtol))*eTrm1*dTrm2 + &
              (2.0_dp*dftdBO + d2ftdBO2*(eTrm1 - reaxFFtol))*(eTrm1*dTrm1)**2
          endif
        endif
      else
...
#ifdef TRACE
  call trace_out('reaxffbo_sigma')
#endif
!
  return
  end
!
  subroutine reaxFFbo_pi(nboij,nspeci,nspecj,rij,BO,dBOdr,d2BOdr2,lgrad1,lgrad2,ltaper)
!
!  Subroutine to calculate the bond order for reaxFF. This is the primed
!  version to be combined with other terms to yield the full expression.
!
...
!   1/09 lreaxFFpboOK added 
!   2/18 Trace added
!   7/22 Second derivative added
!   8/23 Tapering corrected by reaxfftol shift
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
    ...
    if (eTrm1.gt.reaxFFtol) then
      if (lTaper.and.eTrm1.lt.reaxFFtaperscale*reaxFFtol) then
        ...
        if (lgrad1) then
          ...
          dBOdr = dBOdr + (ft + dftdBO*(eTrm1 - reaxFFtol))*eTrm1*dTrm1
          if (lgrad2) then
            dTrm2 = dTrm1*(dTrm1 + (reaxFFpbo(4,nboij) - 1.0_dp)/rij)
            d2BOdr2 = d2BOdr2 + (ft + dftdBO*(eTrm1 - reaxFFtol))*eTrm1*dTrm2 + &
              (2.0_dp*dftdBO + d2ftdBO2*(eTrm1 - reaxFFtol))*(eTrm1*dTrm1)**2
          endif
        endif
...
#ifdef TRACE
  call trace_out('reaxffbo_pi')
#endif
!
  return
  end
!
  subroutine reaxFFbo_pipi(nboij,nspeci,nspecj,rij,BO,dBOdr,d2BOdr2,lgrad1,lgrad2,ltaper)
!
!  Subroutine to calculate the bond order for reaxFF. This is the primed
!  version to be combined with other terms to yield the full expression.
!
    ...
    if (eTrm1.gt.reaxFFtol) then
      if (lTaper.and.eTrm1.lt.reaxFFtaperscale*reaxFFtol) then
        ...
        if (lgrad1) then
          ...
          dBOdr = dBOdr + (ft + dftdBO*(eTrm1 - reaxFFtol))*eTrm1*dTrm1
          if (lgrad2) then
            dTrm2 = dTrm1*(dTrm1 + (reaxFFpbo(6,nboij) - 1.0_dp)/rij)
            d2BOdr2 = d2BOdr2 + (ft + dftdBO*(eTrm1 - reaxFFtol))*eTrm1*dTrm2 + &
              (2.0_dp*dftdBO + d2ftdBO2*(eTrm1 - reaxFFtol))*(eTrm1*dTrm1)**2
          endif
        endif
...
!  df1ddpj         = first derivative of f1 w.r.t. deltapj if lgrad1
...
!        location
!   2/18 Trace added
!   9/22 Second derivative corrected
!   3/23 Trap added for exponential overflow
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!  Copyright Curtin University 2023
!
!  Julian Gale, CIC, Curtin University, March 2023
!
  use reaxFFdata
#ifdef TRACE
  use trace,          only : trace_in, trace_out
...
!
  loverflow_exp6 = (pval6*delta_angle.gt.80.0_dp)
  if (loverflow_exp6) then
    f8 = 1.0_dp
    if (lgrad1) then
      df8ddeltai = 0.0_dp
      if (lgrad2) then
    ...
    if (lgrad1) then
      ...
      if (lgrad2) then
        d2f8ddeltai2 = - pval5m1*trm1*(pval6*pval6*exp6 - trm1*(2.0_dp*pval6*exp6*trm2 + &
                         (2.0_dp + exp6)*(pval6*pval6*exp6 + pval7*pval7*exp7) - &
                         2.0_dp*trm1*(2.0_dp + exp6)*trm2*trm2))
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('reaxff_f8')
#endif
!
  return
  end
...
!
  ...
  if (lgrad1) then
!
!  For theta = 180 case ensure derivatives are zeroed ready for return
!
    if (l180) then
      the1d1 = 0.0_dp
      the1d2 = 0.0_dp
...
!
  subroutine reaxFF_f9(deltai,f9,df9ddeltai,d2f9ddeltai2,lgrad1,lgrad2)
!
...
!   8/07 Created
!  10/07 Second derivatives added
!  11/12 Unused arguments removed
!   2/18 Trace added
!
!  Conditions of use:
!
...
!  10/07 Second derivatives added
!   2/18 Trace added
!   3/23 Overflow of exponentials handled
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!  Local variables
!
  logical                             :: loverflow_exp3
  logical                             :: loverflow_exp4
  real(dp)                            :: exp3
  real(dp)                            :: exp4
  real(dp)                            :: ptor3
  real(dp)                            :: ptor4
...
!
!  Check for potential overflow in exponentials
!
  loverflow_exp3 = (ptor3*sumdelta.gt.80.0_dp)
  loverflow_exp4 = (ptor4*sumdelta.gt.80.0_dp)
!
!  Compute exponentials
!
  if (loverflow_exp4) then
    f11 = 0.0_dp
    if (lgrad1) then
      df11ddeltaij = 0.0_dp
      if (lgrad2) then
        d2f11ddeltaij2 = 0.0_dp
      endif
    endif
  else
    if (loverflow_exp3) then
      exp3 = 0.0_dp
    else
...
!   2/18 Trace added
!  10/22 Strict case added to second derivatives
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!  Copyright Curtin University 2022
!
!  Julian Gale, CIC, Curtin University, October 2022
!
  use reaxFFdata
#ifdef TRACE
  use trace,          only : trace_in, trace_out
...
!
  if (lgrad1) then
    ...
    if (lgrad2) then
      if (lStrict) then
        d2f12dBOij2     = 4.0_dp*pcot2*pcot2*f12*(BOij - 1.5_dp)*(BOij - 1.5_dp) - 2.0_dp*pcot2*f12
        d2f12dBOijdBOjk = 4.0_dp*pcot2*pcot2*f12*(BOij - 1.5_dp)*(BOjk - 1.5_dp)
        d2f12dBOijdBOkl = 4.0_dp*pcot2*pcot2*f12*(BOij - 1.5_dp)*(BOkl - 1.5_dp)
        d2f12dBOjk2     = 4.0_dp*pcot2*pcot2*f12*(BOjk - 1.5_dp)*(BOjk - 1.5_dp) - 2.0_dp*pcot2*f12
        d2f12dBOjkdBOkl = 4.0_dp*pcot2*pcot2*f12*(BOjk - 1.5_dp)*(BOkl - 1.5_dp)
        d2f12dBOkl2     = 4.0_dp*pcot2*pcot2*f12*(BOkl - 1.5_dp)*(BOkl - 1.5_dp) - 2.0_dp*pcot2*f12
      else
        d2f12dBOij2     = 4.0_dp*pcot2*pcot2*(BOij - 1.5_dp)*(BOij - 1.5_dp)*expij*(expjk - exptol)*(expkl - exptol) &
                          - 2.0_dp*pcot2*expij*(expjk - exptol)*(expkl - exptol)
        d2f12dBOijdBOjk = 4.0_dp*pcot2*pcot2*(BOij - 1.5_dp)*(BOjk - 1.5_dp)*expij*expjk*(expkl - exptol)
        d2f12dBOijdBOkl = 4.0_dp*pcot2*pcot2*(BOij - 1.5_dp)*(BOkl - 1.5_dp)*expij*expkl*(expjk - exptol)
        d2f12dBOjk2     = 4.0_dp*pcot2*pcot2*(BOjk - 1.5_dp)*(BOjk - 1.5_dp)*expjk*(expij - exptol)*(expkl - exptol) &
                        - 2.0_dp*pcot2*expjk*(expij - exptol)*(expkl - exptol)
        d2f12dBOjkdBOkl = 4.0_dp*pcot2*pcot2*(BOjk - 1.5_dp)*(BOkl - 1.5_dp)*expjk*expkl*(expij - exptol)
        d2f12dBOkl2     = 4.0_dp*pcot2*pcot2*(BOkl - 1.5_dp)*(BOkl - 1.5_dp)*expkl*(expij - exptol)*(expjk - exptol) &
                          - 2.0_dp*pcot2*expkl*(expij - exptol)*(expjk - exptol)
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('reaxff_f12')
#endif
!
  return
  end
...
#ifdef TRACE
      call trace_out('reaxff_sintheta')
#endif
      return
    endif
!***************************************
...
!  d2fdr2         = second derivative of taper function
!
!  11/07 Created
!   2/18 Trace added
!  12/22 Second derivatives corrected
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!
  ...
  logical,     intent(in)             :: lgrad2
#ifdef TRACE
  call trace_in('p7reaxffvdwtaper')
#endif
!
  if (r .ge. rmax) then
!*************
...
!
    ...
    if (lgrad1) then
      ...
      if (lgrad2) then
        d2fdr2 = ((35.0_dp*reaxFFtaperVDW(8)*r + 24.0_dp*reaxFFtaperVDW(7))*r + 15.0_dp*reaxFFtaperVDW(6))*r + &
                   8.0_dp*reaxFFtaperVDW(5)
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('p7reaxffvdwtaper')
#endif
!
  return
  end
...
!
    ...
    if (lgrad1) then
      ...
      if (lgrad2) then
        d2fdr2 = ((35.0_dp*reaxFFtaperQ(8)*r + 24.0_dp*reaxFFtaperQ(7))*r + 15.0_dp*reaxFFtaperQ(6))*r + &
                   8.0_dp*reaxFFtaperQ(5)
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('p7reaxffqtaper')
#endif
!
  return
  end
...
!
  ...
  real(dp)                            :: rrp1
#ifdef TRACE
  call trace_in('reaxffbocut_sigma')
#endif
!
!  If radii for either species is >= 0, then term is zero
!
...
#ifdef TRACE
  call trace_out('reaxffbocut_sigma')
#endif
!
  return
  end
...
!
  ...
  real(dp)                            :: rrp1
#ifdef TRACE
  call trace_in('reaxffbocut_pipi')
#endif
!
!  If radii for either species is >= 0, then term is zero
!
...
#ifdef TRACE
  call trace_out('reaxffbocut_pipi')
#endif
!
  return
  end
