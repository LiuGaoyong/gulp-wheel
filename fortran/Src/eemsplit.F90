...
!
    do i = 1,neembond
      ib1 = neembonded(1,i)
      ib2 = neembonded(2,i)
      z(i) = z(i) - extpot(nrelf2a(ib1))
      z(i) = z(i) + extpot(nrelf2a(ib2))
    enddo
    if (lmultiqrange) then
      do i = 1,neembond
        ib1 = neembonded(1,i)
...
!
      call dcgsolve(neembond,neembond,nlocptr,ematb,neembond,z,qbond,qitertol,nqitermax,iter,err)
!
      if (ioproc) then
...
!******************
      ...
      call matrix_inversion_library(n,1_i4,neembond,nblocksize,ematb,0_i4,lsymmetric,ifail)
!
...
!
    eself = eself + qi*extpot(nrelf2a(ib1))
    eself = eself - qi*extpot(nrelf2a(ib2))
!
    nregioni = nregionno(nrelf2a(ib1))
    eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + eself - eself_before
    nregioni = nregionno(nrelf2a(ib2))
    eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + eself - eself_before
!
