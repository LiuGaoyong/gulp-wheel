...
!   7/12 Default k point for frequency observable made 1
!   1/13 Bug in normalisation of input eigenvectors corrected
!   5/13 Volume observable added
!   7/13 Setting of lprop flag removed as it is not needed
!  11/13 Strain derivatives created as a separate observable from stress
!  11/16 End trapping added to error trapping on line reads
!   7/17 Absolute dipole observable added
!  10/17 Free energy added as an observable
...
!   7/21 Modified so that split works even without variable option 
!   7/21 feqp added
!  11/22 nobstyle added
!  11/22 freaction added as an observable
!   7/23 Check on au keyword corrected
!
...
!  Local variables
!
  character(len=maxwordlength)                 :: word2
  integer(i4)                                  :: i
  integer(i4)                                  :: iadd
  integer(i4)                                  :: ii
  integer(i4)                                  :: ind
...
!
  ...
  if (index(word,'stra').eq.1) goto 261
  ...
  if (index(word,'absd').eq.1) goto 261
  if (index(word,'sqom').eq.1) goto 261
  if (index(word,'mode ').eq.1) goto 261
  if (index(word,'vol').eq.1) goto 261
  if (index(word,'fene').eq.1) goto 261
  if (index(word,'feqp').eq.1) goto 261
  if (index(word,'frea').eq.1) goto 261
  if (index(word,'frxe').eq.1) goto 261
  ...
  if (index(word,'split').eq.1) goto 291
  return
!********************
!  Default weights  *
!********************
250 continue
  if (nword.ge.2.and.nfloat.ge.1) then
    call stolc(words(2),maxwordlength)
    ...
    elseif (index(words(2),'dipo').eq.1) then
      delwht_dipole = abs(floats(1))
    ...
    elseif (index(words(2),'qtn').eq.1) then
      delwht_qtn = abs(floats(1))
    elseif (index(words(2),'rad').eq.1) then
      delwht_radius = abs(floats(1))
    elseif (index(words(2),'stra').eq.1) then
      delwht_strain = abs(floats(1))
    elseif (index(words(2),'stre').eq.1) then
...
! 28 => Dipole moment (absolute)
! 29 => Mode (frequency with eigenvector)
! 30 => Volume
! 31 => Stresses
! 32 => Free energy
! 33 => Free energy (equipartition)
! 34 => Reaction free energy
! 35 => Reaction free energy (equipartition)
!
260 line = '  '
  read(nru,'(a)',err=98,end=98) line
  iline = iline + 1
  if (index(line,'#').eq.1) goto 260
  call linepro(nru,line,iline)
261 continue
  do i = 1,nword
    call stolc(words(i),maxwordlength)
  enddo
  word = words(1)
...
!
    ...
    if (nword.gt.1) then
      do n = 2,nword
        if (index(words(n),'au ').eq.1) then
          units = autoev
        elseif (index(words(n),'kc').eq.1) then
          units = kcaltoev
        elseif (index(words(n),'kj').eq.1) then
          units = kjmtoev
        elseif (index(words(n),'pos').eq.1) then
          nobstyle(nobs) = 1
        elseif (index(words(n),'neg').eq.1) then
          nobstyle(nobs) = -1
        endif
      enddo
    endif
    ...
    if (nword.ge.1) then
      call stolc(words(1),maxwordlength)
...
!
    ...
    if (nword.gt.1) then
      do n = 2,nword
        if (index(words(n),'pos').eq.1) then
          nstyle = 1
        elseif (index(words(n),'neg').eq.1) then
          nstyle = -1
        endif
...
!
    ...
    if (nword.gt.1) then
      ...
      elseif (index(words(2),'au').eq.1) then
        units = autoev
      elseif (index(words(2),'sum').eq.1) then
        lsumcoreshell(ncurr) = .true.
      endif
    endif
265 line = '  '
    read(nru,'(a)',err=98,end=98) line
...
!
    ...
    enddo
    goto 260
  elseif (index(word,'stra').eq.1) then
!
!  Strain derivatives
!
    units = 1.0_dp
267 line = '  '
    ...
    nfstrain = nfstrain + 1
    if (nfstrain.gt.maxfstrain) then
      maxfstrain = nfstrain + 10
      call changemaxfstrain
    endif
    if (nfloat.lt.2) then
      call outerror('Insufficient info specified for strain derivatives',iline)
      call stopnow('fitword')
    endif
    nfstraincfg(nfstrain) = ncurr
    nfstraint(nfstrain) = nint(floats(1))
    fstrain(nfstrain) = floats(2)
    if (nfloat.ge.3) then
      fstrainweight(nfstrain) = abs(floats(3))
    else
      fstrainweight(nfstrain) = delwht_strain
    endif
    goto 267
...
!
    if (ndimen(ncurr).eq.0) then
      call outerror('Stress observable given for a non periodic case',iline)
      call stopnow('fitword')
    endif
    n = 1
    if (nfloat.ge.1) then
    ...
    do k = 1,n
      ...
      call linepro(nru,line,iline)
      if (nfloat.lt.2) then
        call outerror('Missing data for stress input',iline)
        call stopnow('fitword')
      endif
      i = nint(floats(1))
      ...
      if (nfloat.ge.3) then
        ...
        weight(nobs) = delwht_stress
      endif
!
!  Check that stress pointer is consistent with dimensionality
!
      if (ndimen(ncurr).eq.3) then
        if (i.lt.1.or.i.gt.6) then
          call outerror('Stress pointer is outside allowed range of 1-6 for 3-D',iline)
          call stopnow('fitword')
        endif
      elseif (ndimen(ncurr).eq.2) then
        if (i.lt.1.or.i.gt.3) then
          call outerror('Stress pointer is outside allowed range of 1-3 for 2-D',iline)
          call stopnow('fitword')
        endif
      elseif (ndimen(ncurr).eq.1) then
        if (i.ne.1) then
          call outerror('Stress pointer is not equal to allowed value of 1 for 1-D',iline)
          call stopnow('fitword')
        endif
      endif
      nobptr(nobs) = i
    ...
    goto 260
  elseif (index(word,'fene').eq.1) then
!
!  Free energy value
!
    nobs = nobs + 1
    ...
    goto 260
  elseif (index(word,'feqp').eq.1) then
!
!  Free energy (equipartition) value
!
    nobs = nobs + 1
    if (nobs.gt.maxobs) then
      maxobs = nobs + 10
...
!
    ...
    units = 1.0_dp
    if (index(keyword,' au').ne.0.or.index(keyword,'au').eq.1) then
      units = autoangs
    endif
    do k = 1,n
      nobs = nobs + 1 
    ...
    if (nword.gt.1) then
      do nw = 2,nword
        if (index(words(nw),'au ').eq.1) then
          units = autoev
        elseif (index(words(nw),'kc').eq.1) then
          units = kcaltoev
        elseif (index(words(nw),'kj').eq.1) then
          units = kjmtoev
        elseif (index(words(nw),'pos').eq.1) then
          nstyle = 1
        elseif (index(words(nw),'neg').eq.1) then
          nstyle = -1
        endif
      enddo
    endif
...
!
    ...
    enddo
    goto 260
  elseif (index(word,'frea').eq.1) then
!     
...
!
    do l = 1,n
      ...
      call linepro(nru,line,iline)
      if (nfloat.lt.1) then
        call outerror('Missing data for reaction free energy observable input',iline)
        call stopnow('fitword')
      endif
!
!  Set number of reaction configurations to be read in and check that there is sufficient data
!
    ...
    enddo
    goto 260
  elseif (index(word,'frxeq').eq.1) then
!     
!  Reaction energy
!     
    n = 1
...
!
    ...
    enddo
    goto 260
  elseif (index(word,'absd').eq.1) then
!
!  Absolute dipole moment
!
    if (ndimen(ncurr).ne.0) then
      call outerror('Dipole moment observable given for non 0-D case',iline)
      call stopnow('fitword')
    endif
    nobs = nobs + 1
    if (nobs.gt.maxobs) then
    ...
    if (nfloat.eq.0) then
      ...
      call linepro(nru,line,iline)
      if (nfloat.lt.1) then
        call outerror('Missing data in absolute dipole moment input',iline)
        call stopnow('fitword')
      endif
    endif
    fobs(nobs) = floats(1)*units
...
!
        normalise = 1.0_dp/sqrt(normalise)
        do i = 1,nobsmodeat(nobsmode)
          fobsmode(1,i,nobsmode) = normalise*fobsmode(1,i,nobsmode)
          fobsmode(2,i,nobsmode) = normalise*fobsmode(2,i,nobsmode)
          fobsmode(3,i,nobsmode) = normalise*fobsmode(3,i,nobsmode)
        enddo
        goto 261
      elseif (nfloat.eq.0) then
!
!  No numbers on line means that this is the end of the eigenvector input
!
!
!  Enforce normalisation of eigenvectors
!
        normalise = 1.0_dp/sqrt(normalise)
        do i = 1,nobsmodeat(nobsmode)
          fobsmode(1,i,nobsmode) = normalise*fobsmode(1,i,nobsmode)
...
!
      normalise = normalise + fobsmode(1,nobsmodeat(nobsmode),nobsmode)**2 + &
                              fobsmode(2,nobsmodeat(nobsmode),nobsmode)**2 + &
                              fobsmode(3,nobsmodeat(nobsmode),nobsmode)**2
    enddo
    goto 260
...
263 call outerror('S(Q,omega) file cannot be opened',iline)
    call stopnow('fitword')
  elseif (index(word,'vol').eq.1) then
!
!  Volume
!
    if (ndimen(ncurr).ne.3) then
      call outerror('Volume observable given for non 3-D case',iline)
      call stopnow('fitword')
    endif
    nobs = nobs + 1
    if (nobs.gt.maxobs) then
    ...
    if (nfloat.eq.0) then
      ...
      call linepro(nru,line,iline)
      if (nfloat.lt.1) then
        call outerror('Missing data in volume input',iline)
        call stopnow('fitword')
      endif
    endif
    fobs(nobs) = floats(1)
    ...
    do while (nread.lt.nin)
      ...
      do while (i.le.ntot)
        if (nlorder(i).eq.1) then
          nwo = nwo + 1
          word2 = words(nwo)
          call stolc(word2,maxwordlength)
...
!#################################
      ...
      do i = 1,nread
        ...
        if (nword.gt.0) then
          do ii = 1,nword
            call stolc(words(ii),maxwordlength)
          enddo
        endif
...
!
            ind1 = int(floats(1))
            ind2 = int(floats(2))
            ncvarindcfg(ncontot) = ind1
            ncvartypcfg(ncontot) = iopt_radius
            ncfixindcfg(ncontot) = ind2
            ncfixtypcfg(ncontot) = iopt_radius
          elseif (index(words(1),'co').ne.0) then
!
!  Centre of mass constraint
!
            ncvarindcfg(ncontot) = int(floats(1))
            ncfixindcfg(ncontot) = int(floats(2))
            if (index(words(1),'x').eq.1) then
              ncvartypcfg(ncontot) = iopt_xcom
            elseif (index(words(1),'y').eq.1) then
              ncvartypcfg(ncontot) = iopt_ycom
            elseif (index(words(1),'z').eq.1) then
              ncvartypcfg(ncontot) = iopt_zcom
            else
              call outerror('Missing data for constraint',iline)
              call stopnow('fitword')
            endif
            if (index(words(2),'x').eq.1) then
              ncfixtypcfg(ncontot) = iopt_xcom
            elseif (index(words(2),'y').eq.1) then
              ncfixtypcfg(ncontot) = iopt_ycom
            elseif (index(words(2),'z').eq.1) then
              ncfixtypcfg(ncontot) = iopt_zcom
            else
              call outerror('Missing data for constraint',iline)
              call stopnow('fitword')
            endif
          elseif (index(words(1),'qt').ne.0) then
!
...
!
            ...
            ncfixindcfg(ncontot) = int(floats(2))
            if (index(words(1),'x').eq.1) then
              ncvartypcfg(ncontot) = iopt_xqtn
            elseif (index(words(1),'y').eq.1) then
              ncvartypcfg(ncontot) = iopt_yqtn
            elseif (index(words(1),'z').eq.1) then
              ncvartypcfg(ncontot) = iopt_zqtn
            else
              call outerror('Missing data for constraint',iline)
              call stopnow('fitword')
            endif
            if (index(words(2),'x').eq.1) then
              ncfixtypcfg(ncontot) = iopt_xqtn
            elseif (index(words(2),'y').eq.1) then
              ncfixtypcfg(ncontot) = iopt_yqtn
            elseif (index(words(2),'z').eq.1) then
              ncfixtypcfg(ncontot) = iopt_zqtn
            else
              call outerror('Missing data for constraint',iline)
              call stopnow('fitword')
            endif
          elseif (index(words(1),'ce').eq.1) then
!
!  Cell constraint
!
            ncvarindcfg(ncontot) = int(floats(1))
            ncvartypcfg(ncontot) = iopt_cell
            ncfixindcfg(ncontot) = int(floats(2))
            ncfixtypcfg(ncontot) = iopt_cell
          elseif (index(words(1),'s').eq.1) then
!
!  Strain constraint
!
            ncvarindcfg(ncontot) = int(floats(1))
            ncvartypcfg(ncontot) = iopt_strain
            ncfixindcfg(ncontot) = int(floats(2))
            ncfixtypcfg(ncontot) = iopt_strain
          else
!
...
!
            ...
            ind2 = int(floats(2)) - 1
            if (index(words(1),'x').eq.1) then
              ncvartypcfg(ncontot) = iopt_xf
            elseif (index(words(1),'y').eq.1) then
              ncvartypcfg(ncontot) = iopt_yf
            elseif (index(words(1),'z').eq.1) then
              ncvartypcfg(ncontot) = iopt_zf
            else
              call outerror('Missing data for constraint',iline)
              call stopnow('fitword')
            endif
            if (index(words(2),'x').eq.1) then
              ncfixtypcfg(ncontot) = iopt_xf
            elseif (index(words(2),'y').eq.1) then
              ncfixtypcfg(ncontot) = iopt_yf
            elseif (index(words(2),'z').eq.1) then
              ncfixtypcfg(ncontot) = iopt_zf
            else
              call outerror('Missing data for constraint',iline)
              call stopnow('fitword')
            endif
...
!
        if ((ncvartypcfg(ncontot).eq.iopt_strain.and.ncfixtypcfg(ncontot).ne.iopt_strain) &
        .or.(ncvartypcfg(ncontot).ne.iopt_strain.and.ncfixtypcfg(ncontot).eq.iopt_strain)) then
