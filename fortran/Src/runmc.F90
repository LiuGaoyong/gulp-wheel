...
!   9/12 Call to dumpdur moved from outmc to dumpdur
!   4/16 nmcswappair added
!   5/16 Multiple mcswaps added
!   9/16 Deallocation of nptrswapable removed since this is no longer needed
!   8/17 Call to flush renamed to gflush
!   1/18 Trace added
!   3/19 Multiple temperature ramps added
!   3/19 Temperature ramp now uses current arrays
!  10/21 Transform added 
!  11/23 Trap to prevent GFN-FF with GCMC added
!
!  Conditions of use:
!
...
!
  ...
  integer(i4)                              :: nswap
  integer(i4)                              :: nswap2try
  integer(i4)                              :: nswaptried
  integer(i4)                              :: ntran
  integer(i4)                              :: ntran2try
  integer(i4)                              :: ntrantried
  integer(i4)                              :: ntest
  integer(i4)                              :: ntr
  integer(i4)                              :: status
  logical                                  :: laccept
...
!  
!  GCMC not allowed for GFN-FF
!  
  if (lgfnff.and.((pcreate+pdestroy).gt.0.0)) then
    call outerror('GCMC not only currently allowed for GFN-FF',0_i4)
    call stopnow('runmc')
  endif
!
...
!  ntran         = number of accepted transform trials so far
...
!  ntrantried    = number of attempted transform trials so far
!
  naccepted = 0
  nacceptedlasttime = 0
  ncreate = 0
...
!
  ptotal = pcreate + pdestroy + pmove + protate + pstrain + pswapsum + ptransum
  plcreate = pcreate/ptotal
  pldestroy = plcreate + pdestroy/ptotal
  plmove = pldestroy + pmove/ptotal
  plrotate = plmove + protate/ptotal
  plswap = plrotate + pswapsum/ptotal
  pltran = plswap + ptransum/ptotal
  plstrain = 1.0_dp
!
...
!
  do ns = 1,nmcswaps
    ...
    call setswapptr(ns)
!
!  Check that number of pairs to be swapped doesn't exceed the number available
!
    if (2*nmcswappair(ns).gt.nswapable(ns)) then
      call outerror('number of swapable atoms is less than number of pairs to swap',0_i4)
      call stopnow('runmc')
    endif
  enddo
!
!  Setup pointer to transformable atoms
!
  do ns = 1,nmctrans
    ntranable(ns) = 0
    call settranptr(ns)
...
!
  lvariableT = (ntemperatureramp.gt.0)
!
...
!
    ...
    write(ioout,'(''  Probability of species change = '',f12.6)') pltran-plswap
    write(ioout,'(''  Probability of strain         = '',f12.6)') plstrain-pltran
    ...
    write(ioout,'(''  Number of possible swaps      = '',i12)') nmcswaps
    do ns = 1,nmcswaps
      write(ioout,'(''  Pairs for swap '',i11,''    = '',i12)') ns,nmcswappair(ns)
    enddo
    write(ioout,'(''  Number of possible trans      = '',i12)') nmctrans
...
!
    if (lvariableT) then
      do ntr = 1,ntemperatureramp
        if (nmcstep.gt.ntemperaturestepstart(ntr).and.nmcstep.le.ntemperaturestepstop(ntr)) then
          temperature = temperaturestart(ntr) + dble(nmcstep-ntemperaturestepstart(ntr))*temperaturestep(ntr)
          betamc = evtoj/(boltz*temperature)
          debrogliefct = 2.0_dp*pi*1.0d-3*boltz*temperature/(avogadro*(planck**2))
          debrogliefct = debrogliefct**1.5_dp
          cpfactor = exp(chempot*betamc)*debrogliefct*volm3
...
!  Find specific swap
!
        if (nmcswaps.eq.1) then
          nswap2try = 1
        else
          nswap2try = 0
          lfound = .false.
          ns = 0
          do while (ns.lt.nmcswaps.and..not.lfound)
            ns = ns + 1
            if (randnum.le.(plrotate+(pswap(ns)/ptotal))) then
              lfound = .true.
              nswap2try = ns
            endif
          enddo
          if (nswap2try.eq.0) nswap2try = nmcswaps
        endif
      elseif (pcreate.gt.0.0) then
        noperation = 1
        ncreatetried = ncreatetried + 1
    ...
    elseif (randnum.le.pltran) then
      if (nmctrans.gt.0) then
        noperation = 7
        ntrantried = ntrantried + 1
!
!  Find specific transformation
!
        if (nmctrans.eq.1) then
          ntran2try = 1
        else
          ntran2try = 0
          lfound = .false.
          ns = 0
          do while (ns.lt.nmctrans.and..not.lfound)
            ns = ns + 1
            if (randnum.le.(plswap+(ptran(ns)/ptotal))) then
              lfound = .true.
              ntran2try = ns
            endif
          enddo
          if (ntran2try.eq.0) ntran2try = nmctrans
        endif
      elseif (pcreate.gt.0.0) then
        noperation = 1
        ncreatetried = ncreatetried + 1
...
!
    ...
    elseif (noperation.eq.6) then
      call mcswap(1_i4,nswap2try)
    elseif (noperation.eq.7) then
      call mctran(1_i4,ntran2try)
    else
      call mcstrain(1_i4)
    endif
    t3 = g_cpu_time()
...
!
    ...
    elseif (noperation.eq.4) then
      call mcrotate(2_i4)
    elseif (noperation.eq.6) then
      call mcswap(2_i4,nswap2try)
    elseif (noperation.eq.7) then
      call mctran(2_i4,ntran2try)
    endif
!
!  Convert linear structure array to main structure arrays
!
...
!
      ...
      elseif (noperation.eq.4) then
        call mcrotate(3_i4)
      elseif (noperation.eq.6) then
        call mcswap(3_i4,nswap2try)
      elseif (noperation.eq.7) then
        call mctran(3_i4,ntran2try)
      else
        call mcstrain(2_i4)
      endif
!
...
!
    if (idump.gt.0) then
      ntest = nmcstep/ncycd
      ntest = nmcstep - ntest*ncycd
      if (ntest.eq.0) then
        call dumpdur(idump,nmcstep)
        call gflush(idump)
      endif
    endif
!
...
!
    ...
    if (ntrantried.gt.0) then
      percenttran = dble(ntran)/dble(ntrantried)*100.0_dp
      write(ioout,'('' Transform  '',8x,i12,8x,i12,8x,f9.4)') ntrantried,ntran,percenttran
    endif
