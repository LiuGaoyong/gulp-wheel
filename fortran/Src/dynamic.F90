...
!  12/21 Valence bond contribution added
!  12/21 pGFNFF modifications added
!   1/22 Calls to d2charge_finish moved here from realp
!   1/22 dedqc and d2edqc initialised
!   5/22 Alternative routine added to gfnff_phond for better scaling in dispersion
!  12/22 m_qderiv added
!   4/23 Call to real space sum now only called if lewald, lwolf or npote > 0
!   4/23 ReaxFF added
!   5/23 Sparse reaxff call added
!   7/23 Memory checks added for size of charge derivative arrays
!
!  Conditions of use:
!
...
!
  ...
  use ksample
  use ksample_scatter
  use m_qderiv,          only : d2charge_finish, d2chargepd_finish
  use parallel,          only : nprocs, natomsonnode, lscaleMPI
...
!
      ...
      if (nprocs.gt.1) then
        call dcharged(.false.,emat,nmax,.true.,.false.,.true.)
      else
...
!
  ...
  enddo
!
!  Variable charge derivatives
!
  if (lDoQDeriv2.and..not.loldd2q) then
    d2edq2(1:numat) = 0.0_dp
    d2edqdq(1:numat,1:numat) = 0.0_dp
    if (nprocs.gt.0) then
...
!*******************************************************************************
  if (lgfnff) then
    if (nprocs.gt.1) then
      if (lscaleMPI) then
        call gfnff_phond_s(xkv,ykv,zkv)
      else
        call gfnff_phond(xkv,ykv,zkv)
      endif
    else
      call gfnff_phon(xkv,ykv,zkv)
    endif
  endif
!*******************************
...
!**********************
  if (lreaxFF) then
    if (nprocs.gt.1) then
      call reaxffds_phon(xkv,ykv,zkv)
    else
      call reaxffs_phon(xkv,ykv,zkv)
    endif
  endif
!***************************************
...
!****************************
  if (nvalener.gt.0) then
    if (nprocs.gt.1) then
      call valencebondpd(xkv,ykv,zkv)
    else
      call valencebondp(xkv,ykv,zkv)
    endif
  endif
!*****************************
