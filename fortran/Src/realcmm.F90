...
!
!  Zero moments - quadupole moments store in linear form:
!
...
!
!  Moments beginning with "q" are of the energy, while those starting with "f" 
!
    qmonopole = 0.0_dp
    if (icmm.gt.1) then
      qdipolex = 0.0_dp
    ...
    if (lgrad1) then
!
!  Don't need separate storage for the following force terms as they are trivially related to
!  the energy terms (the last factor of two is to compensate for the factor of half for double
!  counting in the energy which doesn't apply to the forces):
!
!  fmonopole = - qmonopole*2.0_dp
!  fdipole = - 2.0_dp*qdipole*2.0_dp
!
      if (icmm.gt.2) then
        do k = 1,6
...
!
!  Scale moments by factor of half to allow for double counting. 
!  For the quadrupole there is also the factor of half in the normal expression.
!
...
!
!  Decide whether the box j interacts with box i by multipole moments or explicitly.
!
      ldofull = .true.
...
!
        ...
        do i = ilower,nbati
          ...
          if (leem.and.lmultiqrange.and.neemrptr(ii).ne.0) then
            nqri = nqrnow(neemrptr(ii))
          else
            nqri = 1
          endif
!
...
!
            ...
            fct = ofct*angstoev
            factor = qli*qlj*fct
            if (leem.and.lmultiqrange.and.neemrptr(jj).ne.0) then
              nqrj = nqrnow(neemrptr(jj))
            else
              nqrj = 1
            endif
!
...
!*************************************************
!
!  Set core-shell to true to handle case where defect ion is close to a perfect lattice site
!
            eatom_before = eatom
            ereal_before = ereal
            ec6_before   = ec6
...
!
            ...
            if (leem.or.lgfnff) then
              ...
              elseif (lgfnff) then
                ...
                call gfnffbody1(eqeq,zetaij,lgrad1,.false.,nor,1_i4,dist,deriv,deriv2, &
                                fct,qli,qlj,d1i,d1j,d2i2,d2ij,d2j2)
              endif
!
...
!
          do i = 1,nbati
            ii = iptri(i)
            lbreathe = lbsmat(nrelf2a(ii))
            lopi = (.not.lfreeze.or.lopf(ii))
            nati = nat(ii)
            ntypi = nftype(ii)
            oci = occuf(ii)
            radi = radf(ii)
            if (lbreathe.and.lopi) then
              eatom_before = eatom
...
!
            ...
            if (icmm.gt.2) then
              rrx5 = rrx2*rrx3
!
!  Quadrupole - off diagonals multiplied by two as we are working in lower-half triangular form
!
