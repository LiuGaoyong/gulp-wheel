  subroutine twobody(eatom,ereal,ec6,lgrad1,lgrad2,lgrad3,nor,nor0,npots,npotl, &
                     cut2r,cut2e,cut2s,nmolonly,factorin,sfct,spfct,radsum, &
                     sctrm1,sctrm2,qli,qlj,nsi,nsj,lcspair,lperiodic,lregion1, &
                     lskipsr,lskipq)
!
...
!  derive0 = on return contains electrostatic energy component without the charges
...
!  factorin= product of charges and unit conversion factor (includes sfct)
!  sfct    = symmetry equivalence factor (includes oci*ocj)
!  spfct   = symmetry equivalence factor for spring constant (includes oci)
...
!  qli     = charge of ion i in pair
!  qlj     = charge of ion j in pair
!  nsi     = species number of i (currently only used during MD for TI)
!  nsj     = species number of j (currently only used during MD for TI)
...
!  11/12 Original Wolf modified to match F&G correction change
!   7/13 Modifed to include a separate occupancy weighting factor for the core-shell spring
!        constant
!   3/14 derfc changed to g_derfc for benefit of ChemShell
!   3/14 lorder12 removed from argument list
!   8/14 Baskes a4 potential added
!   8/14 Baskes potentials now handled via a call to a 
!        subroutine.
!   8/14 Taper range passed to eamrho/meamrho
!   8/14 Baskes potentials removed from this routine
!   9/14 ZBL potential added
!  12/14 rtrm1 changed from scalar to array
!   2/15 MM3buck added
!   3/15 MM3stretch added
!   3/15 MM3stretch modified 
!   3/15 Modified for x14 option
!   3/15 lnoqeem added
!   8/15 Bug corrected in third derivatives of sw2 potential
!   8/15 Rmax for sw2 defined using local potential value instead of cut2r
!  10/16 Lennard-Jones with fixed coefficients added
!   5/17 Modified to handle lnewdefalg case where different handling of 
!        small distances is required. 
!   6/17 Correction to logical for mmexc = 5 made
!  11/17 Buffered_LJ potential added
!   2/18 Trace added
!   2/18 Switch of lines for small distance handling
!   2/18 Slater potential added
!   6/18 Coulomb subtract parameter moved to twobody 1
!   7/18 Correction to allow the Becke-Johnson potential to be tapered
!   8/18 7th order polynominal taper added
!   8/18 Energy shift added for epsilon-sigma Lennard-Jones
!   5/19 Check on whether polarisation is being used added before skipping electrostatics
!   6/19 j2 potential added
!   8/19 Damping option added to electric field for polarisation
!   3/20 General form of sw2 added
!   3/20 Location of angstoev changed to current
!   3/20 Screened Coulomb potential added
!  11/21 Modifications for TI added
!  11/21 Species numbers added to input arguments
!  11/21 qtaper and qerfc changed
!   8/22 OpenMM taper added
!   6/23 AA-CLP twobody added
!
!  Conditions of use:
!
...
!
  use g_constants
  use control
  use current,       only : lewald, ndim, angstoev, dielectric
  use eam
  use general,       only : cutw, etaw, rkw, selfwolf, selfwolfforce
  use kspace
  use m_aaclp
  use m_eht,         only : nvalenceelectron, npqn
  use m_ti
  use molecule
  use numbers,       only : sixth, third
  use polarise,      only : lpolar, lpoldamp, bpdamp, rpdamp
  use realvectors
  use species
  use splinedata
#ifdef TRACE
...
!
  ...
  integer(i4),    intent(in)      :: nsi
  integer(i4),    intent(in)      :: nsj
...
!
!  Set charges as local variables in case of lambda scaling
!
  qi = qli
  qj = qlj
!
...
!
          if (lvalid) then
            ...
            if (mmexc(npot).eq.6.and.l3bonds(k)) lvalid = .false.
...
!
            cpt = twopot(3,npot)*twopot(4,npot)*sfct*twopot(5,npot)**6
            c6tot = c6tot + cpt
          endif
        enddo
...
!
          if (lvalid) then
            ...
            if (mmexc(npot).ge.2.and.lbonded(k)) lvalid = .false.
            if (mmexc(npot).ge.3.and.l2bonds(k)) lvalid = .false.
            if (mmexc(npot).ge.6.and.l3bonds(k)) lvalid = .false.
            if (mmexc(npot).eq.4.and..not.l3bonds(k)) lvalid = .false.
!
!  Bond order checks for validity
!
...
!
!  NB: During individual potential calculation the following definitions apply:
!
!  derivk  => (1/r)(dE/dR)
!  deriv2k => (d2E/dR2)
!  deriv3k => (d3E/dR3)
...
!*********************************************
                ...
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl + apt*(trm1 - ept)
                    apt = apt*lambda
                    derivk = derivk*lambda
                  elseif (ltib2body(npot)) then
                    dUdl = dUdl - apt*(trm1 - ept)
                    apt = apt*(1.0_dp - lambda)
                    derivk = derivk*(1.0_dp - lambda)
                  endif
                endif
...
!***********************************************
                ...
                trm1 = trm1*trm1
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rk*twopot(4,npot)*pscale*dUdl_f
                  if (ltif2body(npot)) then
                    dUdl = dUdl + apt*(trm1 - ept)
                    apt = apt*lambda
                    derivk = derivk*lambda
...
!*******************************************
                ...
                if (cpt.ne.0.0_dp) then
                  ...
                  if (lti) then
                    if (ltif2body(npot)) then
                      dUdl = dUdl + sixth*cpt*rd2*rd
                      cpt = cpt*lambda
                    elseif (ltib2body(npot)) then
                      dUdl = dUdl - sixth*cpt*rd2*rd
                      cpt = cpt*(1.0_dp - lambda)
                    endif
                  endif
                ...
                if (dpt.ne.0.0_dp) then
                  ...
                  if (lti) then
                    if (ltif2body(npot)) then
                      dUdl = dUdl + 0.25_dp*sixth*dpt*rd2*rd2
                      dpt = dpt*lambda
                    elseif (ltib2body(npot)) then
                      dUdl = dUdl - 0.25_dp*sixth*dpt*rd2*rd2
                      dpt = dpt*(1.0_dp - lambda)
                    endif
                  endif
...
!*********************************************
                ...
                rd2 = rd*rd
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rk*tpot(1,npot)*pscale*dUdl_f
                  if (ltif2body(npot)) then
                    dUdl = dUdl + 0.5_dp*apt*rd2
                    apt = apt*lambda
                  elseif (ltib2body(npot)) then
...
!*****************************************
                apt = twopot(1,npot)*spfct
                bpt = twopot(2,npot)*r24*spfct
                trm = bpt*r2
...
!**************************
                dpte = twopot(1,npot)*pscale
                dpt = factor*dpte
                dpte = dpte*sfct
                ereal = ereal - dpt*rk
!
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rk*twopot(1,npot)*pscale*dUdl_f
                endif
!
                if (lgrad1) then
                  derive0(k) = derive0(k) - dpte*rk
...
!*********************
                ...
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + dfun(1)*psfct*rtistep
                    efct = efct*lambda
                  elseif (ltib2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - dfun(1)*psfct*rtistep
                    efct = efct*(1.0_dp - lambda)
                  endif
                endif
                eatom = eatom + dfun(1)*psfct*efct
                if (lgrad1) deriv(k) = deriv(k) + dfun(2)*rk*psfct*efct
                if (lgrad2) deriv2(k) = deriv2(k) + dfun(3)*psfct*efct
...
!**************************************
                apt = twopot(1,npot)*psfct
                bpt = twopot(2,npot)        ! bpt = rho
                cpt = twopot(3,npot)        ! cpt = B
                rmax = rpot(npot)
                if (r.lt.rmax) then
                  trm1 = 1.0_dp/(r-rmax)    ! trm1 = 1/(r - rmax)
                  apt = apt*exp(bpt*trm1)   ! apt = A*exp(rho/(r - rmax))
                  trm2 = rk2*rk2*cpt        ! trm2 = B/r**4
                else
                  apt = 0.0_dp
                  trm1 = 0.0_dp
                  trm2 = 0.0_dp
                endif
                trm3 = trm2 - 1.0_dp        ! trm3 = (B/r**4 - 1)
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + apt*trm3*rtistep
                    apt = apt*lambda
                  elseif (ltib2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - apt*trm3*rtistep
                    apt = apt*(1.0_dp - lambda)
                ...
                if (lgrad1) then
                  trm4 = trm1*trm1          ! trm4 = 1/(r - rmax)**2
                  trm6 = 4.0_dp*trm2*rk     ! trm6 = 4*B/r**5
                  deriv(k) = deriv(k) - apt*rk*(bpt*trm4*trm3 + trm6)
                  if (lgrad2) then
                    trm5 = trm4*trm4        ! trm5 = 1/(r - rmax)**4
                    trm7 = 5.0_dp*trm6*rk
                    deriv2(k) = deriv2(k) + apt*(bpt*bpt*trm5*trm3 + &
                                                 2.0_dp*bpt*trm4*trm6 + &
                                                 2.0_dp*bpt*trm4*trm1*trm3 + &
                                                 trm7)
                    if (lgrad3) then
                      deriv3(k) = deriv3(k) - apt*(6.0_dp*bpt*trm5*trm3 + &
                                                   6.0_dp*bpt*bpt*trm5*trm1*trm3 + &
                                                   6.0_dp*bpt*trm4*trm1*trm6 + &
                                                   bpt*bpt*bpt*trm4*trm5*trm3 + &
                                                   3.0_dp*bpt*bpt*trm5*trm6 + &
                                                   3.0_dp*bpt*trm4*trm7 + &
                                                   6.0_dp*trm7*rk)
                    endif
                  endif
                endif
...
!
                  ...
                  if (lti) then
                    if (ltif2body(npot)) then
                      dUdl = dUdl - apt*rk6 - bpt*rk8 - cpt*rk10
                      apt = apt*lambda
                      bpt = bpt*lambda
                      cpt = cpt*lambda
                    elseif (ltib2body(npot)) then
...
!
                  if (lti) then
                    if (ltif2body(npot)) then
                      dUdl = dUdl - apt*rk6*f6 - bpt*rk8*f8 - cpt*rk10*f10
                      apt = apt*lambda
                      bpt = bpt*lambda
                      cpt = cpt*lambda
                    elseif (ltib2body(npot)) then
...
!**************************
                if (.not.lMEAMden) then
                  ...
                  call eamrho(nspec1(npot),nptyp1(npot),nspec2(npot),nptyp2(npot),r,rpot(npot),xtmp(k),ytmp(k),ztmp(k), &
                              ...
                              drhoij3,drhoji3,1.0_dp,1.0_dp,.false.,.false.,.false.,.false.,twopot(1,npot))
...
!*****************************************
                ...
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - (1.0_dp + trm1)*exptrm*rtistep
                    exptrm = exptrm*lambda
                  elseif (ltib2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + (1.0_dp + trm1)*exptrm*rtistep
                    exptrm = exptrm*(1.0_dp - lambda)
                  endif
                endif
...
!
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - tpfn*rk*dUdl_f
                  if (ltif2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + (1.0_dp - tpfn)*e02*rtistep
                    e02 = e02*lambda
                  elseif (ltib2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - (1.0_dp - tpfn)*e02*rtistep
                    e02 = e02*(1.0_dp - lambda)
                  endif
                endif
!
                ereal = ereal + tpfn*e01 + (1.0_dp - tpfn)*e02
                if (lgrad1) then
                  e11 = - e01*rk
                  dtpfn = (((5.0_dp*ts5*r+4.0_dp*ts4)*r+3.0_dp*ts3)*r+2.0_dp*ts2)*r+ts1
                  derive(k) = derive(k) + (tpfn*e11 + dtpfn*e01)*rk
                  if (lgrad2) then
                    e21 = - 2.0_dp*e11*rk
                    d2tpfn = ((20.0_dp*ts5*r+12.0_dp*ts4)*r+6.0_dp*ts3)*r + 2.0_dp*ts2
                    derive2(k) = derive2(k) + tpfn*e21 + 2.0_dp*dtpfn*e11 + d2tpfn*e01
                    if (lgrad3) then
                      e31 = - 3.0_dp*e21*rk
                      d3tpfn = (60.0_dp*ts5*r+24.0_dp*ts4)*r + 6.0_dp*ts3
                      derive3(k) = derive3(k) + tpfn*e31 + 3.0_dp*dtpfn*e21 + 3.0_dp*d2tpfn*e11 + d3tpfn*e01
                    endif
                  endif
                endif
...
!**********
                apt = twopot(1,npot)
                rapt = 1.0_dp/apt
                erffc = g_derfc(r*rapt)
                trm0 = erffc*rk*factor
!
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + erffc*rk*dUdl_f
                endif
!
                ereal = ereal + trm0
                if (lgrad1) then
                  ...
                  if (lgrad2) then
                    ...
                    derive2(k) = derive2(k) + (2.0_dp*trm2 + trm3)
                    if (lgrad3) then
                      derive3(k) = derive3(k) - (rk*(6.0_dp*trm2+2.0_dp*trm3) + 2.0_dp*trm3*r*rapt2)
                    endif
                  endif
                endif
...
!  Tsuneyuki  *
!**************
                apt = twopot(1,npot)*twopot(2,npot)*psfct*angstoev
                zeta = twopot(3,npot)
                ...
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + apt*gofr*rk*rtistep
                    apt = apt*lambda
                  elseif (ltib2body(npot)) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - apt*gofr*rk*rtistep
                    apt = apt*(1.0_dp - lambda)
                  endif
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - pscale*dUdl_f*gofr*rk
                endif
                eatom = eatom + (apt - pscale*factor)*gofr*rk
                if (lgrad1) then
                  if (mpt.eq.1) then
                    dgofr = zeta*trm1
                  elseif (mpt.eq.2) then
                  ...
                  derivk = derivk - (apt - pscale*factor)*rk2*(gofr*rk - dgofr)
                  if (lgrad2) then
                    if (mpt.eq.1) then
                    ...
                    deriv2k = deriv2k + (apt - pscale*factor)*rk*(d2gofr - 2.0_dp*dgofr*rk + 2.0_dp*gofr*rk2)
                    if (lgrad3) then
                      if (mpt.eq.1) then
                      ...
                      deriv3k = deriv3k + (apt - pscale*factor)*rk*(d3gofr - 3.0_dp*rk*d2gofr + 6.0_dp*rk2*(dgofr - gofr*rk))
                    endif
                  endif
                endif
                ...
                if (lgrad1) then
                  ...
                  if (lgrad2) then
                    ...
                    deriv2(k) = deriv2(k) + apt*(bpt*bpt*trm5*trm3 + &
                                                 ...
                                                 2.0_dp*bpt*trm4*trm1*trm3 + &
                                                 trm7)
                    d1i(k) = d1i(k) + apt*rk*(bpt*trm4*trm3 + trm6)*rqtrm*rqtrm
                    d1j(k) = d1j(k) + apt*rk*(bpt*trm4*trm3 + trm6)*rqtrm*rqtrm
...
!**********************************************
                ...
                bpt = twopot(2,npot)
                cpt = cosh(r/bpt)
                eatm = eatm + apt*bpt*bpt*(cpt - 1.0_dp)
                if (lgrad1) then
                  dpt = sinh(r/bpt)
...
!
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rk2*dUdl_f
                endif
!
                eatom = eatom + trm0
                if (lgrad1) then
...
!***********
                ...
                if (r.lt.twopot(1,npot)) then
                  efct = psfct*(tpot(1,npot)*rd4 + tpot(2,npot)*rd3 + tpot(3,npot)*rd2 + &
                                tpot(4,npot)*rd  + tpot(5,npot))
                  if (lti) then
                    if (ltif2body(npot)) then
                      dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + efct*rtistep
                      efct = lambda
                    elseif (ltib2body(npot)) then
                      dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - efct*rtistep
                      efct = (1.0_dp - lambda)
...
!
                  ...
                  rd5 = rd4*rd
                  rd6 = rd5*rd
                  efct = psfct*(tpot(6,npot)*rd6 + tpot(7,npot)*rd5 + tpot(8,npot)*rd4 + &
...
!*********************************************************
                ...
                eatm = eatm + eatmtrm*etpfn*efct
                if (lgrad1) then
                  trm2 = 2.0_dp*apt*bpt*trme
                  derivktrm = trm2*trm1*rk
                  derivk = derivk + (derivktrm*etpfn + eatmtrm*detpfn*rk)*efct
                  if (lgrad2) then
                    trm2 = trm2*bpt
                    deriv2ktrm = trm2*(2.0_dp*trme-1.0_dp)
                    deriv2k = deriv2k + (deriv2ktrm*etpfn + 2.0_dp*detpfn*derivktrm*r + d2etpfn*eatmtrm)*efct
                    if (lgrad3) then
                      deriv3ktrm = trm2*bpt*(1.0_dp-4.0_dp*trme)
                      deriv3k = deriv3k + (deriv3ktrm*etpfn + 3.0_dp*(detpfn*deriv2ktrm + d2etpfn*derivktrm*r) + &
                        d3etpfn*eatmtrm)*efct
                    endif
                  endif
                endif
...
!*************
                ...
                eatm = eatm + trm0*rk*efct
                if (lgrad1) then
                  exptrm1 = exp(-(bpt*r)**2)
                  exptrm2 = exp(-(cpt*r)**2)
                  trm1 = (2.0_dp*apt/sqrtpi)*(bpt*exptrm1*erfctrm - erftrm*cpt*exptrm2)
                  derivk = derivk + (trm1*rk2 - trm0*rk2*rk)*efct
                  if (lgrad2) then
                    ...
                    deriv2k = deriv2k + (trm2*rk - 2.0_dp*trm1*rk2 + 2.0_dp*trm0*rk2*rk)*efct
                    if (lgrad3) then
                      trm3 = 2.0_dp*((bpt**5)*r*exptrm1*erfctrm - (cpt**5)*r*exptrm2*erftrm)
                      deriv3k = deriv3k + ((2.0_dp*apt/sqrtpi)*(trm1 + r*trm3 + 3.0_dp*trm2*r*(bpt*bpt + cpt*cpt))*rk)*efct
                      deriv3k = deriv3k - (3.0_dp*trm2*rk2 - 6.0_dp*trm1*rk2*rk + 6.0_dp*trm0*rk2*rk2)*efct
                    endif
                  endif
                endif
...
!*************
                ...
                if (lgrad1) then
                  exptrm1 = exp(-(bpt*r)**2)
                  derivk = derivk - apt*rk2*((2.0_dp/sqrtpi)*bpt*exptrm1 + erfctrm*rk)*efct
                  if (lgrad2) then
                    trm1 = (4.0_dp/sqrtpi)*bpt*exptrm1*(r*bpt*bpt + rk)
                    deriv2k = deriv2k + apt*rk*(trm1 + 2.0_dp*rk2*erfctrm)*efct
                    if (lgrad3) then
                      trm2 = (4.0_dp/sqrtpi)*bpt*exptrm1*(2.0_dp*r*bpt*bpt*(bpt*bpt + rk2) + 3.0_dp*rk2*rk)
                      deriv3k = deriv3k - apt*(trm2 + 6.0_dp*erfctrm*rk2*rk2)*efct
                    endif
                  endif
                endif
...
!************
                ...
                if (lgrad1) then
                  exptrm1 = exp(-(bpt*r)**2)
                  derivk = derivk + apt*rk2*((2.0_dp/sqrtpi)*bpt*exptrm1 - erftrm*rk)*efct
                  if (lgrad2) then
                    trm1 = (4.0_dp/sqrtpi)*bpt*exptrm1*(r*bpt*bpt + rk)
                    deriv2k = deriv2k - apt*rk*(trm1 - 2.0_dp*rk2*erftrm)*efct
                    if (lgrad3) then
                      trm2 = (4.0_dp/sqrtpi)*bpt*exptrm1*(2.0_dp*r*bpt*bpt*(bpt*bpt + rk2) + 3.0_dp*rk2*rk)
                      deriv3k = deriv3k + apt*(trm2 - 6.0_dp*erftrm*rk2*rk2)*efct
                    endif
                  endif
                endif
...
!
                ...
                if (lgrad1) then
                  ...
                  dfdmp = r4*r2*fdmp
                  trm1 = apt*fdmp
                  derivk = derivk + 6.0_dp*trm1*dfdmp*rk2
                  if (lgrad2) then
                    trm2 = trm1*fdmp
                    deriv2k = deriv2k + trm2*r4*(30.0_dp - 72.0_dp*dfdmp)
                    if (lgrad3) then
                      deriv3k = deriv3k + trm2*r*r2*(120.0_dp - 1080.0_dp*dfdmp + 1296.0*dfdmp**2)
                    endif
                  endif
                endif
              elseif (nptyp.eq.56) then
!******************
!  ZBL potential  *
!******************
                apt  = tpot(3,npot)*psfct   ! (Zi*Zj*e^2/4*pi*epsilon0)
                dpt  = tpot(4,npot)/twopot(9,npot) 
!
!  Exponential 1
!
                bpt  = twopot(1,npot)       ! b1
                cpt  = twopot(2,npot)*dpt   ! c1*(1/a)
                exptrm = apt*bpt*exp(-cpt*r)
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl + exptrm*rk
                    exptrm = exptrm*lambda
                ...
                eatm = eatm + exptrm*rk
                if (lgrad1) then
                  derivk = derivk - exptrm*rk2*(rk + cpt)
                  if (lgrad2) then
                    deriv2k = deriv2k + exptrm*rk*(cpt*cpt + 2.0_dp*(cpt*rk + rk2))
                    if (lgrad3) then
                      deriv3k = deriv3k - exptrm*rk*(cpt*cpt*cpt + 3.0_dp*cpt*cpt*rk + &
                                                     6.0_dp*cpt*rk2 + 3.0_dp*rk*rk2)
                    endif
                  endif
                endif
!
!  Exponential 2
!
                bpt  = twopot(3,npot)       ! b2
                cpt  = twopot(4,npot)*dpt   ! c2*(1/a)
                exptrm = apt*bpt*exp(-cpt*r)
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl + exptrm*rk
...
!  Exponential 3
!
                bpt  = twopot(5,npot)       ! b3
                cpt  = twopot(6,npot)*dpt   ! c3*(1/a)
                exptrm = apt*bpt*exp(-cpt*r)
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl + exptrm*rk
...
!  Exponential 4
!
                bpt  = twopot(7,npot)       ! b4
                cpt  = twopot(8,npot)*dpt   ! c4*(1/a)
                exptrm = apt*bpt*exp(-cpt*r)
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl + exptrm*rk
...
!  MM3 Buckingham potential  *
!*****************************
                apt = twopot(1,npot)*twopot(4,npot)*psfct
                bpt = rhopot(npot)*twopot(2,npot)
                cpt = twopot(3,npot)*twopot(4,npot)*psfct*twopot(5,npot)**6
                if (r.lt.repcut(npot)) then
                  trm1 = apt*exp(-bpt*(r-radsum))
                  if (lti) then
                    if (ltif2body(npot)) then
              ...
              elseif (nptyp.eq.58) then
!************************************************
!  MM3 stretch potential - with coulomb offset  *
!************************************************
                apt = twopot(1,npot)*twopot(3,npot)*psfct
                bpt = twopot(2,npot)
                tpte = tpot(1,npot)*pscale
                ...
                tpte = tpte*sfct
                cpt = twopot(4,npot)
                dpt = twopot(5,npot)*cpt
                rd = r - bpt
                rd2 = rd*rd
                trm1 = (1.0_dp - cpt*rd*(1.0_dp - dpt*rd))
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rk*tpot(1,npot)*pscale*dUdl_f
                  if (ltif2body(npot)) then
                    dUdl = dUdl + apt*rd2*trm1
                    apt = apt*lambda
                  elseif (ltib2body(npot)) then
                    dUdl = dUdl - apt*rd2*trm1
                    apt = apt*(1.0_dp - lambda)
                ...
                eatm = eatm + apt*rd2*trm1
                ereal = ereal - tpt*rk
                if (lgrad1) then
                  derive0(k) = derive0(k) - tpte*rk
                  derive(k) = tpte*rk2*rk + derive(k)
                  derivk = derivk + apt*rk*(2.0_dp*rd*trm1 - cpt*rd2*(1.0_dp - 2.0_dp*dpt*rd))
                  if (lgrad2) then
                    deriv2k = deriv2k + apt*(2.0_dp*trm1 - 4.0_dp*cpt*rd + 10.0_dp*cpt*dpt*rd2)
                    derive2(k) = derive2(k) - 2.0_dp*tpte*rk2*rk
                    if (lgrad3) then
                      deriv3k = deriv3k + 6.0_dp*apt*cpt*(4.0_dp*dpt*rd - 1.0_dp)
                      derive3(k) = derive3(k) + 6.0_dp*tpte*rk2*rk2
                    endif
                  endif
                endif
              elseif (nptyp.eq.60) then
!**********************************
!  Buffered_LJ twobody potential  *
!**********************************
                apt = twopot(1,npot)*psfct
                bpt = twopot(2,npot)
                cpt = twopot(3,npot)
                dpt = twopot(4,npot)
                rho = r/bpt
                trm1 = ((1.0_dp + cpt)/(rho + cpt))**7
                trm2 = (1.0_dp + dpt)/(rho**7 + dpt)
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl + apt*trm1*(trm2 - 2.0_dp)
                    apt = apt*lambda
                  elseif (ltib2body(npot)) then
                    dUdl = dUdl - apt*trm1*(trm2 - 2.0_dp)
                    apt = apt*(1.0_dp - lambda)
                ...
                eatm = eatm + apt*trm1*(trm2 - 2.0_dp)
                if (lgrad1) then
                  dtrm1dr = - 7.0_dp*trm1/(bpt*(rho + cpt))
                  dtrm2dr = - 7.0_dp*trm2*rho**6/(bpt*(rho**7 + dpt))
                  derivk = derivk + apt*rk*(dtrm1dr*(trm2 - 2.0_dp) + trm1*dtrm2dr)
                  if (lgrad2) then
                    d2trm1dr2 = 56.0_dp*trm1/(bpt*(rho + cpt))**2
                    d2trm2dr2 = trm2*((98.0_dp*rho**12)/(bpt*bpt*(rho**7 + dpt)**2) - &
                                      (42.0_dp*rho**5)/(bpt*bpt*(rho**7 + dpt)))
                    deriv2k = deriv2k + apt*(d2trm1dr2*(trm2 - 2.0_dp) + &
                                             2.0_dp*dtrm1dr*dtrm2dr + &
                                             trm1*d2trm2dr2)
                    if (lgrad3) then
                      d3trm1dr3 = - 504.0_dp*trm1/(bpt*(rho + cpt))**3
                      d3trm2dr3 = - trm2*((2058.0_dp*rho**18)/((bpt**3)*(rho**7 + dpt)**3) - &
                                          (1764.0_dp*rho**11)/((bpt**3)*(rho**7 + dpt)**2) + &
                                          (210.0_dp*rho**4)/((bpt**3)*(rho**7 + dpt)))
                      deriv3k = deriv3k + apt*(d3trm1dr3*(trm2 - 2.0_dp) + &
                                               3.0_dp*d2trm1dr2*dtrm2dr + &
                                               3.0_dp*dtrm1dr*d2trm2dr2 + &
                                               trm1*d3trm2dr3)
                    endif
                  endif
                endif
...
!*********************
                ...
                if (r.lt.repcut(npot)) then
                  ...
                  eatm = eatm + trm1*trm2
                  if (lgrad1) then
                    trm1 = trm1*bpt/3.0_dp
                    derivk = derivk - trm1*br*(br + 1.0_dp)*rk
                    rtrm1k = rtrm1k + trm1*br*(br + 1.0_dp)
                    if (lgrad2) then
                      trm1 = trm1*bpt
                      deriv2k = deriv2k + trm1*(br*br - br - 1.0_dp)
                      rtrm2k = rtrm2k + trm1*(br*br - br - 1.0_dp)
                      rderivk = rderivk - trm1*rk*(br*br - br - 1.0_dp)
                      if (lgrad3) then
                        trm1 = trm1*bpt*br
                        deriv3k = deriv3k - trm1*(br - 3.0_dp)
                        rtrm32k = rtrm32k + trm1*(br - 3.0_dp)
                        rtrm3k = rtrm3k + trm1*rk*(br - 3.0_dp)
                      endif
                    endif
                  endif
                endif
              ...
              elseif (nptyp.eq.62) then
!*****************
!  j2 potential  *
!*****************
                spin1 = spinspec(nspecptr1(npot))
                spin2 = spinspec(nspecptr2(npot))
                apt = twopot(1,npot)*psfct*spin1*spin2
                if (lti) then
                  if (ltif2body(npot)) then
                    dUdl = dUdl - apt
                    apt = apt*lambda
                ...
                eatm = eatm - apt
              elseif (nptyp.eq.63) then
!****************************************************
!  Stillinger-Weber 2 body potential - general form *
!****************************************************
                apt = twopot(1,npot)*psfct
                bpt = twopot(2,npot)        ! bpt = sigma
                cpt = twopot(3,npot)        ! cpt = B
                p   = twopot(4,npot)        ! p
                q   = twopot(5,npot)        ! q
                rmax = rpot(npot)
                if (r.lt.rmax) then
                  rrm = 1.0_dp/(r-rmax)
                  trm1 = apt*exp(bpt*rrm)
                  sr = bpt*rk
                  trm2 = cpt*sr**p
                  trm3 = sr**q
                  if (lti) then
                    if (ltif2body(npot)) then
                      dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + trm1*(trm2 - trm3)*rtistep
                      trm1 = trm1*lambda
                    elseif (ltib2body(npot)) then
                      dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - trm1*(trm2 - trm3)*rtistep
                      trm1 = trm1*(1.0_dp - lambda)
                  ...
                  eatom = eatom + trm1*(trm2 - trm3)
                  if (lgrad1) then
                    dtrm1 = - trm1*bpt*rrm*rrm
                    dtrm2 = - p*trm2*rk
                    dtrm3 = - q*trm3*rk
                    deriv(k) = deriv(k) + rk*(dtrm1*(trm2 - trm3) + trm1*(dtrm2 - dtrm3))
                    if (lgrad2) then
                      d2trm1 = trm1*(bpt*rrm*rrm)**2 + 2.0_dp*trm1*bpt*rrm**3
                      d2trm2 = p*(p + 1.0_dp)*trm2*rk2
                      d2trm3 = q*(q + 1.0_dp)*trm3*rk2
                      deriv2(k) = deriv2(k) + d2trm1*(trm2 - trm3) + 2.0_dp*dtrm1*(dtrm2 - dtrm3) + trm1*(d2trm2 - d2trm3)
                      if (lgrad3) then
                        d3trm1 = - trm1*(bpt*rrm*rrm)**3 - 6.0_dp*trm1*bpt*bpt*rrm**5 - 6.0_dp*trm1*bpt*rrm**4
                        d3trm2 = - p*(p + 1.0_dp)*(p + 2.0_dp)*trm2*rk2*rk
                        d3trm3 = - q*(q + 1.0_dp)*(q + 2.0_dp)*trm3*rk2*rk
                        deriv3(k) = deriv3(k) + d3trm1*(trm2 - trm3) + 3.0_dp*d2trm1*(dtrm2 - dtrm3) +  &
                                                3.0_dp*dtrm1*(d2trm2 - d2trm3) + trm1*(d3trm2 - d3trm3)
                      endif
                    endif
                  endif
                endif
              elseif (nptyp.eq.64) then
!***************************************************************
!  Screened Coulomb potential : Distance dependent dielectric  *
!***************************************************************
                apt = twopot(1,npot)        ! Saturation dipole
                bpt = twopot(2,npot)        ! r_me
                cpt = twopot(3,npot)        ! sigma_e
                dpte = pscale
                dpt = dpte*factor
                dpte = dpte*sfct
!
!  Compute distance dependent dielectric
!
                trm3 = tanh((r - bpt)/cpt)
                epsr = 0.5_dp*(apt + dielectric) + 0.5_dp*(dielectric - apt)*trm3
                trm2 = (dielectric/epsr - 1.0_dp)
!
                if (lti) then
                  dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rk*trm2*pscale*dUdl_f
                endif
!
                ereal = ereal + dpt*rk*trm2
                if (lgrad1) then
                  depsr = 0.5_dp*(dielectric - apt)*(1.0_dp - trm3**2)/cpt
                  dtrm2 = - dielectric*depsr/epsr**2
                  derive(k) = derive(k) + dpte*rk2*(-rk*trm2 + dtrm2)
                  derive0(k) = derive0(k) + dpte*rk*trm2
                  if (lgrad2) then
                    d2epsr = - (dielectric - apt)*trm3*(1.0_dp - trm3**2)/cpt**2
                    d2trm2 = dielectric*(2.0_dp*depsr*depsr/epsr - d2epsr)/epsr**2
                    derive2(k) = derive2(k) + dpte*rk*(2.0_dp*rk2*trm2 - 2.0_dp*rk*dtrm2 + d2trm2)
                    if (lgrad3) then
                      d3epsr = (dielectric - apt)*(3.0_dp*trm3 - 1.0_dp)*(1.0_dp - trm3**2)/cpt**3
                      d3trm2 = - dielectric*(6.0_dp*depsr*depsr*depsr/epsr**2 - 6.0_dp*d2epsr*depsr/epsr + d3epsr)/epsr**2
                      derive3(k) = derive3(k) +  dpte*rk*(d3trm2 - 3.0_dp*rk*d2trm2 + 6.0_dp*(rk2*dtrm2 - rk2*rk*epsr))
                    endif
                  endif
                endif
              elseif (nptyp.eq.65) then
!***************************************************************************
!  AA-CLP twobody potential : Depends on charge of atoms and species data  *
!***************************************************************************
!
!  Trap call where species numbers aren't set
!
                if (nsi.gt.0.and.nsj.gt.0) then
                  nati = natspec(nsi)
                  natj = natspec(nsj)
! 
!  Polarisability
!
                  poli0 = aaclp_pol_spec(nsi)
                  polj0 = aaclp_pol_spec(nsj)
                  vali = dble(nvalenceelectron(nati))
                  valj = dble(nvalenceelectron(natj))
                  poli = poli0*(vali - qli)/vali   ! This is the effective polarisability of i
                  polj = polj0*(valj - qlj)/valj   ! This is the effective polarisability of j
                  polij = sqrt(poli*polj)
! 
                  pij = polij*abs(qli*qlj)*aaclp_pscale_ev*psfct
! 
                  nqni = npqn(nati)   
                  nqnj = npqn(natj)   
                  dij = polij*dble(nqni*nqnj)*sqrt(aaclp_ip_spec(nsi)*aaclp_ip_spec(nsj))*aaclp_dscale_ev*psfct
! 
                  tij = (vali - qli)*(valj - qlj)*aaclp_rscale_ev*sqrt(aaclp_b_spec(nsi)*aaclp_b_spec(nsj))*psfct
                  hij = aaclp_h_spec(nsi)*aaclp_h_spec(nsj)
                  if (hij.lt.0.0_dp) then
                    tij = tij*(1.0_dp + hij)
                  endif
!
                  r6 = rk2*rk2*rk2
                  r12 = r6*r6
!
                  eatm = eatm + tij*r12 - dij*r6 - pij*rk2*rk2
                  if (lgrad1) then
                    derivk = derivk - (12.0_dp*tij*r12 + 6.0_dp*dij*r6 - 4.0_dp*pij*rk2*rk2)*rk2
                    if (lgrad2) then
                      deriv2k = deriv2k + (168.0_dp*tij*r12 + 48.0_dp*dij*r6 - 24.0_dp*pij*rk2*rk2)*rk2*rk2
                      if (lgrad3) then
                        deriv3k = deriv3k - (2688.0_dp*tij*r12 + 480.0_dp*dij*r6 - 192.0_dp*pij*rk2*rk2)*rk2*rk2*rk2
                      endif
                    endif
                  endif
                endif
...
!
            ...
            elseif (tapertype.eq.6) then
              call p7taper(r,tapermax,tpfn,dtpfn,d2tpfn,d3tpfn,lgrad1,lgrad2,lgrad3)
            elseif (tapertype.eq.7) then
              call openmmtaper(r,tapermin,tapermax,tpfn,dtpfn,d2tpfn,d3tpfn,lgrad1,lgrad2,lgrad3)
            else
              call ctaper(r,tapermin,tapermax,tpfn,dtpfn,d2tpfn,d3tpfn,lgrad1,lgrad2,lgrad3)
            endif
            eatom = eatom + tpfn*eatm
            if (lti) then
              dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + dUdl*tpfn*rtistep
            endif
            if (lgrad1) then
              deriv(k) = deriv(k) + tpfn*derivk + dtpfn*eatm*rk
              rtrm1(k) = rtrm1(k) + tpfn*rtrm1k
              if (lgrad2) then
          ...
          eatom = eatom + eatm
          if (lti) then
            dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + dUdl*rtistep
          endif
          if (lgrad1) then
...
!  Bulk electrostatics
!
          if ((r2.le.cut2e.or.lptrmol(k)).and.(lewald.or.lwolf).and.(abs(factorin).gt.1.0d-12.or.lpolar)) then
            if (k.ne.nmolonly.or..not.lcoulsub) then
              ...
              if (r2.le.cut2s.and.r2.gt.small2.and.lcspair) then
                ...
                effc = g_derfc(setaloc*r)
                effc = effc*rk
              endif
              ereal = ereal + effc*factor
!
              if (lti) then
                dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + effc*dUdl_f
              endif
!
              if (lneedgrad1) then
                ...
                if (lpoldamp.and.r.lt.rpdamp) then
!
!  Polarisation damping for electric field
!
                  call tangtoennies(4_i4,bpdamp,r,ftt,dfttdr,d2fttdr2,d3fttdr3,.true.,lgrad2,lgrad3)
                  derivqd(k) = derivqd(k) + rk2*dfttdr - rk*rk2*(ftt - 1.0_dp)
                endif
                if (lgrad2) then
                  derive2(k) = derive2(k) + 2.0_dp*(trm2 + trm1*etaloc)
                  if (lpoldamp.and.r.lt.rpdamp) then
                    derivqd2(k) = derivqd2(k) + rk*d2fttdr2 - 2.0_dp*rk2*(dfttdr - rk*(ftt - 1.0_dp))
                  endif
                  if (lgrad3) then
                    derive3(k) = derive3(k) - 6.0_dp*trm2*rk
                    derive3(k) = derive3(k) - 4.0_dp*trm1*etaloc*(rk + etaloc*r)
                    if (lpoldamp.and.r.lt.rpdamp) then
                      derivqd3(k) = derivqd3(k) + rk*d3fttdr3 - 3.0_dp*rk2*(d2fttdr2 - 2.0_dp*rk*dfttdr + &
                                                 2.0_dp*rk2*(ftt - 1.0_dp))
                    endif
                  endif
                endif
              endif
...
!
              if (((leem.or.lDoQDeriv2).and..not.lnoqeem).and.lgrad1) then
...
!
!  Fennell and Gezelter modification
!
                if (lwolffennell) then
                  ereal = ereal + selfwolfforce*(r - cutw)*factor
!
                  if (lti) then
                    dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + selfwolfforce*(r - cutw)*dUdl_f
                  endif
!
                  if (lgrad1) then
                    trm2 = selfwolfforce*sfct*rk
                    derive(k) = derive(k) + trm2
                    derive0(k) = derive0(k) - effc*sfct + selfwolfforce*(r - cutw)*sfct
!
!  Variable charge terms
!
                    if ((leem.or.lDoQDeriv2).and..not.lnoqeem) then
                      d1i(k) = d1i(k) + trm2*angstoev*qj
                      d1j(k) = d1j(k) + trm2*angstoev*qi
                    endif
                  endif
...
!
                  if (lgrad1) then
                    ...
                    if (lgrad2) then
!
!  The terms below need verifying
!
                      erffc = effc*sfct
...
!
!  Modified potential consistent Wolf sum
!
                  derive0(k) = derive0(k) - effc*sfct
                endif
              endif
...
!
              if (lti) then
                dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rk*dUdl_f
              endif
!
              if (lgrad1) then
                trm1 = sfct*rk*rk2
...
!
!  Combine polarisation terms if needed
!
    if (lpolar.and.lpoldamp) then
      if (lgrad2) then
        derivqd2(k) = rk2*(derivqd2(k) - derivqd(k))
        if (lgrad3) then
          derivqd3(k) = rk*(rk2*derivqd3(k) - 3.0_dp*derivqd2(k)*rk)
        endif
      endif
    endif
!
...
!
    ...
    if (lpolar) then
!
!  Charge-dipole interactions for polarisation
!
      if (lpoldamp) then
        derivqd(k) = angstoev*derivqd(k) + derive(k)
        if (lgrad2) then
          derivqd2(k) = angstoev*derivqd2(k) + derive2(k)
          if (lgrad3) then
            derivqd3(k) = angstoev*derivqd3(k) + derive3(k)
          endif
        endif
      else
        derivqd(k) = derive(k)
        if (lgrad2) then
          derivqd2(k) = derive2(k)
          if (lgrad3) then
            derivqd3(k) = derive3(k)
          endif
        endif
      endif
    endif
...
!*******************************************************************
  subroutine twobody1(eatom,ereal,ec6,lgrad1,lgrad2,lgrad3,nor,nor0,dist1,x1,y1,z1,d0i1,d0j1,deriv1, &
                      ...
                      cut2s,lptrmol1,nmolonly,factor,sfct,spfct,radsum,rtrm11,rtrm21,rtrm31,rtrm321,sctrm1, &
                      sctrm2,qi,qj,nsi,nsj,lcspair,lperiodic,lregion1,lbonded1,l2bonds1,l3bonds1,nbotype_1, &
                      nbotype2_1,lskipsr,lskipq,d1i1,d1j1,d2i21,d2ij1,d2j21)
!
...
!   6/09 Charge as a coordinate option added
!   7/13 Occupancy factor for core-shell spring constant added to pass through to twobody
!   3/14 lorder12 removed from argument list
!  12/14 rtrm1 changed from scalar to array
!   2/18 Trace added
!  11/21 Species numbers added to input arguments
