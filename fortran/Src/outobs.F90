...
!  12/13 Strain derivative and stress observables separated
!   8/17 Fitting of properties enabled based on numerical 
!        second derivatives
!   9/17 Absolute dipole moment added as an observable for molecules
!  10/17 Free energy added as an observable
!   3/19 iopt replaced by ioptindex and iopttype
!   7/20 Rigid molecules added to observable output
!   4/21 Output format changed to handle larger numbers of observables
!   7/21 feqp added
!  10/21 lfcphon set to true for feqp case
!  11/22 nobstyle added
!  11/22 Reaction free energies added
!   4/23 Handling of reaction free energies improved for 0-D case
!
!  Conditions of use:
!
...
!
      ...
      if (nt.eq.9.or.nt.eq.13.or.nt.eq.14.or.nt.eq.29.or.nt.eq.32.or.nt.eq.33) then
        lfcphon(ncf) = .true.
        if (ndimen(ncf).eq.3.and.(.not.lnoanald2.or.lnumerical)) then
!
!  Turn on calculation of Born charges and properties in case they are needed for phonons
!
          lfcborn(ncf) = .true.
          lfcprop(ncf) = .true.
        endif
...
!
!  Check k-point is specified, otherwise use Gamma point
!
        k = 0
        ninsert = 0
        nkncf = 0
      ...
      if (nt.eq.34.or.nt.eq.35) then
!
!  Set lfcphon for free energies of reaction
!
        do ic = 1,ncfg
          if (abs(freaction(ic,i)).gt.1.0d-12) then
            lfcphon(ic) = .true.
            ncf = ic
            if (ncf.ne.ncflast) then
              call setup(.true.)
...
!
            ...
            do while (k.lt.nkpt)
              k = k + 1 
              if (nkptcfg(k).eq.ic) nkncf = nkncf + 1 
              if (nkptcfg(k).eq.ic-1) ninsert = k + 1 
            enddo
            if (nkncf.eq.0) then
              do k = nkpt,ninsert,-1
                xkpt(k+1) = xkpt(k)
...
!
      if (nt.eq.1) then
        write(ioout,'(4x,i7,6x,a1,1x,a13,1x,f12.6,2x,f12.4,2x,11x,i4)') &
          i,astyle,nameobs(nt),fobs(i),weight(i),nobcfg(i)
!
!  Derivatives
!
      elseif (nt.eq.2) then
        nptr = nobptr(i)
        ind = ioptindex(nptr)
        if (iopttype(nptr).eq.iopt_cell.or.iopttype(nptr).eq.iopt_strain) then
!
!  Cell derivative or strain
!
!  Correction for rhombohedral setting
          if ((ifhr(ncf).eq.1.or.ndim.eq.2).and.ind.eq.3.and.lrelax) ind = 4
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,2x,a5,4x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),namecell(ind),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_xf) then
!
!  x coordinate derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,6x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(1),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_yf) then
!
!  y coordinate derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,6x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(2),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_zf) then
!
!  z coordinate derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,6x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(3),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_radius) then
!
!  Radial derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,6x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(4),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_xcom) then
!
!  x centre of mass derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,''com'',3x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(1),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_ycom) then
!
!  y centre of mass derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,''com'',3x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(2),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_zcom) then
!
!  z centre of mass derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,''com'',3x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(3),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_xqtn) then
!
!  x quaternion derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,''qtn'',3x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(1),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_yqtn) then
!
!  y quaternion derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,''qtn'',3x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(2),nobcfg(i)
        elseif (iopttype(nptr).eq.iopt_zqtn) then
!
!  z quaternion derivative
!
          write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,a,''qtn'',3x,i4)') &
            i,nameobs(nt),fobs(i),weight(i),ind,namecrd(3),nobcfg(i)
        endif
...
!
      elseif (nt.eq.3) then
        ni = nobptr(i)/7
        nj = nobptr(i) - 7*ni
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,2i3,5x,i4)') &
...
!
      elseif (nt.eq.7.or.nt.eq.8) then
        ni = nobptr(i)/7
        nj = nobptr(i) - 7*ni
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,2x,a1,5x,i4)') &
...
!
      elseif (nt.eq.9) then
        ni = nobptr(i)
        write(ioout,'(4x,i7,6x,a1,1x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,i4,3x,i4)') &
          i,astyle,nameobs(nt),fobs(i),weight(i),nobptr2(i),ni,nobcfg(i)
!
!  Electrostatic potential
!
      elseif (nt.eq.10) then
        ni = nobptr(i)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,8x,i4)') &
          i,nameobs(nt),fobs(i),weight(i),ni,nobcfg(i)
!
!  Bulk/shear modulus
!
      elseif (nt.eq.11.or.nt.eq.12) then
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,13x,i4)') &
...
!
      elseif (nt.eq.17) then
        ni = nobptr(i)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,1x,i4,8x,i4)') &
          i,nameobs(nt),fobs(i),weight(i),ni,nobcfg(i)
!
...
!
      elseif (nt.eq.18) then
        ...
        idk = (idj-1)/3 + 1
        idj = idj - 3*(idk-1)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,1x,i4,1x,2a1,5x,i4)') &
...
!
      elseif (nt.eq.22) then
        ni = nobptr(i)
        nj = nobptr2(i)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,1x,i4,1x,i3,4x,i4)') &
          i,nameobs(nt),fobs(i),weight(i),ni,nj,nobcfg(i)
!
!  Bond angle
!
      elseif (nt.eq.23) then
        ...
        nj = nobptr2(i)
        nk = nobptr3(i)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,1x,i4,1x,i3,1x,i3,i4)') &
          i,nameobs(nt),fobs(i),weight(i),ni,nj,nk,nobcfg(i)
!
!  Reaction energy 
!
      elseif (nt.eq.24.or.nt.eq.34.or.nt.eq.35) then
        write(ioout,'(4x,i7,6x,a1,1x,a13,1x,f12.6,2x,f12.4,2x,11x,i4)') &
          i,astyle,nameobs(nt),fobs(i),weight(i),nobcfg(i)
!
!  Young's moduli
!
      elseif (nt.eq.25) then
        ni = nobptr(i)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,4x,a1,8x,i4)') &
...
!
      elseif (nt.eq.26) then
        ...
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,4x,2a1,7x,i4)') &
...
!
      elseif (nt.eq.29) then
        ni = nobptr(i)
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,i3,1x,i4,3x,i4)') &
          i,nameobs(nt),fobs(i),weight(i),nobptr2(i),ni,nobcfg(i)
!
!  Volume
!
      elseif (nt.eq.30) then
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,5x,8x,i4)') &
          i,nameobs(nt),fobs(i),weight(i),nobcfg(i)
!
!  Stresses
!
...
!
      elseif (nt.eq.32) then
        write(ioout,'(4x,i7,8x,a13,1x,f12.6,2x,f12.4,2x,11x,i4)') i,nameobs(nt),fobs(i),weight(i),nobcfg(i)
