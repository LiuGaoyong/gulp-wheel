...
!   7/22 Changed to remove use of d1j
!   6/23 Electrostatic cutoff squared now passed to rsearch subroutines
!   7/23 Argument list for reaxFF_bomd updated by adding mneigh
...
!
  ...
  integer(i4)                                      :: maxneigh2e
  integer(i4)                                      :: maxneigh2t
  integer(i4)                                      :: mneigh
  integer(i4)                                      :: n
  integer(i4)                                      :: nati
  integer(i4)                                      :: nboij
...
!  Sigma
!
            call reaxFFbo_sigma(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.true.,.false., &
...
!  Pi
!
              call reaxFFbo_pi(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.true.,.false., &
                                 .not.lStrict)
              BOp_pi(ni,i) = BOpij
              deltap(i) = deltap(i) + BOpij
...
!  Pi_pi
!
              call reaxFFbo_pipi(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.true.,.false., &
                                 .not.lStrict)
              BOp_pipi(ni,i) = BOpij
              deltap(i) = deltap(i) + BOpij
...
!***************************************
        call reaxFF_bomd(i,j,nspeci,nspecj,ni,nj,deltapi,deltapj,mneigh,nneigh,BOp,BOp_pi,BOp_pipi, &
                         d1BOp,d1BOp_pi,d1BOp_pipi,BOij_s,BOij_pi,BOij_pipi, &
                         d1BOi_s,d1BOi_pi,d1BOi_pipi,.true.)
!
...
!
    if (ii.le.numattodo_main) then
      call d1addfc(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,ineigh,maxlhs, &
                   d1cell,matom,nbodummyptr,d1i,.false.,.false.)
    endif
  enddo iloop
!***************************************************
...
!
            ...
            loverflow_expuc3_j = (reaxFFlam(9)*sumdeltabopi_j.gt.80.0_dp)
            if (loverflow_expuc3_j) then
              expuc3_j = 0.0_dp
              utrm2_j  = 0.0_dp
            else
...
!
        call d1addfc(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,ineigh,maxlhs, &
                     d1cell,matom,nbodummyptr,d1i,.true.,lextended)
!
...
!
        call d1addfc(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,ineigh,maxlhs, &
                     d1cell,matom,nbodummyptr,d1i,.false.,lextended)
!
!  End of QMMM condition
!
      endif
