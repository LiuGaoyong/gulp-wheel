  subroutine phase_newfcd(xk,yk,zk,nfccells,nfcsuper,fccells,fcderv2,maxd2fc,nd2fc)
!
...
!  NB: For the parallel version in this routine, the fcderv2 array has already been reduced
!      to the smaller cell in the right-hand index
!
!  Note that at present the reciprocal space and 1-D real space
!  Coulomb sums are computed using the standard algorithm.
!
!   8/23 Created from phase_newfc 
!
!  Conditions of use:
!
...
!
  ...
  real(dp)                                     :: zkv
#ifdef TRACE
  call trace_in('phase_newfcd')
#endif
!****************************
!  Zero second derivatives  *
!****************************
  maxlimu = 3*natomsonnode
  maxlim = 3*numat
...
!
        derv2(jx,ix) = derv2(jx,ix) + fcderv2(jxs,ix)*cosk
        derv2(jy,ix) = derv2(jy,ix) + fcderv2(jys,ix)*cosk
        derv2(jz,ix) = derv2(jz,ix) + fcderv2(jzs,ix)*cosk
        derv2(jx,iy) = derv2(jx,iy) + fcderv2(jxs,iy)*cosk
        derv2(jy,iy) = derv2(jy,iy) + fcderv2(jys,iy)*cosk
        derv2(jz,iy) = derv2(jz,iy) + fcderv2(jzs,iy)*cosk
        derv2(jx,iz) = derv2(jx,iz) + fcderv2(jxs,iz)*cosk
        derv2(jy,iz) = derv2(jy,iz) + fcderv2(jys,iz)*cosk
        derv2(jz,iz) = derv2(jz,iz) + fcderv2(jzs,iz)*cosk
!
        dervi(jx,ix) = dervi(jx,ix) + fcderv2(jxs,ix)*sink
        dervi(jy,ix) = dervi(jy,ix) + fcderv2(jys,ix)*sink
        dervi(jz,ix) = dervi(jz,ix) + fcderv2(jzs,ix)*sink
        dervi(jx,iy) = dervi(jx,iy) + fcderv2(jxs,iy)*sink
        dervi(jy,iy) = dervi(jy,iy) + fcderv2(jys,iy)*sink
        dervi(jz,iy) = dervi(jz,iy) + fcderv2(jzs,iy)*sink
        dervi(jx,iz) = dervi(jx,iz) + fcderv2(jxs,iz)*sink
        dervi(jy,iz) = dervi(jy,iz) + fcderv2(jys,iz)*sink
        dervi(jz,iz) = dervi(jz,iz) + fcderv2(jzs,iz)*sink
!
