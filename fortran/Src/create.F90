...
!
  ...
  ltranat(natom) = .false.
  ltranatminus(natom) = .false.
  oxa(natom) = 0.0_dp
  cna(natom) = 0.0_dp
  extpotcfg(natom) = 0.0_dp
...
!  Old variable order  *
!***********************
    ioptindex(nvar+1) = 1_i4
    iopttype(nvar+1)  = iopt_xf
!
    ioptindex(nvar+2) = 1_i4
    iopttype(nvar+2)  = iopt_yf
!
    ioptindex(nvar+3) = 1_i4
    iopttype(nvar+3)  = iopt_zf
    nvar = nvar + 3
  else
!***********************
!  New variable order  *
!***********************
!
!  Move cell variables to make space
!
    do i = ncellmax,ncellmin,-1
      ioptindex(i+3) = ioptindex(i)
      iopttype(i+3)  = iopttype(i)
    enddo
!
!  Insert new atom
!
    ioptindex(ninternalmax+1) = 1_i4
    iopttype(ninternalmax+1)  = iopt_xf
    ioptindex(ninternalmax+2) = 1_i4
    iopttype(ninternalmax+2)  = iopt_yf
    ioptindex(ninternalmax+3) = 1_i4
    iopttype(ninternalmax+3)  = iopt_zf
!
!  Update variable numbers
!
    nvar = nvar + 3
    ninternalmax = ninternalmax + 3
    ncellmin = ncellmin + 3
    ncellmax = ncellmax + 3
  endif
!
