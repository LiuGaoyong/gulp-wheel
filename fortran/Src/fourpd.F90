...
!
  use g_constants
  use control,        only : lgroupvelocity, lnotorXduplicates
  use current
  use derivatives
...
!
  ...
  if (status/=0) call outofmemory('fourpd','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourpd','ntypmiddle')
  allocate(nptrnfornonoop(nfor),stat=status)
  if (status/=0) call outofmemory('fourpd','nptrnfornonoop')
  allocate(nuniqueiptr(numat),stat=status)
  if (status/=0) call outofmemory('fourpd','nuniqueiptr')
  allocate(nuniquekptr(numat),stat=status)
  if (status/=0) call outofmemory('fourpd','nuniquekptr')
  allocate(nuniquelptr(numat),stat=status)
  if (status/=0) call outofmemory('fourpd','nuniquelptr')
  if (ndim.gt.0) then
    allocate(xvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fourpd','xvec')
    allocate(yvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fourpd','yvec')
    allocate(zvec(maxvector),stat=status)
    if (status/=0) call outofmemory('fourpd','zvec')
  else
...
!
      ...
      if (status/=0) call deallocate_error('fourpd','zvec')
      deallocate(yvec,stat=status)
      if (status/=0) call deallocate_error('fourpd','yvec')
      deallocate(xvec,stat=status)
      if (status/=0) call deallocate_error('fourpd','xvec')
...
!
              d2k  = - e1d(1)*cos21
              d2ks = - e1d(1)*sin21
              cdk(1) = dcmplx(d2k*x21,d2ks*x21)*dcmplx(0.0_dp,1.0_dp)
              cdk(2) = dcmplx(d2k*y21,d2ks*y21)*dcmplx(0.0_dp,1.0_dp)
              cdk(3) = dcmplx(d2k*z21,d2ks*z21)*dcmplx(0.0_dp,1.0_dp)
!
              derv2dk(1:3,n11,n21) = derv2dk(1:3,n11,n21) - cdk(1:3)
!
              d2k  = e1d(4)*cos32
              d2ks = e1d(4)*sin32
              cdk(1) = dcmplx(d2k*x32,d2ks*x32)*dcmplx(0.0_dp,1.0_dp)
              cdk(2) = dcmplx(d2k*y32,d2ks*y32)*dcmplx(0.0_dp,1.0_dp)
              cdk(3) = dcmplx(d2k*z32,d2ks*z32)*dcmplx(0.0_dp,1.0_dp)
!
              derv2dk(1:3,n31,n21) = derv2dk(1:3,n31,n21) - cdk(1:3)
!
              d2k  = e1d(5)*cos42
              d2ks = e1d(5)*sin42
              cdk(1) = dcmplx(d2k*x42,d2ks*x42)*dcmplx(0.0_dp,1.0_dp)
              cdk(2) = dcmplx(d2k*y42,d2ks*y42)*dcmplx(0.0_dp,1.0_dp)
              cdk(3) = dcmplx(d2k*z42,d2ks*z42)*dcmplx(0.0_dp,1.0_dp)
!
              derv2dk(1:3,n41,n21) = derv2dk(1:3,n41,n21) - cdk(1:3)
            endif
!
...
!
                ...
                cdk(3) = dcmplx(d2k*z32,d2ks*z32)*dcmplx(0.0_dp,1.0_dp)
!
                derv2dk(1:3,n32,n21) = derv2dk(1:3,n32,n21) + cdk(1:3)
!
                d2k  = - t12*cos21
                d2ks = - t12*sin21
                cdk(1) = dcmplx(d2k*x21,d2ks*x21)*dcmplx(0.0_dp,1.0_dp)
                ...
                cdk(3) = dcmplx(d2k*z21,d2ks*z21)*dcmplx(0.0_dp,1.0_dp)
!
                derv2dk(1:3,n11,n22) = derv2dk(1:3,n11,n22) + cdk(1:3)
!
                d2k  = t24*cos42
                d2ks = t24*sin42
                cdk(1) = dcmplx(d2k*x42,d2ks*x42)*dcmplx(0.0_dp,1.0_dp)
                ...
                cdk(3) = dcmplx(d2k*z42,d2ks*z42)*dcmplx(0.0_dp,1.0_dp)
!
                derv2dk(1:3,n42,n21) = derv2dk(1:3,n42,n21) + cdk(1:3)
              endif
            enddo
          enddo
        enddo
...
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"fourpd","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!
              ...
              cdk(3) = dcmplx(d2k*z21,d2ks*z21)*dcmplx(0.0_dp,1.0_dp)
!
              derv2dk(1:3,n21,n11) = derv2dk(1:3,n21,n11) - conjg(cdk(1:3))
!
              d2k  = - e1d(2)*cos31
              d2ks = - e1d(2)*sin31
              cdk(1) = dcmplx(d2k*x31,d2ks*x31)*dcmplx(0.0_dp,1.0_dp)
              cdk(2) = dcmplx(d2k*y31,d2ks*y31)*dcmplx(0.0_dp,1.0_dp)
              cdk(3) = dcmplx(d2k*z31,d2ks*z31)*dcmplx(0.0_dp,1.0_dp)
!
              derv2dk(1:3,n31,n11) = derv2dk(1:3,n31,n11) - conjg(cdk(1:3))
!
              d2k  = e1d(3)*cos41
              d2ks = e1d(3)*sin41
              cdk(1) = dcmplx(d2k*x41,d2ks*x41)*dcmplx(0.0_dp,1.0_dp)
              cdk(2) = dcmplx(d2k*y41,d2ks*y41)*dcmplx(0.0_dp,1.0_dp)
              cdk(3) = dcmplx(d2k*z41,d2ks*z41)*dcmplx(0.0_dp,1.0_dp)
!
              derv2dk(1:3,n41,n11) = derv2dk(1:3,n41,n11) - cdk(1:3)
            endif
!
!  Loop over second coordinate
!
...
!
                ...
                cdk(3) = dcmplx(d2k*z21,d2ks*z21)*dcmplx(0.0_dp,1.0_dp)
!
                derv2dk(1:3,n22,n11) = derv2dk(1:3,n22,n11) + conjg(cdk(1:3))
!
                d2k  = - t13*cos31
                d2ks = - t13*sin31
                cdk(1) = dcmplx(d2k*x31,d2ks*x31)*dcmplx(0.0_dp,1.0_dp)
                ...
                cdk(3) = dcmplx(d2k*z31,d2ks*z31)*dcmplx(0.0_dp,1.0_dp)
!
                derv2dk(1:3,n32,n11) = derv2dk(1:3,n32,n11) + conjg(cdk(1:3))
!
                d2k  = t14*cos41
                d2ks = t14*sin41
                cdk(1) = dcmplx(d2k*x41,d2ks*x41)*dcmplx(0.0_dp,1.0_dp)
                ...
                cdk(3) = dcmplx(d2k*z41,d2ks*z41)*dcmplx(0.0_dp,1.0_dp)
!
                derv2dk(1:3,n42,n11) = derv2dk(1:3,n42,n11) + cdk(1:3)
              endif
            enddo
          enddo
        enddo
...
!****************************
  if (noofp.gt.0) call fourooppd(xkv,ykv,zkv)
!************************
!  Improper potentials  *
!************************
  if (nimproper.gt.0) call fourimppd(xkv,ykv,zkv)
!
!  Free local memory
!
  deallocate(zvec,stat=status)
  ...
  if (status/=0) call deallocate_error('fourpd','nuniquelptr')
  deallocate(nuniquekptr,stat=status)
  if (status/=0) call deallocate_error('fourpd','nuniquekptr')
  deallocate(nuniqueiptr,stat=status)
  if (status/=0) call deallocate_error('fourpd','nuniqueiptr')
  deallocate(nptrnfornonoop,stat=status)
  if (status/=0) call deallocate_error('fourpd','nptrnfornonoop')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourpd','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourpd','natmiddle')
!
!  Timing
!
  time2 = g_cpu_time()
