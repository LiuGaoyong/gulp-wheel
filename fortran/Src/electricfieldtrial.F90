...
#ifdef TRACE
  call trace_in('electricfieldtrial')
#endif
!
...
!
  do nt = 1+procid,ntrialatom,nprocs
    i = nptrtrialatom(nt)
    efield = efield - dble(neqv(i))*fieldx*(xalat(i) - xvoffset)*qa(i)
    efield = efield - dble(neqv(i))*fieldy*(yalat(i) - yvoffset)*qa(i)
    efield = efield - dble(neqv(i))*fieldz*(zalat(i) - zvoffset)*qa(i)
  enddo
#ifdef TRACE
  call trace_out('electricfieldtrial')
#endif
!
  return
  end
