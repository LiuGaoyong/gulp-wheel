  subroutine twobodymd(eatom,ereal,ec6,nor,nor0,npots,npotl,cut2r,cut2e,cut2s,nmolonly, &
                       factorin,sfct,spfct,radsum,sctrm1,sctrm2,qli,qlj,nsi,nsj,lcspair, &
                       lperiodic,lskipsr,lskipq)
!
...
!  spfct   = symmetry equivalence factor (core-shell spring constant)
...
!  nsi     = species number of i
!  nsj     = species number of j
...
!   7/13 Separate weighting factor added for core-shell spring constant
...
!   8/14 Baskes potentials replaced by subroutine
...
!   3/20 General form of sw2 potential added
...
!
  ...
  use eam
  use general,       only : cutw, etaw, selfwolf, selfwolfforce
  use kspace
  use m_aaclp
...
!*********************
              ...
              deriv(k) = deriv(k) + dfun(2)*rk*psfct*efct
...
!***********
              ...
              if (r.lt.twopot(1,npot)) then
                efct = sfct*(tpot(1,npot)*rd4 + tpot(2,npot)*rd3 + tpot(3,npot)*rd2 + &
...
!
                ...
                rd5 = rd4*rd
                rd6 = rd5*rd
                efct = sfct*(tpot(6,npot)*rd6 + tpot(7,npot)*rd5 + tpot(8,npot)*rd4 + &
...
!*************
              ...
              exptrm1 = exp(-(bpt*r)**2)
              exptrm2 = exp(-(cpt*r)**2)
              derivk = derivk + (2.0_dp*apt*rk2/sqrtpi)*(bpt*exptrm1*erfctrm - erftrm*cpt*exptrm2)*efct
              derivk = derivk - trm0*rk2*rk*efct
...
!
              ...
              eatm = eatm - apt*fdmp
              r4 = r2*r2
              derivk = derivk + 6.0_dp*apt*fdmp*fdmp*r4
...
!****************************************************
              ...
              if (r.lt.rmax) then
                ...
                dtrm2 = - p*trm2*rk
                dtrm3 = - q*trm3*rk
                derivk = derivk + rk*(dtrm1*(trm2 - trm3) + trm1*(dtrm2 - dtrm3))
              endif
...
!
          ...
          elseif (tapertype.eq.6) then
            call p7taper(r,tapermax,tpfn,dtpfn,d2tpfn,d3tpfn,.true.,.false.,.false.)
          elseif (tapertype.eq.7) then
            call openmmtaper(r,tapermin,tapermax,tpfn,dtpfn,d2tpfn,d3tpfn,.true.,.false.,.false.)
          else
...
!
                call tangtoennies(4_i4,bpdamp,r,ftt,dfttdr,d2fttdr2,d3fttdr3,.true.,.false.,.false.)
...
!
!  Fennell and Gezelter force consistent Wolf sum
!
