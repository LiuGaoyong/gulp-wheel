...
!   8/12 Format for output of shift modified to allow for larger numbers
!  12/12 Time-dependent field added
!  12/12 delayforce and endforce renamed to delay_force and end_force
!  12/12 delay_field and end_field added
!  12/12 Modified to allow for multiple time-dependent fields
!   3/13 Dimension of itmp2 increased to prevent out of bounds error for move2a1 case
!   3/13 Use of scratch files for defect information removed
!   3/13 deflist option replaced with dlist_vacancy/interstitial
!   5/13 Volume observable added
!   8/13 Thermal conductivity omega_af added
!   9/13 Raman directions added
!  12/13 Strain derivatives separated from stress for fitting
!   4/14 lbornkin added
!  10/14 Uninitialised referencing of pr_cons trapped.
!  10/14 fc_supercell option added
!  11/14 hbr now accessed from symmetry module
!   4/15 Ghost supercell array added
!   4/15 Error in format statement for rigid region 2 corrected
!   7/15 External potential added
!   9/15 translate noise added
!   5/16 SPME added
!   8/16 nqkgrid default increased
!   9/16 hmssgio added
!   3/17 fix_atom option added
!   4/17 qonsas option added
!   9/17 Output of absolute dipole moment observable added
!  10/17 Free energy added to observables
!  10/17 Initial coordinates added to restart info
!   1/18 Trace added
!   2/18 Trantherm added
!   2/18 nxks, nyks, nzks converted to a single array
!   3/18 Sign option added to translate
!   4/18 Scan_cell option added
!   5/18 Strain sub-option for scan_cell added
!   6/18 Strain cell option added
!   8/18 Correction to cell strain output
!  11/18 cellstrain renamed to cstrain
!  11/18 Handling of cell vectors output modified for strain case
!        where the unit cell is centred
!  12/18 Shear force added
!  12/18 lusevectors added
!   1/19 Correction to handling of ldumpcart when lmd is true
!   1/19 Changes for maxlinelength made
!   2/19 Output of cell increased to more decimal places 
!   3/19 ioptcfg replaced by ioptindexcfg and iopttypecfg
...
!   3/19 Multiple temperature ranges added
!   8/19 molatom option added
!   9/19 nrel2 / nrelat changed to nrela2f / nrelf2a
!   9/19 Use of intochar replace by string write
!   9/19 Alamode options output
!   9/19 Primitive cell added for Alamode
!  12/19 Flag output simplified to handle rigid molecules
!   2/20 npotsitescfg added
!   2/20 lksorigin added
!   3/20 Sum core and shell gradients sub-option added to gradients
!   3/20 Dielectric constant added
!   3/20 Fractional coordinates for dump added in rigid molecule case
!   4/20 Restart info for rigid molecules added
!   4/20 Centre of mass for rigid molecules now output in fractional
!   5/20 Rigid molecule constraints added
!   6/20 nmolcore changes added
!   7/20 Correction to optimisation flag array referencing
!   7/20 More than 10 atoms in molecules now handled in molrigid output
!   8/20 ordersuper added
!   4/21 Correction to connection types
!   5/21 lusevectors forced when ldumpcart is true to ensure cell and coordinates
!        have the same orientation on restart
!   7/21 feqp added
!  11/21 Modifications for TI added
!  11/21 Einstein positions changed
!  12/21 Restart info added for lambda in TI
!  12/21 pGFNFF modifications added
!   3/22 pr_cons only checked for nensemble = 4
...
!
  ...
  use symmetry
  use thermalcond,        only : lomega_af_in, omega_af, v_s_cfg, B_pr_cfg, v_p_cfg
#ifdef TRACE
...
!
  ...
  character(len=maxwordlength)                 :: cdumps
  character(len=maxwordlength)                 :: cextension
  character(len=maxlinelength)                 :: line
  character(len=maxlinelength)                 :: line2
  character(len=maxlinelength)                 :: dfilelocal
  integer(i4)                                  :: endpoint
  integer(i4)                                  :: i
  integer(i4)                                  :: iblank
  integer(i4)                                  :: ic
...
!
      dfilelocal = ' ' 
      dfilelocal(1:maxwordlength) = dfile(1:maxwordlength)
!
!  Find the point to insert the number
!
      insertpoint = index(dfile,'.')
      if (insertpoint.eq.0) then
        call endstring(dfile,len(dfile),insertpoint)
        lextension = .false.
      else
        lextension = .true.
      endif
      call endstring(dfile,len(dfile),endpoint) 
      endpoint = endpoint - 1
      if (lextension) then
        if (endpoint-insertpoint+1.gt.maxwordlength) then
...
!
!  Initialise necessary local variables to avoid writing to global equivalents
!
    nlasym = nascfg(nc)
    nlsft = 0
    do i = 1,nc-1
...
!  
      if (nc.eq.ncf.and..not.lstraincell) then
        do i = 1,3
          rvp(1,i) = rv(1,i)
          rvp(2,i) = rv(2,i)
          rvp(3,i) = rv(3,i)
...
!
    ...
    if (lflags) then
      ...
      enddo
!
!  Use lopfi to set flags for atoms
!
      do i = 1,nlasym
        if (lopfi(3*(nlsft+i-1)+1)) itmp(nstrains+3*(i-1)+1) = 1
        if (lopfi(3*(nlsft+i-1)+2)) itmp(nstrains+3*(i-1)+2) = 1
        if (lopfi(3*(nlsft+i-1)+3)) itmp(nstrains+3*(i-1)+3) = 1
      enddo
!
!  Set flags for cell
!
      do i = 1,nlvar
        ii = ioptindexcfg(i+nlfst)
        if (iopttypecfg(i+nlfst).eq.iopt_cell) then
          itmp(ii) = 1
        endif
      enddo
...
!
        if (ldump80) then
          ...
          if (ndimen(nc).eq.3) then
            write(iout,'(''vectors '')')
            write(iout,'(3f16.9)') (rv(j,1),j=1,3)
            write(iout,'(3f16.9)') (rv(j,2),j=1,3)
            write(iout,'(3f16.9)') (rv(j,3),j=1,3)
            write(iout,'(6i2)') (itmp(j),j=1,6)
          elseif (ndimen(nc).eq.2) then
            write(iout,'(''svectors '')')
            write(iout,'(2f16.9)') (rv(j,1),j=1,2)
            write(iout,'(2f16.9)') (rv(j,2),j=1,2)
            write(iout,'(3i2)') (itmp(j),j=1,3)
          elseif (ndimen(nc).eq.1) then
            write(iout,'(''pvectors '')')
            write(iout,'(f16.9)') rv(1,1)
            write(iout,'(1i2)') itmp(1)
          endif
        endif
!
!  Turn off ldumpcart if it has been explicitly specified since coordinates are already Cartesian
!
        lsavedumpcart = ldumpcart 
        ldumpcart = .false.
!
        do nr = 1,nregions(nc)
          fixstring = ' '
          ...
          do i = 1,nlasym
            if (nregionnocfg(i+nlsft).eq.nr) then
              ...
              if (ltranatminus(i+nlsft)) then
                tchar = 'T-'
              elseif (ltranat(i+nlsft)) then
                tchar = 'T '
              else
                tchar = '  '
              endif
...
!  Reset ldumpcart 
!
        ldumpcart = lsavedumpcart 
      else
        if (lusevectors.or.ldumpcart) then
!
!  Some methods require the Cartesian cell orientation to be preserved and so vectors must be output
!
          if (ldump80) then
            if (ndimen(nc).eq.3) then
              write(iout,'(''vectors '')')
              write(iout,'(3f12.6)') (rvp(j,1),j=1,3)
              write(iout,'(3f12.6)') (rvp(j,2),j=1,3)
              write(iout,'(3f12.6)') (rvp(j,3),j=1,3)
              write(iout,'(6i2)') (itmp(j),j=1,6)
            elseif (ndimen(nc).eq.2) then
              write(iout,'(''svectors '')')
              write(iout,'(2f12.6)') (rvcfg(j,1,nc),j=1,2)
              write(iout,'(2f12.6)') (rvcfg(j,2,nc),j=1,2)
              write(iout,'(3i2)') (itmp(j),j=1,3)
            elseif (ndimen(nc).eq.1) then
              write(iout,'(''pvectors '')')
              write(iout,'(f12.6)') rvcfg(1,1,nc)
              write(iout,'(1i2)') itmp(1)
            endif
          else
            if (ndimen(nc).eq.3) then
              write(iout,'(''vectors '')')
              write(iout,'(3f16.9)') (rvp(j,1),j=1,3)
              write(iout,'(3f16.9)') (rvp(j,2),j=1,3)
              write(iout,'(3f16.9)') (rvp(j,3),j=1,3)
              write(iout,'(6i2)') (itmp(j),j=1,6)
            elseif (ndimen(nc).eq.2) then
              write(iout,'(''svectors '')')
              write(iout,'(2f16.9)') (rvcfg(j,1,nc),j=1,2)
              write(iout,'(2f16.9)') (rvcfg(j,2,nc),j=1,2)
              write(iout,'(3i2)') (itmp(j),j=1,3)
            elseif (ndimen(nc).eq.1) then
              write(iout,'(''pvectors '')')
              write(iout,'(f16.9)') rvcfg(1,1,nc)
              write(iout,'(1i2)') itmp(1)
            endif
          endif
          if (lstraincell) then
            if (ndimen(nc).eq.3) then
              write(iout,'(''cstrain'',6(1x,f12.8))') (straincfg(j,nc),j=1,6)
            elseif (ndimen(nc).eq.2) then
              write(iout,'(''cstrain'',3(1x,f12.8))') (straincfg(j,nc),j=1,3)
            elseif (ndimen(nc).eq.1) then
              write(iout,'(''cstrain'',1x,f12.8)') straincfg(1,nc)
            endif
          endif
        else
!
...
!
          if (ldump80) then
            ...
            if (ndimen(nc).eq.3) then
              write(iout,'(''cell '')')
              write(iout,'(6f13.8,6i2)') acl,bcl,ccl,alpcl,betcl,gamcl,(itmp(j),j=1,6)
            elseif (ndimen(nc).eq.2) then
              write(iout,'(''scell '')')
              write(iout,'(3f13.8,3i2)') acl,bcl,alpcl,(itmp(j),j=1,3)
            elseif (ndimen(nc).eq.1) then
              write(iout,'(''pcell '')')
              write(iout,'(f13.8,i2)') acl,itmp(1)
            endif
          endif
        endif
!
              ...
              if (inat.le.maxele.or..not.lhideshells) then
                ...
                if (ndimen(nc).eq.3) then
                  if (nc.eq.ncf.and.nreg1.eq.0) then
                    if (.not.lsymopt) then
                      if (lrigid) then
                        call wcoord(iout,lab1,lab2,xfdmp(i),yfdmp(i),zfdmp(i),qlcfg(i+nlsft),occucfg(i+nlsft), &
                    ...
                    elseif (lrhombo) then
                      if (lrigid) then
                        call wcoord(iout,lab1,lab2,xfdmp(nri),yfdmp(nri),zfdmp(nri),qlcfg(i+nlsft), &
        ...
        if (llsym) then
          if (ngocfg(nc).gt.1) then
            ...
            if (iflags(nc).eq.0) then
              ...
              write(iout,'(16a1)') (hmssgio(i,nc),i=1,16)
            endif
...
!
          if (ldump80) then
            ...
            if (ndimen(nc).eq.3) then
              write(iout,'(''cell'')')
              write(iout,'(6f13.8)') acl,bcl,ccl,alpcl,betcl,gamcl
            elseif (ndimen(nc).eq.2) then
              write(iout,'(''scell'')')
              write(iout,'(3f13.8)') acl,bcl,alpcl
            elseif (ndimen(nc).eq.1) then
              write(iout,'(''pcell'')')
              write(iout,'(f13.8)') acl
            endif
          endif
        endif
!
              ...
              if (inat.le.maxele.or..not.lhideshells) then
                if (ndimen(nc).eq.3) then
                  if (nc.eq.ncf.and.nreg1.eq.0) then
                    if (.not.lsymopt) then
                      if (lrigid) then
                        call wcoord(iout,lab1,lab2,xfdmp(i),yfdmp(i),zfdmp(i),qlcfg(i+nlsft), &
...
!
!  Primitive cell for Alamode runs
!
    if (lalamode.and.ndimen(nc).eq.3) then
      write(iout,'(''primitive_vectors '',i6)') ncorepcfg(nc)
      write(iout,'(3f16.9)') (rvpcfg(j,1,nc),j=1,3)
      write(iout,'(3f16.9)') (rvpcfg(j,2,nc),j=1,3)
      write(iout,'(3f16.9)') (rvpcfg(j,3,nc),j=1,3)
    endif
!
!  Write out initial coordinates if required
!
    if (linitcfg(nc)) then
      write(iout,'(''initial_coordinates'')')
      do i = 1,numat
        write(iout,'(3(f16.9,1x))') xinitcfg(nlsft+i),yinitcfg(nlsft+i),zinitcfg(nlsft+i)
      enddo
    endif
!
!  Write out GFNFF reference configuration
!
    if (lgfnff) then
      if (ndimen(nc).gt.0) then
        write(iout,'(''gfnff_vectors '')')
        write(iout,'(3f16.9)') (gfnff_ref_rv(j,1,nc),j=1,3)
        if (ndimen(nc).gt.1) then
          write(iout,'(3f16.9)') (gfnff_ref_rv(j,2,nc),j=1,3)
          if (ndimen(nc).gt.2) then
            write(iout,'(3f16.9)') (gfnff_ref_rv(j,3,nc),j=1,3)
          endif
        endif
      endif
      write(iout,'(''gfnff_coords '')')
      do i = 1,numat
        write(iout,'(3(f16.9,1x),f9.6)') (gfnff_ref_crd(j,i,nc),j=1,3),gfnff_ref_q(i,nc)
      enddo
    endif
!
!  QM/MM mode
...
!  Symmetry number 
!
    if (symnocfg(nc).ne.1) then
      write(iout,'(''symmetry_number '',i4)') symnocfg(nc)
    endif
!
...
!  Fix atom data
!
    if (nfixatomtypecfg(nc).ne.0) then
      if (nfixatomtypecfg(nc).eq.1) then
        write(iout,'(''fix_atom first '')') 
      elseif (nfixatomtypecfg(nc).eq.2) then
        write(iout,'(''fix_atom last '')') 
      elseif (nfixatomtypecfg(nc).eq.3) then
        write(iout,'(''fix_atom centre '')') 
      elseif (nfixatomtypecfg(nc).eq.4) then
        write(iout,'(''fix_atom '',i8)') nfixatomcfg(nc)
      elseif (nfixatomtypecfg(nc).eq.5) then
        write(iout,'(''fix_atom none '')') 
      endif
    endif
!
!  Electric field data
...
!
    if (ntdfieldcfg(nc).gt.0) then
      do k = 1,ntdfieldcfg(nc)
        if (ndimen(nc).eq.2) then
          write(iout,'(''td_field '',f24.10,'' z '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
        elseif (ndimen(nc).eq.1) then
          if (abs(td_fielddirectioncfg(2,k,nc)).gt.1.0d-2) then
            write(iout,'(''td_field '',f24.10,'' y '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          else
            write(iout,'(''td_field '',f24.10,'' z '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          endif
        elseif (ndimen(nc).eq.0) then
          norm = td_fielddirectioncfg(1,k,nc)**2 + td_fielddirectioncfg(2,k,nc)**2 + td_fielddirectioncfg(3,k,nc)**2
          norm = 1.0_dp/sqrt(norm)
          if (norm*td_fielddirectioncfg(3,k,nc).gt.0.99999999_dp) then
            write(iout,'(''td_field '',f24.10,'' z '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          elseif (norm*td_fielddirectioncfg(2,k,nc).gt.0.99999999_dp) then
            write(iout,'(''td_field '',f24.10,'' y '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          elseif (norm*td_fielddirectioncfg(1,k,nc).gt.0.99999999_dp) then
            write(iout,'(''td_field '',f24.10,'' x '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          else
            write(iout,'(''td_field '',f24.10,1x,3f8.5,f12.6,1x,f8.5)') td_fieldcfg(1,k,nc), &
                  (td_fielddirectioncfg(j,k,nc),j=1,3),td_fieldcfg(2,k,nc),td_fieldcfg(3,k,nc)
          endif
        elseif (ndimen(nc).eq.3) then
          if (abs(td_fielddirectioncfg(2,k,nc)).gt.1.0d-2) then
            write(iout,'(''td_field '',f24.10,'' b '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          elseif (abs(td_fielddirectioncfg(3,k,nc)).gt.1.0d-2) then
            write(iout,'(''td_field '',f24.10,'' c '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          else
            write(iout,'(''td_field '',f24.10,'' a '',f12.6,1x,f8.5)') (td_fieldcfg(j,k,nc),j=1,3)
          endif
        endif
      enddo
    endif
...
!
    ...
    if (nsc.ne.nlshift.and.shc.ne.0.0_dp) then
      write(iout,'(''shift'',1x,f18.8)') shc
      nlshift = nsc
    endif
    if (shscalecfg(nc).ne.1.0_dp) then
      write(iout,'(''sshift'',1x,f18.8)') shscalecfg(nc)
    endif
    lfirstout = .true.
    do i = 1,nlasym
      if (leinsteinat(nlsft+i).and.ltifeinsteinat(nlsft+i)) then
        if (lfirstout) then
          lfirstout = .false.
          write(iout,'(''einstein flambda'')')
        endif
        write(iout,'(i5,3(1x,f10.6),1x,f12.6)') i,xfeinsteinat(nlsft+i), &
          yfeinsteinat(nlsft+i),zfeinsteinat(nlsft+i),keinsteinat(nlsft+i)
      endif
    enddo
    lfirstout = .true.
    do i = 1,nlasym
      if (leinsteinat(nlsft+i).and.ltibeinsteinat(nlsft+i)) then
        if (lfirstout) then
          lfirstout = .false.
          write(iout,'(''einstein blambda'')')
        endif
        write(iout,'(i5,3(1x,f10.6),1x,f12.6)') i,xfeinsteinat(nlsft+i), &
          yfeinsteinat(nlsft+i),zfeinsteinat(nlsft+i),keinsteinat(nlsft+i)
      endif
    ...
    lfirstout = .true.
    do i = 1,nlasym
      if (leinsteinat(nlsft+i).and..not.ltibeinsteinat(nlsft+i).and..not.ltifeinsteinat(nlsft+i)) then
        if (lfirstout) then
          lfirstout = .false.
          write(iout,'(''einstein'')')
        endif
...
!
            ...
            if (nbondedtype(1,icm,i).gt.1) then
              ...
              elseif (nbondedtype(1,icm,i).eq.7) then
                conword(nconword) = 'custom'
              elseif (nbondedtype(1,icm,i).eq.8) then
                conword(nconword) = 'half'
              elseif (nbondedtype(1,icm,i).eq.9) then
                conword(nconword) = 'quarter'
              elseif (nbondedtype(1,icm,i).eq.10) then
                conword(nconword) = 'third'
              endif
            endif
        ...
        if (nconnectcfg(ic) .eq. nc) then
          ...
          if (nconnecttype(1,ic).gt.1) then
            ...
            elseif (nconnecttype(1,ic).eq.6) then
              conword(nconword) = 'amide'
            elseif (nconnecttype(1,ic).eq.7) then
              conword(nconword) = 'custom'
            elseif (nconnecttype(1,ic).eq.8) then
              conword(nconword) = 'half'
            elseif (nconnecttype(1,ic).eq.9) then
              conword(nconword) = 'quarter'
            elseif (nconnecttype(1,ic).eq.10) then
              conword(nconword) = 'third'
            endif
          endif
...
!
!  Molecule lists if input
!
    if (lmolatom.and.nmolcfg(nc).gt.0) then
      ii = 0
      do i = 1,nmolcfg(nc)
        write(iout,'(''molatom '',10i6)') (nmollistcfg(ii+j,nc),j=1,nmolatomcfg(i,nc))
        ii = ii + nmolatomcfg(i,nc)
      enddo
    endif
!
!  Rigid molecule lists if relevant
!
    if (lrigid.and.nmolcfg(nc).gt.0) then
      ii = 0
      do i = 1,nmolcfg(nc)
        write(iout,'(''molrigid '',2i6)') nmolatomcfg(i,nc),nmolcorecfg(i,nc)
        if (nmolatomcfg(i,nc).le.10) then
          write(iout,'(10i6)') (nmollistcfg(ii+j,nc),j=1,nmolatomcfg(i,nc))
        else
          ngroup = (nmolatomcfg(i,nc)-1)/10 + 1
          ind = 0
          do ng = 1,ngroup
            if (ng.lt.ngroup) then
              write(iout,'(10i6,'' &'')') (nmollistcfg(ii+ind+j,nc),j=1,10)
              ind = ind + 10
            else
              write(iout,'(10i6)') (nmollistcfg(ii+ind+j,nc),j=1,(nmolatomcfg(i,nc)-ind))
            endif
          enddo
        endif
        if (nc.eq.ncf) then
          write(iout,'(2x,3f14.8,'' # Quaternions'')') (molQ(j,i),j=1,3)
          write(iout,'(2x,3f14.8,'' # COM'')') (molcom(j,i),j=1,3)
          do j = 1,nmolcore(i)
            write(iout,'(2x,3f14.8,'' # Atom '',i6)') (molQxyz(k,j,i),k=1,3),j
          enddo
        else
          write(iout,'(2x,3f14.8,'' # Quaternions'')') (molQcfg(j,i,nc),j=1,3)
          write(iout,'(2x,3f14.8,'' # COM'')') (molcomcfg(j,i,nc),j=1,3)
          do j = 1,nmolcorecfg(i,nc)
            write(iout,'(2x,3f14.8,'' # Atom '',i6)') (molQxyzcfg(k,ii+j,nc),k=1,3),j
          enddo
        endif
        ii = ii + nmolatomcfg(i,nc)
      enddo
        ...
        if (nc.eq.nobcfg(i).and.(nt.ne.2.and.nt.ne.6)) then
          ...
          if (nt.eq.1) then
            ...
            elseif (nobstyle(i).eq.1) then
              write(iout,'(''energy pos '')')
            elseif (nobstyle(i).eq.-1) then
              write(iout,'(''energy neg '')')
            endif
          ...
          elseif (nt.eq.9) then
            ...
            elseif (nobstyle(i).eq.1) then
              write(iout,'(''frequency pos '')')
            elseif (nobstyle(i).eq.-1) then
              write(iout,'(''frequency neg '')')
            endif
            ni = nobptr(i)
          ...
          elseif (nt.eq.28) then
            write(iout,'(''absdipolemoment'')')
            write(iout,'(f12.5,1x,f16.6)') fobs(i),weight(i)
          elseif (nt.eq.29) then
            ...
            enddo
          elseif (nt.eq.30) then
            write(iout,'(''volume '',f12.5,1x,f16.6)') fobs(i),weight(i)
          elseif (nt.eq.31) then
            write(iout,'(''stress'')')
            ni = nobptr(i)
            write(iout,'(i2,f12.5,1x,f16.6)') ni,fobs(i),weight(i)
          elseif (nt.eq.32) then
            write(iout,'(''fenergy'')')
            write(iout,'(f16.8,1x,f16.6)') fobs(i),weight(i)
          elseif (nt.eq.33) then
            write(iout,'(''feqp'')')
            write(iout,'(f16.8,1x,f16.6)') fobs(i),weight(i)
          endif
        endif
...
!
    ...
    if (nlfgrad.gt.0) then
      ...
      if (lsumcoreshell(nc)) then
        write(iout,'(''gradients sum_shells '')')
      else
...
!
!  User specified strain derivatives for fitting
!
    nlfstrain = 0
    ...
    if (nlfstrain.gt.0) then
      write(iout,'(''observables'')')
      write(iout,'(''strain_derivative'')')
      do k = nlfstr,nufstr
        write(iout,'(i4,1x,f14.6)') nfstraint(k),fstrain(k)
      enddo
      write(iout,'(''end'')')
    endif
!
...
!
    ...
    if (nlcon.gt.0.and.(nconin.gt.0.or.index(keyword,'outc').ne.0)) then
      ...
      do i = 1,nlcon
        nfv = ncfixindcfg(i+nlcfst)
        if (ncfixtypcfg(i+nlcfst).eq.iopt_radius) then
          cft = 'r'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_xf) then
          cft = 'x'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_yf) then
          cft = 'y'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_zf) then
          cft = 'z'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_cell) then
          cft = 'c'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_strain) then
          cft = 's'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_xcom) then
          cft = 'xcom'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_ycom) then
          cft = 'ycom'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_zcom) then
          cft = 'zcom'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_xqtn) then
          cft = 'xqtn'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_yqtn) then
          cft = 'yqtn'
        elseif (ncfixtypcfg(i+nlcfst).eq.iopt_zqtn) then
          cft = 'zqtn'
        endif
        nvv = ncvarindcfg(i+nlcfst)
        if (ncvartypcfg(i+nlcfst).eq.iopt_radius) then
          cvt = 'r'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_xf) then
          cvt = 'x'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_yf) then
          cvt = 'y'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_zf) then
          cvt = 'z'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_cell) then
          cvt = 'c'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_strain) then
          cvt = 's'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_xcom) then
          cvt = 'xcom'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_ycom) then
          cvt = 'ycom'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_zcom) then
          cvt = 'zcom'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_xqtn) then
          cvt = 'xqtn'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_yqtn) then
          cvt = 'yqtn'
        elseif (ncvartypcfg(i+nlcfst).eq.iopt_zqtn) then
          cvt = 'zqtn'
        endif
        write(iout,'(i4,1x,a4,1x,i4,1x,a4,1x,f12.6,1x,f12.6)') nvv,cvt,nfv,cft,concocfg(i+nlcfst),conaddcfg(i+nlcfst)
      enddo
      write(iout,'(''end'')')
    endif
!******************************
!  External potential option  *
!******************************
    lfirstout = .true.
    do i = 1,nlasym
      if (abs(extpotcfg(nlsft+i)).gt.1.0d-6) then
        if (lfirstout) then
          lfirstout = .false.
          write(iout,'(''external_potential'')')
        endif
        write(iout,'(i6,1x,f14.6)') i,extpotcfg(nlsft+i)
      endif
    enddo
!**************************
...
!******************************
    if (ntran(nc).ne.0) then
      if (ltrantherm(nc)) then
        write(iout,'(''translate '',3(f12.6,1x),i5,'' thermal '',f8.5)') &
          xtran(nc),ytran(nc),ztran(nc),ntran(nc),trantherm(nc)
      elseif (ltrannoise(nc)) then
        write(iout,'(''translate '',3(f12.6,1x),i5,'' noise '',f8.5)') &
          xtran(nc),ytran(nc),ztran(nc),ntran(nc),trannoise(nc)
      else
...
!  Cell scan option  *
!*********************
    if (ncscan(nc).ne.0) then
      if (lcscanstrain(nc)) then
        if (ndimen(nc).eq.3) then
          write(iout,'(''scan_cell strain '',3(f10.5,1x),'' &'')') (cscan(j,nc),j=1,3)
          write(iout,'(17x,3(f10.5,1x),i5)') (cscan(j,nc),j=4,6),ncscan(nc)
        elseif (ndimen(nc).eq.2) then
          write(iout,'(''scan_cell strain '',3(f12.5,1x),i5)') (cscan(j,nc),j=1,3),ncscan(nc)
        elseif (ndimen(nc).eq.1) then
          write(iout,'(''scan_cell strain '',f12.5,1x,i5)') cscan(1,nc),ncscan(nc)
        endif
      else
        if (ndimen(nc).eq.3) then
          write(iout,'(''scan_cell '',3(f10.5,1x),'' &'')') (cscan(j,nc),j=1,3)
          write(iout,'(10x,3(f10.5,1x),i5)') (cscan(j,nc),j=4,6),ncscan(nc)
          write(iout,'(''scan_cell '',6(f10.5,1x),i5)') (cscan(j,nc),j=1,6),ncscan(nc)
        elseif (ndimen(nc).eq.2) then
          write(iout,'(''scan_cell '',3(f12.5,1x),i5)') (cscan(j,nc),j=1,3),ncscan(nc)
        elseif (ndimen(nc).eq.1) then
          write(iout,'(''scan_cell '',f12.5,1x,i5)') cscan(1,nc),ncscan(nc)
        endif
      endif
    endif
!***********************
!  Shear force option  *
!***********************
    if (lshearforcecfg(nc)) then
      write(iout,'(''shear_force '',f14.8,3(1x,f10.7),'' &'')') shearforcecfg(nc),(shearforcedircfg(i,nc),i=1,3)
      write(iout,'(26x,3(1x,f10.7))') (shearforcenormcfg(i,nc),i=1,3)
    endif
!******************
!  COSMO options  *
!******************
    ...
    if (qonsastarget(nc).ne.0.0_dp) then
      write(iout,'(''qonsas '',f10.4)') qonsastarget(nc)
    endif
!***************************************
...
!*******************************
!  Raman intensity directions  *
!*******************************
    sum = 0.0_dp
    do i = 1,6
      sum = sum + abs(ramandir(i,nc)-1.0_dp)
    enddo
    if (sum.gt.0.0_dp) then
      if (ramandirtype(nc).eq.2) then
        write(iout,'(''rdirection frac in'',3(1x,f7.4),'' out'',3(1x,f7.4))') (ramandir(i,nc),i=1,6)
      else
        write(iout,'(''rdirection in'',3(1x,f7.4),'' out'',3(1x,f7.4))') (ramandir(i,nc),i=1,6)
      endif
    endif
!************************************************************
!  Frequency cutoff for Allen-Feldman thermal conductivity  *
!************************************************************
    if (lomega_af_in(nc)) then
      if (v_s_cfg(nc).gt.0.0_dp.or.v_p_cfg(nc).gt.0.0_dp.or.B_pr_cfg(nc).gt.0.0_dp) then
        write(iout,'(''omega_af '',f18.6,1x,f12.6,2(1x,f12.6))') omega_af(nc),B_pr_cfg(nc),v_s_cfg(nc),v_p_cfg(nc)
      else
        write(iout,'(''omega_af '',f18.6)') omega_af(nc)
      endif
    endif
!****************************
...
!**********************
    if (ndimen(nc).eq.3) then
      if (nks(1,nc)*nks(2,nc)*nks(3,nc).gt.0) then
        nxk = nks(1,nc)
        nyk = nks(2,nc)
        nzk = nks(3,nc)
        if (nxk.eq.nyk.and.nyk.eq.nzk) then
          if (lksorigin(nc)) then
            write(iout,'(''shrink origin '',3i3)') nxk
          else
            write(iout,'(''shrink '',3i3)') nxk
          endif
        else
          if (lksorigin(nc)) then
            write(iout,'(''shrink origin '',3i3)') nxk,nyk,nzk
          else
    ...
    elseif (ndimen(nc).eq.2) then
      if (nks(1,nc)*nks(2,nc).gt.0) then
        nxk = nks(1,nc)
        nyk = nks(2,nc)
        if (nxk.eq.nyk) then
          if (lksorigin(nc)) then
          ...
          if (lksorigin(nc)) then
            write(iout,'(''shrink origin '',3i3)') nxk,nyk
          else
...
!********************
    if (ndimen(nc).eq.3) then
      if (nksala(1,nc)*nksala(2,nc)*nksala(3,nc).gt.0) then
        nxk = nksala(1,nc)
        nyk = nksala(2,nc)
        nzk = nksala(3,nc)
        if (nxk.eq.nyk.and.nyk.eq.nzk) then
          write(iout,'(''ala_shrink '',3i3)') nxk
        else
          write(iout,'(''ala_shrink '',3i3)') nxk,nyk,nzk
        endif
      endif
    elseif (ndimen(nc).eq.2) then
      if (nksala(1,nc)*nksala(2,nc).gt.0) then
        nxk = nksala(1,nc)
        nyk = nksala(2,nc)
        if (nxk.eq.nyk) then
          write(iout,'(''ala_shrink '',3i3)') nxk
        else
          write(iout,'(''ala_shrink '',3i3)') nxk,nyk
        endif
      endif
    elseif (ndimen(nc).eq.1) then
      if (nksala(1,nc).gt.0) then
        nxk = nksala(1,nc)
        write(iout,'(''ala_shrink '',3i3)') nxk
      endif
    endif
...
!  Dielectric constant  *
!************************
    if (dielectriccfg(nc).gt.1.0_dp) then
      write(iout,'(''dielectric_constant '',f10.5)') dielectriccfg(nc)
    endif
!****************
!  Temperature  *
!****************
    if (tempcfg(nc).ne.0.0_dp) then
      if (ntempramp(nc).gt.1) then
!
!  Multiple temperature ramps
!
        if (ntempstpstart(1,nc).ne.0) then
          write(iout,'(''temperature multi '',f10.3,1x,f15.9,1x,2(i10,1x))') &
            tempcfg(nc),tempstp(1,nc),ntempstp(1,nc),ntempstpstart(1,nc)
        else
          write(iout,'(''temperature multi '',f10.3,1x,f15.9,1x,i10)') &
            tempcfg(nc),tempstp(1,nc),ntempstp(1,nc)
        endif
        do i = 2,ntempramp(nc)
          if (ntempstpstart(i,nc).ne.0) then
            write(iout,'(29x,f15.9,1x,2(i10,1x))') &
              tempstp(i,nc),ntempstp(i,nc),ntempstpstart(i,nc)
          else
            write(iout,'(29x,f15.9,1x,i10)') &
              tempstp(i,nc),ntempstp(i,nc)
          endif
        enddo
      elseif (ntempramp(nc).eq.1) then
!
!  Single temperature ramp
!
        if (ntempstpstart(1,nc).ne.0) then
          write(iout,'(''temperature '',f10.3,1x,f15.9,1x,2(i10,1x))') &
            tempcfg(nc),tempstp(1,nc),ntempstp(1,nc),ntempstpstart(1,nc)
        else
          write(iout,'(''temperature '',f10.3,1x,f15.9,1x,i10)') &
            tempcfg(nc),tempstp(1,nc),ntempstp(1,nc)
        endif
      else
!
...
!  Potential sites  *
!********************
    if (npotsitescfg(nc).gt.0) then
      lfirstout = .true.
      do j = 1,npotsites
        if (npotsitecfg(j).eq.nc) then
          if (lfirstout) then
            lfirstout = .false.
            if (lpotsitecartcfg(nc)) then
              write(iout,'(''potsites cart '')')
            else
              write(iout,'(''potsites'')')
            endif
          endif
...
!***********************
    ...
    if (tmdforcestart(nc).gt.1.0d-12) then
      write(iout,'(''delay_force '',f12.4,'' ps'')') tmdforcestart(nc)
    endif
    if (tmdforcestop(nc).gt.1.0d-12) then
      write(iout,'(''end_force   '',f12.4,'' ps'')') tmdforcestop(nc)
    endif
    if (tmdfieldstart(nc).gt.1.0d-12) then
      write(iout,'(''delay_field '',f12.4,'' ps'')') tmdfieldstart(nc)
    endif
    if (tmdfieldstop(nc).gt.1.0d-12) then
      write(iout,'(''end_field   '',f12.4,'' ps'')') tmdfieldstop(nc)
    endif
...
!
            if (nwptr+16.gt.maxlinelength) then
              write(iout,'(a)') trim(line2)
              line2 = ' '
              write(line2,'(''tether '')')
              nwptr = 7
              lfirstfind = .true.
...
!  Will writing exceed line length?
!
            if (nwptr+8.gt.maxlinelength) then
              write(iout,'(a)') trim(line2)
              line2 = ' '
              write(line2,'(''tether '')')
              nwptr = 7
...
!
      if (lti) then
        write(iout,'(''dudlambda '',i8,1x,i8)') nlambdanow,ntinow
        lam = lambda_initial
        do i = 1,nlambdanow
          write(iout,'(f9.6,1x,f24.12)') lam,dUdlambda(i)
          lam = lam + lambda_step
        enddo
      endif
      write(iout,'(''# '')')
...
!************
    if (nldef.gt.0) then
      ...
      if (nc.lt.ncf) then
!
!  Find start of configuration in arrays
!
        do i = 1,nc-1
...
!
        write(iout,'(''region_1 '',i4)') nreg1cfg(nc)
        do i = 1,nreg1cfg(nc)
          inat = natdefecfg(ndind+i)
          itype = ntypdefecfg(ndind+i)
          xal = xdefecfg(ndind+i)
          yal = ydefecfg(ndind+i)
          zal = zdefecfg(ndind+i)
          q  = qdefecfg(ndind+i)
          oc = occdefecfg(ndind+i)
          ri = radefecfg(ndind+i)
          nm = ndefmolcfg(ndind+i)
          nmi = ndefindcfg(ndind+i)
          lbre = ldefbsmatcfg(ndind+i)
          ldqm = ldqmatomcfg(ndind+i)
          id1 = idoptcfg(1,ndind+i)
          id2 = idoptcfg(2,ndind+i)
          id3 = idoptcfg(3,ndind+i)
!
          call label(inat,itype,lab1)
          if (ldqm) then
            if (lbre) then
        ...
        if (mode2a.ge.3) then
          ...
          enddo
          if (nintecfg(nc).gt.0) then
            write(iout,'(''dlist_interstitial '',i4,1x,i4)') nintecfg(nc)
            write(iout,'(15(i4,1x))')(ndintptrcfg(niind+i),i=1,nintecfg(nc))
          endif
          if (nvacacfg(nc).gt.0) then
            write(iout,'(''dlist_vacancy '',i4,1x,i4)') nvacacfg(nc)
            do i = 1,nvacacfg(nc)
              inat = natdvacptrcfg(nvind+i)
              itype = ntypdvacptrcfg(nvind+i)
              call label(inat,itype,lab1)
              if (lbrdvacptrcfg(nvind+i)) then
                if (inat.gt.maxele) then
                  lab2 = 'bshe'
                else
                  lab2 = 'bcor'
              ...
              write(iout,'(a5,1x,a4,3(1x,f10.5))') lab1,lab2,(xyzdvacptrcfg(j,nvind+i),j=1,3)
            enddo
          endif
        endif
        if (lmol) then
          write(iout,'(''reldef '')')
          write(iout,'(15(i4,1x))')(nreldefcfg(ndind+i),i=1,nreg1cfg(nc))
        endif
...
!
        ...
        if (ldsym) then
!
!  Dimenion of array below can usually be ndasym, except if move2a1 is being
!  used in which bounds would be exceeded. Use full region 1 size to be safe.
!
          allocate(itmp2(3*nreg1),stat=status)
          ...
          do i = 1,nvar
            if (idopttype(i).eq.idopt_dx) then
              itmp2(3*idoptindex(i)-2) = 1
            elseif (idopttype(i).eq.idopt_dy) then
              itmp2(3*idoptindex(i)-1) = 1
            elseif (idopttype(i).eq.idopt_dz) then
              itmp2(3*idoptindex(i)) = 1
            endif
          enddo
!
...
!
          ...
          do i = 1,nvar
            if (idopttype(i).eq.idopt_dx) then
              itmp(3*idoptindex(i)-2) = 1
            elseif (idopttype(i).eq.idopt_dy) then
              itmp(3*idoptindex(i)-1) = 1
            elseif (idopttype(i).eq.idopt_dz) then
              itmp(3*idoptindex(i)) = 1
            endif
          enddo
        endif
...
!
!  ghost_supercell option
!
    if ((nsuperghost(1,nc)+nsuperghost(2,nc)+nsuperghost(3,nc)).gt.3) then
      if (ordersuper(nc).eq.2) then
        write(iout,'(''ghost_supercell zyx '',3i4)') (nsuperghost(j,nc),j=1,3)
      else
        write(iout,'(''ghost_supercell '',3i4)') (nsuperghost(j,nc),j=1,3)
      endif
    endif
!
!  fc_supercell option
!
    if ((nd2cellcfg(1,nc)+nd2cellcfg(2,nc)+nd2cellcfg(3,nc)).gt.0) then
      write(iout,'(''fc_supercell '',3i4)') (nd2cellcfg(j,nc),j=1,3)
    endif
!
!  SPME grid
!
    if (nqkgrid(1,nc).ne.24.or.nqkgrid(2,nc).ne.24.or.nqkgrid(3,nc).ne.24) then
      write(iout,'(''qgrid '',3i6)') (nqkgrid(j,nc),j=1,3)
    endif
!******************************
