...
!    6 => nonbonded only potential (excludes atoms up to 3 bonds away)
!
...
!  11/12 Duplication of zero check in Lennard-Jones removed
!  11/12 symbol2 array now set when potentials are automatically generated for L-J potentials
!  11/12 Missing copy of scale14 for automatic generation added 
!   5/13 Custom bond type added
!   7/13 Product combination rule added
!   3/14 Fractional bond orders added
!   8/14 Taper range added to manybody potential
!   8/14 lorder12 added
!   9/14 ZBL potential added
!   2/15 MM3buck and lmm3se added
!   3/15 all sub-option added to mm3buck
!   3/15 MM3stretch added
!   3/15 x14 sub-option added
!   3/15 mmexcse added
!   3/15 Error in addressing symbolbondQ corrected
!   4/15 Error in setting cutoffs for mm3buck in 1 case corrected
!  10/16 Lennard-Jones with fixed coefficients added
!   2/17 Change to defaults on lenn option input line
!   2/18 Slater potential added
!   1/19 Worsy changed to use word length parameter
!   3/20 Use of angstoev changed to inverse_angstroms_to_ev
!  11/21 Modifications for TI added
!   3/22 Handling of multiple species matches added for selected potentials
!  11/22 TI flags corrected for lautolj case
!   2/23 Molecule warning extended
!   6/23 AA-CLP twobody potential added
!
!  Conditions of use:
!
...
!
  ...
  character(len=5), allocatable, save :: syms(:,:)
  integer(i4)                         :: i
  integer(i4)                         :: iadd
  integer(i4)                         :: ilp
  integer(i4)                         :: ind
  ...
  integer(i4)                         :: nvar1
  integer(i4)                         :: nvar2
  integer(i4),      allocatable, save :: itypes(:,:)
  integer(i4),      allocatable, save :: nvars(:,:)
  logical                             :: lautolj
  logical                             :: lautomm3
  logical                             :: letaper
  logical                             :: lk3
...
!
  ...
  if (index(word,'zbl').eq.1)  goto 420
  if (index(word,'mm3b').eq.1) goto 430
  if (index(word,'mm3s').eq.1) goto 440
  if (index(word,'slat').eq.1) goto 450
  if (index(word,'aaclp_t').eq.1) goto 460
  return
!*************************************************
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'zer').eq.1) then
        iadd = 0
      elseif (index(words(i),'c1c').eq.1) then
        iadd = 47
      ...
      elseif (index(words(i),'geo').eq.1) then
        ...
        lcombine(npote+1) = .true.
      elseif (index(words(i),'pro').eq.1) then
        ncombipower(npote+1) = 1
        lcombine(npote+1) = .true.
      elseif (index(words(i),'bon').eq.1) then
        mmexc(npote+1) = 1
        lintra(npote+1) = .true.
...
!
!  Set exponents and/or 1-4 scaling factor if specified
!
  if (nfloat.ge.3) then
!
!  For 3 values exponents and 1-4 scale must be specified
!
  ...
  elseif (nfloat.eq.2) then
!
!  For 2 values exponents are likely input
!
    tpot(1,npote+1) = floats(1)
    tpot(2,npote+1) = floats(2)
  elseif (nfloat.eq.1) then
!
!  For 1 value 1-4 scale factor is likely input
!
    rscale = abs(floats(1))
    if (rscale.ge.0.0_dp.and.rscale.lt.1.0_dp) then
      scale14(npote+1) = rscale
...
!
  ...
  elseif (nptype(npote).eq.59) then
    twopot(3,npote) = 4.0_dp
    twopot(4,npote) = 4.0_dp
  endif
  if (lautolj) then
    read(iin,'(a)',end=198) line
    iline = iline + 1
...
!  Generate symbols
!
          call label(nspec1(npote),nptyp1(npote),symbol2(1,npote))
          call label(nspec2(npote),nptyp2(npote),symbol2(2,npote))
!
...
!
          ...
          ltib2body(npote) = ltib2body(npot1)
          ltif2body(npote) = ltif2body(npot1)
        enddo
      enddo
    else
!
...
210 units = 1.0_dp
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'kj').eq.1) then
        units = kjmtoev
      elseif (index(words(i),'mm3').eq.1) then
        lmm3 = .true.
      elseif (index(words(i),'bon').eq.1) then
        nmmexcse = 1
      elseif (index(words(i),'x12').eq.1) then
...
!
  ...
  if (nvar1.eq.nvar2) then
    ...
    if (itype1.lt.itype2) then
      ...
      symbolbondQ(1,nbondQ) = sym21
      symbolbondQ(2,nbondQ) = sym22
    else
      nbondQtyp1(nbondQ) = itype2
      nbondQtyp2(nbondQ) = itype1
      symbolbondQ(1,nbondQ) = sym22
      symbolbondQ(2,nbondQ) = sym21
...
!
  if (llibrary) then
    allocate(nvars(2,maxpair))
    allocate(itypes(2,maxpair))
    allocate(syms(2,maxpair))
  else
    allocate(nvars(2,1))
    allocate(itypes(2,1))
    allocate(syms(2,1))
...
!
  if (llibrary) then
    call getpotsymbols2(iline,llibrary,maxpair,npair,nvars,itypes,syms,nbeg,lvalidpot)
!
!  Check dimension maxpair
!
...
!
!  Check that there will be enough space in the arrays for the pairs to be added
!
  if (npote+npair-1.gt.maxpot) then
    maxpot = npote + npair
    call changemaxpot
  endif
!*************************************
!  Loop over valid pairs of species  *
!*************************************
  do np = 1,npair
    if (np.gt.1) then
      npote = npote + 1
!
!  Copy potential information set so far
!
      leshift(npote) = leshift(npot0)
      lgshift(npote) = lgshift(npot0)
      lintra(npote) = lintra(npot0)
      linter(npote) = linter(npot0)
      ltib2body(npote) = ltib
      ltif2body(npote) = ltif
      mmexc(npote) = mmexc(npot0)
      n2botype(1:2,npote) = n2botype(1:2,npot0)
      nptype(npote) = nptype(npot0)
      scale14(npote) = scale14(npot0)
      twopot(1:4,npote) = twopot(1:4,npot0)
      tpot(1,npote) = tpot(1,npot0)
      rpot(npote) = rpot(npot0)
      rpot2(npote) = rpot2(npot0)
    endif
!
!  Add atom types for this pair
!
    if (nvars(1,np).eq.nvars(2,np)) then
      nspec1(npote) = nvars(1,np)
      nspec2(npote) = nvars(2,np)
      if (itypes(1,np).lt.itypes(2,np)) then
        nptyp1(npote) = itypes(1,np)
        nptyp2(npote) = itypes(2,np)
        symbol2(1,npote) = syms(1,np)
        symbol2(2,npote) = syms(2,np)
      else
        nptyp1(npote) = itypes(2,np)
        nptyp2(npote) = itypes(1,np)
        symbol2(1,npote) = syms(2,np)
        symbol2(2,npote) = syms(1,np)
      endif
    elseif (nvars(1,np).lt.nvars(2,np)) then
      nspec1(npote) = nvars(1,np)
      nspec2(npote) = nvars(2,np)
      nptyp1(npote) = itypes(1,np)
      nptyp2(npote) = itypes(2,np)
      ...
      lorder12(npote) = .false.
      nspec1(npote) = nvars(2,np)
      nspec2(npote) = nvars(1,np)
      nptyp1(npote) = itypes(2,np)
      nptyp2(npote) = itypes(1,np)
      symbol2(1,npote) = syms(2,np)
      symbol2(2,npote) = syms(1,np)
...
!  coulomb subtraction is turned off.
!
  if (abs(nvars(1,1)-nvars(2,1)).eq.maxele.and.rpot2(npote).eq.0.0_dp) then
    if (tpot(1,npote).eq.1) then
      nwarn = nwarn + 1
      if (ioproc) then
        write(ioout,'(/,''**** Warning - core-shell spring constant set for coulomb subtraction ****'')')
      ...
      tpot(1,npot1:npote) = 0.0_dp
      nptype(npot1:npote) = 5
    endif
  endif
!
  goto 365
...
418 if (.not.l55) l1000 = .true.
  ...
  return
!*********************************
!  Parameters for ZBL potential  *
!*********************************
420 if (npote.eq.maxpot) then
    maxpot = npote + 10
    call changemaxpot
  endif
  call init2bodydefaults(npote+1_i4)
...
!
  if (mmexc(npote).eq.1) then
    if (nfloat.gt.10) nfloat = nfloat - 9
!
!  Assign coefficients and cutoffs
!
    if (nfloat.ge.10) then
      ...
      twopot(10,npote) = floats(10+nbeg)
    elseif (nfloat.eq.0) then
      twopot(1,npote)  = 0.18175_dp
      twopot(2,npote)  = 3.19980_dp
      twopot(3,npote)  = 0.50986_dp
      twopot(4,npote)  = 0.94229_dp
      twopot(5,npote)  = 0.28022_dp
      twopot(6,npote)  = 0.40290_dp
      twopot(7,npote)  = 0.02817_dp
      twopot(8,npote)  = 0.20162_dp
      twopot(9,npote)  = 0.46850_dp
      twopot(10,npote) = 0.23_dp
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('potword21')
    endif
...
!
!  If number of floats is greater than 12, assume that fitting flags have been left on line
!
    if (nfloat.gt.12) nfloat = nfloat - 9
!
!  Assign coefficients and cutoffs
!
    if (nfloat.eq.12) then
      twopot(1,npote)  = floats(1+nbeg)
      twopot(2,npote)  = floats(2+nbeg)
      twopot(3,npote)  = floats(3+nbeg)
      twopot(4,npote)  = floats(4+nbeg)
      ...
      twopot(9,npote)  = floats(9+nbeg)
      twopot(10,npote) = floats(10+nbeg)
      rpot2(npote)     = floats(11+nbeg)
      rpot(npote)      = floats(12+nbeg)
    elseif (nfloat.eq.2) then
      twopot(1,npote)  = 0.18175_dp
      twopot(2,npote)  = 3.19980_dp
      twopot(3,npote)  = 0.50986_dp
...
!
!  Compute coefficient that depends on atomic numbers
!
  tpot(3,npote) = inverse_angstroms_to_ev*dble(nvar1*nvar2)
  tpot(4,npote) = dble(nvar1)**twopot(10,npote) + dble(nvar2)**twopot(10,npote)
!
  if (nvar1.eq.nvar2) then
    nspec1(npote) = nvar1
    nspec2(npote) = nvar2
...
748 if (.not.l55) l1000 = .true.
  ...
  return
!********************************************
!  Parameters for MM3 Buckingham potential  *
!********************************************
430 if (npote.eq.maxpot) then
    maxpot = npote + 10
    call changemaxpot
  endif
  call init2bodydefaults(npote+1_i4)
...
!
  ...
  if (.not.lcombine(npote+1)) lautomm3 = .false.
  if (.not.lfound) then
    lintra(npote+1) = lint
    linter(npote+1) = linr
  endif
...
!
  ...
  if (nword.gt.0.and..not.lautomm3) then
    word = words(1)
    call worsy(word,lsymbol,.true.)
    if (.not.lsymbol) then
      l55 = .true.
...
!
      if (nfloat.eq.5) then
        ...
        twopot(2,npote) = floats(2+nbeg)
        twopot(3,npote) = floats(3+nbeg)
        twopot(4,npote) = floats(4+nbeg)*units
        twopot(5,npote) = floats(5+nbeg)
      elseif (nfloat.eq.4) then
        twopot(1,npote) = floats(1+nbeg)
        twopot(2,npote) = floats(2+nbeg)
...
!  Check rho is non-zero
!
    if (twopot(5,npote).lt.1.0d-8) then
      call outerror('MM3 Buckingham potential has rv of zero',iline)
      call stopnow('potword21')
    endif
  endif
  if (.not.lautomm3) then
...
438 if (.not.l55) l1000 = .true.
  ...
  lwordok = .true.
!
!  Do automatic generation of further MM3buck potentials if necessary
!
  if (lautomm3) then
    npot1 = npote
    npote = npote - 1
...
!
    ...
    enddo
    do np = npot1,npote
      if (leshift(np).and..not.lautomm3) call potcut(np)
    enddo
  endif
  return
!*******************************************************************
!  Parameters for MM3 bond stretch with Coulomb subtracted option  *
!*******************************************************************
440 if (npote.eq.maxpot) then
    maxpot = npote + 10
    call changemaxpot
  endif
  call init2bodydefaults(npote+1_i4)
...
!
  if (mmexc(npote).eq.1) then
    if (nfloat.gt.6) nfloat = nfloat - 5
    if (nfloat.eq.6) then
      twopot(1,npote) = floats(1+nbeg)*units
      twopot(2,npote) = floats(2+nbeg)
      twopot(3,npote) = floats(3+nbeg)
      twopot(4,npote) = floats(4+nbeg)
      twopot(5,npote) = floats(5+nbeg)
      tpot(1,npote)   = floats(6+nbeg)
    elseif (nfloat.eq.5) then
      twopot(1,npote) = floats(1+nbeg)*units
      twopot(2,npote) = floats(2+nbeg)
      twopot(3,npote) = floats(3+nbeg)
      twopot(4,npote) = floats(4+nbeg)
      twopot(5,npote) = 7.0_dp/12.0_dp
      tpot(1,npote)   = floats(5+nbeg)
    elseif (nfloat.eq.4) then
      twopot(1,npote) = floats(1+nbeg)*units
      twopot(2,npote) = floats(2+nbeg)
      twopot(3,npote) = floats(3+nbeg)
      twopot(4,npote) = 2.55_dp
      twopot(5,npote) = 7.0_dp/12.0_dp
      tpot(1,npote)   = floats(4+nbeg)
    elseif (nfloat.eq.3) then
      twopot(1,npote) = floats(1+nbeg)*units
      twopot(2,npote) = floats(2+nbeg)
      twopot(3,npote) = 71.94_dp
      twopot(4,npote) = 2.55_dp
      twopot(5,npote) = 7.0_dp/12.0_dp
      tpot(1,npote)   = floats(3+nbeg)
    elseif (nfloat.eq.2) then
    ...
    if (nfloat.gt.8) nfloat = nfloat - 5
    if (nfloat.eq.8) then
      twopot(1,npote) = floats(1+nbeg)*units
      twopot(2,npote) = floats(2+nbeg)
      twopot(3,npote) = floats(3+nbeg)
...
448 if (.not.l55) l1000 = .true.
  ...
  return
!************************************
!  Parameters for Slater potential  *
!************************************
...
458 if (.not.l55) l1000 = .true.
  ...
  return
!****************************************
!  Parameters for AA-CLP two potential  *
!****************************************
