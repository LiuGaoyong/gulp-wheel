...
!   8/22 Choice between mass-weighted eigenvectors or not added
!
!  Conditions of use:
!
...
!
  ...
  use phonout,        only : leigenvectors_original
#endif
  use projectdos
  use species
#ifdef TRACE
...
!
  ...
  real(dp)                                     :: drQ(3,3)
  real(dp)                                     :: drR(3,3,3)
  real(dp),    dimension(:),   allocatable     :: egr
  real(dp)                                     :: egrl(6,6)
...
!
  allocate(egr(mtv),stat=status)
  if (status/=0) call outofmemory('peigengd','egr')
  allocate(enorm(mtvloc),stat=status)
  if (status/=0) call outofmemory('peigengd','enorm')
!
...
!************************************************************************
  if (.not.leigenvectors_original) then
    do m = 1,mtvloc
      enorm(m) = 0.0_dp
      do i = 1,mtv
         enorm(m) = enorm(m) + (eigr(i,m)*rfmass(i))**2
      enddo
...
!
        if (mm.gt.0) then
          if (leigenvectors_original) then
            ...
            do n = 1,ntmp
              etmp(n,1) = eigr(n,mm)*enorm(mm)*rfmass(n)
            enddo
          endif
!
!  Post send
...
!
        ...
        if (mm.gt.0) then
          ...
          write(53,'(f15.6)') freq(m)
          if (leigenvectors_original) then
            egr(1:mtv) = eigr(1:mtv,mm)
          else
            do n = 1,mtv
              egr(n) = eigr(n,mm)*enorm(mm)*rfmass(n)
            enddo
          endif
          ii = 0
          do i = 1,nphonatc
            write(53,'(3f10.6)') (egr(ii+j),j=1,3)
            ii = ii + 3
          enddo
          if (lrigid) then
            do i = 1,nphonatm
              write(53,'(6f10.6)') (egr(ii+j),j=1,6)
              ii = ii + 6
            enddo
          endif
        endif
...
!
          do i = 1,nphonatc
            ig = ig + 1
            write(54,'(2i5,2f16.12,4x,2f16.12,4x,2f16.12)') m,ig, &
              egr(ii+1),0.0_dp,egr(ii+2),0.0_dp,egr(ii+3),0.0_dp
            ii = ii + 3
          enddo
          if (lrigid) then
            do i = 1,nphonatm
          ...
          if (mtvrptr(indi+1).gt.0) then
            do k = 1,3
              if (leigenvectors_original) then
                ...
                do n = 1,mtv
                  etmp(n,k) = eigr(n,mtvrptr(indi+k))*enorm(mtvrptr(indi+k))*rfmass(n)
                enddo
              endif
            enddo
!
...
!
          ...
          if (mtvrptr(indi+1).gt.0) then
            ...
            do j = 1,njloop
              do k = 1,3
                if (leigenvectors_original) then
                  egrl(1:3,k) = eigr(indj+1:indj+3,mtvrptr(indi+k))
                else
                  do n = 1,3
                    egrl(n,k) = eigr(indj+n,mtvrptr(indi+k))*enorm(mtvrptr(indi+k))*rfmass(indj+n)
                  enddo
                endif
              enddo
!
              write(ioout,'(i6,'' x '',5x,3f10.6)') j,(egrl(1,k),k=1,3)
              write(ioout,'(i6,'' y '',5x,3f10.6)') j,(egrl(2,k),k=1,3)
              write(ioout,'(i6,'' z '',5x,3f10.6)') j,(egrl(3,k),k=1,3)
              indj = indj + 3
            enddo
            if (lrigid) then
              do j = 1,nphonatm
                do k = 1,3
                  if (leigenvectors_original) then
                    egrl(1:6,k) = eigr(indj+1:indj+6,mtvrptr(indi+k))
                  else
                    do n = 1,6
                      egrl(n,k) = eigr(indj+n,mtvrptr(indi+k))*enorm(mtvrptr(indi+k))*rfmass(indj+n)
                    enddo
...
!
  ...
  if (status/=0) call deallocate_error('peigengd','enorm')
  deallocate(egr,stat=status)
  if (status/=0) call deallocate_error('peigengd','egr')
#ifdef TRACE
  call trace_out('peigengd')
#endif
#else
!
!  Dummy setting of ir to avoid compiler warning
!
  ir(1) = 0.0_dp
!
