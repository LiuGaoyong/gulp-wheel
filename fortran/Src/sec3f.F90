...
!
  ...
  if (status/=0) call outofmemory('sec3f','ioptfindex')
...
!
  if (lsdebug.and.ioproc) then
    ...
    do i = 1,n3f
      write(ioout,'(2x,9(f9.4))')(derv2(i,j),j=1,n3a)
    enddo
    write(ioout,'(/)')
  endif
!***********************************************************************
...
!
      if (nbsm.gt.0) then
        ...
        do i = 1,nfa
          do j = 1,nff
            jj = npf(j)
            if (lbsmat(i).and.lbsmat(jj)) then
...
!
  ...
  if (status/=0) call deallocate_error('sec3f','ioptfindex')
!
!  Timing
!
  t2 = g_cpu_time()
