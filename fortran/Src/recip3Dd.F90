...
!   1/22 d2charge call changed to d2charged
!   1/22 Corrections to d2charge loop
!   2/22 Parallel correction to handle division of quantities by nproc where
!        they are non-zero on entry to this routine
...
!
  ...
  if (status/=0) call outofmemory('recip3Dd','xdrl')
  allocate(ydrl(numat),stat=status)
  if (status/=0) call outofmemory('recip3Dd','ydrl')
  allocate(zdrl(numat),stat=status)
  if (status/=0) call outofmemory('recip3Dd','zdrl')
!
!  Setup
!
  eta4 = 0.25_dp/eta
                  ...
                  do kk = 1,6
                    ...
                    do ll = kk,6
                      if (lrigid) then
                        dGrds2 = xrkk*drxyzds(ll,1) + yrkk*drxyzds(ll,2) + zrkk*drxyzds(ll,3) + &
                                 xd*dgds(iv,1,ll) + yd*dgds(iv,2,ll) + zd*dgds(iv,3,ll)
                        sderv2(ll,kk) = sderv2(ll,kk) - 0.5_dp*(cosq*dGrds1*dGrds2 + sinq*( &
                                                        xrkk*d2rxyzds2(ll,kk,1) + xd*d2gds2(iv,1,ll,kk) + &
                                                        yrkk*d2rxyzds2(ll,kk,2) + yd*d2gds2(iv,2,ll,kk) + &
                                                        zrkk*d2rxyzds2(ll,kk,3) + zd*d2gds2(iv,3,ll,kk) + &
                                                        dgds(iv,1,ll)*drxyzds(kk,1) + dgds(iv,1,kk)*drxyzds(ll,1) + &
...
!***********************************
!
!  If nprocs > 1 then multiply some quantities by the number processors as they will be later divided by this
!
    if (nprocs.gt.1.and.lstr) then
...
!
          if (lgrad2) then
            ...
            do jj = 1,nchargec6
              ...
              derv2(jx,iy) = derv2(jx,iy) + argck*d2g2dx2(iv,6)
              derv2(jy,iy) = derv2(jy,iy) + argck*d2g2dx2(iv,2)
              derv2(jz,iy) = derv2(jz,iy) + argck*d2g2dx2(iv,4)
              derv2(jx,iz) = derv2(jx,iz) + argck*d2g2dx2(iv,5)
              derv2(jy,iz) = derv2(jy,iz) + argck*d2g2dx2(iv,4)
...
!
        ...
        call d2charged(ii,i,j,nkvec,ix,iy,iz,jx,jy,jz,phsq,ktrm4,d1ix,d1iy,d1iz, &
                       d1jx,d1jy,d1jz,d1is,d1js,csin,argc,sine,d2self,0.0_dp,0.0_dp, &
                       .false.,.false.)
      enddo
    enddo
  endif
...
!
  ...
  if (status/=0) call deallocate_error('recip3Dd','zdrl')
  deallocate(ydrl,stat=status)
  if (status/=0) call deallocate_error('recip3Dd','ydrl')
  deallocate(xdrl,stat=status)
  if (status/=0) call deallocate_error('recip3Dd','xdrl')
