  subroutine recip3Dpd(xkv,ykv,zkv)
!
!  Calculate phased second derivatives in reciprocal space for phonon calculations.
!  Distribute memory version.
!
...
!
  ...
  if (status/=0) call outofmemory('recip3Dpd','ktrm0')
  allocate(ktrm3(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip3Dpd','ktrm3')
  allocate(ktrm6(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip3Dpd','ktrm6')
  allocate(ktrm60(maxkvec),stat=status)
  if (status/=0) call outofmemory('recip3Dpd','ktrm60')
  allocate(tmp6(6,maxkvec),stat=status)
  if (status/=0) call outofmemory('recip3Dpd','tmp6')
  if (lgroupvelocity) then
    allocate(ktrmdk(3_i4,maxkvec),stat=status)
    if (status/=0) call outofmemory('recip3Dpd','ktrmdk')
    allocate(ktrm6dk(3_i4,maxkvec),stat=status)
    if (status/=0) call outofmemory('recip3Dpd','ktrm6dk')
  endif
!
!  Setup 
!
...
!
        ...
        if (ldoc6) then
          ...
          do iv = 1,nkvec
            ...
            d2trmi = sinq - sin6
            do ii = 1,6
              d2qr(ii) = d2qr(ii) + d2trmr*tmp6(ii,iv)
              d2qi(ii) = d2qi(ii) - d2trmi*tmp6(ii,iv)
            enddo
            if (lgroupvelocity) then
...
!
                cdk(1,ii) = cdk(1,ii) + dcmplx(cosa*d2trmx*tmp6(ii,iv),-sina*d2trmx*tmp6(ii,iv))
                cdk(2,ii) = cdk(2,ii) + dcmplx(cosa*d2trmy*tmp6(ii,iv),-sina*d2trmy*tmp6(ii,iv))
                cdk(3,ii) = cdk(3,ii) + dcmplx(cosa*d2trmz*tmp6(ii,iv),-sina*d2trmz*tmp6(ii,iv))
              enddo
!
...
!
              if (lrigid) then
                ...
                do ii = 1,6
                  cdk(1,ii) = cdk(1,ii) + dcmplx(-sina*xcom*trmc*tmp6(ii,iv),-cosa*xcom*trmc*tmp6(ii,iv))
                  cdk(2,ii) = cdk(2,ii) + dcmplx(-sina*ycom*trmc*tmp6(ii,iv),-cosa*ycom*trmc*tmp6(ii,iv))
                  cdk(3,ii) = cdk(3,ii) + dcmplx(-sina*zcom*trmc*tmp6(ii,iv),-cosa*zcom*trmc*tmp6(ii,iv))
                enddo
              endif
            endif
          enddo
...
!
          ...
          do iv = 1,nkvec
            ...
            sinq = sina*ktrm(iv)
            do ii = 1,6
              d2qr(ii) = d2qr(ii) + cosq*tmp6(ii,iv)
              d2qi(ii) = d2qi(ii) - sinq*tmp6(ii,iv)
            enddo
            if (lgroupvelocity) then
              do ii = 1,6
!
!  Terms from derivatives of G terms
!
                cdk(1,ii) = cdk(1,ii) + dcmplx(cosa*ktrmdk(1,iv)*tmp6(ii,iv),-sina*ktrmdk(1,iv)*tmp6(ii,iv))
                cdk(2,ii) = cdk(2,ii) + dcmplx(cosa*ktrmdk(2,iv)*tmp6(ii,iv),-sina*ktrmdk(2,iv)*tmp6(ii,iv))
                cdk(3,ii) = cdk(3,ii) + dcmplx(cosa*ktrmdk(3,iv)*tmp6(ii,iv),-sina*ktrmdk(3,iv)*tmp6(ii,iv))
              enddo
!
!  Terms from derivatives of G_alpha.G_beta terms
!
...
!
        ...
        d1iy = d1iy*qlj*oci
        d1iz = d1iz*qlj*oci
        call d2chargepd(iloc,i,j,nkvec,ix,iy,iz,jx,jy,jz,xkv,ykv,zkv,ktrm6,ktrm60, &
                        d1ix,d1iy,d1iz,d1jx,d1jy,d1jz,csin,argc,sine,d2self, &
                        0.0_dp,0.0_dp,.false.,.false.)
      enddo
    enddo
  endif
!
...
!
  if (lgroupvelocity) then
    ...
    if (status/=0) call deallocate_error('recip3Dpd','ktrm6dk')
    deallocate(ktrmdk,stat=status)
    if (status/=0) call deallocate_error('recip3Dpd','ktrmdk')
  endif
  deallocate(tmp6,stat=status)
  if (status/=0) call deallocate_error('recip3Dpd','tmp6')
  deallocate(ktrm60,stat=status)
  if (status/=0) call deallocate_error('recip3Dpd','ktrm60')
  deallocate(ktrm6,stat=status)
  if (status/=0) call deallocate_error('recip3Dpd','ktrm6')
  deallocate(ktrm3,stat=status)
  if (status/=0) call deallocate_error('recip3Dpd','ktrm3')
  deallocate(ktrm0,stat=status)
  if (status/=0) call deallocate_error('recip3Dpd','ktrm0')
!
!  Timing
!
  time1 = g_cpu_time()
