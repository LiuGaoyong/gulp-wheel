  subroutine eemd(lmain,lgrad1,lgrad2)
!
...
!   6/21 Call to dbcgsolve changed to use dcgsolve for symmetric matrix case
!   1/22 Matrix inversion solve added in parallel
!   4/22 Parallel correction for case where nprocs > neem
!   7/23 nregionnocfg added
!   7/23 Update of qlcfg restricted to non-supercell calls
!   7/23 extpot added
!   9/23 Sign of vfield corrected
!   9/23 Atomic number checked to exclude shells
!
!  Conditions of use:
!
...
!
  ...
  integer(i4)                                  :: nr
  integer(i4)                                  :: nregioni
  integer(i4)                                  :: procplus1     ! Processor that has extra column
  integer(i4)                                  :: status
#ifdef MPI
  integer                                      :: MPIerror
  integer                                      :: ntag
...
!
  if (lsymopt) then
    do i = 1,nasym
      ...
      qi = qa(i)
      if (lelementOK(ni).and.nregionno(i).eq.1) then
        neem = neem + 1
        neemptr(neem) = i
        neemrptr(i) = neem
...
!
  if (nprocs.eq.1) then
    ...
    enddo
!
!  Find processor that is to be responsible for extra column based on block cyclic distribution
!
    procplus1 = neem/nprocs
    procplus1 = neem - procplus1*nprocs
    if (procid.eq.procplus1) then
      nlocptr(nloc+1) = neem + 1
    endif
  endif
!
  if (procid.eq.procplus1) then
...
!
      if (lsymopt) then
        call dbcgsolve(neem+1_i4,nloc1,nlocptr,emat,maxp,z,qf,qitertol,nqitermax,iter,err)
      else
        call dcgsolve(neem+1_i4,nloc1,nlocptr,emat,maxp,z,qf,qitertol,nqitermax,iter,err)
      endif
!
      if (ioproc) then
...
!******************
      ifail = 0
      n = neem + 1
      lsymmetric = (.not.lsymopt.and..not.lqeq)
      call matrix_inversion_library(n,1_i4,maxp,1_i4,emat,0_i4,lsymmetric,ifail)
!
...
!
      qa(1:neem+1) = 0.0_dp
      do i = 1,nloc1
        ii = neemptr(nlocptr(i))
        do j = 1,neem + 1
          qa(ii) = qa(ii) + z(j)*emat(j,i)
        enddo
      enddo
!
!  Sum charges in qa/qf
!
      call sumall(qa,qf,neem+1,"qf","eemd")
      do i = 1,neem
        ii = neemptr(i)
        qa(ii) = qf(ii)
      enddo
...
!*****************************
  enddo
!
!  Store charges in configurational array (unless this is a temporary supercell call)
!
  if (nascfg(ncf).eq.nasym) then
    do i = 1,nasym
      qlcfg(nsft+i) = qa(i)
    enddo
  endif
...
!
    eself = eself + qi*reqv*extpot(ii)
!
    nregioni = nregionno(ii)
