...
!   4/17 Order of loops for printing of tmat switched to match parallel case
!   2/18 Trace added
...
!   3/20 ninternal changed to ninternalatm to exclude rigid molecules
!   5/20 Rigid molecule modifications added for indexing during cmat compression
!   6/21 Correction to breathing shell handling in setting of cmat
...
!  Check size of tmat
!
  if (n3a+ninternalatm.gt.maxn3a) then
    maxn3a = n3a + ninternal
    call changemaxn3a
  endif
        ...
        do i = 1,nfa
          ...
          if (lbsmat(ii)) then
            ...
            do j = 1,numat
              ...
              elseif (.not.lfreeze.or.lopf(nrelf2a(j))) then
                ind = ind + 1
              endif
            enddo
          endif
      ...
      if (nbsm.gt.0) then
        do i = 1,nfa
          ...
          if (lbsmat(ia)) then
            ind = 0
            do j = 1,numat
              if (nrelf2a(j).eq.ia) then
                ind = ind + 1
                tmat(noff+ind,nofa+i) = 1.0_dp
              elseif (lopf(nrelf2a(j))) then
                ind = ind + 1
              endif
            enddo
          endif
...
!
        do j = 1,n3f
          tmat(j,iint) = 0.0_dp
          do k = 1,n3a
            tmat(j,iint) = tmat(j,iint) + derv2(j,k)*cmat(k)
          enddo
        enddo
      else
        do j = 1,n3a
          tmat(j,iint+n3a) = cmat(j)
        enddo
      endif
    enddo
  endif
...
!
      ...
      do i = 1,n3a
        write(ioout,'(2x,12f6.3)')(tmat(i,n3a+j),j=1,ninternalatm)
      enddo
      write(ioout,'(/)')
    else
!
