...
!
    ...
    if (status/=0) call deallocate_error('brenner','ineigh')
...
!
  ...
  if (status/=0) call outofmemory('brenner','ineigh')
...
!
          if (lgrad1) then
            call d1add(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,d1j,.true.,.false.,.true.)
            if (lgrad2) then
              if (ldophonon) then
                call d2addp(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nREBOatomRptr,d1j,d2j,xkv,ykv,zkv, &
                  .true.,.false.)
              else
                call d2add(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,d1j,d2j,.true.,.false.)
              endif
            endif
          endif
!
...
!
    if (lgrad1) then
      call d1add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,d1i,.true.,.false.,.true.)
      if (lgrad2) then
        if (ldophonon) then
          call d2addp(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nREBOatomRptr,d1i,d2i,xkv,ykv,zkv,.true.,.false.)
        else
          call d2add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,d1i,d2i,.true.,.false.)
        endif
      endif
    endif
  enddo
