...
!  11/13 Extra trap added for case of nboA/nboR for ij-/j-i = 0
...
!   9/14 Check on nboij numbers being 0 in setting of terms removed
!        since this leads to incorrect first derivatives from this
!        routine. 
!   9/14 Error in second derivatives corrected
!  11/14 ineigh added
...
!   3/16 Algorithm changed slightly so that instead of using lower half
...
!   4/23 lextended argument added to d2addp
...
!
  ...
  if (status/=0) call outofmemory('bondorder','Zcn')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('bondorder','ineigh')
...
!
  ...
  if (status/=0) call outofmemory('bondorder','ineigh')
...
!
    if (nboZ.gt.0) then
      ...
      if (lfound) then
        eii = BOecoeffZ(nboZi)
        ebondorder = ebondorder + eii
!
        eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + eii
!
        siteenergy(i) = siteenergy(i) + eii
      endif
    endif
!
...
!
    if (lgrad1) then
      call d1add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,d1i,.false.,.false.,.true.)
      if (lgrad2) then
        if (ldophonon) then
          call d2addp(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nboatomRptr,d1i,d2i,xkv,ykv,zkv,.false.,.false.)
        else
          call d2add(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nboatomRptr,d1i,d2i,.false.,.false.)
        endif
      endif
    endif
  enddo
...
!
      ...
      if (lfound) then
        ...
        eii = deltaZ*(BOccoeffZ(1,nboZi) + BOccoeffZ(2,nboZi)*deltaZ)
        ebondorder = ebondorder + eii
!
        eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + eii
!
...
!
  ...
  if (status/=0) call deallocate_error('bondorder','Zcn')
