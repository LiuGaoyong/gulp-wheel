...
!   2/20 Conversion of site potential coordinates now limited to fractionals
...
!
  ...
  use control
  use current
  use potentialsites,  only : npotsitecfg, npotsites, xpotsite, ypotsite, zpotsite, lpotsitecartcfg
  use scan
  use symmetry
#ifdef TRACE
...
!
  ...
  logical,          dimension(:), allocatable       :: ltemp3
  real(dp)                                          :: rr(6)
  real(dp)                                          :: rvf(3,3)
  real(dp)                                          :: rvt(3,3)
  real(dp)                                          :: v(3)
...
!*******************
  ...
  allocate(ltemp3(nasym),stat=status)
  if (status/=0) call outofmemory('symoff','ltemp3')
  do i = nasum,nstart2, - 1
    ...
    ltranatminus(i + nnew) = ltranatminus(i)
  enddo
  do i = 1,nasym
    ltemp2(i) = ltranat(i + nsft)
    ltemp3(i) = ltranatminus(i + nsft)
  enddo
  if (lfull.and.icf.gt.1) then
    ...
    do i = 1,numat
      do j = 1,icf
        ...
        nspecptrcfg(j + iadd) = nspecptr(nrelf2a(i))
        qlcfg(j + iadd) = qf(i)
        occucfg(j + iadd) = occuf(i)
        oxcfg(j + iadd) = oxf(i)
        radcfg(j + iadd) = radf(i)
        lbsmatcfg(j + iadd) = lbsmat(nrelf2a(i))
        ltranat(j + iadd) = ltemp2(nrelf2a(i))
        ltranatminus(j + iadd) = ltemp3(nrelf2a(i))
      enddo
      iadd = iadd + icf
    enddo
  else
    do i = 1,numat
      ...
      nspecptrcfg(i + nsft) = nspecptr(nrelf2a(i))
      qlcfg(i + nsft) = qf(i)
      occucfg(i + nsft) = occuf(i)
      oxcfg(i + nsft) = oxf(i)
      radcfg(i + nsft) = radf(i)
      lbsmatcfg(i + nsft) = lbsmat(nrelf2a(i))
      ltranat(i + nsft) = ltemp2(nrelf2a(i))
      ltranatminus(i + nsft) = ltemp3(nrelf2a(i))
    enddo
  endif
  deallocate(ltemp3,stat=status)
  if (status/=0) call deallocate_error('symoff','ltemp3')
...
!****************
  ...
  if (lfull) then
    ...
    enddo
    ifail = 0
    call matrix_inversion(rvf,3_i4,3_i4,rr,ifail)
    do i = 1,3
      do j = 1,3
...
!**********************************
  if (.not.lfull.and..not.lpotsitecartcfg(ncf)) then
!
