  ...
  subroutine setreaxFFneigh
!
!  Initialise arrays and computes the neighbour list for ReaxFF.
!
!  12/17 Created from reaxffmd
!   9/19 nrel2 / nrelat changed to nrela2f / nrelf2a
!   7/22 Arguments added to reaxFFbo_sigma call for second derivatives
!
!  Conditions of use:
!
...
!
!  Julian Gale, CIC, Curtin University, July 2022
!
  use datatypes
...
!
  ...
  if (status/=0) call outofmemory('setreaxFFneigh','nboatomptr')
  allocate(nboatomRptr(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nboatomRptr')
  allocate(nbodummyptr(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nbodummyptr')
!
!  Allocate memory that does not depend on maxneigh
!
  allocate(numattodoptr(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','numattodoptr')
  allocate(numattodoRptr(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','numattodoRptr')
  allocate(nbos(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nbos')
  allocate(nbosptr(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nbosptr')
  allocate(latomdone(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','latomdone')
  allocate(nfreeatom(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nfreeatom')
  allocate(nfreeqptr(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nfreeqptr')
  allocate(nneigh(numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','nneigh')
!
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('setreaxFFneigh','BOp_pipi')
    deallocate(BOp_pi,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','BOp_pi')
    deallocate(BOp,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','BOp')
    deallocate(BO_pipi,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','BO_pipi')
    deallocate(BO_pi,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','BO_pi')
    deallocate(BO,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','BO')
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','rneigh')
    deallocate(ineigh,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','ineigh')
    deallocate(neighnoRptr,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','neighnoRptr')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('setreaxFFneigh','neighno')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('setreaxFFneigh','neighno')
  allocate(neighnoRptr(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','neighnoRptr')
  allocate(ineigh(3_i4,maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','ineigh')
  allocate(rneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','rneigh')
  allocate(xneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','xneigh')
  allocate(yneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','yneigh')
  allocate(zneigh(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','zneigh')
  allocate(BO(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','BO')
  allocate(BO_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','BO_pi')
  allocate(BO_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','BO_pipi')
  allocate(BOp(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','BOp')
  allocate(BOp_pi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','BOp_pi')
  allocate(BOp_pipi(maxneigh,numat),stat=status)
  if (status/=0) call outofmemory('setreaxFFneigh','BOp_pipi')
!****************************
...
!  Sigma
!
            call reaxFFbo_sigma(nboij,nspeci,nspecj,rij,BOpij,dBOpijdr,d2BOpijdr2,.false.,.false., &
...
!
  t2 = g_cpu_time()
  treaxFF = treaxFF + t2 - t1
!
  return
  end subroutine setreaxFFneigh

  subroutine reaxFFfd(matom,mcrd,step,ereaxFF,lgrad1)
!
...
!
!  12/17 Created from reaxFFmd.f90
!   7/22 Arguments added to reaxFFbo_sigma/pi/pipi calls for second derivatives
!   7/22 Changed to remove used of BOj arrays
...
!
  ...
  use control,        only : keyword, lseok, lreaxFFQ, literativeQ
...
!***************************************
        call reaxFF_bomd(i,j,nspeci,nspecj,ni,nj,deltapi,deltapj,nneigh,BOp,BOp_pi,BOp_pipi, &
                         d1BOp,d1BOp_pi,d1BOp_pipi,BOij_s,BOij_pi,BOij_pipi, &
                         d1BOi_s,d1BOi_pi,d1BOi_pipi,lgrad1)
!
...
!
    if (lgrad1) then
      call d1add(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nbodummyptr,d1i,.false.,.false.,.true.)
    endif
  enddo iloop
!***************************************************
...
!
        if (abs(reaxFFlp(2,nspeci)).gt.1.0d-12) then
          ...
          elone = elone + reaxFFlp(2,nspeci)*deltalp(i)*trm1lp
        endif
!
!  Bond penalty energy
!
...
!
        ...
        eover = eover + sumover*otrm2*deltalp_corr_i*otrm3
!
...
!
        ...
        eunder  = eunder - reaxFFuc1(nspeci)*(1.0_dp - expuc1)*utrm1*utrm2
!
...
!
  return
  end subroutine reaxFFfd

  subroutine unsetreaxFFneigh
!
!  Deallocates memory reserved for reaxFF neighbour list.
!
!  12/17 Created from reaxFFmd
!
!  Conditions of use:
...
!
  deallocate(BOp_pipi,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','BOp_pipi')
  deallocate(BOp_pi,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','BOp_pi')
  deallocate(BOp,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','BOp')
  deallocate(BO_pipi,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','BO_pipi')
  deallocate(BO_pi,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','BO_pi')
  deallocate(BO,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','BO')
  deallocate(zneigh,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','zneigh')
  deallocate(yneigh,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','yneigh')
  deallocate(xneigh,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','xneigh')
  deallocate(rneigh,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','rneigh')
  deallocate(ineigh,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','ineigh')
  deallocate(neighnoRptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','neighnoRptr')
  deallocate(neighno,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','neighno')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nneigh')
  deallocate(nfreeqptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nfreeqptr')
  deallocate(nfreeatom,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nfreeatom')
  deallocate(latomdone,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','latomdone')
  deallocate(nbosptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nbosptr')
  deallocate(nbos,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nbos')
  deallocate(numattodoRptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','numattodoRptr')
  deallocate(numattodoptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','numattodoptr')
  deallocate(nbodummyptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nbodummyptr')
  deallocate(nboatomRptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nboatomRptr')
  deallocate(nboatomptr,stat=status)
  if (status/=0) call deallocate_error('unsetreaxFFneigh','nboatomptr')
!
  t2 = g_cpu_time()
  treaxFF = treaxFF + t2 - t1
!
  return
  end subroutine unsetreaxFFneigh
