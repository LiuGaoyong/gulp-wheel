...
!  11/12 Defaults now set at the start of each potential input using call to 
!        initmaxthbdefault to ensure consistency
!   2/13 bagcross potential added
!   5/13 Custom bond type added
!   6/13 balcross potential added
!   8/13 Checks for k3 and k4 when using exponential or Vessal potentials added
!   8/13 Swapping of minimum distances added when maximum values are swapped
!   3/14 Fractional bond orders added
!   1/15 k3 and k4 terms added for cosine form
!   1/15 Modified Stillinger-Weber 3-body added
!   3/15 mm3angle added
!   3/15 Sub-option to convert per degree to radians
...
!
  ...
  logical                             :: lvessal
  logical                             :: lwarnp
  real(dp)                            :: units              ! Units for energy
  real(dp)                            :: unitsangle         ! Units for angular part of force constants
  real(dp)                            :: var1
!
...
!
  ...
  if (index(word,'bagc').eq.1) goto 490
  if (index(word,'balc').eq.1) goto 500
  if (index(word,'mm3a').eq.1) goto 510
  if (index(word,'j3').eq.1)   goto 520
  if (index(word,'ppp3').eq.1) goto 530
  return
!****************************
...
!
  if (llibrary) then
    allocate(nvars(3,maxtrio))
    allocate(itypes(3,maxtrio))
    allocate(syms(3,maxtrio))
  else
    allocate(nvars(3,1))
    allocate(itypes(3,1))
    allocate(syms(3,1))
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'kj').eq.1) then
        units = kjmtoev
      elseif (index(words(i),'de').eq.1) then
        unitsangle = radtodeg
      ...
      elseif (index(words(i),'cus').eq.1) then
        n3botype(1,nbondtypein1,nthb+1) = 7
        nbondtypein1 = nbondtypein1 + 1
        nbondtypein1 = min(nbondtypein1,2_i4)
      elseif (index(words(i),'hal').eq.1) then
        n3botype(1,nbondtypein1,nthb+1) = 8
        nbondtypein1 = nbondtypein1 + 1
        nbondtypein1 = min(nbondtypein1,2_i4)
      elseif (index(words(i),'quar').eq.1) then
        n3botype(1,nbondtypein1,nthb+1) = 9
        nbondtypein1 = nbondtypein1 + 1
        nbondtypein1 = min(nbondtypein1,2_i4)
      elseif (index(words(i),'thi').eq.1) then
        n3botype(1,nbondtypein1,nthb+1) = 10
        nbondtypein1 = nbondtypein1 + 1
        nbondtypein1 = min(nbondtypein1,2_i4)
...
!
!  Check for mutually exclusive specifications
!
  if (lk3.or.lk4) then
    if (lvessal) then
      nwarn = nwarn + 1
      if (ioproc) then
        call outwarning('k3 and k4 options do not apply to Vessal threebody potential',0_i4)
      endif
    elseif (lexpo) then
      nwarn = nwarn + 1
      if (ioproc) then
        call outwarning('k3 and k4 options do not apply to exponential threebody potential',0_i4)
      endif
    endif
  endif
!
...
!
    if (llibrary) then
      call getpotsymbols3(iline,llibrary,maxtrio,ntrio,nvars,itypes,syms,nbeg,lvalidpot)
!
!  Check dimension maxtrio
!
      if (ntrio.gt.maxtrio) then
        maxtrio = ntrio
!
        deallocate(syms)
        deallocate(itypes)
...
!
      ...
      if (nfloat.ge.4) then
        thbk(nthb) = floats(1+nbeg)*units*unitsangle*unitsangle
...
!
!  Check that there will be enough space in the arrays for the trios to be added
!
    if (nthb+ntrio-1.gt.maxthb) then
      maxthb = nthb + ntrio
      call changemaxthb
    endif
!*************************************
!  Loop over valid trios of species  *
!*************************************
    do nt = 1,ntrio
      if (nt.gt.1) then
        nthb = nthb + 1
...
!
        mmtexc(nthb) = mmtexc(npot0)
        nthrty(nthb) = nthrty(npot0)
        n3botype(1:2,1:2,nthb) = n3botype(1:2,1:2,npot0)
        n3bondnono(1:2,nthb) = n3bondnono(1:2,npot0)
        if (n3bondnono(1,nthb).gt.0) then
          n3bondno(n3bondnono(1,nthb),1,nthb) = n3bondno(n3bondnono(1,nthb),1,npot0)
        endif
        if (n3bondnono(2,nthb).gt.0) then
          n3bondno(n3bondnono(2,nthb),2,nthb) = n3bondno(n3bondnono(2,nthb),2,npot0)
        endif
        lthetataper(nthb) = lthetataper(npot0)
        ltdreiding(nthb) = ltdreiding(npot0)
        ltib3body(nthb) = ltib
        ltif3body(nthb) = ltif
        thetatapermax(nthb) = thetatapermax(npot0)
        thetatapermin(nthb) = thetatapermin(npot0)
        thbk(nthb) = thbk(npot0)
        theta(nthb) = theta(npot0)
        thrho1(nthb) = thrho1(npot0)
        thrho2(nthb) = thrho2(npot0)
        thr1min(nthb) = thr1min(npot0)
        thr1(nthb) = thr1(npot0)
        thr2min(nthb) = thr2min(npot0)
        thr2(nthb) = thr2(npot0)
        thr3min(nthb) = thr3min(npot0)
        thr3(nthb) = thr3(npot0)
      endif
!
!  Assign pivot atom parameters
!
      ntspec1(nthb) = nvars(1,nt)
      ntptyp1(nthb) = itypes(1,nt)
      symbol3(1,nthb) = syms(1,nt)
!
!  Assign terminal atom parameters
!
      if (nvars(2,nt).eq.nvars(3,nt)) then
        ntspec2(nthb) = nvars(2,nt)
        ntspec3(nthb) = nvars(3,nt)
        if (itypes(2,nt).lt.itypes(3,nt)) then
          ntptyp2(nthb) = itypes(2,nt)
          ntptyp3(nthb) = itypes(3,nt)
          symbol3(2,nthb) = syms(2,nt)
          symbol3(3,nthb) = syms(3,nt)
          if (lfit) then
            if (n3.eq.1) then
              nfit = nfit + 1
              if (nfit.ge.maxfit) then
          ...
          ntptyp2(nthb) = itypes(3,nt)
          ntptyp3(nthb) = itypes(2,nt)
          var1 = thr1(nthb)
          thr1(nthb) = thr2(nthb)
          thr2(nthb) = var1
          var1 = thr1min(nthb)
          thr1min(nthb) = thr2min(nthb)
          thr2min(nthb) = var1
          symbol3(2,nthb) = syms(3,nt)
          symbol3(3,nthb) = syms(2,nt)
!
...
!
      ...
      elseif (nvars(2,nt).lt.nvars(3,nt)) then
        ntspec2(nthb) = nvars(2,nt)
        ntspec3(nthb) = nvars(3,nt)
        ntptyp2(nthb) = itypes(2,nt)
        ntptyp3(nthb) = itypes(3,nt)
        ...
        ntspec2(nthb) = nvars(3,nt)
        ntspec3(nthb) = nvars(2,nt)
        ntptyp2(nthb) = itypes(3,nt)
        ntptyp3(nthb) = itypes(2,nt)
        symbol3(2,nthb) = syms(3,nt)
        symbol3(3,nthb) = syms(2,nt)
...
!
    ...
    if (lk3.and.lk4) then
      if (mmtexc(nthb).eq.1) then
        ...
        if (nfloat.ge.4) then
          thbk(nthb) = floats(1+nbeg)*units*unitsangle**2
          thrho2(nthb) = floats(2+nbeg)*units*unitsangle**3
          thrho1(nthb) = floats(3+nbeg)*units*unitsangle**4
    ...
    elseif (lk4) then
      if (mmtexc(nthb).eq.1) then
        ...
        if (nfloat.ge.3) then
          thbk(nthb) = floats(1+nbeg)*units*unitsangle**2
          thrho1(nthb) = floats(2+nbeg)*units*unitsangle**4
          theta(nthb) = floats(3+nbeg)
        else
          call outerror('Incorrect potential coefficient input',iline)
          call stopnow('potword3')
  ...
  if (nword.gt.1) then
    ...
    do while (i.le.nword)
      ...
      elseif (index(words(i),'mod').eq.1) then
        lmodified = .true.
      elseif (index(words(i),'gar').eq.1) then
        lgarofalini = .true.
      elseif (index(words(i),'kj').eq.1) then
        units = kjmtoev
      elseif (index(words(i),'sin').eq.1) then
...
!
  ...
  if (mmtexc(nthb).eq.1) then
    if (nfloat.ge.5) then
      thbk(nthb) = floats(1+nbeg)*units*unitsangle
      thrho3(nthb) = floats(2+nbeg)*units*unitsangle
      thrho1(nthb) = floats(3+nbeg)
      thrho2(nthb) = floats(4+nbeg)
    ...
    elseif (nfloat.ge.3) then
      thbk(nthb) = floats(1+nbeg)*units*unitsangle
      thrho3(nthb) = floats(1+nbeg)*units*unitsangle
      thrho1(nthb) = floats(2+nbeg)
      thrho2(nthb) = floats(2+nbeg)
      theta(nthb) = floats(3+nbeg)
    else
...
!
  call initmaxthbdefaults(npot1)
  thrho1(npot1) = 12.0_dp
  thrho2(npot1) = 10.0_dp
  thrho3(npot1) = 4.0_dp
  if (nword.gt.1) then
    i = 2
    do while (i.le.nword)
...
!
  ...
  if (mmtexc(nthb).eq.1) then
    if (nfloat.ge.7) then
      ...
      thrho2(nthb) = floats(3+nbeg)
      theta(nthb) = floats(4+nbeg)
      threepoly(1,nthb) = abs(floats(5+nbeg))
      threepoly(2,nthb) = abs(floats(6+nbeg))
      threepoly(3,nthb) = abs(floats(7+nbeg))
    elseif (nfloat.ge.6) then
      thbk(nthb) = floats(1+nbeg)*units
      thrho1(nthb) = floats(2+nbeg)
      thrho2(nthb) = floats(2+nbeg)
      theta(nthb) = floats(3+nbeg)
      threepoly(1,nthb) = abs(floats(4+nbeg))
      threepoly(2,nthb) = abs(floats(5+nbeg))
      threepoly(3,nthb) = abs(floats(6+nbeg))
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('potword3')
    endif
  else
    if (nfloat.ge.13) then
      thbk(nthb) = floats(1+nbeg)*units
      thrho1(nthb) = floats(2+nbeg)
      thrho2(nthb) = floats(3+nbeg)
      theta(nthb) = floats(4+nbeg)
      ...
      threepoly(2,nthb) = abs(floats(6+nbeg))
      threepoly(3,nthb) = abs(floats(7+nbeg))
      thr1min(nthb) = floats(8+nbeg)
      thr1(nthb) = floats(9+nbeg)
      thr2min(nthb) = floats(10+nbeg)
      thr2(nthb) = floats(11+nbeg)
      thr3min(nthb) = floats(12+nbeg)
      thr3(nthb) = floats(13+nbeg)
    elseif (nfloat.eq.12) then
      thbk(nthb) = floats(1+nbeg)*units
      thrho1(nthb) = floats(2+nbeg)
      thrho2(nthb) = floats(3+nbeg)
      theta(nthb) = floats(4+nbeg)
...
!
  ...
  if (mmtexc(nthb).eq.1) then
    if (nfloat.ge.7) then
      ...
      thrho1(nthb) = floats(2+nbeg)
      thrho2(nthb) = floats(3+nbeg)
      if (floats(4+nbeg).lt.0.0_dp) then
        theta(nthb) = - 1.0_dp
      else
        theta(nthb) = 1.0_dp
      endif
      threepoly(3,nthb) = abs(floats(5+nbeg))
      threepoly(1,nthb) = abs(floats(6+nbeg))
      threepoly(2,nthb) = abs(floats(7+nbeg))
    else
      call outerror('Incorrect potential coefficient input',iline)
      call stopnow('potword3')
    endif
...
!
  ...
  return
!*******************************
!  MM3 angle bending potential *
!*******************************
510 if (nthb.eq.maxthb) then
    maxthb = nthb + 10
    call changemaxthb
  endif
!
...
!
  ...
  if (mmtexc(nthb).eq.1) then
    ...
    if (nfloat.ge.7) then
      ...
      theta(nthb)       = floats(2+nbeg)
      threepoly(1,nthb) = floats(3+nbeg)
      threepoly(2,nthb) = floats(4+nbeg)*unitsangle
      threepoly(3,nthb) = floats(5+nbeg)*unitsangle**2
      threepoly(4,nthb) = floats(6+nbeg)*unitsangle**3
      threepoly(5,nthb) = floats(7+nbeg)*unitsangle**4
    elseif (nfloat.eq.6) then
      thbk(nthb)        = floats(1+nbeg)*units*unitsangle*unitsangle
      theta(nthb)       = floats(2+nbeg)
      threepoly(1,nthb) = floats(3+nbeg)
      ...
      threepoly(3,nthb) = floats(5+nbeg)*unitsangle**2
      threepoly(4,nthb) = floats(6+nbeg)*unitsangle**3
      threepoly(5,nthb) = 9.0_dp*unitsangle**4
    elseif (nfloat.eq.5) then
      thbk(nthb)        = floats(1+nbeg)*units*unitsangle*unitsangle
      theta(nthb)       = floats(2+nbeg)
      threepoly(1,nthb) = floats(3+nbeg)
      threepoly(2,nthb) = floats(4+nbeg)*unitsangle
      threepoly(3,nthb) = floats(5+nbeg)*unitsangle**2
      threepoly(4,nthb) = 7.0_dp*unitsangle**3
      threepoly(5,nthb) = 9.0_dp*unitsangle**4
    elseif (nfloat.eq.4) then
      thbk(nthb)        = floats(1+nbeg)*units*unitsangle*unitsangle
      theta(nthb)       = floats(2+nbeg)
      threepoly(1,nthb) = floats(3+nbeg)
      threepoly(2,nthb) = floats(4+nbeg)*unitsangle
      threepoly(3,nthb) = 5.6_dp*unitsangle**2
      threepoly(4,nthb) = 7.0_dp*unitsangle**3
      threepoly(5,nthb) = 9.0_dp*unitsangle**4
    elseif (nfloat.eq.3) then
      thbk(nthb)        = floats(1+nbeg)*units*unitsangle*unitsangle
      theta(nthb)       = floats(2+nbeg)
      threepoly(1,nthb) = floats(3+nbeg)
      threepoly(2,nthb) = 0.014_dp*unitsangle
      threepoly(3,nthb) = 5.6_dp*unitsangle**2
      threepoly(4,nthb) = 7.0_dp*unitsangle**3
      threepoly(5,nthb) = 9.0_dp*unitsangle**4
    elseif (nfloat.eq.2) then
    ...
    if (nfloat.gt.13) nfloat = nfloat - 7
    thrho1(nthb) = 0.0_dp
    if (nfloat.ge.13) then
      thbk(nthb)        = floats(1+nbeg)*units*unitsangle*unitsangle
      theta(nthb)       = floats(2+nbeg)
...
!
  ...
  return
!*****************************
!  PPP three-body potential  *
!*****************************
530 if (nthb.eq.maxthb) then
    maxthb = nthb + 10
    call changemaxthb
  endif
!
...
!
!  Check that rmin values are sensible
!
  if (thrho1(nthb).gt.thr1(nthb)) then
    call outerror('PPP 3body rmin must be less than rcut',iline)
    call stopnow('potword3')
  elseif (thrho2(nthb).gt.thr2(nthb)) then
    call outerror('PPP 3body rmin must be less than rcut',iline)
    call stopnow('potword3')
  elseif (abs(thrho1(nthb)-thr1(nthb)).lt.1.0d-12) then
    call outerror('PPP 3body rmin is too close to rcut',iline)
    call stopnow('potword3')
  elseif (abs(thrho2(nthb)-thr2(nthb)).lt.1.0d-12) then
    call outerror('PPP 3body rmin is too close to rcut',iline)
    call stopnow('potword3')
  endif
  goto 535
