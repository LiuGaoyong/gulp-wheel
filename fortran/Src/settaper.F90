...
!   9/14 Unneeded argument, lorder12, to twobody1 removed
!  11/21 Species added to twobody calls
!   6/23 Actual species numbers passed to twobody rather than 1
!
!  Conditions of use:
!
...
!
    do i = 1,npote
      ...
      call twobody1(eatom,ereal,ec6,.true.,.false.,.false.,1_i4,1_i4,dist,0.0_dp,0.0_dp,0.0_dp,d0i,d0j, &
                    ...
                    0_i4,0.0_dp,1.0_dp,1.0_dp,0.0_dp,rtrm1,rtrm2,rtrm3,rtrm32,sctrm1,sctrm2,0.0_dp,0.0_dp,ns1,ns2, &
                    .false.,.false.,.false.,.false.,.false.,.false.,0_i4,0_i4,.false.,.false.,d1i,d1j,d2i2,d2ij,d2j2)
      taperpot(i) = eatom
...
!
          rmin = denpar(5,1,j,i)
          r0 = denpar(6,1,j,i)
          if (tapermax.lt.r0.and.tapermax.ge.rmin) then
            apt = denpar(1,1,j,i)
            bpt = denpar(2,1,j,i)
            cpt = denpar(3,1,j,i)
            dpt = denpar(4,1,j,i)
            trm = (tapermax - r0)
            sctrm1 = eamalloy(1,i)*(apt*trm**3 + bpt*trm**2 + cpt*trm + dpt)
            sctrm2 = eamalloy(1,i)*(3.0_dp*apt*trm**2 + 2.0_dp*bpt*trm + cpt)
          else
            sctrm1 = 0.0_dp
            sctrm2 = 0.0_dp
          endif
