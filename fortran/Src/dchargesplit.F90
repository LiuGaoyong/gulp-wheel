...
!
    ...
    do i = 1,numat
      ...
      if (lelementOK(ni).and.nregionno(nrelf2a(i)).eq.1) then
        ...
        jeem = 0
        do j = 1,numat
          if (lelementOK(nat(j)).and.nregionno(nrelf2a(j)).eq.1) then
            jeem = jeem + 1
...
!     
    n = neembond
    lsymmetric = (.not.lqeq)
    call matrix_inversion_library(n,1_i4,maxeb,nblocksize,ematb,0_i4,lsymmetric,ifail)
!
...
!
    ...
    do i = 1,numat
      ...
      if (ni.le.maxele) then
        if (lelementOK(ni).and.nregionno(nrelf2a(i)).eq.1) ieem = ieem + 1
      endif
!
...
!
      ...
      do j = 1,numat
        ...
        if (nj.le.maxele) then
          if (lelementOK(nj).and.nregionno(nrelf2a(j)).eq.1) jeem = jeem + 1
        endif
!
