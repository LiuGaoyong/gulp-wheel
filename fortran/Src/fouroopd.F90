  subroutine fouroopd(eoop,esregion12,esregion2,eattach,lgrad1,lgrad2,xderv,yderv,zderv)
!
...
!
!   1/17 Created from fouroop
!   1/17 ioptptr replaced by noptatrptr
!   3/17 Correction for degtorad factors added for potentials with angles
...
!
  ...
  integer(i4)                                  :: ntypl
  integer(i4)                                  :: nunique
  integer(i4)                                  :: nuniqueloc
  integer(i4), dimension(:), allocatable       :: nuniqueptr
  integer(i4), dimension(:), allocatable       :: nuniquelocptr
  integer(i4)                                  :: status
  logical                                      :: l2bonds
  logical                                      :: lattach
  logical                                      :: lbonded
...
!
  ...
  if (status/=0) call outofmemory('fouroopd','nuniqueptr')
  allocate(nuniquelocptr(numat),stat=status)
  if (status/=0) call outofmemory('fouroopd','nuniquelocptr')
!
...
!**************************************************
!  Loop over middle site 1 / i - centre atom case *
!**************************************************
    ixloc = - 2
    iyloc = - 1
    izloc =   0
    liloop: do iloc = 1,natomsonnode
      i = node2atom(iloc)
      ni = nat(i)
      ntypi = nftype(i)
...
!
              ...
              xcom(4) = xcom(2) - xcom(1)
              ycom(4) = ycom(2) - ycom(1)
              zcom(4) = zcom(2) - zcom(1)
            else
              lmolok = .false.
              xcom(2) = - xcomi
              ycom(2) = - ycomi
...
!
                  ...
                  xcom(5) = xcom(3) - xcom(1)
                  ycom(5) = ycom(3) - ycom(1)
                  zcom(5) = zcom(3) - zcom(1)
                  xcom(6) = xcom(3) - xcom(2)
                  ycom(6) = ycom(3) - ycom(2)
                  zcom(6) = zcom(3) - zcom(2)
                else
                  lmolok = .false.
                  xcom(3) = - xcomi
                  ycom(3) = - ycomi
...
!  Check i is allowed for n
!
      if (.not.lmatch(ni,ntypi,nt1,ntyp1,.true.)) cycle liloop2
!
!  Only 3 bonds check
!
      if (lbtyp.and.lonly3oop(n).and.nbonds(i).ne.3) cycle liloop2
!
!  QM/MM handling : i is a QM atom and potential is of bonded type => exclude
!
      if (QMMMmode(ncf).gt.0) then
        if (nregiontypi.eq.1.and.lbtyp) cycle liloop2
      endif
!
!  Set loop range for other atoms
!
        ...
        if (nbonds(i).gt.0) then
          ...
          do jloop = 1,nbonds(i)
            if (atom2local(nbonded(jloop,i)).ne.0) then
              lunique = .true.
              do lu = 1,nuniqueloc
                if (nbonded(jloop,i).eq.nuniquelocptr(lu)) lunique = .false.
              enddo
              if (lunique) then
                nuniqueloc = nuniqueloc + 1
                nuniquelocptr(nuniqueloc) = nbonded(jloop,i)
              endif
            endif
!
            lunique = .true.
...
!  Distance checking
!
          if ((r212.gt.tr1.or.r212.lt.tr1min).and.(.not.lbtyp.or..not.lbonded)) cycle iiloop2
          r21 = sqrt(r212)
          x21 = x21t + xvec1cell(ii)
          y21 = y21t + yvec1cell(ii)
          z21 = z21t + zvec1cell(ii)
...
!  Check for intra and but not in same molecule
!
            if (lintra_only.and..not.lmolok) cycle lkloop2
            if (lbtyp.and..not.lmolok) cycle lkloop2
            xc3t = xclat(k)
            yc3t = yclat(k)
            zc3t = zclat(k)
            x31t = xc3t - xc1
...
!  Distance checking
!
              if ((r312.gt.tr2.or.r312.lt.tr2min).and.(.not.lbtyp.or..not.lbonded)) cycle jjloop2
              r31 = sqrt(r312)
              x31 = x31t + xvec1cell(jj)
              y31 = y31t + yvec1cell(jj)
              z31 = z31t + zvec1cell(jj)
...
!  Distance checking
!
                  if ((r412.gt.tr3.or.r412.lt.tr3min).and.(.not.lbtyp.or..not.lbonded)) cycle llloop2
!
                  x41 = x41t + xvec1cell(ll)
                  y41 = y41t + yvec1cell(ll)
                  z41 = z41t + zvec1cell(ll)
...
!
                  ...
                  if (lgrad2) then
                    ...
                    n2z = jzl
!
!  lopj must be true for the following
!
                    n2x1 = jxl
                    n2x3 = jxl
                    n2x4 = jxl
...
!  Free local memory
!
  deallocate(nuniquelocptr,stat=status)
  if (status/=0) call deallocate_error('fouroopd','nuniquelocptr')
  deallocate(nuniqueptr,stat=status)
  if (status/=0) call deallocate_error('fouroopd','nuniqueptr')
!
!  All tidying up of derivatives is handled by four so we can just return here
!
#ifdef TRACE
  call trace_out('fouroopd')
#endif
  return
  end
