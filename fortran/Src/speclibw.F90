...
!   6/23 AA-CLP force field parameters added
!
!  Conditions of use:
!
...
!
  ...
  real(dp)                     :: radi
#ifdef TRACE
  call trace_in('speclibw')
#endif
!
!  Initialise local variables
!
  if (index(word,'spec').eq.1) goto 100
  if (index(word,'aaclp_s').eq.1) goto 110
#ifdef TRACE
  call trace_out('speclibw')
#endif
  return
!********************************************************
...
!
  ...
  if (nword.ge.2) then
    ...
    if (index(words(2),'bco').eq.1.or.index(words(2),'bsh').eq.1) then
      ...
      ind2 = 2
      nw = 2
    elseif (index(words(2),'co').eq.1.or.index(words(2),'sh').eq.1) then
      ind2 = 1
      nw = 2
    else
      ind2 = 1
...
!
!  Check for secondary symbol (library only)
!
  if (nword.gt.nw) then
    word = words(nw+1)
    call worsy(word,lsymbol,.false.)
    if (lsymbol) then
      llib2ndspec(ilp) = .true.
      lib2ndspec(ilp) = word(1:16)
    endif
  endif
!
...
108 if (.not.l55) l1000 = .true.
  ...
  return
!***********************************************
!  AA-CLP force field information for species  *
!***********************************************
110 continue
  laaclp = .true.
  if (nfloat.gt.0) then
...
!
    if (nfloat.ge.4) then
      aaclp_qscale = abs(floats(1))
      aaclp_dscale = abs(floats(2))
      aaclp_pscale = abs(floats(3))
      aaclp_rscale = abs(floats(4))
...
!  Check for old fashion specification of number of atoms
!
  if (nword.eq.0.and.nfloat.eq.1) goto 115
  if (nword.gt.0) then
    word = words(1)
    call worsy(word,lsymbol,.false.)
    if (.not.lsymbol) then
...
!
  if (nfloat.ge.4) then
    aaclp_pol_spec(ilp) = abs(floats(1))
    aaclp_ip_spec(ilp) = abs(floats(2))
    aaclp_b_spec(ilp) = abs(floats(3))
    aaclp_h_spec(ilp) = floats(4)
  else
    call outerror('Insufficient parameters given for AA-CLP species',iline)
