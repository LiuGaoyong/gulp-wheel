...
!   1/16 rvdw changed to vdwspec
!   6/19 Spin added
!   8/23 Output of wildcard species excluded
!
!  Conditions of use:
!
...
!*******************
  ...
  do i = 1,nspec
    ...
    if (na.ne.maxele) then
      ...
      if (lspininspec(i)) lspinneeded = .true.
!
...
!
      ...
      write(ioout,'(4x,a5,4x,a6,3x,i4,5x,f6.2,1x,f10.6,2x,f6.3,1x,f6.3,1x,f6.3,1x,a9)') &
        lab,stype(ind),na,atm,qlspec(i),rcov(na),radspec(i),vdwspec(i),libspec(i)(1:9)
    endif
  enddo
  write(ioout,'(''--------------------------------------------------------------------------------'')')
  write(ioout,'(/)')
...
!
  if (lspinneeded) then
    write(ioout,'(''  Species with non-zero spin for Ising Hamiltonian: '',/)')
    write(ioout,'(''--------------------------------------------------------------------------------'')')
    write(ioout,'(''  Species    Type    Spin    '')')
    write(ioout,'(''--------------------------------------------------------------------------------'')')
    do i = 1,nspec
      if (lspininspec(i)) then
        ...
        write(ioout,'(4x,a5,4x,a6,1x,f7.4)') lab,stype(ind),spinspec(i)
      endif
    enddo
    write(ioout,'(''--------------------------------------------------------------------------------'')')
    write(ioout,'(/)')
