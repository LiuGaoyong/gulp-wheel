...
!
  ...
  if (status/=0) call outofmemory('recip2D','xdrl')
  allocate(ydrl(numat),stat=status)
  if (status/=0) call outofmemory('recip2D','ydrl')
  allocate(zdrl(numat),stat=status)
  if (status/=0) call outofmemory('recip2D','zdrl')
!**********
!  Setup  *
!**********
  iv = 0
...
!
          if (lti) then
            dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - vol4pi*(zd*derfez + dexpz*rpieta)*dUdl
          endif
!
          if (lreg2one) then
                ...
                if (lrigid) then
                  if (latomicstress) then
                    do kk = 1,nstrains
                      ...
                      strderv(kk) = strderv(kk) - sinq*dGrds*kexperfc
...
!
                        sderv2(kk,ll) = sderv2(kk,ll) - sinq*strm1*(dg2ds(iv,kk)*dGrds2 + dg2ds(iv,ll)*dGrds1)
                        sderv2(kk,ll) = sderv2(kk,ll) - cosq*kexperfc*dGrds1*dGrds2
                        sderv2(kk,ll) = sderv2(kk,ll) - sinq*kexperfc*d2Grds12
                      endif
...
!
                    if (lrigid) then
                      ...
                      derv3(jz,kk) = derv3(jz,kk) - sinq*ztrm1*dGrds1
                    endif
                  enddo
                endif
              endif
...
!
        ...
        if (procid.eq.0) then
          if (lti) then
            dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - vol4pi*rpieta*dUdl
          endif
!
          if (lreg2one) then
...
!
      if (lti) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + csinq*dUdl
      endif
!
!  Lattice energy
!
...
!
  ...
  if (status/=0) call deallocate_error('recip2D','zdrl')
  deallocate(ydrl,stat=status)
  if (status/=0) call deallocate_error('recip2D','ydrl')
  deallocate(xdrl,stat=status)
  if (status/=0) call deallocate_error('recip2D','xdrl')
